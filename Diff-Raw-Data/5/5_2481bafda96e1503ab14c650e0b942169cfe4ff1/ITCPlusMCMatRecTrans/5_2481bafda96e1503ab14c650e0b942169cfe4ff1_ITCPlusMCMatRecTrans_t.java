 ...
 public class ITCPlusMCMatRecTrans extends MatRecTrans implements ITCPlusMCMatRecTransRemote {
     ...
     if (siteidMC != null && ((isNew() && isNull("STATUS")) || (isModified() && getString("STATUS").equalsIgnoreCase("COMP")))) {
       ...
       if (isReceipt()) {
         ...
         if (!isNull("PONUM") && isNull("INVUSEID")) {
           ...
           log.debug(">>>> isNull(\"PONUM\") || !isNull(\"INVUSEID\")");
           Inventory inventoryMC = (Inventory) getInventoryMC(fromstoreloc, siteidMC);
         ...
         log.debug(">>>> isKitMake()");
         Inventory toInventory = (Inventory) getInventoryMC(fromstoreloc, fromsiteid);
   ...
   private void setToOldValuesMC(String siteidMC) throws MXException, RemoteException {
     ...
     if (siteidMC != null && isStore() && ((isNew() && isNull("STATUS")) || (isModified() && getString("STATUS").equalsIgnoreCase("COMP")))) {
       ...
       Inventory inventory = (Inventory) getSharedInventory(tostoreloc, siteid);
       Inventory inventoryMC = (Inventory) getInventoryMC(tostoreloc, siteidMC);
   ...
   private void setToNewValuesMC(String siteidMC) throws MXException, RemoteException {
     ...
     if (siteidMC != null && isStore() && ((isNew() && isNull("STATUS")) || (isModified() && getString("STATUS").equalsIgnoreCase("COMP")))) {
       ...
       if (inventory != null && inventoryMC != null) {
         log.debug(">>>> inventory != null && inventoryMC != null");
         log.debug(">>>> inventory.siteid: " + inventory.getString("SITEID"));
         log.debug(">>>> inventoryMC.siteid: " + inventoryMC.getString("SITEID"));
         ...
         if (invbalances != null) {
           log.debug(">>>> invbalances != null");
           log.debug(">>>> invbalances.siteid: " + invbalances.getString("SITEID"));
         ...
         if (invcostMC != null) {
           log.debug(">>>> invcostMC != null");
           log.debug(">>>> invcostMC.siteid: " + invcostMC.getString("SITEID"));
   ...
   private void updateAverageCostMC(String siteidMC) throws MXException, RemoteException {
     ...
     if (siteidMC != null && ((isNew() && isNull("STATUS")) || (isModified() && getString("STATUS").equalsIgnoreCase("COMP"))) && !isHolding()) {
       log.debug(">>>> siteidMC != null && ((isNew() && isNull(\"STATUS\")) || (isModified() && getString(\"STATUS\").equalsIgnoreCase(\"COMP\")))");
       Inventory toInventory = (Inventory) getInventoryMC(getString("TOSTORELOC"), siteidMC);
       ...
       if (toInventory == null) {
        ...
        log.debug(">>>> sqf2.format(): " + sqf2.format());
       ...
       if (!isNull("FROMSTORELOC") && isFromStore()) {
         log.debug(">>>> !isNull(\"FROMSTORELOC\") && isFromStore()");
         fromInventory = (Inventory) getInventoryMC(getString("FROMSTORELOC"), siteidMC);
   ...
   /**
    * Este metodo obtiene el Inventory segun el almacen y el id de la planta
    * multimoneda
    *
    * @param storeLoc
    * @param siteid
    ...
    * @throws RemoteException
    */
   private MboRemote getInventoryMC(String storeLoc, String siteidMC) throws MXException, RemoteException {
     log.debug("<<<< Entering itc.plusmc.app.inventory.ITCPlusMCMatRecTrans:getInventoryMC(storeLoc, siteidMC) >>>>");
    SqlFormat sqf = new SqlFormat(this, "itemnum = :itemnum and location = :1 and siteid = :2 and itemsetid=:itemsetid");
     ...
     sqf.setObject(2, "INVENTORY", "SITEID", siteidMC);
 
     log.debug(">>>> storeLoc: " + storeLoc);
     log.debug(">>>> siteidMC: " + siteidMC);
     log.debug(">>>> sqf.format(): " + sqf.format());
 
     MboSetRemote invSet = getMboSet("$INVMC", "INVENTORY", sqf.format());
 
     if (invSet != null && !invSet.isEmpty()) {
       log.debug(">>>> invSet != null && !invSet.isEmpty()");
       log.debug(">>>> return invSet.getMbo(0);");
       log.debug("<<<< ==> Exiting itc.plusmc.app.inventory.ITCPlusMCMatRecTrans:getInventoryMC(storeLoc, siteidMC) >>>>");
   ...
   private MboRemote getInvCostRecordMC(MboRemote toInventory, String siteidMC) throws MXException, RemoteException {
     ...
     for (MboRemote invCost = invcostSet.moveFirst(); invCost != null; invCost = invcostSet.moveNext()) {
       ...
       if (invCost.getString("ITEMNUM").equals(getString("ITEMNUM")) && invCost.getString("LOCATION").equals(getString("TOSTORELOC")) && invCost.getString("ITEMSETID").equals(getString("ITEMSETID")) && invCost.getString("CONDITIONCODE").equals(getString("CONDITIONCODE")) && invCost.getString("SITEID").equals(siteidMC)) {
         log.debug(">>>> invCost.getString(\"ITEMNUM\").equals(getString(\"ITEMNUM\")) && invCost.getString(\"LOCATION\").equals(getString(\"TOSTORELOC\")) && invCost.getString(\"ITEMSETID\").equals(getString(\"ITEMSETID\")) && invCost.getString(\"CONDITIONCODE\").equals(getString(\"CONDITIONCODE\")) && invCost.getString(\"SITEID\").equals(siteidMC)");
