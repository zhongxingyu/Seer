  ...
  * @throws IOException 
    */
   public Tuple toTuple(Message msg) throws IOException {
     if (msg == null) {
  ...
  * @throws IOException 
  ...
  public Object messageToTuple(FieldDescriptor fieldDescriptor, Object fieldValue) throws IOException {
     ...
     if (fieldDescriptor.isRepeated()) {
       ...
       List<Message> messageList = (List<Message>) (fieldValue != null ? fieldValue : new ArrayList<Message>());
   ...
   public Object singleFieldToTuple(FieldDescriptor fieldDescriptor, Object fieldValue) {
     ...
     if (fieldDescriptor.isRepeated()) {
       ...
       DataBag bag = bagFactory_.newDefaultBag();
       List<Object> fieldValueList = (List<Object>) (fieldValue != null ? fieldValue : new ArrayList<Object>());
   ...
   public Schema toSchema(Descriptor msgDescriptor) {
 	  return _toSchema(msgDescriptor, 0);
   }
 
   private Schema _toSchema(Descriptor msgDescriptor, int level) {
 	    Schema schema = new Schema();
 	    
 	    try {
 	      // Walk through all the possible fields in the message.
 	      for (FieldDescriptor fieldDescriptor : msgDescriptor.getFields()) {
 	    	  final FieldSchema field = (fieldDescriptor.getType() == FieldDescriptor.Type.MESSAGE)?
 	    			  messageToFieldSchema(fieldDescriptor, level) : 
 	    			  singleFieldToFieldSchema(fieldDescriptor, level);
    ...
    */
   private FieldSchema messageToFieldSchema(FieldDescriptor fieldDescriptor, int level) throws FrontendException {
     assert fieldDescriptor.getType() == FieldDescriptor.Type.MESSAGE : "messageToFieldSchema called with field of type " + fieldDescriptor.getType();
 ...
 //    }
    ...
    * @throws FrontendException if Pig decides to.
    */
   private FieldSchema singleFieldToFieldSchema(FieldDescriptor fieldDescriptor, int level) throws FrontendException {
