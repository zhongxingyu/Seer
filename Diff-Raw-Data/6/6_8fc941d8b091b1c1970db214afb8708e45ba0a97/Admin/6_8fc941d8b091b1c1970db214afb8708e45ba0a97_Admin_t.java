         ...
         synchronized (getCachedUser()) {
             if (ORDERSTATUS.valueOf(status) != null) {
                 ...
                 if(order!=null){
                    ...
                    if(ORDERSTATUS.valueOf(status).equals(ORDERSTATUS.SUBMITTED)){
                        order.status =   ORDERSTATUS.valueOf(status);
                        order.timestamp = System.nanoTime();
                        order.update();
                    } else if(ORDERSTATUS.valueOf(status).equals(ORDERSTATUS.PROCESSED)){
                         order.status =   ORDERSTATUS.valueOf(status);
                         order.timestamp = System.nanoTime();
                         order.update();
                         try {
                     ...
                     }   else if(ORDERSTATUS.valueOf(status).equals(ORDERSTATUS.DISCARDED)){
                          if(order.status.equals(ORDERSTATUS.PROCESSED) || order.status.equals(ORDERSTATUS.SUBMITTED) ){
                              synchronized (order) {
 
                                  List<OrderTireLine> lines = OrderTireLine.findByOrder(order.id);
                                  Iterator<OrderTireLine> lineIterator = lines.iterator();
 
                                  lineIterator = lines.iterator();
                                  while (lineIterator.hasNext()) {
                                      OrderTireLine line = lineIterator.next();
                                      Tire tire = line.tire;
                                      synchronized (line) {
                                          //Update TIRE
                                          tire.stockUnitsAvailable = tire.stockUnitsAvailable + line.quantity;
                                          tire.update();
 
                                          StockLog log = new StockLog();
                                          log.tire = tire;
                                          log.timestamp = System.currentTimeMillis();
                                          log.quantityDiff = line.quantity;
                                          log.currentUnits = tire.stockUnitsAvailable;
                                          log.user = getCachedUser();
                                          log.save();
                                      }
                              ...
                              try {
                                  NotificationManager.queueNotification(order.user.email,
                                          "Norpneu - Ordem de encomenda " + order.getFriendlyId() + " anulada",
                                          "A ordem de encomenda #" + order.getFriendlyId() + " foi anulada. Se considerar que esta acção não foi de encontro às suas expectativas ou necessitar de mais esclarecimentos, por favor contacte a NorPneu. Obrigado!");
                              } catch (QueueException e) {
                                  Logger.error("Mail notification failed", e);
                              }
                          }   else {
                              result.put("status", false);
                              result.put("message", "Ordem " + orderid + " só pode ser anulada nos estados Confirmada ou Processada");
                              return ok(result);
                          }
                     }
