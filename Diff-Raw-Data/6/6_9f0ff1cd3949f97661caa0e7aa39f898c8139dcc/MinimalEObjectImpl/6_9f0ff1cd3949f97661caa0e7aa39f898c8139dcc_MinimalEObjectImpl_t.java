...
import org.eclipse.acceleo.common.internal.utils.compatibility.OCLVersion;
 ...
 public class MinimalEObjectImpl extends BasicEObjectImpl implements EObject, EStructuralFeature.Internal.DynamicValueHolder {
 	...
 	/**
 	 * The {@link #eFlags bit flag} for indicating that a dynamic {@link #eContainer() container} field is
 	 * allocated. A derived implementation wishing to allocate a static container field should override
 	 * {@link #eInternalContainer()} and {@link #eBasicSetContainer(InternalEObject)}.
 	 */
 	private static final int CONTAINER = 1 << 1;
 
 	/**
 	 * The {@link #eFlags bit flag} for indicating that a dynamic {@link #eAdapters() adapters} field is
 	 * allocated. A derived implementation wishing to allocate a static adapter field should override
 	 * {@link #eBasicHasAdapters()}, {@link #eBasicAdapterArray()}, and
 	 * {@link #eBasicSetAdapterArray(Adapter[])}.
 	 */
 	 ...
 	 * A derived implementation wishing to allocate a static class field should override
 	 * {@link #eDynamicClass()} and {@link #eSetClass(EClass)}.
 	 */
 	private static final int CLASS = 1 << 4;
 
 	/**
 	 * The {@link #eFlags bit flag} for indicating that a dynamic {@link #eSettings() settings} field is
 	 * allocated. A derived implementation wishing to allocate a static settings field should override
 	 * {@link #eHasSettings()}, {@link #eBasicSettings()}, and {@link #eBasicSetSettings(Object[])}.
 	 */
 	private static final int SETTING = 1 << 5;
 
 	/**
 	 * The {@link #eFlags bit flag} for indicating that a dynamic {@link #eProxyURI() proxy URI} field is
 	 * allocated. A derived implementation wishing to allocate a static proxy URI field should override
 	 * {@link #eIsProxy()}, {@link #eProxyURI()}, and {@link #eSetProxyURI(URI)}.
 	 */
 	private static final int PROXY = 1 << 6;
 
 	/**
 	 * The {@link #eFlags bit flag} for indicating that a dynamic {@link #eResource() resource} field is
 	 * allocated. A derived implementation wishing to allocate a static resource field should override
 	 * {@link #eDirectResource()} and {@link #eSetDirectResource(Resource.Internal)}.
 	 */
 	 ...
 	 */
 	private static final int FIELD_MASK = CONTAINER | ADAPTER | ADAPTER_LISTENER | CLASS | SETTING | PROXY
 			| RESOURCE;
 
 	/**
 	 * A bit flag field with bits for {@link #NO_DELIVER}, {@link #CONTAINER}, {@link #ADAPTER},
 	 * {@link #ADAPTER_LISTENER}, {@link #CLASS}, {@link #SETTING}, {@link #PROXY}, and {@link #RESOURCE}. The
 	 * high order 16 bits are used to represent the {@link #eContainerFeatureID() container feature ID}, a
 	 * derived implementation wishing to allocate a static container feature ID field should override
 	...
 	public EList<Adapter> eAdapters() {
 		class ArrayDelegatingAdapterList extends ArrayDelegatingEList<Adapter> implements BasicNotifierImpl.EObservableAdapterList {
 			...
 			protected void didRemove(int index, Adapter oldObject) {
 				...
 				if (eDeliver()) {
 					Notification notification = new NotificationImpl(Notification.REMOVING_ADAPTER,
 							oldObject, null, index) {
 						@Override
 						...
 						if (listeners[i] == listener) {
 							if (listeners.length == 1) {
 								...
 								if (i != newListeners.length) {
 									System.arraycopy(listeners, i + 1, newListeners, i, newListeners.length
 											- i);
 								}
 								listeners = newListeners;
 							}
 	...
 	protected EStructuralFeature.Internal.DynamicValueHolder eSettings() {
 		if (!eHasSettings()) {
 			...
 			if (size != 0) {
 				eBasicSetSettings(size == 0 ? new Object[0] : new Object[size]);
 			}
 		}
