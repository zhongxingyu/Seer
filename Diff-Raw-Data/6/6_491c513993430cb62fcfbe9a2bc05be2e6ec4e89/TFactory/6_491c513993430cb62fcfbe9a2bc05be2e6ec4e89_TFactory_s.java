 ...
 public class TFactory extends Observable {
 	 ...
 	 * @author Chris Welden
 	 */
 	public static TRoute findShortestPath(List<TStation> stations) {
 		TRoute path = new TRoute();
 		if (stations.size() == 1) {
 			ArrayList<TStop> stops = new ArrayList<TStop>();
 			stops.add(stations.get(0).getStops().get(0));
 			return new TRoute(stops);
 		}
 		else if (stations.size() > 1) {
 			
 			for (int i = 0; i < stations.size() - 1; i++) {
 				path = new TRoute(path, findShortestPath(stations.get(i), stations.get(i+1)));
 			}
 		}
 	...
 	private static ArrayList<TStation> getTransferStations(){
 		...
 		for (TStation station : getAllStations()) {
 			if (station.getLineData().size() > 1 || station.getStops().size() > 2)
