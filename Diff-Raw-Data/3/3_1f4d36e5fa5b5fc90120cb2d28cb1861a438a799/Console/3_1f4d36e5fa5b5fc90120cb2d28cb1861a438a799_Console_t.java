 ...
 public class Console extends Job{
 	...
 	public Console(Integer PID, Pipe stdERR) {
 		...
 		try{
 			...
 			this.frame.setIconImage(new ImageIcon
 					(this.getClass().getResource("/gui/ico/icon64.png")).getImage());
 		} catch (Exception e) {
 	...
 	public void closeTheConsole(boolean closeShell){
 		...
 		try {
 			((PermPipe)this.getStdIn()).closeForReal();
 			((PermPipe)this.getStdOut()).closeForReal();
 			((PermPipe)this.getStdErr()).closeForReal();
 		} catch (IOException e) {
 			//Who cares, we are closing
