...
import static org.auraframework.impl.util.UserAgent.*;
 ...
 public class BrowserInfo {
     ...
     /**
      * See if this user agent String is for a known mobile client. This is a comprehensive
      * check for hundreds of possible mobile clients.
      *
      * @see Browser#isBrowserMobile() if a quicker test of all supported and many common
      * unsupported clients is sufficient
      *
     ...
     public static boolean isMobileClient(String userAgent) {
         ...
         // It is easier to maintain with no third party dependencies and ~6x faster.
         if (userAgent != null) {
             String ua = userAgent.toLowerCase();
             // check against the common ones first (mainstream users get faster perf)
     ...
     /* Browser checks
     *
     * The pre-182 checks (isBrowserXXX()) match against both the old 4 digit
     * browserTypes and the new 8 digit ones too. The parseBrowser() method will
     * always return the new type from 182 onward, but 4 digit ones may be in the
     * DB or a cookie, or be hard-coded in various places.
     *
     * The 182+ checks (isBrowser(XXX)) will check and match against only new 8 digit
     * browserTypes. Please use these for any new functionality, as they require
     * much less upkeep and rework as new browsers come out and they keep the
     * API list shorter.
     */
    ...
    /**
     * Determines if this browser type is a match for the UserAgent,
     * and if the browser version refers to a version in the given range.
     *
    ...
    /**
     * This checks if we want to treat the browser as a Webkit browser. This mostly
     * maps to &quot;Does it have 'AppleWebkit' in the user agent?&quot; but there can
     * be exceptions to that to form the correct behavior for supported clients.
     *
     * Before 182 this returned true for Safari, Chrome, and Chromeframe and some
     * other common desktop and mobile Webkit clients, but false for many Android
    ...
    public boolean isBrowserWebkit() {
        return isBrowser(CHROME)
                || isBrowser(SAFARI)
                || isBrowser(ANDROID_WEBKIT)
                || isBrowser(OTHER_WEBKIT)
    ...
    /* Platform checks */
     ...
     /*
     * Please use caution when adding new platform methods - each user agent
     * can only map to 1 platform currently. This causes problems with our
     * use patterns. Some examples:
     *
     * If Linux, Mac and Windows are platforms, why not IOS?
     * Because we need to track iPad and iPhone separately.
     *
     * What about iPad Mini? Is that a new platform or the same as iPad?
     *
     * What about iPod Touch? Same OS, screen, events and browser as iPhone,
     * but different platform?
     *
     * So Android is a Platform...what about the difference between
     * Android phones and tablets? What about Netbooks that have notebook-
     * like screens and capabilities but run Android?
     *
     */
    ...
    public boolean isPlatformModernWin() {
        // 1 found use as of 182
        // TODO: deprecate and merge with isPlatformWin() - we don't have many surviving /Win95/95/Me/3.1 users
    ...
    /**
     * Identifies if a browser is advertising itself as being mobile in its user agent String.
     * This will return true if keywords such as mobile, phone, tablet, or touch are present,
     * of it the browser is one that is only available as a mobile version, such as Blackberry.
     *
     * This will detect all supported mobile clients and many common unsupported ones.
     *
     * This does not necessarily indicate that the user should get our mobile or Touch UI,
     * as some mobile browsers are very limited in compatibility or resolution so they should
     * get nothing or plain text; and others are netbooks or tablets with keyboards that may
     * as well be laptops for all they can do.
     *
     * @see Browser#isMobileClient(String) for a more comprehensive but slower check of known
     * user agents
     *
    ...
    /**
     * See if this Browser is a known mobile client. This is a comprehensive check for
     * hundreds of possible mobile clients.
     *
     * @see Browser#isBrowserMobile() if a quicker test of all supported and many common
     * unsupported clients is sufficient
     *
    ...
    public boolean isMobileClient() {
        ...
        // It is easier to maintain with no third party dependencies and ~7x faster.
    ...
    public boolean isSmartPhoneClient() {
        return this.isPlatformIPhone()
                || this.isPlatformMobileWebkit()
                || this.isPlatformAndroid()
                || this.isPlatformAndroid22OrGreater()
     ...
     *
     * Note: As of 182 this is actually
     *    &quot;Is it an iPad or an Android 2.2 or higher tablet?&quot;
     *
     * This definition will likely expand as new tablets with other
    ...
    public boolean isTabletClient() {
         ...
         * - Windows RT (not detected)
         * - Kindle (rejected because it is a mobile clients)
    ...
    /**
     * Returns the {@link FormFactor} for this instance, usually based on
     * the user agent String passed in to the constructor of this Browser.
     *
     * Be cautious with assumptions based on the return value.
     * For example:
     * - Phones may not always have lower resolutions than tablets,
     * - Tablets may sometimes have cellular capabilities,
     * - Phones may sometimes have cellular disabled,
     * - Some desktops and laptops now come with touch screens
     * - Windows may some day be the most popular tablet OS.
     * - Some tablets can dock and become desktops
     *
