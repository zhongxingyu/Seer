 ...
 public class Face {
 	...
 	public boolean detectFeatures() throws Exception {
		...
		featureClassifiers.put(FeatureType.NOSE,     ClassifierType.NOSE);
 
 		//20% for a feature is quite big, let's change it to about 5% of the picture.
		int featureSize = Math.round(image.height() * 0.05f);
 		...
 		for (HashMap.Entry<FeatureType, ClassifierType> entry : featureClassifiers.entrySet()) {
 			...
 			if (featureRects.capacity() >= 1) {
 				...
 				for(int i = 0; i < total; i ++){
					Log.i("Features", "Feature " + featureType + " "+i+" is found in rect " + IplImageHelper.rectFromCvRect(featureRects.position(i)).flattenToString());
 				}
