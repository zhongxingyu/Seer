...
public class NodeFeatureInverse implements Feature1Inverse {
	...
	public NodeFeatureInverse (String nodetype, int annotationIndex, int characterIndex,
			 int numFeature)
	{
	...
	{	
		return node.getNodeType().equals(this.nodetype) || (this.nodetype.equals("RootLogical") && 
	...
	{
		return !node.getCharacterValue(this.characterIndex);
	}

	@Override
	public double getValue(Node node, int annotationIndex)
	{
		return getAnnotationTestValue(annotationIndex) && isNodeObservable(node) && 
				getCharacterTestValue(node)? 1.0 : 0.0;
	}
