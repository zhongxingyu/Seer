 ...
 */
 ...
 public class VizMapAttrTab extends VizMapTab {
 	 ...
 	 *    @param    c
 	 */
 	public VizMapAttrTab(VizMapUI mainUI, JTabbedPane tabContainer, int tabIndex,
 	                     VisualMappingManager VMM, byte type) {
 	...
 	protected void drawDefault(Object defaultObj) {
 		...
 		this.defaultValueDisplayer = ValueDisplayer.getDisplayFor(mainUIDialog, getName(),
 		                                                          defaultObj);
 		...
 		defPanel.setBorder(BorderFactory.createTitledBorder(defBorder, "Default",
 		...
 		public void actionPerformed(ActionEvent e) {
 			...
 			try {
 				...
 				JOptionPane.showMessageDialog(mainUIDialog,
 				                              ...
 				                              "Invalid Mapper", JOptionPane.ERROR_MESSAGE);
 	...
 	private String getCalculatorName(Calculator c) {
 		...
 		while (true) {
 			...
 			if (ret == null) {
 				...
 				JOptionPane.showMessageDialog(mainUIDialog,
 				                              "Please enter a name for the calculator.",
 				                              "Calculator Name Input Error",
 			...
 			int alt = JOptionPane.showConfirmDialog(mainUIDialog,
 			                                        "Calculator with name " + ret
 			                                        + " already exists,\nrename to " + newName
 			                                        + " okay?", "Duplicate calculator name",
 	...
 	private class RmCalcListener extends AbstractAction {
 		public void actionPerformed(ActionEvent e) {
 			...
 			if ((conflicts.size() != 0)
 			    && !((conflicts.size() == 1) && (((Vector) conflicts.get(0)).size() == 2))) {
 				...
 				int conf = JOptionPane.showConfirmDialog(mainUIDialog, errPane,
 				                                         "Calculator In Use",
 				                                         JOptionPane.YES_NO_OPTION,
