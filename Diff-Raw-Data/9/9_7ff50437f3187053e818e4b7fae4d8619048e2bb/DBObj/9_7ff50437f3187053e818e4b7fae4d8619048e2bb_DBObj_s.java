 ...
 public class DBObj implements Database {
 	...
 	public boolean isIdInRoomDB(String id) {
		String q = "select max(date) from log where id='"+id+"';";
 		String rs = execute_query(q,"date",false,true);
		q = "select event from log where date="+rs+" and id='"+id+"';";
		rs = execute_query(q,"event", false, true);
		Integer ev = new Integer(rs);
		if (ev==1)
 			return true;
 		return false;
 	}
