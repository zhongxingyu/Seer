 ...
 public class ManageDataversesPage extends VDCBaseBean implements Serializable {
     ...
     @EJB
     RoleRequestServiceLocal roleRequestService;
     @EJB
     ...
     private void checkForwardPaging(DataverseGrouping grouping, int amountToIncrement) {
         if (amountToIncrement < 0) { // reversing
         } else if (amountToIncrement == 0) { // on last page
             FacesContext context = FacesContext.getCurrentInstance();
             context.renderResponse();
         } else if (grouping.getFirstRow() + amountToIncrement < grouping.getDataModelRowCount()) {
             grouping.setFirstRow(grouping.getFirstRow() + amountToIncrement);
             grouping.setPageAction(true);
             grouping.getManagedDataModel();
         } else {
             checkForwardPaging(grouping, (amountToIncrement - 10));
         }
     }
     ...
     private String getActivityClass(Long activity) {
         ...
         switch (activity.intValue()) {
             case 0:
                 activityClass = "activitylevelicon al-0";
                 break;
             case 1:
                 activityClass = "activitylevelicon al-1";
                 break;
             case 2:
                 activityClass = "activitylevelicon al-2";
                 break;
             case 3:
                 activityClass = "activitylevelicon al-3";
                 break;
             case 4:
                 activityClass = "activitylevelicon al-4";
                 break;
             case 5:
                 activityClass = "activitylevelicon al-5";
                 break;
         }
