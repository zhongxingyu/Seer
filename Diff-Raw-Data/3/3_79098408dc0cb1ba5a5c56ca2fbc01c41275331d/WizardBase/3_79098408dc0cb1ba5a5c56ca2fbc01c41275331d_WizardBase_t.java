 ...
 public class WizardBase implements IRegistrationListener, SelectionListener, ControlListener, DisposeListener
 {
     // HOLDS ALL TASKS ADDED TO THIS INVOCATION...THIS IS NOT A CACHE
     private transient LinkedHashMap availableTasks = null;
 
     // HOLDS COLLECTION OF TASK LABELS IN SEQUENCE...INDEX OF LABEL MATCHES CORRESPONDING INDEX OF TASK ID IN vTaskIDs
     ...
     private transient String sWizardID = "org.eclipse.birt.framework.taskwizard.prototype.SampleWizard";
     ...
     public void addTask(String sTaskID)
     {
         ...
         String sLabel = TasksManager.instance().getTask(sTaskID).getDisplayLabel(Locale.getDefault());
     ...
     public Shell createPopupContainer()
     {
         ...
         if(shellPopup == null || shellPopup.isDisposed())
         {
             ...
             shellPopup.setLocation((shell.getLocation().x + shell.getSize().x), (shell.getLocation().y + 20));
         }
         return shellPopup;
     }
     ...
     public WizardBase()
     {
         // Initialize tasks manager...so that extensions get processed if they haven't already
         TasksManager.instance();
         // Initialize instance variables
         availableTasks = new LinkedHashMap();
         vTaskLabels = new Vector();
     ...
     /**
      * Clears the cached task instances. This can be used between invocations when a wizard instance
      * is being reused in an application. Calling this will cause fresh instances of tasks to be fetched
      * from the TasksManager when the wizard is invoked.
      */
     ...
     public void displayException(Throwable t)
     {
         ...
         new ErrorDialog("WizardBase: Exception Encountered", "The following exception was encountered in WizardBase", t);
     }
     
     /**
      * Displays the errors in a common Error Display UI mechanism. Also displayed are possible solutions to the problems.
      * The user can also be given the option of switching to a different task where the fix needs to be made. (This is not implemented yet).
      * 
      * @param sErrors Array of error strings
      * @param sFixes Array of strings listing possible solutions to above errors
      * @param sTaskIDs Array of task IDs which the user can switch to. The appropriate task labels should be indicated in the solutions to
      * allow users to make the connection 
      * @param context Updated IWizardContext instance...this instance will include the erroneous settings
      * @param errorHints Object array that will be passed to the target task...which can be used to indicate specific problems or to customize
     ...
     private void placeComponents()
     {
         ...
         cmpTaskContainer.setToolTipText("This is a canvas!");
         cmpTaskContainer.setLayoutData(new GridData(GridData.FILL_BOTH));
     }
 
     /**
      * Notification method called by the org.eclipse.birt.frameworks.taskwizard.interfaces.TasksManager instance when a new ITask instance is successfully registered. Default
     ...
     {
         // DO NOTHING...NEWLY REGISTERED TASKS DO NOT AFFECT AN EXISTING WIZARD IN MOST CASES
     }
      ...
      * Default behavior is to do nothing. This can be overridden by individual wizards to handle deregistration of tasks currently available in the wizard.
      * 
      * @param sTaskID The ID for the deregistered task
      */
     public void taskDeregistered(String sTaskID)
     {
         // DO NOTHING...IF EXISTING TASKS ARE DEREGISTERED, THEY WOULD NOT DIRECTLY AFFECT A RUNNING WIZARD...
         // HOWEVER, CUSTOM WIZARDS MAY INTERPRET SUCH A DEREGISTRATION AS INDICATING NON-AVAILABILITY OF SOME
     ...
     public void widgetSelected(SelectionEvent e)
     {
         if(e.getSource() instanceof Button)
         {
             ...
             else
             {
                 ...
                 int i = vTaskIDs.indexOf(this.sCurrentActiveTask);
                 buttonpanel.setButtonEnabled(ButtonPanel.NEXT, (i < vTaskIDs.size() - 1) ? true : false);
                 buttonpanel.setButtonEnabled(ButtonPanel.BACK, (i > 0) ? true : false);
             }
         }
     }
 ...
 class TaskList extends Composite implements DisposeListener
 {
     ...
     private transient Color clrActive = new Color(Display.getCurrent(), 230, 230, 230);
