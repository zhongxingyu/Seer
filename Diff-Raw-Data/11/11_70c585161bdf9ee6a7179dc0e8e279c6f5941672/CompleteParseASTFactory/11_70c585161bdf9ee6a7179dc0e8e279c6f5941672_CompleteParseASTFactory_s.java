 ...
 public class CompleteParseASTFactory extends BaseASTFactory implements
       IASTFactory {
    protected static final char[]                     THIS                                 = new char[] {
          't', 'h', 'i', 's'                                                              };
    ...
    /*
     * Adds a reference to a reference list Overrides an existing reference if it
     * has the same name and offset
     */
    ...
    /*
     * Test if the provided list is a valid parameter list Parameters are list of
     * TypeInfos
     */
    ...
    protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
          char[] name, List references, boolean throwOnError, LookupType lookup)
          throws ASTSemanticException {
       return lookupQualifiedName(startingScope, name, ITypeInfo.t_any, null, 0,
    ...
    protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
          ...
          throws ASTSemanticException {
       return lookupQualifiedName(startingScope, name, references, throwOnError,
             LookupType.UNQUALIFIED);
    }
          ...
          case 1:
             ...
             if (result != null) {
                if (lookup == LookupType.FORPARENTSCOPE
                   ...
                   if (args != null)
                      ((ITemplateFactory) startingScope).pushTemplateId(result,
                            args);
                   else
                      ((ITemplateFactory) startingScope).pushSymbol(result);
                }
                if (references != null)
                   addReference(references, createReference(result, image, name
                         .getStartOffset()));
                ...
                if (startingScope.getASTExtension() != null
                   ...
                   if (((IASTCodeScope) startingScope.getASTExtension()
                      ...
                      IASTClassSpecifier classSpecifier = ((IASTMethod) ((IASTCodeScope) startingScope
                            ...
                            .getContainingFunction()).getOwnerClassSpecifier();
             ...
             for (;;) {
                ...
                if (templateArgLists != null && templateArgLists[idx] != null) {
                   if (t != last && t.getNext().getType() == IToken.tLT)
                      t = TokenFactory.consumeTemplateIdArguments(t.getNext(),
                            last);
                }
                ...
                try {
                   while (!(result instanceof IContainerSymbol)) {
                      if (result.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
                         ...
                         if (throwOnError)
                            handleProblem(IProblem.SEMANTIC_NAME_NOT_PROVIDED,
                                  image, t.getOffset(), t.getEndOffset(), t
                                        .getLineNumber(), true);
                         else
                            return null;
                      }
                   ...
                   args = (templateArgLists != null) ? getTemplateArgList(templateArgLists[idx])
                         : null;
                   if (t == last)
                      result = lookupElement((IContainerSymbol) result, image,
     ...
     *      org.eclipse.cdt.core.parser.ITokenDuple, int, int)
     */
     ...
     *      boolean, org.eclipse.cdt.core.parser.ITokenDuple, int, int)
     */
    public IASTUsingDeclaration createUsingDeclaration(IASTScope scope,
       ...
       if (name.getSegmentCount() > 1) {
          ...
          try {
             ...
             handleProblem(e.createProblemID(), name.getLastToken()
                   .getCharImage(), startingOffset, endingOffset, startingLine,
                   true);
          }
       } else
          try {
       ...
       ASTUsingDeclaration using = new ASTUsingDeclaration(scope, name
             .getLastToken().getCharImage(), endResult.getReferencedSymbols(),
             isTypeName, startingOffset, startingLine, endingOffset, endingLine,
             references, filename, name.getStartOffset(), name.getEndOffset(),
             name.getLineNumber());
    ...
    public IASTClassSpecifier createClassSpecifier(IASTScope scope,
             ...
             if (currentScopeSymbol == null)
                handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND,
                      containerSymbolName.toCharArray(), containerSymbolName
                            .getFirstToken().getOffset(), containerSymbolName
                            .getLastToken().getEndOffset(), containerSymbolName
       ...
       if (!CharArrayUtils.equals(newSymbolName, EMPTY_STRING) && !isTemplateId) {
          ...
          if (classSymbol != null && classSymbol.getType() != pstType) {
             ...
             handleProblem(IProblem.SEMANTIC_INVALID_OVERLOAD, newSymbolName,
                   nameOffset, nameEndOffset, nameLine, isError);
          }
       }
       ...
       try {
          ...
          if (e2.reason == ParserSymbolTableException.r_InvalidOverload)
             handleProblem(e2.createProblemID(), newSymbolName, nameOffset,
                   nameEndOffset, nameLine, false); // to fix 65569 for this
                                                    // patch
          else
             handleProblem(e2.createProblemID(), newSymbolName, nameOffset,
       ...
       ASTClassSpecifier classSpecifier = new ASTClassSpecifier(newSymbol, kind,
             type, access, startingOffset, startingLine, nameOffset,
    ...
    protected void handleProblem(int id, char[] attribute)
          throws ASTSemanticException {
       handleProblem(null, id, attribute, problemStartOffset, problemEndOffset,
             problemLineNumber, true);
    }
 
    protected void handleProblem(IASTScope scope, int id, char[] attribute)
          throws ASTSemanticException {
       handleProblem(scope, id, attribute, problemStartOffset, problemEndOffset,
             problemLineNumber, true);
    }
    ...
    public void addBaseSpecifier(IASTClassSpecifier astClassSpec,
       ...
       if (symbol == null)
          handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND, parentClassName
                ...
                parentClassName.getEndOffset(), parentClassName.getLineNumber(),
                true);
       if (symbol instanceof ITemplateSymbol
          ...
          handleProblem(IProblem.SEMANTIC_INVALID_TEMPLATE_ARGUMENT,
                parentClassName.toCharArray(), parentClassName.getStartOffset(),
                parentClassName.getEndOffset(), parentClassName.getLineNumber(),
                true);
    ...
    protected IASTReference createReference(ISymbol symbol,
          char[] referenceElementName, int offset) throws ASTSemanticException {
       ...
       if (symbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)
          ...
          if (declaration instanceof IASTNamespaceDefinition)
             ...
             try {
                namespace = ((IASTNamespaceAlias) declaration).getNamespace();
 ...
 
       {
          if (symbol.getContainingSymbol().getType() == ITypeInfo.t_class
             ...
             return new ASTFieldReference(offset,
                   (IASTField) (definition != null ? definition : declaration));
    ...
    public IASTEnumerationSpecifier createEnumerationSpecifier(IASTScope scope,
       ...
       IDerivableContainerSymbol classSymbol = pst.newDerivableContainerSymbol(
             name, pstType);
       try {
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.cdt.core.parser.ast.IASTFactory#createExpression(org.eclipse.cdt.core.parser.ast.IASTExpression.Kind,
     *      org.eclipse.cdt.core.parser.ast.IASTExpression,
     *      org.eclipse.cdt.core.parser.ast.IASTExpression,
     *      org.eclipse.cdt.core.parser.ast.IASTExpression, java.lang.String,
    ...
    public IASTExpression createExpression(IASTScope scope, Kind kind,
       ...
       List references = new ArrayList();
       ISymbol symbol = getExpressionSymbol(scope, kind, lhs, rhs, idExpression,
             references);
 
       // Try to figure out the result that this expression evaluates to
       ExpressionResult expressionResult = getExpressionResultType(scope, kind,
             lhs, rhs, thirdExpression, typeId, literal, symbol, extra);
       ...
       if (extension.overrideCreateExpressionMethod())
          expression = (ASTExpression) extension.createExpression(scope, kind,
                lhs, rhs, thirdExpression, typeId, idExpression, literal,
    ...
    private boolean createConstructorReference(ISymbol classSymbol,
       ...
       if (constructor != null) {
          ...
          try {
             reference = createReference(constructor, duple.toCharArray(), duple
                   .getStartOffset());
    ...
    private ISymbol getExpressionSymbol(IASTScope scope, Kind kind,
       ...
       if (idExpression != null)
          symbol = lookupQualifiedName(startingScope, idExpression, references,
                false);
    ...
    protected ITypeInfo usualArithmeticConversions(IASTScope scope,
          ...
          if (kind == IASTExpression.Kind.MULTIPLICATIVE_MULTIPLY
             ...
             if (!lhs.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator))
                handleProblem(scope, IProblem.SEMANTIC_INVALID_CONVERSION_TYPE,
                      ...
                      lhsExp.getEndingOffset(), lhsExp.getStartingLine(), true);
             if (!rhs.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator))
                handleProblem(scope, IProblem.SEMANTIC_INVALID_CONVERSION_TYPE,
                      null, rhsExp.getStartingOffset(),
                      rhsExp.getEndingOffset(), rhsExp.getStartingLine(), true);
            ...
            //  the left operand is a pointer to a completely defined object
             ...
             if (!((isLhsPointer && (isIntegralType(rhs, isRhsPointer) || rhs
                ...
                if (!(isIntegralType(lhs, isLhsPointer) || lhs.isType(
                      ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
                   handleProblem(scope,
                         IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null, lhsExp
                               .getStartingOffset(), lhsExp.getEndingOffset(),
                         lhsExp.getStartingLine(), true);
                if (!(isIntegralType(rhs, isRhsPointer) || rhs.isType(
                      ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
                   handleProblem(scope,
                         IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null, rhsExp
                               .getStartingOffset(), rhsExp.getEndingOffset(),
                         rhsExp.getStartingLine(), true);
             }
       ...
       if ((lhs.checkBit(ITypeInfo.isUnsigned) && lhs.checkBit(ITypeInfo.isLong))
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.cdt.core.parser.ast.IASTFactory#createConstructorMemberInitializer(org.eclipse.cdt.core.parser.ITokenDuple,
     *      org.eclipse.cdt.core.parser.ast.IASTExpression)
     */
    public IASTConstructorMemberInitializer createConstructorMemberInitializer(
       ...
       if (duple != null) {
          try {
             ...
             //TODO: resolve unresolved references in the expressionList using
       ...
       return new ASTConstructorMemberInitializer(expressionList,
             ...
             duple == null ? 0 : duple.getFirstToken().getOffset(), references,
             requireReferenceResolution);
    }
 
    /*
     * (non-Javadoc)
     * 
     * @see org.eclipse.cdt.core.parser.ast.IASTFactory#createSimpleTypeSpecifier(org.eclipse.cdt.core.parser.ast.IASTSimpleTypeSpecifier.Type,
    ...
    public IASTSimpleTypeSpecifier createSimpleTypeSpecifier(IASTScope scope,
       ...
       if (extension.overrideCreateSimpleTypeSpecifierMethod(kind))
          return extension.createSimpleTypeSpecifier(pst, scope, kind, typeName,
                isShort, isLong, isSigned, isUnsigned, isTypename, isComplex,
                isImaginary, isGlobal, extensionParms);
          ...
          for (;;) {
             ...
             if (argLists != null && argLists[idx] != null) {
                if (current != last && current.getNext().getType() == IToken.tLT)
                   current = TokenFactory.consumeTemplateIdArguments(current
                         .getNext(), last);
             }
             ...
             while (!(typeSymbol instanceof IContainerSymbol)) {
                if (typeSymbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
                   ...
                   handleProblem(IProblem.SEMANTIC_INVALID_TYPE, image, current
                         .getOffset(), current.getEndOffset(), current
                         .getLineNumber(), true);
                }
             }
             try {
                ...
                if (typeSymbol != null) {
                   addReference(references, createReference(typeSymbol, image,
                         offset));
                   if (argLists != null && argLists[idx] != null) {
                   ...
                   handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND, image,
                         current.getOffset(), current.getEndOffset(), current
                               .getLineNumber(), true);
             } catch (ParserSymbolTableException e) {
                handleProblem(e.createProblemID(), image, typeName
                      .getStartOffset(), typeName.getEndOffset(), typeName
                      .getLineNumber(), true);
             }
          }
     ...
     *      org.eclipse.cdt.core.parser.ast.IASTExceptionSpecification, boolean,
     *      boolean, boolean, int, int,
     *      org.eclipse.cdt.core.parser.ast.IASTTemplate)
     */
    public IASTFunction createFunction(IASTScope scope, ITokenDuple name,
       ...
       if (name.getSegmentCount() > 1) {
          ...
          if (symbol instanceof IContainerSymbol)
             ...
             parentScope = ((IDeferredTemplateInstance) symbol).getTemplate()
                   .getTemplatedSymbol();
          ...
          if ((parentScope != null)
             ...
             if (parentScope.getASTExtension().getPrimaryDeclaration() instanceof IASTElaboratedTypeSpecifier) {
                //we are trying to define a member of a class for which we only
                // have a forward declaration
                handleProblem(scope, IProblem.SEMANTICS_RELATED, name
                      .toCharArray(), startOffset, nameEndOffset, startLine,
                      true);
             }
             ...
             return createMethod(methodParentScope, newName, parameters,
                   ...
                   startOffset, startLine, newName.getFirstToken().getOffset(),
                   nameEndOffset, nameLine, ownerTemplate, isConst, isVolatile,
                   isVirtual, isExplicit, isPureVirtual,
                   ASTAccessVisibility.PRIVATE, constructorChain, references,
       ...
       for (int i = 0; i < size; i++) {
          ...
          if (param.getSymbol() == null)
             handleProblem(IProblem.SEMANTICS_RELATED, param.getNameCharArray(),
                   param.getNameOffset(), param.getEndingOffset(), param
       ...
       functionDeclaration = (IParameterizedSymbol) lookupQualifiedName(
             ownerScope, name.extractNameFromTemplateId(), ITypeInfo.t_function,
             functionParameters, 0, null, false, LookupType.FORDEFINITION);
       ...
       if (previouslyDeclared == false || isFunctionDefinition) {
          try {
             ...
             handleProblem(e.createProblemID(), name.toCharArray(), nameOffset,
                   nameEndOffset, nameLine, true);
          }
       } else {
       ...
       ASTFunction function = new ASTFunction(symbol, nameEndOffset, parameters,
             returnType, exception, startOffset, startLine, nameOffset,
             nameLine, ownerTemplate, references, previouslyDeclared,
             hasFunctionTryBlock, isFriend, filename);
    ...
    protected void setParameters(IParameterizedSymbol symbol, List references,
       ...
       for (int i = 0; i < size; i++) {
          setParameter(symbol, (IASTParameterDeclaration) params.get(i), true,
                references);
       }
    }
    ...
    protected ITypeInfo getParameterTypeInfo(IASTAbstractDeclaration absDecl)
       ...
       if (absDecl.getTypeSpecifier() instanceof IASTSimpleTypeSpecifier) {
          ...
          provider.setType(ITypeInfo.t_type);
          provider
                .setTypeSymbol(((ASTClassSpecifier) absDecl.getTypeSpecifier())
                      .getSymbol());
    ...
    protected void setParameter(IParameterizedSymbol symbol,
          IASTAbstractDeclaration absDecl, boolean isParameter, List references)
          throws ASTSemanticException {
       ...
       if (absDecl.getTypeSpecifier() instanceof IASTSimpleTypeSpecifier) {
          if (((IASTSimpleTypeSpecifier) absDecl.getTypeSpecifier()).getType() == IASTSimpleTypeSpecifier.Type.CLASS_OR_TYPENAME) {
             xrefSymbol = ((ASTSimpleTypeSpecifier) absDecl.getTypeSpecifier())
                   .getSymbol();
    ...
    public IASTMethod createMethod(IASTScope scope, ITokenDuple name,
       ...
       return createMethod(scope, name, parameters, returnType, exception,
             ...
             nameEndOffset, nameLine, ownerTemplate, isConst, isVolatile,
             isVirtual, isExplicit, isPureVirtual, visibility, constructorChain,
             null, isFunctionDefinition, hasFunctionTryBlock,
             hasVariableArguments);
    }
    ...
    public IASTMethod createMethod(IASTScope scope, ITokenDuple nameDuple,
          List parameters, IASTAbstractDeclaration returnType,
          IASTExceptionSpecification exception, boolean isInline,
          boolean isFriend, boolean isStatic, int startOffset, int startingLine,
          int nameOffset, int nameEndOffset, int nameLine,
          IASTTemplate ownerTemplate, boolean isConst, boolean isVolatile,
          boolean isVirtual, boolean isExplicit, boolean isPureVirtual,
          ASTAccessVisibility visibility, List constructorChain,
       ...
       if (returnType.getTypeSpecifier() == null) {
          ...
          if (CharArrayUtils.equals(parentName, methodName)) {
             isConstructor = true;
                ...
                && CharArrayUtils.equals(methodName, 1, methodName.length - 1,
       ...
       if (isFunctionDefinition || isFriend) {
          ...
          functionDeclaration = (IParameterizedSymbol) lookupQualifiedName(
                ownerScope, nameDuple, isConstructor ? ITypeInfo.t_constructor
                      : ITypeInfo.t_function, functionParameters, null, false,
          ...
          if (isFriend) {
             if (functionDeclaration != null
                ...
                handleProblem(IProblem.SEMANTIC_ILLFORMED_FRIEND, nameDuple
                      .toCharArray(), nameDuple.getStartOffset(), nameDuple
                      .getEndOffset(), nameDuple.getLineNumber(), true);
             }
             ...
             visibility = ((IASTMethod) (functionDeclaration.getASTExtension()
       ...
       try {
          ...
          handleProblem(e.createProblemID(), nameDuple.toCharArray(), nameDuple
                .getStartOffset(), nameDuple.getEndOffset(), nameDuple
                .getLineNumber(), true);
       }
       ...
       ASTMethod method = new ASTMethod(symbol, parameters, returnType,
             exception, startOffset, startingLine, nameOffset, nameEndOffset,
             nameLine, ownerTemplate, references, previouslyDeclared,
             isConstructor, isDestructor, isPureVirtual, visibility,
             constructorChain, hasFunctionTryBlock, isFriend, filename);
             ...
             if (initializer.getNameCharArray().length > 0
                ...
                lookupQualifiedName(container, realInitializer
                      ...
                      realInitializer.getNameOffset(), realInitializer
                            .getReferences(), false, LookupType.QUALIFIED);
    ...
    public IASTVariable createVariable(IASTScope scope, ITokenDuple name,
          ...
          int nameEndOffset, int nameLine, IASTExpression constructorExpression,
       ...
       if (name.getSegmentCount() > 1) {
          ...
          if ((parentScope != null)
             ...
             return createField(fieldParentScope, newName, isAuto,
                   initializerClause, bitfieldExpression, abstractDeclaration,
                   isMutable, isExtern, isRegister, isStatic, startingOffset,
                   startingLine, newName.getStartOffset(), nameEndOffset,
                   nameLine, constructorExpression, ASTAccessVisibility.PRIVATE,
                   references, fn);
          }
       }
       ...
       setVariableTypeInfoBits(isAuto, abstractDeclaration, isMutable, isExtern,
             isRegister, isStatic, newSymbol);
       ...
       if (!isStatic) {
          ...
          if (variableDeclaration != null
             ...
             ITypeInfo varInfo = variableDeclaration.getTypeInfo().getFinalType(
                   provider);
       ...
       try {
          ...
          handleProblem(e.createProblemID(),
                name.getFirstToken().getCharImage(), name.getFirstToken()
                      .getOffset(), name.getFirstToken().getEndOffset(), name
                      .getFirstToken().getLineNumber(), true);
       }
    ...
    protected void addDesignatorReferences(ASTInitializerClause clause) {
       if (clause.getKind() == IASTInitializerClause.Kind.DESIGNATED_INITIALIZER_LIST
          ...
          ITypeInfo currentTypeInfo = TypeInfoProvider.newTypeInfo(currentSymbol
                .getTypeInfo());
          ...
          for (int i = 0; i < size; i++) {
             ...
             if (designator.getKind() == IASTDesignator.DesignatorKind.FIELD) {
                ...
                try {
                   lookup = ((IContainerSymbol) currentSymbol).lookup(designator
                         .fieldNameCharArray());
                } catch (ParserSymbolTableException e) {
                   break;
                }
                ...
                try {
                   if (lookup != null)
                      addReference(clause.getReferences(), createReference(
                            lookup, designator.fieldNameCharArray(), designator
                                  .fieldOffset()));
    ...
    public IASTField createField(IASTScope scope, ITokenDuple name,
          ...
          int startingOffset, int startingLine, int nameOffset,
          int nameEndOffset, int nameLine, IASTExpression constructorExpression,
          ASTAccessVisibility visibility, char[] fn) throws ASTSemanticException {
       return createField(scope, name, isAuto, initializerClause,
             ...
             nameEndOffset, nameLine, constructorExpression, visibility, null,
             fn);
    }
    ...
    public IASTField createField(IASTScope scope, ITokenDuple name,
       ...
       if (!isStatic && !image.equals(EMPTY_STRING)) {
          ...
          if (fieldDeclaration != null
             ...
             ITypeInfo newInfo = newSymbol.getTypeInfo().getFinalType(provider);
             ITypeInfo fieldInfo = fieldDeclaration.getTypeInfo().getFinalType(
                   provider);
             ...
             if (newInfo.equals(fieldInfo)) {
                ...
                // set the definition visibility = declaration visibility
                visibility = ((IASTField) fieldDeclaration.getASTExtension()
                      .getPrimaryDeclaration()).getVisiblity();
             }
             provider.returnTypeInfo(newInfo);
       ...
       try {
          ...
          if (name == null)
             ...
             handleProblem(e.createProblemID(), image, name.getStartOffset(),
                   name.getEndOffset(), name.getLineNumber(), true);
       }
    ...
    public IASTTemplateDeclaration createTemplateDeclaration(IASTScope scope,
       ...
       ASTTemplateDeclaration ast = new ASTTemplateDeclaration(template, scope,
             templateParameters, filename);
    ...
    public IASTTemplateParameter createTemplateParameter(ParamKind kind,
       ...
       if (kind == ParamKind.TEMPLATE_LIST) {
          ...
          for (int i = 0; i < size; i++) {
             ...
             try {
                ...
                handleProblem(e.createProblemID(), param.getNameCharArray(),
                      param.getStartingOffset(), param.getEndingOffset(), param
                            .getStartingLine(), true); //$NON-NLS-1$
             }
          }
          symbol = template;
       } else {
          ...
          if (kind == ParamKind.CLASS || kind == ParamKind.TYPENAME) {
             ...
             symbol = cloneSimpleTypeSymbol(
                   ((ASTParameterDeclaration) parameter).getNameCharArray(),
                   parameter, null);
       ...
       ASTTemplateParameter ast = new ASTTemplateParameter(symbol, defaultValue,
    ...
    public IASTTypedefDeclaration createTypedef(IASTScope scope, char[] name,
          IASTAbstractDeclaration mapping, int startingOffset, int startingLine,
          int nameOffset, int nameEndOffset, int nameLine, char[] fn)
          throws ASTSemanticException {
       IContainerSymbol containerSymbol = scopeToSymbol(scope);
    ...
    public IASTElaboratedTypeSpecifier createElaboratedTypeSpecifier(
          ...
          int startingOffset, int startingLine, int endOffset, int endingLine,
          boolean isForewardDecl, boolean isFriend) throws ASTSemanticException {
       setProblemInfo(name);
       IContainerSymbol currentScopeSymbol = scopeToSymbol(scope);
       ...
       if (name.getSegmentCount() != 1) // qualified name
       {
          ...
          if (currentScopeSymbol == null)
             handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND, containerSymbolName
                   .toCharArray(), containerSymbolName.getFirstToken()
                   .getOffset(), containerSymbolName.getLastToken()
                   .getEndOffset(), containerSymbolName.getLastToken()
                   .getLineNumber(), true);
          nameToken = name.getLastSegment().getFirstToken();
       }
       //template-id
             ...
             if (isFriend && isForewardDecl
                ...
                if (!isForewardDecl)
                   checkSymbol = currentScopeSymbol.elaboratedLookup(pstType,
                         newSymbolName); // for using - goes outside scope with
                                         // filter
                else
                   checkSymbol = currentScopeSymbol.lookupMemberForDefinition(
                         newSymbolName, pstType); // for declaring - stays inside
                                                  // scope with filter
             }
          } catch (ParserSymbolTableException e) {
       ...
       if (scope instanceof IASTTemplateInstantiation) {
          ...
          ASTElaboratedTypeSpecifier elab = new ASTElaboratedTypeSpecifier(
                checkSymbol, kind, startingOffset, startingLine, name
                      ...
                      .getEndOffset(), name.getLastToken().getLineNumber(),
                endOffset, endingLine, references, isForewardDecl, filename);
    ...
    public IASTNamespaceAlias createNamespaceAlias(IASTScope scope,
       ...
       if (namespaceSymbol.getType() != ITypeInfo.t_namespace)
          handleProblem(IProblem.SEMANTIC_INVALID_OVERLOAD, alias.toCharArray(),
                startingOffset, endOffset, startingLine, true);
    ...
    public IASTScope getDeclaratorScope(IASTScope scope, ITokenDuple duple) {
       if (duple != null && duple.getSegmentCount() > 1) {
          ...
          try {
             symbol = lookupQualifiedName(ownerScope,
                   duple.getLeadingSegments(), null, false,
    ...
    public IASTParameterDeclaration createParameterDeclaration(boolean isConst,
       ...
       return new ASTParameterDeclaration(null, isConst, isVolatile,
             ...
             startingLine, nameOffset, nameEndOffset, nameLine, endingOffset,
             endingLine, filename);
    }
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.cdt.core.parser.ast.IASTFactory#createTypeId(org.eclipse.cdt.core.parser.ast.IASTSimpleTypeSpecifier.Type,
    ...
    public IASTTypeId createTypeId(IASTScope scope, Type kind, boolean isConst,
          boolean isVolatile, boolean isShort, boolean isLong, boolean isSigned,
          boolean isUnsigned, boolean isTypename, ITokenDuple name,
          List pointerOps, List arrayMods, char[] completeSignature)
          throws ASTSemanticException {
       if (kind != Type.CLASS_OR_TYPENAME) {
    ...
    protected ISymbol createSymbolForTypeId(IASTScope scope, IASTTypeId id)
       ...
       if (result.getType() == ITypeInfo.t_type) {
          ISymbol typeSymbol = lookupQualifiedName(scopeToSymbol(scope), typeId
                .getTokenDuple(), refs, true);
    ...
    public void signalEndOfClassSpecifier(IASTClassSpecifier astClassSpecifier) {
       ...
       for (int i = 0; i < size; i++) {
          ...
          try {
             lookupQualifiedName(duple.getScope(), duple.getName(), references,
                   false);
          } catch (ASTSemanticException ase) {
          }
       }
    ...
    public IASTInitializerClause createInitializerClause(IASTScope scope,
          IASTInitializerClause.Kind kind, IASTExpression assignmentExpression,
             ...
             if (expression.getExpressionKind() == IASTExpression.Kind.ID_EXPRESSION
                ...
                try {
                   ISymbol symbol = getExpressionSymbol(scope, expression
                         .getExpressionKind(), expression.getLHSExpression(),
                         expression.getRHSExpression(), null, null);
                   if (symbol == null)
                      return null;
                ...
                if (ownerExpression.getExpressionKind()
                   ...
                   try {
                      s = lookupQualifiedName(getSearchScope(ownerExpression
                            .getExpressionKind(), ownerExpression
                            .getLHSExpression(), scopeToSymbol(scope)), duple,
                            null, false);
                   } catch (ASTSemanticException e) {
                      return null;
                   }
    ...
    public IASTNode expressionToMostPreciseASTNode(IASTScope scope,
       ...
       if (expression.getExpressionKind() == IASTExpression.Kind.ID_EXPRESSION) {
          if (expression instanceof ASTExpression) {
             try {
                return lookupSymbolInContext(
                      scope,
                      ((ASTIdExpression) expression).getIdExpressionTokenDuple(),
                      null);
             } catch (ASTNotImplementedException e) {
                //	            	assert false : e;
             }
