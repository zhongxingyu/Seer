 ...
 public class RailDroid extends Mob {
     ...
     public void tick() {
         ...
         if (!xcenterIsh) ur = false;
         if (!xcenterIsh) dr = false;
 
         if (!ycenterIsh) lr = false;
         if (!ycenterIsh) rr = false;
 ...
 //                dir = 0;
                 ...
                 if (res < lWeight) dir = 1;
                 else if (res < uWeight) dir = 2;
                 else if (res < rWeight) dir = 3;
 ...
 //        level.getTile(xt, yt)
     ...
     public Bitmap getSprite() {
         if (lDir == 1) return Art.slave[1][1];
         if (lDir == 2) return Art.slave[0][1];
         if (lDir == 3) return Art.slave[1][0];
         if (lDir == 4) return Art.slave[0][0];
     ...
     public void handleCollision(Entity entity, double xa, double ya) {
         ...
         if (entity instanceof RailDroid) {
             ...
             if (other.carrying != carrying && carrying) {
                 if (lDir == 1 && other.pos.x > pos.x - 4) return;
                 if (lDir == 2 && other.pos.y > pos.y - 4) return;
                 if (lDir == 3 && other.pos.x < pos.x + 4) return;
                 if (lDir == 4 && other.pos.y < pos.y + 4) return;
 
                 if (other.lDir == 1 && pos.x > other.pos.x - 4) return;
                 if (other.lDir == 2 && pos.y > other.pos.y - 4) return;
                 if (other.lDir == 3 && pos.x < other.pos.x + 4) return;
                 if (other.lDir == 4 && pos.y < other.pos.y + 4) return;
