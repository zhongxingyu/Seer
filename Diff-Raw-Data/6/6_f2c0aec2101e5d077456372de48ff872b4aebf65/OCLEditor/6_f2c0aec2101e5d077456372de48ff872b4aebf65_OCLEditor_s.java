 ...
 public class OCLEditor
 	...
 	protected static class OCLEditorDocument extends DefaultStyledDocument {
 		...
 		protected void checkEditLocation(int offset, int length)
 			...
 			while (!eAffected.isLeaf()) {
 				eAffected =
 					eAffected.getElement(eAffected.getElementIndex(offset));
 			}
 
 			// check all affected elements
 			while ((eAffected != null)
 				&& ((eAffected.getStartOffset() <= offset)
 					|| (eAffected.getEndOffset() >= offset + length))) {
 				if (eAffected.getAttributes().getAttribute("allowEdit")
 					== Boolean.FALSE) {
 					// do not allow edit
 		...
 		protected Element getNeighbouringElement(Element e) {
 			...
 			while ((eTemp != null) && (!eTemp.isLeaf())) {
 				eTemp =
 					eTemp.getElement(eTemp.getElementIndex(e.getEndOffset()));
 			}
 			...
 			if (column == 0) {
 				if (m_oclemModel != null) {
 					ConstraintRepresentation cr =
 						m_oclemModel.getConstraintAt(row);
 
 					if (cr != null) {
 						try {
 							cr.setName(
 								value.toString(),
 								(OCLEditor) m_wrocle.get());
 						} catch (IllegalArgumentException iae) {
 							JOptionPane.showMessageDialog(
 								null,
 								"Invalid name: " + iae.getMessage(),
 								"Error",
 								JOptionPane.ERROR_MESSAGE);
 						} catch (IllegalStateException ise) {
 							JOptionPane.showMessageDialog(
 								null,
 								"Couldn't set name: " + ise.getMessage(),
 								"Error",
 								JOptionPane.ERROR_MESSAGE);
 						}
 					}
 	...
 	public OCLEditor() {
 		...
 		m_jtConstraintList.getSelectionModel().setSelectionMode(
 	...
 	public void setEditMode(boolean fEditMode) {
 		...
 		if (!m_fInEditMode) {
 			...
 			m_jbEdit.setIcon(
 				new javax.swing.ImageIcon(
 					getClass().getResource(
 						"/tudresden/ocl/images/Cancel16.gif")));
 			m_jbEdit.setToolTipText(
 				"Click to cancel editing without saving changes");
 			...
 			if (m_fCreatedFreshConstraint && !m_fConstraintChanged) {
 			...
 			m_jbEdit.setIcon(
 				new javax.swing.ImageIcon(
 					getClass().getResource(
 						"/tudresden/ocl/images/Edit16.gif")));
 			m_jbEdit.setToolTipText(
 				"Click to edit the currently selected constraint");
 	 ...
 	 */
 	public OclTree parseAndCheckConstraint(
 		final String sConstraint,
 		final ModelFacade mfFacade)
 		throws OclParserException, java.io.IOException, OclTypeException {
 		// Parse and syntax check
 		final OclTree tree = OclTree.createTree(sConstraint, mfFacade);
 
 		// Type check
 		if ((tree != null) && (m_fDoTypeCheck)) {
 	...
 	public List splitConstraint(OclTree ocltConstraint) {
 		...
 		class Splitter extends DepthFirstAdapter {
 			public void caseAConstraint(AConstraint node) {
 				...
 				for (Iterator i = node.getConstraintBody().iterator();
 					i.hasNext();
 					) {
 	...
 	protected void setEditorText(ConstraintRepresentation cr) {
 		...
 		if (cr != null) {
 			if (m_fNoContextEdit) {
 				((OCLEditorDocument) m_jtpConstraintEditor.getDocument())
 					.stopChecking();
 				...
 				int nPrePos = s.indexOf("pre");
 				if (nPrePos == -1) {
 					nPrePos = s.length();
 				}
 				int nPostPos = s.indexOf("post");
 				if (nPostPos == -1) {
 					nPostPos = s.length();
 				}
 
 				int nContextEnd =
 					Math.min(Math.min(nInvPos, nPrePos), nPostPos);
 
 				m_jtpConstraintEditor.setCharacterAttributes(
 					s_sasNoEditText,
 					true);
 				m_jtpConstraintEditor.replaceSelection(
 					s.substring(0, nContextEnd));
 
 				m_jtpConstraintEditor.setCharacterAttributes(
 					s_sasNormalText,
 					true);
 				m_jtpConstraintEditor.replaceSelection(
 					s.substring(nContextEnd));
 			} else {
 	...
 	private void initComponents() { //GEN-BEGIN:initComponents
 		...
 		m_jbNew.setIcon(
 			new javax.swing.ImageIcon(
 				getClass().getResource("/tudresden/ocl/images/New16.gif")));
 		...
 		m_jbRemove.setIcon(
 			new javax.swing.ImageIcon(
 				getClass().getResource("/tudresden/ocl/images/Delete16.gif")));
 		m_jbRemove.setToolTipText(
 			"Click to remove the currently selected constraint");
 		...
 		m_jbEdit.setIcon(
 			new javax.swing.ImageIcon(
 				getClass().getResource("/tudresden/ocl/images/Edit16.gif")));
 		m_jbEdit.setToolTipText(
 			"Click to edit the currently selected constraint");
 		...
 		m_jbSaveEditResult.setIcon(
 			new javax.swing.ImageIcon(
 				getClass().getResource("/tudresden/ocl/images/Ok16.gif")));
 		m_jbSaveEditResult.setToolTipText(
 			"Check OCL syntax and save constraint into model");
 		...
 		m_jbPreferences.setIcon(
 			new javax.swing.ImageIcon(
 				getClass().getResource(
 					"/tudresden/ocl/images/Preferences16.gif")));
 		m_jbPreferences.setToolTipText(
 			"Click to inspect and modify OCL editor preferences.");
 		...
 		m_jcbQuickBar.setToolTipText(
 			"Check to see the syntax assistant toolbar");
 		...
 		m_jpEditorPanel.setBorder(
 			new javax.swing.border.TitledBorder("Edit constraint"));
 		...
 		m_jpConstraintListPane.setPreferredSize(
 			new java.awt.Dimension(100, 37));
 		m_jtConstraintList.setToolTipText(
 			"Lists all constraints. Selecting a constraint, shows its body in the right hand preview pane. Clicking twice on a constraint allows editing the constraint's name.");
 		...
 		m_jpPreviewGroup.setBorder(
 			new javax.swing.border.TitledBorder("Preview"));
 	...
 	private void onEditButton(
 		java.awt.event.ActionEvent evt) { //GEN-FIRST:event_onEditButton
 		// Toggle edit mode
 		setEditMode(!isInEditMode());
 	} //GEN-LAST:event_onEditButton
 
 	private void onNewConstraintButton(
 		java.awt.event.ActionEvent evt) {
 		//GEN-FIRST:event_onNewConstraintButton
 		if (m_oclemModel != null) {
 	...
 	private void onPreferencesButton(
 		java.awt.event.ActionEvent evt) { //GEN-FIRST:event_onPreferencesButton
 	...
 	private void onCaretUpdate(
 		javax.swing.event.CaretEvent evt) { //GEN-FIRST:event_onCaretUpdate
 		...
 		SwingUtilities.invokeLater(new Runnable() {
 			public void run() {
 				...
 				Element eAffected =
 					m_jtpConstraintEditor.getDocument().getDefaultRootElement();
 				while (!eAffected.isLeaf()) {
 					eAffected =
 						eAffected.getElement(
 							eAffected.getElementIndex(e.getDot()));
 				}
 
 				if (eAffected.getAttributes().getAttribute("isField")
 					== Boolean.TRUE) {
 	...
 	private void onQuickBarButton(
 		java.awt.event.ActionEvent evt) { //GEN-FIRST:event_onQuickBarButton
 	...
 	/**
 	  ...
 	  */
 	private void onSubmitConstraintButton(
 		java.awt.event.ActionEvent evt) {
 		//GEN-FIRST:event_onSubmitConstraintButton
 		if (m_oclemModel != null) {
 			int nIdx = m_jtConstraintList.getSelectedRow();
 			...
 			if (nIdx != -1) {
 				try {
 					m_oclemModel.getConstraintAt(nIdx).setData(
 						m_jtpConstraintEditor.getText(),
 						this);
 
 					// Stop editing if successful
 					m_fConstraintChanged = true;
 					// somehow wasn't set correctly before...
 					setEditMode(false);
 					...
 					String sWrappedMessage = wrapMessage(ope.getMessage(), 50);
 					...
 					String sWrappedMessage = wrapMessage(ote.getMessage(), 50);
 	...
 	/**
 	  ...
 	  */
 	private void onRemoveConstraintButton(
 		java.awt.event.ActionEvent evt) {
 		//GEN-FIRST:event_onRemoveConstraintButton
 		if (m_oclemModel != null) {
 			int nIdx = m_jtConstraintList.getSelectedRow();
 	...
 	public void valueChanged(final javax.swing.event.ListSelectionEvent p1) {
 		// due to bug #4700643 in JDK 1.4 this statement checking the validity
 	...
 	/**
 	  ...
 	  */
 	public void constraintAdded(ConstraintChangeEvent cce) {
 		m_jtConstraintList.setRowSelectionInterval(
 			cce.getIndex(),
 			cce.getIndex());
 	}
