  ...
  */
 public class CramDeckOptions extends PreferenceActivity implements
 		OnSharedPreferenceChangeListener {
 
 	private JSONObject mDeck;
 	private Collection mCol;
 
 	private String[] dynExamples = new String[]{null, "{'search'=\"is:new\", 'resched'=False, 'steps'=\"1\", 'order'=5}",
 	...
 	public class DeckPreferenceHack implements SharedPreferences {
 		...
 		public class Editor implements SharedPreferences.Editor {
 			...
 			public boolean commit() {
 				Log.d(AnkiDroidApp.TAG,
 						"CramDeckOptions - commit() changes back to database");
 				...
 				for (OnSharedPreferenceChangeListener listener : listeners) {
 					listener.onSharedPreferenceChanged(DeckPreferenceHack.this,
 							null);
 				}
 		...
 		public String getString(String key, String defValue) {
 			Log.d(this.getClass().toString(), String.format(
 					"getString(key=%s, defValue=%s)", key, defValue));
 	...
 	protected void updateSummaries() {
 		...
 		for (String key : mPref.mValues.keySet()) {
 			...
 			if (!mPref.mSummaries.containsKey(key)) {
 				...
 				mPref.mSummaries.put(key, s != null ? pref.getSummary()
 						.toString() : null);
 			}
