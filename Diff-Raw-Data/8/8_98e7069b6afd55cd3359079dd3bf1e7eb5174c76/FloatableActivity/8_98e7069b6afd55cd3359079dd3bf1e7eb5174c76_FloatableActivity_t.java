 ...
 public abstract class FloatableActivity extends Activity {
 	...
 	private void dismissCurrentActivity() {
 		...
 		if (!mDontFinishActivity) {
			...
			// FIXME: VERY VERY dirty hack. Following two statements ensures that
			// the activity goes into the STOPPED state.
			// Without following code, the activity sometimes stays at the PAUSED
			// state and enters into the STOPPED state only after the top activity
			// has changed.
 	...
 	public boolean isInFloatingMode() {
 		return mFloatingWindow != null && !mIsSwitchingToFloatingMode;
 	}
 	...
 	/* package */void handleAttachToFloatingWindow(FloatingWindow w) {
 		...
 		mIsSwitchingToFloatingMode = false; // the switch is completed
