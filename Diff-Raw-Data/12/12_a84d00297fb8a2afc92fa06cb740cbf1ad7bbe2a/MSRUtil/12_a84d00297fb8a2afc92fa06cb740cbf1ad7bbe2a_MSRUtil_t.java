 ...
 public class MSRUtil {
 	...
 	public ActionForward onFirstTimeLoad(ActionMapping mapping, HttpServletRequest request) throws BizLogicException
 	{
 			...
 			try {
 				//request.setAttribute(Constants.GRIDGROUPERMAP, GridGrouperUtil.getStemAndGroups());
 				request.setAttribute(Constants.GRIDGROUPERSTEMS, GridGrouperUtil.getStems());
 			} catch (Exception e) {
 				e.printStackTrace();
 			}
 			request.setAttribute(Constants.GRID_PRIVILEGE_DATA_LIST_ONLOAD, CPGridGrouperPrivilege.getActiveCPGridGrouperPrivileges((List<CPGridGrouperPrivilege>)session.getAttribute(Constants.GRID_ROW_ID_OBJECT_BEAN_MAP)));
 	...
 	public ActionForward setAJAXResponse(HttpServletRequest request, HttpServletResponse response, String cpOperation) throws IOException, JSONException,BizLogicException, CSException  
 	{
 		...
 		else if ((Constants.OPERATION_ADD_GRID_PRIVILEGE).equals(cpOperation))
 		{
 			addGridPrivilegeMethod(request, response);
 		}
 		else if ((Constants.OPERATION_REMOVE_GRID_PRIVILEGE).equals(cpOperation))
 		{
 			removeGridPrivilegeMethod(request, response);
 		}
 		return null;
 	}
 	 ...
 	 * @throws IOException
 	 */
 	private void addGridPrivilegeMethod(HttpServletRequest request, HttpServletResponse response) throws BizLogicException, JSONException, CSException, IOException {
 		final HttpSession session = request.getSession();
 		final AssignPrivilegePageBizLogic apBizLogic = getAssignPrivilegePageBizLogic();
 		final String pageOf = (String) request.getParameter(Constants.PAGE_OF);
 		final String stemName= request.getParameter(Constants.SELECTED_GRID_GROUPER_STEM_NAME);
 		final String groupName= request.getParameter(Constants.SELECTED_GRID_GROUPER_GROUP_NAME);
 		final String actionIds = (String) request.getParameter(Constants.SELECTED_ACTION_IDS);
 		final String operation = (String) request.getParameter(Constants.OPERATION);
 		final String userIds = (String) request.getParameter(Constants.SELECTED_USER_IDS);
 		final String roleId = (String) request.getParameter(Constants.SELECTED_ROLE_IDS);
 		...
 		Set<NameValueBean> actionBeanList = new HashSet<NameValueBean>(apBizLogic
 				.getPrivilegesNameValueBeanList(Arrays.asList(actionIds.split(","))));
 		boolean found = false;
 		for(CPGridGrouperPrivilege gridP : gridPrivilegeList){
 			if(gridP.getGroupName().equals(groupName) && gridP.getStemName().equals(stemName)){
 				gridP.setPrivileges(actionBeanList);
 				gridP.setStatus(CPGridGrouperPrivilege.ACTIVE);
 				gridP.setRoleId(roleId);
 				found = true;
 				break;
 			}
 		}
 		if(!found){
 			gridPrivilegeList.add(new CPGridGrouperPrivilege(groupName, stemName, actionBeanList,roleId));
 		}
 		session.setAttribute(Constants.GRID_ROW_ID_OBJECT_BEAN_MAP, gridPrivilegeList);
 		response.flushBuffer();
 		response.getWriter().write(StringUtils.join(CPGridGrouperPrivilege.getActiveCPGridGrouperPrivileges(gridPrivilegeList), ";;;"));
 	}
    ...
    private void removeGridPrivilegeMethod(HttpServletRequest request, HttpServletResponse response) throws BizLogicException, JSONException, CSException, IOException {
 		...
 		for(NameValueBean nv : items){
 			for(CPGridGrouperPrivilege gridP : gridPrivilegeList){
 				if(gridP.getGroupDisplayString().equals(nv.getValue()) && gridP.getStemName().equals(nv.getName())){
 					if(gridP.getId() == null){
 						gridPrivilegeList.remove(gridP);
 					}else{
 						gridP.setStatus(CPGridGrouperPrivilege.DELETE);
 					}
 					break;
 				}
 			}
