 ...
 public class ComparativeValueChunkProcessor extends AbstractChunkProcessor {
 	...
 	/**
 	 * 3 times n[...than...]
 	   lengths 0.50.6+ times <bodies>
 	   ca .3.5 times length of <throat>
        13 times {pinnately} {lobed}
        12 times shape[{shape~list~pinnately~lobed~or~divided}]
        4 times longer than wide
 	 	content: 0.50.6+ times a[type[bodies]]
 	 	subjects2
 	 */
 	private LinkedList<DescriptionTreatmentElement> processComparativeValue(Chunk content,
 		...
 		for(Chunk chunk : onAndAfterTimes) {
 			//1.52.5 times n[{longer} than (throat)]
 		...
 		if(containsThan) {
			...
			processingContextState.setClauseModifierContraint(comparisonChunk.getTerminalsText());
 			...
 			//characters:13 times {pinnately} {lobed}
