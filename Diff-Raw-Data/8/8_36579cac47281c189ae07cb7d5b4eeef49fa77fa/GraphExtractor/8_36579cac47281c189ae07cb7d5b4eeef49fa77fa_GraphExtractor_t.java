 ...
 public class GraphExtractor implements ResultSetExtractor
 {
 	...
 	{
 		// FIXME: totally ugly, but the query has fixed column names (and needs
 		// its own column aliasing)
 		// TableAccessStrategyFactory wants a corpus selection strategy
 		// solution: build AnnisNodes with API and refactor SqlGenerator to
 		// accept GROUP BY nodes
 	...
 	{
 		ParameterizedSingleColumnRowMapper<String> planRowMapper = new ParameterizedSingleColumnRowMapper<String>();
 	...
 	public List<AnnotationGraph> queryAnnotationGraph(
 			JdbcTemplate jdbcTemplate, List<Long> corpusList, int nodeCount,
 			...
 			Map<Long, Properties> corpusProperties)
 	{
 		return (List<AnnotationGraph>) jdbcTemplate.query(
 				getContextQuery(corpusList, left, right, limit, offset,
 						nodeCount, corpusProperties), this);
 	}
 
 	public List<AnnotationGraph> queryAnnotationGraph(
 			JdbcTemplate jdbcTemplate, long textID)
 	{
 		return (List<AnnotationGraph>) jdbcTemplate.query(getTextQuery(textID),
 				this);
 	}
 
 	private IslandPolicies getMostRestrictivePolicy(List<Long> corpora,
 			Map<Long, Properties> props)
 	{
 		...
 		for (Long l : corpora)
 		{
 			if (props.get(l) != null)
 			{
 				IslandPolicies newPolicy = IslandPolicies.valueOf(props.get(l)
 						.getProperty("islands-policy", defaultIslandsPolicy));
 	...
 	public String getContextQuery(List<Long> corpusList, int left, int right,
 			long limit, long offset, int nodeCount,
 			Map<Long, Properties> corpusProperties)
 	{
 
 		IslandPolicies islandsPolicy = getMostRestrictivePolicy(corpusList,
 				corpusProperties);
 		...
 		if (corpusList != null)
 		{
 			...
 			sb.append(corpusList.isEmpty() ? "NULL" : StringUtils.join(
 					corpusList, ","));
 			sb.append(") AND\n");
 		}
 		sb.append("\t(\n");
 			...
 			throws SQLException, DataAccessException
 	{
 		...
 		while (resultSet.next())
 		{
 			...
 			// add annotation data
 			Annotation nodeAnnotation = nodeAnnotationRowMapper.mapRow(
 					resultSet, rowNum);
 			if (nodeAnnotation != null)
 			{
 				node.addNodeAnnotation(nodeAnnotation);
 			}
 			Annotation edgeAnnotation = edgeAnnotationRowMapper.mapRow(
 					resultSet, rowNum);
 			...
 			Map<Long, AnnisNode> nodeById)
 	{
 		...
 		AnnisNode parent = parentEdge != null ? parentEdge.getDestination()
 				: null;
 		// log.debug("looking for node with rank.pre = " + pre + "; found: " +
 		// parent);
