 ...
 public class PlayboardRenderer {
 				...
 				for (int row = 0; row < boxes[col].length; row++) {
 					if (!renderAll) {
 						if (!currentWord.checkInWord(col, row)
 								&& (reset != null) && !reset.checkInWord(col, row)) {
 							continue;
 						}
 					}
 					int x = col * boxSize;
 					int y = row * boxSize;
 					this.drawBox(canvas, x, y, row, col, scale,
 							boxes[col][row], currentWord);
 				}
 			}
 	...
 	private void drawBox(Canvas canvas, int x, int y, int row, int col,
 			float scale, Box box, Word currentWord) {
 		int boxSize = (int) (BOX_SIZE * scale);
 		...
 		if (box == null) {
 			...
 			if (box.isAcross() | box.isDown()) {
 				canvas.drawText(Integer.toString(box.getClueNumber()), x + 2, y
 			...
 			canvas.drawText(Character.toString(box.getResponse()), x
 					+ (boxSize / 2), y + (int) (letterTextSize * 1.25),
 					thisLetter);
 		}
 		Paint boxColor = ((highlight.across == col) && (highlight.down == row)) && currentWord != null ? this.currentLetterBox
 				: this.blackLine;
 	...
 	public Bitmap drawWord() {
 		...
 		for (int i = 0; i < word.length; i++) {
 			int x = (int) (i * boxSize);
 			int y = 0;
 			this.drawBox(canvas, x, y, word[i].down, word[i].across,
 					this.logicalDensity, boxes[i], null);
 		}
 	...
 	public float fitTo(int shortDimension) {
 		...
 		double newScale = (double) shortDimension
 				/ (double) (this.board.getBoxes().length) / (double) BOX_SIZE;
