 ...
 /**
     ...
     Changes by Ramesh Reddy
      - Added a custom parser on top of the Bean Shell parser to interpret
      certain commands in a specific way. Look at (acceptLine() method)
 */
 ...
 {
     private final static String	CUT = "Cut"; //$NON-NLS-1$
     private final static String	COPY = "Copy"; //$NON-NLS-1$
     private final static String	PASTE =	"Paste"; //$NON-NLS-1$
     ...
     // Ramesh Changes
     private SimpleParser mmparser = null;
     public void setParser(SimpleParser p) {
         this.mmparser=p;
     }    
 	...
 	public JConsole( InputStream cin, OutputStream cout )  
 	{
 		...
 		Font font = new	Font("Monospaced",Font.PLAIN,14); //$NON-NLS-1$
 		...
 		// create popup	menu
 		menu = new JPopupMenu("JConsole	Menu"); //$NON-NLS-1$
 		...
 		if ( outPipe ==	null ) {
 			...
 			try {
 				...
 				print("Console internal	error (1)...", Color.red);//$NON-NLS-1$
 			}
 		}
 		...
 		if ( inPipe == null ) {
 			...
 			} catch ( IOException e ) { print("Console internal error: "+e); }//$NON-NLS-1$
 		}
 		...
 		switch ( e.getKeyCode()	) 
 		{
 			...
 			case ( KeyEvent.VK_U ):	// clear line
 				if ( (e.getModifiers() & InputEvent.CTRL_MASK) > 0 ) {
 					replaceRange( "", cmdStart, textLength());//$NON-NLS-1$
 					histLine = 0;
 					e.consume();
 				}
 				break;
 			...
 			default:
 				...
 				*/
 				if (e.paramString().indexOf("Backspace") != -1)//$NON-NLS-1$
 				{ 
 				  if (text.getCaretPosition() <= cmdStart) {
 						e.consume();
 						break;
     ...
     private String getCmd() {
 		...
 		try {
 			...
 			// should not happen
 			System.out.println("Internal JConsole Error: "+e); //$NON-NLS-1$
 		}
 		return s;
     }
 	...
 	String ZEROS = "000";//$NON-NLS-1$
 
 	private	void acceptLine( String	line ) 
 	{
 	    // Ramesh Changes
         if (mmparser != null) {	    
             line = mmparser.convert(line);      
         }
         // Changes end.
 		...
 		for(int i=0; i<lineLength; i++) {  
 				...
 				buf.append("\\u" + val);//$NON-NLS-1$
 		} 
 		line = buf.toString();
 		// End unicode patch
 
 		if (outPipe == null )
 			print("Console internal	error: cannot output ...", Color.red);//$NON-NLS-1$
 		else
 			try {                
 				...
 				outPipe	= null;
 				throw new RuntimeException("Console pipe broken..."); //$NON-NLS-1$
 			}
 		//text.repaint();
 	}
 ...
 //    }
 	...
 	private	void inPipeWatcher() throws IOException	{
 		...
 		println("Console: Input	closed...");//$NON-NLS-1$
 	}
 	...
 	public String toString() {
 		return "BeanShell console";//$NON-NLS-1$
 	}
     ...
     public void	propertyChange(PropertyChangeEvent event) {
 		if (event.getPropertyName().equals("lookAndFeel")) {//$NON-NLS-1$
