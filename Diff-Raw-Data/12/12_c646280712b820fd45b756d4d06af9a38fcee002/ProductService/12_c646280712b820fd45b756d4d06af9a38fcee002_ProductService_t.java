 ...
 public class ProductService {
 	...
 	static final transient Logger logger = Logger.getLogger(ProductService.class);
 	...
 	public Product createProduct(RequestContext ctx, User user, String barcode) {
 		return createProduct((HttpServletRequest) ctx.getExternalContext().getNativeRequest(), user, barcode);
 	}
 
 	public Product createProduct(HttpServletRequest request, User user, String barcode) {
 	...
 	public Product createProduct(RequestContext ctx, User user) {
 		return createProduct((HttpServletRequest) ctx.getExternalContext().getNativeRequest(), user);
 	}
 	...
 	public boolean isActionPerformedByAdmin() {
 		try {
 			return User.ROLE_ADMIN.equals(userService.getCurrentUser().getRole());
 	...
 	public Product save(Product p) {
 		if ((!userService.getCurrentUser().equals(p.getUser()) || p.isApproved()) && !isActionPerformedByAdmin()) {
 	...
 	public void delete(Product p) {
 		if (!isActionPerformedByAdmin()) {
 			logger.error("NEGALIMA trinti svetimų produktų! Produktas: " + p.getName() + " vartotojas: " + userService.getCurrentUser().getEmail());
 			throw new IllegalArgumentException("NEGALIMA trinti svetimų produktų!");
 		}
 	...
 	public List<Product> getList(ProductSearchCriteria criteria) {
 		String qString = "select p from Product p " + criteria.getWhereClause() + "order by " + criteria.getOrderByClause();
 
 		criteria.setTotalItems(((Long) criteria.setParameters(em.createQuery("select count(*) from Product p " + criteria.getWhereClause()))
 				.getSingleResult()).longValue());
 		return criteria.setParameters(em.createQuery(qString)).setFirstResult(criteria.getPage() * criteria.getItemsPerPage()).setMaxResults(
 				criteria.getItemsPerPage()).getResultList();
 	}
 	...
 	public List<Product> getProductList(ProductSearchCriteria criteria) {
 		...
 		String qString = "SELECT * " + criteria.getRankingFilter().getRankClause() + " FROM " + tables + criteria.getWhereClause() + " order by "
 				+ criteria.getRankingFilter().getRankColumn() + criteria.getOrderByClause();
 		criteria.setTotalItems(((BigInteger) criteria.setParameters(em.createNativeQuery("select count(*) FROM " + tables + criteria.getWhereClause()))
 				.getSingleResult()).longValue());
 		...
 		logger.debug("first: " + criteria.getPage() * criteria.getItemsPerPage());
 
 		return criteria.setParameters(em.createNativeQuery(qString, Product.class)).setFirstResult(criteria.getPage() * criteria.getItemsPerPage())
 	...
 	public Collection<String> findCompanies(String q) {
 		return em.createQuery(
 				...
 				.setParameter("qstart", q + "%").setParameter("qbothends", "%" + q + "%").getResultList();
 	}
 
 //	public List<Product> getShoppingBasket(User user, SearchCriteria criteria) {
 ...
 //
 //		return new ArrayList<Product>(shoppingBasket.subList(fromIndex, toIndex > shoppingBasket.size() ? shoppingBasket.size() : toIndex));
 //	}
 	...
 	public List<Product> getByBarcode(String barcode) {
 		return getByBarcode(Long.parseLong(barcode), !isActionPerformedByAdmin());
 	}	
 	...
 	public List<Product> getByBarcode(long barcode, boolean approvedOnly) {
 		try {
 			...
 			return (List<Product>) em.createQuery("select p from Product p where p.barcode = :barcode" + approvedClause).setParameter("barcode", barcode).getResultList();
 		} catch (Exception e) {
 	...
 	private void removeEvents(List<? extends TimedEvent> listOfEvents) {
 		for (Iterator<? extends TimedEvent> i = listOfEvents.iterator(); i.hasNext();) {
 			TimedEvent c = i.next();
 			em.remove(c);
 		}
 	}
 	...
 	public void removeAdditive(Product p, E e) {
 		p.setConservantsText(p.getConservantsText().replaceAll(e.getNumber() + ",? ?", ""));
 		p.getConservants().remove(e);	
 	}
 
 	@Transactional
 	public void updateField(Product p, String field, Object value) throws Exception {
 	...
 	public Product loadProductByField(String field, String value) {
 		try {
 			return (Product) em.createQuery("select p from Product p where p." + field + "  = :value").setParameter("value", value).setMaxResults(1).getSingleResult();
 	...
 	@Transactional
 	public void updateProduct(Product p, String field, Object value, User currentUser) throws Exception {
