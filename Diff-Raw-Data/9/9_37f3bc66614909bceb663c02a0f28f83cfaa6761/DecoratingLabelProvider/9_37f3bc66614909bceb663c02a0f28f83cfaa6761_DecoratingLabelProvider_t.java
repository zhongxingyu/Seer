 ...
 public class DecoratingLabelProvider extends LabelProvider implements
 	...
 	/**
 	 * The <code>DecoratingLabelProvider</code> implementation of this
 	 * <code>IBaseLabelProvider</code> method disposes, the nested label
 	 * provider, any resource managers and the label decorator.
 	 */
 	public void dispose() {
 	...
 	private void decorateSettings(ViewerLabel settings, TreePath elementPath) {
 		...
 		if (decorator != null) {
 			if (decorator instanceof LabelDecorator) {
 				...
 				String text = decorator.decorateText(settings.getText(),
 						element);
 				if (text != null && text.length() > 0)
 					settings.setText(text);
 				Image image = decorator.decorateImage(settings.getImage(),
 						element);
 				if (image != null)
 					settings.setImage(image);
 			}
