 ...
 public class ViewOrganizationSettingsAction extends BaseAction {
 	...
 	public static ActionSecurity getSecurity() {
 		ActionSecurity security = new ActionSecurity(
 				"viewOrganizationSettingsAction");
 	...
 	private Properties getFiscalRules() {
 		...
 		fiscalRules.setProperty("allowCalDefForNextYear", FiscalCalendarRules
 				.getDaysForCalendarDefinition().toString());
 		fiscalRules.setProperty("startOfWeek", WeekDay.getWeekDay(
 				FiscalCalendarRules.getStartOfWeek()).getName());
 		fiscalRules.setProperty("offDays", getOffDays());
 		fiscalRules.setProperty("holidayMeeting", FiscalCalendarRules
 				.getScheduleTypeForMeetingOnHoliday());
 	...
 	private Properties getLocaleInfo() {
 		...
 		localeInfo.setProperty("localeCountryCode", configLocale
 				.getCountryCode());
 		localeInfo.setProperty("localeLanguageCode", configLocale
 				.getLanguageCode());
 	...
 	private Properties getAccountingRules() {
 		...
 		accountingRules.setProperty("currency", AccountingRules
 				.getCurrencyCode());
 		accountingRules.setProperty("maxInterest", AccountingRules
 				.getMaxInterest().toString());
 		accountingRules.setProperty("minInterest", AccountingRules
 				.getMinInterest().toString());
 		accountingRules.setProperty("digitsAfterDecimal", AccountingRules
 				.getDigitsAfterDecimal().toString());
 		accountingRules.setProperty("digitsBeforeDecimal", AccountingRules
 				.getDigitsBeforeDecimal().toString());
 		accountingRules.setProperty("intDigitsAfterDecimal", AccountingRules
 				.getDigitsAfterDecimalForInterest().toString());
 		accountingRules.setProperty("intDigitsBeforeDecimal", AccountingRules
 	...
 	private Properties getClientRules() throws ConfigurationException {
 		...
 		clientRules.setProperty("centerHierarchyExists",
 				booleanToYesNo(ClientRules.getCenterHierarchyExists()));
 		clientRules.setProperty("loansForGroups", booleanToYesNo(ClientRules
 				.getGroupCanApplyLoans()));
 		clientRules.setProperty("clientsOutsideGroups",
 				booleanToYesNo(ClientRules.getClientCanExistOutsideGroup()));
 		clientRules.setProperty("nameSequence", StringUtils.join(ClientRules
 				.getNameSequence(), DELIMITER));
 	...
 	private Properties getProcessFlowRules() {
 		...
 		processFlowRules.setProperty("clientPendingState",
 				booleanToYesNo(ProcessFlowRules
 	...
 	private Properties getMiscRules(HttpSession httpSession) {
 		...
 		try {
 			...
 			misc.setProperty("collectionSheetAdvanceDays", advanceDaysVal
 					.toString());
 		}
 		...
 		misc.setProperty("backDatedTransactions",
 				booleanToYesNo(AccountingRules.isBackDatedTxnAllowed()));
 	...
 	private String booleanToYesNo(boolean bool) {
 		...
 		else return m.lookup(YesNoFlag.NO);
 	}
