 ...
 public class DefaultConsumer implements Consumer {
 	...
 	protected void respondToWhoIsIn(Matcher m, Tweet msg) {
 		...
 		while (true) {
 			try {
 				...
 				else if(this.nearbyUsers.size() == 1)
 					usernames.append("is in.");
 				else if(this.nearbyUsers.size() == 0)
 	...
 	protected void respondToIsIn(Matcher m, Tweet msg) {
 		String deviceId = this.db.getDeviceIdFromID(m.group(1));
 		while(true) {
 			try {
 				this.nearbyUsersSemaphore.acquire();
 				if(this.nearbyUsers.contains(deviceId))
 					this.messagesToPublish.add(String.format("@%s is in the room.", m.group(1)));
 				else
 					this.messagesToPublish.add(String.format("@%s is not in the room.", m.group(1)));
 				this.nearbyUsersSemaphore.release();
 				break;
 			} catch (InterruptedException e) {
