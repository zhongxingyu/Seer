 ...
 //Modificado José A. Fernández <aullasistemas@gmail.com>
 public class Xpm
 {
 	...
 	public static Image XpmToImage(String xpm) {
 		...
 		*/
 		...
 		while(colcount-->0)
 		{
 			...
 			if(debugflag){
 				debug("\n colorname on line is "+colorname);
 				debug("now parsing "+
 			...
 			if(charsperpixel==3)
 				...
 				int tmpchar=xpm.charAt(parse++);
 				int tmpchar2=xpm.charAt(parse++);
 				int tmpchar3=xpm.charAt(parse++);  
 				...
 				if(debugflag){
 				debug("tree charsperpixel: substre==" +
 				      xpm.substring(parse-3,parse)+
 				      " which generates char "+
 					(tmpchar&0xffffff));
 				}
 				...
 				int tmpchar2=xpm.charAt(parse++);
 				int tmpchar3=xpm.charAt(parse++);  
  				int tmpchar4=xpm.charAt(parse++);
 				
 				tmpchar = (tmpchar&0xff)<<24;
 				tmpchar |= (tmpchar2&0xff)<<16;
 				tmpchar |= (tmpchar3&0xff)<<8;
 				tmpchar |= (tmpchar4&0xff);
 
 				if(debugflag){
 				debug("four charsperpixel: substre==" +
 				      xpm.substring(parse-4,parse)+
 				      " which generates char "+
 					(tmpchar&0xffffffff));
 				}
 		...
 		while(xpm.charAt(parse) =='"')
 		{
 			...
 			for(int pix=parse; pix<parseend;pix++)
 			{
 				...
 				if(charsperpixel==3)
 					{ //3 charsperpixel
 					tmpchar=xpm.charAt(pix++);
 					int tmpchar2=xpm.charAt(pix++);
 					int tmpchar3=xpm.charAt(pix);  
