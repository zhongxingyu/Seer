  ...
  * </ul>
  * <p>
  * The static analysis is performed by visiting the parsed expression
  * tree and checking certain things against the actual BPEL model.
  * <p>
 ...
 public class XPathValidator extends Validator {
 	...
 	public void start () {
 		...
 		exprString = mModelQuery.lookup(mNode,IModelQueryLookups.LOOKUP_TEXT_TEXT, null, null );
 	...
 	public void rule_SanityCheck_1 () {
 		...
 		if ( isEmptyOrWhitespace( exprString) ) {
 			problem = createError();
 			problem.fill("XPATH_EMPTY_EXPRESSION",
 					fNodeName,
 					fExprByNode );
 			exprString = null;
 	...
 	protected void repointOffsets(IProblem problem, int offset, int offsetEnd ) {
 		...
 		int charStart = mModelQuery.lookup(mNode,IModelQueryLookups.LOOKUP_NUMBER_CHAR_START_2,-1);		
 		int charEnd   = mModelQuery.lookup(mNode,IModelQueryLookups.LOOKUP_NUMBER_CHAR_END_2,-1);
 		
 		int lineNo    = mModelQuery.lookup(mNode,IModelQueryLookups.LOOKUP_NUMBER_LINE_NO_2,-1);
 		int columnNo  = mModelQuery.lookup(mNode,IModelQueryLookups.LOOKUP_NUMBER_COLUMN_NO_2,-1);
 	...
 	/**
 	 * Sets the value of the expression result. This is run as the last rule
 	 * in pass1 and it examines the top element on the stack that the visitor/validator
 	...
 	@ARule(
 		sa = 1015,
 		desc = "Check functions in XPath expressions",
 		author = "michal.chmielewski@oracle.com",
 		date = "03/02/2007",
 		tag = "functions",		
 		order = 1
 	...
 	@ARule(
 		sa = 30,
 		desc = "Arguments to getVariableProperty must be quoted strings",
 		author = "michal.chmielewski@oracle.com",
 		date = "01/29/2007",
 		tag = "bpel.functions"
 	)
 	...
 	@ARule(
 		sa = 31,
 		desc = "The second argument MUST be a string literal conforming to the definition of QName in section 3",
 		author = "michal.chmielewski@oracle.com",
 		date = "01/29/2007",
 		tag = "bpel.functions"
 	)
 	...
 	public void CheckGetVariableProperty2ndArgument ( FunctionCallExpr expr ) {
 		...
 		if (params.size() < 2 || ((params.get(1) instanceof LiteralExpr) == false) ) {
 	...
 	@ARule(
 		sa = 39,
 		desc = "1st arguments to doXslTransform must be quoted string",
 		author = "michal.chmielewski@oracle.com",
 		date = "01/29/2007",
 		tag = "bpel.functions"
 	)
 	...
 	@ARule(
 		sa = 40,
 		desc = "There must be an even number of arguments doXslTransform",
 		author = "michal.chmielewski@oracle.com",
 		date = "01/29/2007",
 		tag = "bpel.functions"
 	)
 	...
 	@ARule(
 		sa = 41,
 		desc = "3rd,5th,7th, etc. arguments must be QName strings",
 		author = "michal.chmielewski@oracle.com",
 		date = "01/29/2007",
 		tag = "bpel.functions"
 	)
 	...
 	public void CheckDoXslTransform_15 ( FunctionCallExpr expr) {
 		...
 		// check the 3rd, 5th, 7th etc. arguments of the XPath expression to make sure
 	...
 	public void rule_CheckVariable_10 ( VariableReferenceExpr expr ) {
 		...
 		if (partName != null) {
 			...
 			variablePart = mModelQuery.lookup(
 					varTypeNode,
 					IModelQueryLookups.LOOKUP_NODE_TYPE_OF_PART,
 					partName );
 		}
 		...
 		if (partName != null) {
 			...
 			if (WSDL_ND_MESSAGE.equals(varTypeNode.nodeName()) == false) {
 				problem = createError();
 				problem.fill("XPATH_VARIABLE_PART",						
 						varName,
 						partName,
 						expr.getText(),
 						0);
 			...
 			// there is no part name specified, but variable does have more then 1 part.
 			if (WSDL_ND_MESSAGE.equals(varTypeNode.nodeName())) {
 				problem = createError();
 				problem.fill("XPATH_VARIABLE_PART",						
 						varName,
 						"text.term.unspecified",
 						expr.getText(),
 						1);
 				repointOffsets(problem, expr);
 	...
 	/**
 	 * @see org.eclipse.bpel.validator.model.Validator#runRules(java.lang.String, java.lang.Object[])
 	 */
