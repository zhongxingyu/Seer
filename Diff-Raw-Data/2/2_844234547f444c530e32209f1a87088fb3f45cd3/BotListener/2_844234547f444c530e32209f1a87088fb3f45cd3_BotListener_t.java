 ...
 public class BotListener implements Listener {
 	...
 	private String connected = "";
 	 ...
 	 public void debug(String msg) {
 		 if(debugmode) {
 			 botclass.getServer().broadcastMessage("\247f[\247bAntiBot\247f] " + msg);
 		 }
 	 }
 	 ...
 	 public boolean hasPerms(Player pl) {
 		 if(useOp(pl) || useWhitelist(pl) || botclass.ownPermission("antibot.join", pl, 1)) {
 			 return true;
 		 } else {
 			 return false;
 		 }
 		...
 		try {
 		 if(reanibo) {
				...
				interval = botclass.defaultinterval;
 				...
 				botattempt = 0;
 				if(notify) {
 			   botclass.getServer().broadcastMessage("\247f[\247bAntiBot\247f] \247aThe minecraft bot invasion has ended. Connection Throttling: \247cDisabled");
 				}
 		     }
 			botcts = 0;
 		 return true;
 	 ...
 	 public void onPlayerJoin(PlayerJoinEvent event) {
 		 ...
 		 if(hasPerms(event.getPlayer())) { //It's a double check.
 			   debug("Whitelisted.");
 			   if(reanibo && botclass.ownPermission("antibot.notify", event.getPlayer(), 1)) {
 				   event.getPlayer().sendMessage("\247f[\247bAntiBot\247f] \247c" + connectInvasion);
 			   }
 			   if(reanibo && botclass.ownPermission("antibot.admin.notify", event.getPlayer(), 2) && interval > 120000) {
 				   event.getPlayer().sendMessage("\247f[\247bAntiBot\247f] \247cThe system needs a flush. Please type /antibot flush. Thanks.");
 			   }
 				 return;
 			}
 	 }
 	 ...
 	 public void onPlayerLogin(PlayerLoginEvent event)
 	   {
 		 ...
 		 debug("User is trying to connect..");
 		   ...
 		   if(botcts > accounts && math < interval) {
 			   ...
 			   if(!reanibo) {
 				   if(notify) {
 				   botclass.getServer().broadcastMessage("\247f[\247bAntiBot\247f] \247cOh no! A minecraft bot invasion has began. Connection Throttling: \247aEnabled");
 				   }
 				   reanibo = true;
 				   debug("Tripswitched!");
 				   kickConnected();
