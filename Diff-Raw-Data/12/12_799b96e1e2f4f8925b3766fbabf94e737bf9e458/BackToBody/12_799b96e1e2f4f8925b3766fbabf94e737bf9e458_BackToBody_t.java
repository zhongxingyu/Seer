 				...
 				if (sender instanceof Player) {
 					if (cmdName.equalsIgnoreCase("btb")) {
						...
						if ((permission == null && player.isOp()) || permission.has(player, "backtobody.btb")) {
 							...
 							if (!prop.keyExists("x")) {
								player.sendMessage(ChatColor.DARK_PURPLE+"You either haven't died, or your corpse no longer exists.");
 								return true;
 							}
 							...
 							if (cost == 0) {
 								...
 								if (iconomy != null) {
 									...
 									if (account.getBalance() > cost) {
										...
										player.sendMessage(ChatColor.DARK_PURPLE+"You hear a sinister laugh,"+ChatColor.DARK_RED+" \"You do not possess enough "+ChatColor.GREEN+iConomy.getBank().getCurrency()+ChatColor.DARK_RED+".\" The toll is "+ChatColor.GREEN+cost+' '+iConomy.getBank().getCurrency()+ChatColor.DARK_RED+'.');
 										return true;
 									}
 								} else {
							...
							player.sendMessage(ChatColor.DARK_PURPLE+"You do not have permissions to use "+ChatColor.RED+"/btb");
							return true;
 						}
 					}
 				}
