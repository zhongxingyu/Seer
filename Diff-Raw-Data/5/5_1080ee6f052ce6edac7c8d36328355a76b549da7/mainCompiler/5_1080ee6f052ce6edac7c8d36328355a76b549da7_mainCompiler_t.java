 ...
 public class mainCompiler {
 					...
 					try {
 						...
 						case "add":
 							...
 							code |= Integer.valueOf(
 									...
 									.intValue() << 16;
 							code |= 0x20;
 							pw.printf("%02x : %08x; -- %s %s\r\n", lineNum, code,
 									getLabel(Symbols, lineNum*4), origInstruction.get(lineNum));
 							break;
 						case "and":
 							code = Integer.valueOf(
 									params[0].replace("$", "").trim())
 	...
 	private static String getLabel(ArrayList<String[]> symbols, int address){
 		for(int i = 0; i < symbols.size(); i++){
 			if (symbols.get(i)[1].equals(String.valueOf(address))){
 				return symbols.get(i)[0]+": ";
 			}
 		}
 		return "";		
 	}
