 ...
 //                    }
     ...
     public int[] getNumberOfInstances(AtomicConcept concept) {
         ...
         if (representative.equals(m_topConcept)) {
             result[0]=m_individuals.length;
             return result;
         }
         return getNumberOfInstances(node,result);
     }
     protected int[] getNumberOfInstances(HierarchyNode<AtomicConcept> node,int[] result) {
         AtomicConceptElement representativeElement=m_conceptToElement.get(node.getRepresentative());
         if (representativeElement!=null) {
         ...
         for (HierarchyNode<AtomicConcept> child : node.getChildNodes())
             if (child!=m_currentConceptHierarchy.m_bottomNode) 
                 getNumberOfInstances(child,result);
         return result;
     }
     ...
     public int[] getNumberOfSuccessors(AtomicRole role, Individual individual) {
         ...
         return getNumberOfRoleSuccessors(node,individual,result);
     }
     protected int[] getNumberOfRoleSuccessors(HierarchyNode<RoleElement> node, Individual individual, int[] result) {
         RoleElement representative=node.getRepresentative();
         Map<Individual,Set<Individual>> map=representative.getKnownRelations();
     ...
     public int[] getNumberOfPredecessors(AtomicRole role, Individual individual) {
         ...
         return getNumberOfRolePredecessors(node,individual,result);
     }
     protected int[] getNumberOfRolePredecessors(HierarchyNode<RoleElement> node, Individual individual, int[] result) {
         RoleElement representative=node.getRepresentative();
         Map<Individual,Set<Individual>> map=representative.getKnownRelations();
         ...
         for (HierarchyNode<RoleElement> child : node.getChildNodes())
             if (child!=m_currentRoleHierarchy.m_bottomNode) 
                 getNumberOfRolePredecessors(child,individual,result);
         return result;
     } 
     ...
     public int[] getNumberOfTypes(Individual individual) {
         int[] result=new int[2];
         DFS<AtomicConcept> dfs=new DFS<AtomicConcept>(m_currentConceptHierarchy.m_bottomNode);
         DFSTypeCounter typeCounter=new DFSTypeCounter(individual);
         dfs.traverse(m_currentConceptHierarchy.m_topNode, typeCounter);
         result[0]=typeCounter.types;
         if (result[0]==0) result[0]=1; // we don't store inds that only have owl:Thing as type
         return result;
     }
     final class DFS<T> {
         ...
         public void traverse(HierarchyNode<T> startNode, InstanceManager.DFSVisitor<T> visitor) {
             traverse(0, startNode, visitor);
         }
         protected void traverse(int level, HierarchyNode<T> node, InstanceManager.DFSVisitor<T> visitor) {
     ...
     protected final class DFSTypeCounter implements InstanceManager.DFSVisitor<AtomicConcept> {
         ...
         public boolean visit(int level,HierarchyNode<AtomicConcept> node) {
             AtomicConceptElement element=m_conceptToElement.get(node.getRepresentative());
