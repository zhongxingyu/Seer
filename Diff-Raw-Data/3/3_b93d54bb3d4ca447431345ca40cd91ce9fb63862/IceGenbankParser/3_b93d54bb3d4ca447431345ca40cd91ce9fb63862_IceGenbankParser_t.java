 ...
 public class IceGenbankParser extends AbstractParser {
         ...
         for (int i = 1; i < lines.length; i++) {
             ...
             if (!(' ' == (line.charAt(apparentFeatureKeyColumn)))) {
                 ...
                 try {
                     ...
                     Matcher startStopMatcher = startStopPattern.matcher(chunk[1]);
 
                     ArrayList<Integer[]> locations = new ArrayList<Integer[]>();
                     while (startStopMatcher.find()) {
                         if (startStopMatcher.groupCount() == 2) {
                             ...
                             locations.add(new Integer[] { genbankStart, end });
                         }
                     }
 
                     if (locations.size() == 0) {
                         Matcher startOnlyMatcher = startOnlyPattern.matcher(chunk[1]);
                     ...
                     if (locations.size() != 0) {
                         locations = joinApeLocation(locations);
                         genbankStart = locations.get(0)[0];
                         end = locations.get(0)[1];
                     } else {
     ...
     /**
      * Try to join split locations generated by ApE. ApE has a nasty habit
      * of splitting features when it's not necessary.
      * 
      * @return
      */
     private ArrayList<Integer[]> joinApeLocation(List<Integer[]> locations) {
 
         int startSize = locations.size();
         ...
         for (Integer[] location : locations) {
             if (result.size() == 0) {
                 ...
                 Integer[] previousLocation = result.get(result.size() - 1);
                 if (location[0] == previousLocation[1] + 2) { // if gap is one bp, join.
         ...
         if (result.size() == 1 && startSize != 1) {
             System.out.println("joinApeLocation activated");
         }
