 ...
 /**
  * Manager fr das CFJ-Typsystem unter Bercksichtigung alternativer Features
  * 
  * Grundlegende Voraussetzung: jedes CFJ-Programm ist auch ein gltiges FJ-Programm, wenn man die Annotationen weglsst
  * 
 ...
 public class CFJTypingManagerAF extends CFJTypingManager {
 	...
 	// Haben wir bentigt, als versucht wurde, auch alternative Klassen, Methoden usw. im Typsystem zu bercksichtigen.
 ...
 //	}
 	...
 	// Versuch, auch alternative Klassen und alternative formale Parameter zu bercksichtigen
 //	/**
 //	 * T-VAR: lokale Variablen
 //	 * @param identifier
 //	 * @param method
 ...
 //	}
 	...
 	// Konstruktoren berprfen wir hier nicht mehr
 ...
 //	}
 	...
 	// Versuch, auch alternative Klassen und alternative Felder zu bercksichtigen
 //	/**
 //	 * T-FIELD: Zugriff auf ein Feld (instance.field oder this.field)
 //	 * @param fieldInvoke
 //	 * @param strategy
 ...
 //	}
 	...
 	public CFJType typeOf(MethodInvoke methodInvoke, Alternative parent, MethodDeclaration methodDeclaration, Set<IFeature> context,
 		...
 		for (Alternative methodInvokeAlternative : methodInvokeAlternatives) {
 			...
 			// (I.4): Expressions mssen well-typed sein
 			...
 			// Fr jede Methode, die aufgerufen werden knnte, mssen Checks durchgefhrt werden
 			for (MethodSignature mtype : mtypes) {
 				...
 				// (I.6): Expressions mssen Subtypen der Typen der formalen Parameter sein
 				...
 				if (methodInvokeAltNode.getExpressionList() != null) {
 					for (int i = 0; i < methodInvokeAltNode.getExpressionList().getExpression().size(); ++i) {
 						...
 						for (Alternative expressionAlternative : expressionAlternatives) {
 							// Im Kontext der Farben von Methodenaufruf und Methode mssen die Expressions und die Parameter
 							// in genau den gleichen Varianten prsent sein.
 							Set<IFeature> source = expressionAlternative.getFeatures();
 							source = addAll(addAll(source, contextWithAlt), featuresOfMethod);
 
 							// (T.3ii) -> (I.2): Annotationen mssen passen
 	...
 	public CFJType typeOf(AllocationExpression allocationExpression, Alternative parent, MethodDeclaration methodDeclaration, Set<IFeature> context,
 		...
 		for (Alternative allocationExpressionAlternative : allocationExpressionAlternatives) {
 			...
 			if (fieldsOfTypeDeclaration == null)
 				// Drfte eigentlich nicht passieren, denn gibt es keine Felder, wird eine leere Liste zurckgegeben.
 				return null;
 
 			// (N.3): Expressions mssen well-typed sein
 			...
 			// (N.5): Expressions mssen Subtypen der Typen der formalen Parameter sein
 			...
 			if (allocationExpressionAltNode.getExpressionList() != null) {
 				for (int i = 0; i < allocationExpressionAltNode.getExpressionList().getExpression().size(); ++i) {
 					...
 					for (Alternative expressionAlternative : expressionAlternatives) {
 						...
 						// (T.4ii) -> (N.2): Annotationen mssen passen
 						if (!strategy.equal(fm, source, target)) {
 	...
 	public CFJType typeOf(InvokeTarget invokeTarget, Alternative parent, MethodDeclaration methodDeclaration, Set<IFeature> context,
 		...
 		if (invokeTarget instanceof InvokeTarget4) {
 			// Wir suchen die Klasse von der methodDeclaration aus und nicht vom invokeTarget, da das invokeTarget mglicherweise als
 			// einzelnes Codefragment geparsed wurde (kann in Alternativen vorliegen) und somit die Informationen ber Vaterknoten usw.
 			// mglicherweise nicht vorhanden sind.
 		...
 		// Tritt nur bei unvollstndiger Fallunterscheidung auf
 		return null;
 	}
 	...
 	public CFJType typeOf(PrimaryExpression primaryExpression, Alternative parent, MethodDeclaration methodDeclaration, Set<IFeature> context,
 		...
 		if (primaryExpression instanceof PrimaryExpression8) {
 			// Wir suchen die Klasse von der methodDeclaration aus und nicht vom primaryExpression, da die primaryExpression mglicherweise als
 			// einzelnes Codefragment geparsed wurde (kann in Alternativen vorliegen) und somit die Informationen ber Vaterknoten usw.
 			// mglicherweise nicht vorhanden sind.
 			TypeDeclaration surroundingTypeDeclaration = findSurroundingTypeDeclaration(methodDeclaration);
 			if (surroundingTypeDeclaration == null) {
