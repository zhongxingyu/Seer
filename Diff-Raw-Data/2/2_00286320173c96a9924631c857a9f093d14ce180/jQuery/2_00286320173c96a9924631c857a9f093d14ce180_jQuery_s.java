 ...
 public class jQuery implements Iterable<WebElement> {
     ...
     public jQuery(jQuery j) {
         ...
         if (StringUtils.isNotBlank(j.selector)) {
     ...
     public jQuery revive() {
         return new jQuery(this);
     }
 
     public <E> E as(Class<E> klazz) {
         return ReflectUtils.getAs(this, klazz);
     }
     ...
     public <E> E find(String selector, Class<E> klazz) {
         if (selector.contains("%s")) {
             selector = String.format(selector, ReflectUtils.getExtraSelector(klazz));
         }
         return ReflectUtils.getAs(subset(".find(arguments[0]);", selector), klazz);
     }
     ...
     public jQuery jget() {
         return jqf.query(get(0));
     }
 
     public jQuery jget(int index) {
         return jqf.query(get(index));
     }
