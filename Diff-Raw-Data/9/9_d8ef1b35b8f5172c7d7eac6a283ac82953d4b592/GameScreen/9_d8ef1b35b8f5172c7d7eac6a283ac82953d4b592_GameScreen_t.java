 ...
 public class GameScreen extends AbstractScreen {
 	...
 	// tick time constant
 	private static float TICK_TIME = 0.8f;
 	// tick level increase
 	...
 	private ArrayList<ArrayList<Actor>> getFullLines() {
		...
		for(int y = 0; y <= 19; y++) {
 			...
 			for(int x = 0; x <= 9; x++) {
 				...
 				Actor brick = null;
 				if((brick = stage.hit(coords.x, coords.y, false)) != null && brick.getClass() == Brick.class) {
 					bricks.add(brick);
 				}
 			}
 			if(bricks.size() == 10) {
 	...
 	public void render(float delta) {
 		...
 		if(tickDelta >= TICK_TIME) {
 			...
 			if(flushedLines.size() > 0) System.out.println(flushedLines);
