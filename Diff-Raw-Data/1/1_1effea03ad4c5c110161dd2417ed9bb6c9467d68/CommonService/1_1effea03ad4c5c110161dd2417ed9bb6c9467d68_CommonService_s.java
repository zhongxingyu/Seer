 ...
 public class CommonService
 {
 	...
 	private static int protAppCounter = 0; // for generating unique protocol
 											// application names
 	...
 	/**
 	 * Helper funtion to upload Dec application files. Note the use of name and
 	 * uniquePostfix!
 	 * 
 	 * @param name
 	 * @param uniquePostfix
 			...
 			throws Exception
 	{
 		...
 		String fileName = name.toLowerCase() + "_" + NameConvention.escapeFileName(uniquePostfix); // use
 																									// DEC
 																									// name
 																									// to
 																									// make
 	...
 	/**
 	 * Retrieve an investigation id based on an investigation name. Returns -1
 	 * if none found.
 	 * 
 	 * @param invName
 	 *            investigation name
 	...
 	/**
 	 * Gets the name of the first investigation owned by the user with name
 	 * 'userName'. Returns null if no investigations found.
 	 * 
 	 * @param userId
 	 * @return
 	 * @throws DatabaseException
 	...
 	/**
 	 * Gets the names of all the investigations owned by the user with name
 	 * 'userName'. Returns null if no investigation found.
 	 * 
 	 * @param userName
 	 * @return
 	 * @throws DatabaseException
 	...
 	/**
 	 * Gets the investigations owned, readable or writable by the user with name
 	 * 'userName'. TODO: also take groups into account.
 	 * 
 	 * @param userName
 	 * @return
 	 */
 	public List<Investigation> getAllUserInvestigations(String userName)
 	{
 		...
 		q.addRules(new QueryRule(Operator.OR));
 		q.addRules(new QueryRule(Investigation.CANREAD_NAME, Operator.EQUALS, "AllUsers")); // FIXME
 																							// evil!!!
 		q.addRules(new QueryRule(Operator.OR));
 	...
 	/**
 	 * Gets the names of the investigations owned, readable or writable by the
 	...
 	/**
 	 * Gets the Ids of the investigations owned, readable or writable by the
 	 * user with name 'userName'.
 	 * 
 	 * @param userName
 	 * @return
 	...
 	/**
 	 * Gets the names of the investigation owned or writable by the user with
 	 * name 'userName'.
 	 * 
 	 * @param userName
 	 * @return
 	 ...
 	 * 
 	 */
 	public ObservationTarget getObservationTargetById(int targetId) throws DatabaseException, ParseException
 	{
 	 ...
 	 * 
 	 */
 	public ObservationTarget getObservationTargetByName(String targetName) throws DatabaseException, ParseException
 	{
 	 ...
 	 * @throws ParseException
 	 */
 	public Location getLocationById(int locationId) throws DatabaseException, ParseException
 	{
 	 ...
 	 * @throws ParseException
 	 */
 	public Individual getIndividualById(int individualId) throws DatabaseException, ParseException
 	{
 	 ...
 	 * 
 	 * @param targetName
 	 *            the name of the ObservationTarget element to look for
 	 * @return an integer matching the id of the ObservationTarget element, or
 	 *         -1 if none was found
 	 */
 	...
 	/**
 	 * Return a list of all observation targets of a certain type in the
 	 * investigations with ID's in investigationIds or in the System
 	 * investigation. If desired, filtered down to only the currently active
 	 * ones.
 	 * 
 	 * @param type
 	 *            : observation target type (Individual, Panel, Location etc.)
 	 *            to filter on, may be null
 	 * @param isActive
 	...
 	/**
 	 * Returns all ObservedValues for the given Measurement ID, sorted
 	 * descending on 'time'.
 	 * 
 	 * @param measurementId
 	 * @return
 	...
 	public List<ObservationTarget> getAllObservationTargets(List<String> investigationNames)
 	{
 		try
 		{
 			return db.query(ObservationTarget.class).in(ObservationTarget.INVESTIGATION_NAME, investigationNames)
 					.find();
 		}
 		catch (Exception e)
 		{
 	 ...
 	 * 
 	 * @param idList
 	 *            : the ID's of the desired ObservationTargets
 	 * @return
 	 * @throws DatabaseException
 	 * @throws ParseException
 	 */
 	...
 	/**
 	 * Returns a list of Individuals belonging to the ID's provided. Returns an
 	 * empty list when no ID's are passed.
 	 * 
 	 * @param idList
 	 *            : the ID's of the desired Individuals
 	 * @return
 	 * @throws DatabaseException
 	 * @throws ParseException
 	 */
 	...
 	/**
 	 * Returns a list of all Panels that have been given a certain mark using a
 	 * 'TypeOfGroup' ObservedValue.
 	 * 
 	 * @param mark
 	 * @return
 	 * @throws DatabaseException
 			...
 			throws DatabaseException, ParseException
 	{
 		...
 		if (investigationNames.size() > 0)
 		{
 			...
 			valueQuery.addRules(new QueryRule(qr1, qr2, qr3)); // only user's
 																// own OR System
 																// investigations
 		}
 		else
 	...
 	/**
 	 * Creates an ObservedValue for adding an ObservationTarget to a Panel, but
 	 * does NOT add this to the database. However, a protocol application is
 	 * created and added to the database, so there is a database transaction
 	 * involved, rendering this method unsuitable for use with batch lists.
 	 */
 	public ObservedValue addObservationTargetToPanel(String investigationName, String targetName, Date tmpDate,
 			String panelName) throws DatabaseException, ParseException, IOException
 	{
 		...
 		if (valueList.size() == 0)
 		{
 			...
 			db.add(app);
 			return createObservedValue(investigationName, app.getName(), tmpDate, null, "Group", targetName, null,
 					panelName);
 		}
 	 ...
 	 */
 	@Deprecated
 	public int makeActor(int investigationId, String actorName) throws DatabaseException, ParseException, IOException
 	{
 	...
 	/**
 	 * Makes a new application of the given protocol but does NOT add it to the
 	 * database. Uses Investigation and Protocol Names so it can be used with
 	 * lists.
 	 */
 	public ProtocolApplication createProtocolApplication(String investigationName, String protocolName)
 			throws ParseException, DatabaseException, IOException
 	{
 		...
 		pa.setName(protocolName + "_" + protAppCounter++ + "_" + now.toString()); // strange
 																					// but
 																					// unique
 																					// name
 	 ...
 	 * 
 	 * @param protocolid
 	 *            the id of the protocol to search for
 	...
 	/**
 	 * Creates an ObservedValue with the given parameters, but does NOT add it
 	 * to the database. Uses investigation, feature, target, protocol
 	 * application and relation names to link, so those do not have to have been
 	 * added to the DB already.
 	 * 
 	 * @param investigationId
 	 * @param protappid
 	 * @param starttime
 	...
 	/**
 	 * First makes a ProtocolApplication and adds it to the Database. Then
 	 * creates an ObservedValue with the given parameters and a link to the
 	 * ProtocolApplication just made, but does NOT add it to the database.
 	 * Warning: because this method involves a database transaction, it is not
 	 * suited for use with batch lists.
 	 */
 	public ObservedValue createObservedValueWithProtocolApplication(String investigationName, Date starttime,
 			Date endtime, String protocolName, String featureName, String subjectTargetName, String valueString,
 			String targetRefName) throws DatabaseException, IOException, ParseException
 	{
 			...
 			ParseException
 	{
 		...
 		if (valueList.size() > 0)
 		{
 			ObservedValue returnValue = valueList.get(0); // default is first
 															// one
 	...
 	/**
 	 * For a given ObservationTarget and ObservableFeature, returns the ID of
 	 ...
 	 * @throws ParseException
 	 */
 	public Protocol createProtocol(String investigationName, String protocolName, String description,
 			List<String> measurementNameList) throws DatabaseException, IOException, ParseException
 	{
 	 ...
 	 * @throws ParseException
 	 */
 	public int getProtocolId(String protocolName) throws DatabaseException, ParseException
 	{
 		Query<Protocol> q = db.query(Protocol.class);
 		q.addRules(new QueryRule(Protocol.NAME, Operator.EQUALS, protocolName));
 	 ...
 	 * @throws ParseException
 	 */
 	public List<Protocol> getAllProtocols(List<Integer> investigationIds) throws DatabaseException, ParseException
 	{
 		Query<Protocol> q = db.query(Protocol.class);
 		QueryRule qr1 = new QueryRule(Measurement.INVESTIGATION, Operator.IN, investigationIds);
 		QueryRule qr2 = new QueryRule(Operator.OR);
 		QueryRule qr3 = new QueryRule(Measurement.INVESTIGATION_NAME, Operator.EQUALS, "System");
 		q.addRules(new QueryRule(qr1, qr2, qr3)); // only user's own OR System
 													// investigation
 		return q.find();
 	}
 	 ...
 	 * 
 	 */
 	public Protocol getProtocol(String protocolName) throws DatabaseException, ParseException
 	{
 		Query<Protocol> q = db.query(Protocol.class);
 		q.eq(Protocol.NAME, protocolName);
 		if (q.find() != null) return q.find().get(0);
 		else
 			throw new DatabaseException("No protocol with name " + protocolName + " could be found");
 	}
 	 ...
 	 * @throws ParseException
 	 */
 	public List<Protocol> getAllProtocolsSorted(String sortField, String sortOrder, List<String> investigationNames)
 			throws DatabaseException, ParseException
 	{
 		Query<Protocol> q = db.query(Protocol.class);
 		QueryRule qr1 = new QueryRule(Measurement.INVESTIGATION_NAME, Operator.IN, investigationNames);
 	...
 	public int getMeasurementId(String measurementName) throws DatabaseException, ParseException
 	{
 		...
 		if (featList.size() > 0) return featList.get(0).getId();
 		else
 			throw new DatabaseException("Id could be found for" + " Measurement with name: " + measurementName);
 	}
 			...
 			ParseException
 	{
 		...
 		if (investigationNames.size() > 0)
 		{
 			...
 			QueryRule qr2 = new QueryRule(Operator.OR);
 			QueryRule qr3 = new QueryRule(Measurement.INVESTIGATION_NAME, Operator.EQUALS, "System");
 			q.addRules(new QueryRule(qr1, qr2, qr3)); // only user's own OR
 	 ...
 	 * @throws ParseException
 	 */
 	public List<Measurement> getAllMeasurementsSorted(String sortField, String sortOrder,
 			List<String> investigationNames) throws DatabaseException, ParseException
 	{
 		Query<Measurement> q = db.query(Measurement.class);
 		if (sortOrder.equals("ASC"))
 		{
 	 ...
 	 * @throws ParseException
 	 */
 	public Measurement getMeasurementByName(String featureName) throws DatabaseException, ParseException
 	{
 		return db.find(Measurement.class, new QueryRule(Measurement.NAME, Operator.EQUALS, featureName)).get(0);
 	}
 
 	/**
 	 * Create and add a Measurement to the database. Note: unlike the other
 	 * makeMeasurement(), this method does NOT set the Investigation, the Unit,
 	 * the TargettypeAllowedForRelation, the panelLabelAllowedForRelation and
 	 * the Temporal fields.
 	 * 
 	 * @param name
 	 ...
 	 */
 	@Deprecated
 	public void makeMeasurement(String name, String description, String dataType) throws DatabaseException, IOException
 	{
 	...
 	public int makeMeasurement(String investigationName, String name, String unitName,
 			String targettypeAllowedForRelationClassName, String panelLabelAllowedForRelation, boolean temporal,
 			String dataType, String description, String userName) throws DatabaseException, IOException, ParseException
 	{
 		Measurement newFeat = createMeasurement(investigationName, name, unitName,
 				targettypeAllowedForRelationClassName, panelLabelAllowedForRelation, temporal, dataType, description,
 				userName);
 		db.add(newFeat);
 	...
 	/**
 	 * Creates a new entry in the Category table and updates the Measurement for
 	 * which it is a Category.
 	 * 
 	 * @param code
 	 *            : the code itself
 	 * @param desc
 	 *            : the description of what the code stands for
 	 * @param feat
 	 *            : the name of the ObservableFeature this Code can be used for
 	 * @throws DatabaseException
 	 * @throws ParseException
 	 * @throws IOException
 	 */
 	...
 	/**
 	 * Creates a new Category table but does NOT add it to the database and does
 	 * NOT update the Measurement for which it is a Category.
 	 * 
 	 * @param code
 	 *            : the code itself
 	 * @param desc
 	...
 	/**
 	 * Finds all Code entities for a given feature name and returns their
 	 * descriptions.
 	 * 
 	 * @param featurename
 	 * @return
 	 ...
 	 * @throws ParseException
 	 */
 	public List<String> getAllCodesForFeatureAsStrings(String featurename) throws DatabaseException, ParseException
 	{
 	 ...
 	 * @throws ParseException
 	 */
 	public List<Category> getAllCodesForFeature(String featureName) throws DatabaseException, ParseException
 	{
 		Measurement m = db.query(Measurement.class).eq(Measurement.NAME, featureName).find().get(0);
 	 ...
 	 * 
 	 * @param targets
 	 *            : the observation targets to be added
 	 * @throws DatabaseException
 	 * @throws IOException
 	 */
 	public void addObservationTargets(List<ObservationTarget> targets) throws DatabaseException, IOException
 	{
 	 ...
 	 * @throws ParseException
 	 */
 	public void addProtocol(String name, String description, List<String> observableFeatureNames)
 			throws DatabaseException, IOException, ParseException
 	{
 		...
 		for (String s : observableFeatureNames)
 		{
 			...
 			{ // this should not happen
 				logger.error("Name could not be found and feature:" + s + " will not be added to the protocol");
 			}
 		}
 	...
 	public List<ObservedValue> getObservedValuesByTargetAndMeasurement(String targetName, String mName,
 	...
 	/**
 	 * Returns all observed values for a given ObservationTarget and list of
 	 * features, sorted so that the most recent one comes first. NOTE: Creates a
 	 * default "" value if observedValue doesn't exist yet for given feature and
 	 * sample. This created default does not link to a ProtocolApplication and
 	 * this method isn't responsible for storing this object in the database.
 	 */
 	public List<ObservedValue> getObservedValuesByTargetAndFeatures(String targetName, List<String> measurementNames,
 			List<String> investigationNames, String investigationToAddToName) throws DatabaseException, ParseException
 	{
 	...
 	/**
 	 * Returns all observed values for a given ObservationTarget and
 	 * ObservableFeature, sorted so that the most recent one comes first. NOTE:
 	 * Creates a default "" value if observedValue doesn't exist yet for given
 	 * feature and sample. This created default does not link to a
 	 ...
 	 * @throws ParseException
 	 */
 	public ProtocolApplication getProtocolApplicationByName(String name) throws DatabaseException, ParseException
 	{
 		Query<ProtocolApplication> q = db.query(ProtocolApplication.class);
 		q.addRules(new QueryRule(ProtocolApplication.NAME, Operator.EQUALS, name));
 		if (!q.find().isEmpty())
 		{
 			return q.find().get(0);
 		}
 	...
 	public Investigation getInvestigation(String name) throws DatabaseException, ParseException
 	{
 		...
 		s.addRules(new QueryRule(Investigation.NAME, Operator.EQUALS, name));
 	...
 	/**
 	 * Get the name of the given MolgenisEntity. Returns null if it doesn't
 	 * exist.
 	 * 
 	 * @param entityId
 	 * @return
 	...
 	/**
 	 * Returns the id of the ObservableFeature the user has chosen as custom
 	 * name for the ObservationTargets, or -1 if none was set.
 	 * 
 	 * @param userId
 	 * @return
 	 */
 	public int getCustomNameFeatureId(String userName)
 	{
 		...
 		try
 		{
 			...
 			return db.query(CustomLabelFeature.class).eq(CustomLabelFeature.USERID, userId).find().get(0)
 					.getFeatureId_Id();
 		}
 		catch (Exception e)
 		{
 	...
 	/**
 	 * Makes a map of all ObservationTarget id's and names. The names are
 	 * retrieved using the feature name specified, or -if no feature is
 	 * specified- the normal database name is taken. To improve performance,
 	 * does not make a map if one already exists.
 	 */
 	public void makeObservationTargetNameMap(String userName, boolean force)
 	{
 		...
 		// First fill with standard names for all the investigations the current
 		// user has rights on
 	...
 	/**
 	 * Get all the prefixes (non-numeric parts) of the ObservationTarget names
 	 * that are in the DB. Example: say there are ObservationTargets with names
 	 * Morris1, Morris2 and Jessica99. This method will then return [Morris,
 	 * Jessica].
 	 * 
 	 * @return
 	 */
 	public List<String> getPrefixes(String targetType) throws DatabaseException
 	{
 		...
 		// List<ObservationTarget> allTargetList =
 	...
 	/**
 	 * Get the highest number found in the DB following the given
 	 * ObservationTarget name base. Example: say there are ObservationTargets
 	 * with names Morris1, Morris2 and Jessica99. This method, when passed
 	 * 'Morris', will return 2.
 	 * 
 	 * @param base
 	 * @return
 	 * @throws DatabaseException
 	...
 	/**
 	 * Get a list of all the remarks that have been set on the target with name
 	 * 'targetName'.
 	 * 
 	 * @param targetId
 	 * @return
