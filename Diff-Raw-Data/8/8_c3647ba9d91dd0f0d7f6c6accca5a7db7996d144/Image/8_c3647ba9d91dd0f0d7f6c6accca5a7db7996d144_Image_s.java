 ...
 **/
 public final class Image extends TagImpl {
 	...
 	private railo.runtime.img.Image source;
 	...
 	public void setSource(Object source) {
 		this.oSource=source;
 		// this.source=railo.runtime.img.Image.createImage(pageContext, source, false, false);
 	}
 	...
 	private void doActionBorder() throws PageException, IOException {
 		...
 		source.addBorder(thickness,color,railo.runtime.img.Image.BORDER_TYPE_CONSTANT);
 		write();
 	...
 	private static void cleanOld(Resource folder) throws IOException {
 		if(!folder.exists())folder.createDirectory(true);
 	...
 	private Struct doActionInfo(railo.runtime.img.Image source) throws PageException {
 		return source.info();
 	}
 	...
 	private void doActionRotate() throws PageException, IOException {
 		...
 		source.rotate(-1, -1, angle, railo.runtime.img.Image.INTERPOLATION_NONE);
 		write();
 	...
 	class Info {
 		Struct struct;
 		private railo.runtime.img.Image img;
 
 		public Info(railo.runtime.img.Image img) {
 			this.img=img;
 		}
