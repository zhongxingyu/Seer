 ...
 */
 ...
 public class PublicGameInfo implements GameInfo {
 	 ...
 	 * if not (i.e. amounts to high compared to bankroll) it is adjusted to the most similar valid action.<br>
 	 * 
 	...
 	private Action correctPlayerErrors(Action act, int s) {
 		...
 		if (act.isCall()) {
 			...
 			if (Utils.roundToCents(act.getAmount() + act.getToCall()) > player.getBankRoll()) {
 				if (getAmountToCall(s) > player.getBankRoll()) {
 					return Action.callAction(player.getBankRoll());
 				} else {
 					double raise = Utils.roundToCents(player.getBankRoll() - getAmountToCall(s));
