 ...
 * $Id: CheckidBean.java,v 1.2 2007-07-06 18:45:28 pbryan Exp $
 ...
 public class CheckidBean extends BackingBean
 {
      ...
      * TODO: Description.
      */
     protected void disableRegistration(CheckidQuery query)
     {
         query.setRequired(null);
         query.setOptional(null);
         query.setPolicyURL(null);
     }
      ...
      * @param result TODO.
      */
     protected void grant(CheckidQuery query, CheckidResult result)
     {
         AssocHandle handle = query.getAssocHandle();
        ...
        handle.getType() != AssocHandle.Type.ASSOCIATED)
         {
             ...
             // stateless handle cannot be used in further checkid_* requests
             handle = AssocHandle.generate(AssocHandle.Type.STATELESS);
         }
 
         result.setMode(Mode.ID_RES);
         result.setAssocHandle(handle);
         result.setIdentity(query.getIdentity());
         result.setReturnTo(query.getReturnTo());
 
         // turn result into map of name-value pairs
         Map<String,String> map = result.encode();
 
         // sign all entries in the map
         Maps.sign(map, Constants.HMAC_ALGORITHM, handle.getSecret());
 
         // respond by redirecting back to consumer through user agent
         sendRedirect(Maps.toQueryString(query.getReturnTo(), map));
     }
