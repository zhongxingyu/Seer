 ...
 public class IRCListener implements Listener {
     ...
     public void onIRCUserQuitServer(IRCUserQuitServerEvent e)
     {
        ...
        for(IRCServer server : IRCApi.getIrcServers())
        {
            if(server.getBotName() != e.getServer().getBotName() || server.getServerAddress() != e.getServer().getServerAddress())
            {
                for(IRCChannel c : server.getChannels())
                {
     ...
     {
         if(e.getCommand().equalsIgnoreCase("list") || e.getCommand().equalsIgnoreCase("players"))
         {
