 ...
 import org.nees.rpi.util.Either;
 ...
 public class M3DVLoader implements ModelLoader
 {
	 ...
	 * @param file
 	...
 	{
 		try {
 			if (input.getType() == Either.Type.LEFT) {
 				config = new XMLConfiguration(input.getLeft());
 			} else {
 				config = new XMLConfiguration(input.getRight());
 			}
 			processModelInfo();
 			processDataFile();
 			...
 			if (filename != null) {
 				try {
 					...
 					if (filename.contains("/") || filename.contains("\\")) {
 						...
 						if (input.getType() == Either.Type.LEFT) {
 							URL parentXMLURL = input.getLeft();
 							parentDir = parentXMLURL.getPath().substring(0, parentXMLURL.getPath().lastIndexOf("/"));
 						} else {
 							parentDir = input.getRight().getParentFile().getAbsolutePath();
 						}
