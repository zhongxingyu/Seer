 ...
 public class GUI_PartRobot extends GUI_Component{
         ...
         try{
             for(int i = 0; i < 8; i++){
                 ...
                 for(int j = 0; j < partsToGet.size(); j++){
                    System.out.println("=================" + partsToGet.get(j));                               
                    System.out.println("=================" + partsToGet.get(j));                               
                     if(partsToGet.get(j).getName().equals(myState.guiNestList.get(i).getPartHeld())){
     ...
     public void updateGraphics() {
         ...
         if(move || goHome){
             if(vrailX < moveToX){
                 vrailX+=2;
                 movePartsInGrippersX(2);
             }
             else if(vrailX > (moveToX+2)){
                 vrailX-=2;
                 movePartsInGrippersX(-2);
             }
             else if(railGripY < moveToY){
                 railGripY+=2;
                 movePartsInGrippersY(2);
             }
             else if(railGripY > (moveToY+2)){
                 railGripY-=2;
                 movePartsInGrippersY(-2);
             }
             else{
         ...
         if(getPart){
             if((gripperCoords[2*curGripper+1] < 35*(curGripper+1)) && !partIn){
                 gripperCoords[2*curGripper+1]+=2;
             }            
     ...
     public void paintComponent() //update PartRobot's drawing
     {
         ...
         myDrawing.subDrawings.add(new Drawing(x+vrailX-gripperRailOffset-5, y+railGripY, "GripperRail.png"));
