 ...
 public class ZScreen extends Canvas {
 	 ...
 	 * System.err.print("mouseDown "); if (inside(x,y)) { requestFocus(); //
 	 * System.err.println("inside"); return true; } else {
 	 * System.err.println("outside"); return false; } }
 	 */
 	...
 	static short unicode_to_zascii(char unicode) throws NoSuchKeyException {
 		...
 		if (unicode == '\b')
 			return 127;
 		else if ((unicode < 0x20) && (unicode != '\r' /* ' ' */)
 				&& (unicode != '\uu001b'))
 	...
 	protected synchronized void drawtext(Graphics g, int y, int x,
 	...
 	public synchronized void scrollLines(int top, int height, int lines) {
 		...
 		try {
 			...
 			g_store.copyArea(0, texttop + lines * fixedmetrics.getHeight(),
 					getSize().width,
 					(height - lines) * fixedmetrics.getHeight(), 0, -lines
 							* fixedmetrics.getHeight());
 			// g_store.setColor(getBackground());
 			g_store.setColor(zbcolor);
 			g_store.fillRect(0,
 					texttop + ((height - 1) * fixedmetrics.getHeight()),
 	...
 	// System.err.println("repaint: " + tm + " " + x + " " + y + " " + width +
 	// " " + height);
 	...
 	public void clear() {
 		...
 		try {
 			// g_store.setColor(getBackground());
 			g_store.setColor(zbcolor);
 			g_store.fillRect(0, 0, mysize.width, mysize.height);
 		} catch (NullPointerException booga) {
