 ...
 //		}
 	...
 	public void eventOccured(String event) {
 		...
 		if(event.contains("QR") && !(qrDetected || qrSeen.equals("yes"))) {
 			qrDetected = true;
 			qrSeen = "yes";
 			System.out.println("Saw something!!");
 			driveTowardsQR();				
 			qrDetected = false;
 		} else if (event.contains("bump") && !bumped) {
 			...
 			bumped = false;
 			locked = false;
 			while (nextMove > System.currentTimeMillis()) { Thread.yield(); }
 //			if(auto && !bumped) { m.getNewDriver(); }
 		}
 	}
 ...
 //		System.out.println("New call to driveRandomly");
 		...
 		while(!qrDetected && !bumped) {
 			if  (locked || nextMove > System.currentTimeMillis()) { Thread.yield(); continue; }
 			if (bumped || qrDetected) { break; }
