 ...
 public class ShellLink implements NativeLibraryClient
 {
    ...
    /** ALL_USERS = 1; the constant to use for selecting the all users. */
     ...
     public ShellLink(int type, String name) throws Exception, IllegalArgumentException
     {
         ...
         if (name == null) { throw (new IllegalArgumentException("the name parameter was null")); }
     ...
     public ShellLink(String name, int userType) throws Exception, IllegalArgumentException
     {
         ...
         if (nameEnd < 0) { throw (new Exception("illegal file name")); }
         ...
         if (loadLink(linkFileName) != SL_OK) { throw (new Exception(
     ...
     {
         if (SetArguments() != SL_OK) { throw (new Exception("could not set arguments")); }
         if (SetDescription() != SL_OK) { throw (new Exception("could not set description")); }
         if (SetHotkey() != SL_OK) { throw (new Exception("could not set hotkey")); }
         if (SetIconLocation() != SL_OK) { throw (new Exception("could not set icon location")); }
         if (SetPath() != SL_OK) { throw (new Exception("could not set target path")); }
         if (SetShowCommand() != SL_OK) { throw (new Exception("could not set show command")); }
         if (SetWorkingDirectory() != SL_OK) { throw (new Exception(
     ...
     {
         if (GetArguments() != SL_OK) { throw (new Exception("could not get arguments")); }
         if (GetDescription() != SL_OK) { throw (new Exception("could not get description")); }
         if (GetHotkey() != SL_OK) { throw (new Exception("could not get hotkey")); }
         if (GetIconLocation() != SL_OK) { throw (new Exception("could not get icon location")); }
         if (GetPath() != SL_OK) { throw (new Exception("could not get target ath")); }
         if (GetShowCommand() != SL_OK) { throw (new Exception("could not get show command")); }
         if (GetWorkingDirectory() != SL_OK) { throw (new Exception(
     ...
     {
         if ((show < MIN_SHOW) || (show > MAX_SHOW)) { throw (new IllegalArgumentException(
     ...
     public void save() throws Exception
     {
         ...
         if (result != SL_OK) { throw (new Exception("cannot resolve target")); }
         ...
         else if (result == SL_NO_SAVE) { throw (new Exception("the save operation failed")); }
     ...
     public void save(String name) throws Exception
     {
         ...
         // make sure the target actually resolves
         if (Resolve() != SL_OK) { throw (new Exception("cannot resolve target")); }
         ...
         // perform the save operation
         if (saveLink(name) != SL_OK) { throw (new Exception("the save operation failed")); }
