 ...
 public class PrintUtility
 {
 	...
 	public static Printer createPrinter( PrintService service )
 	{
 		...
 		PrintServiceAttribute attr = service
 				.getAttribute( PrinterMakeAndModel.class );
 		if ( attr != null )
 		...
 		// Copies attribute
 		printer.setCopiesSupported( service
 				.isAttributeCategorySupported( Copies.class ) );
 		int copies = 0;
 		Copies copiesObj = (Copies) service
 				.getDefaultAttributeValue( Copies.class );
 		if ( copiesObj != null )
 		{
 		...
 		// Collate attribute
 		boolean collateSupported = service
 				.isAttributeCategorySupported( SheetCollate.class );
 		printer.setCollateSupported( collateSupported );
 		if ( collateSupported )
 		{
 			SheetCollate collate = (SheetCollate) service
 					.getDefaultAttributeValue( SheetCollate.class );
 			if ( collate == null )
 		...
 		// Mode attribute
 		boolean modeSupported = service
 				.isAttributeCategorySupported( Chromaticity.class );
 		printer.setModeSupported( modeSupported );
 		if ( modeSupported )
 		{
 			Chromaticity chromaticity = (Chromaticity) service
 					.getDefaultAttributeValue( Chromaticity.class );
 		...
 		// Duplex attribute
 		boolean duplexSupported = service
 				.isAttributeCategorySupported( Sides.class );
 		printer.setDuplexSupported( duplexSupported );
 		if ( duplexSupported )
 		{
 			Sides sides = (Sides) service
