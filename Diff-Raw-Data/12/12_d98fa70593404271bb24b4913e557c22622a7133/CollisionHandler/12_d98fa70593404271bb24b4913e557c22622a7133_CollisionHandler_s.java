 ...
 public class CollisionHandler {
	...
	private static final float COEFFICIENT_OF_FRICTION = 0.01f;
 	...
 	public static void checkAndResolveCollision(PhysicsObject a, PhysicsObject b) {
 		...
 		if (!Float.isNaN(tangent.x))
 		{
			...
			a.angularVelocity -= (j * (r_ap.x * ci.normal.y - r_ap.y * ci.normal.x) / a.momentOfInertia) * COEFFICIENT_OF_FRICTION;
			b.angularVelocity += (j * (r_bp.x * ci.normal.y - r_bp.y * ci.normal.x) / b.momentOfInertia) * COEFFICIENT_OF_FRICTION;
 		}
 		// End friction code
