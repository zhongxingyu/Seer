 ...
 public final class PluginLoader {
 	...
 	private static void loadPlugin(File file) {
 		...
 		for (PluginMetadata.Annotation metadata : metadatas) {
			...
			} catch (NoSuchMethodException | NoClassDefFoundError | SecurityException | InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
 				Logger.error("[Plugin Loader] Failed to construct plugin \"" + metadata.getName() + "\"!");
				Logger.error("[Plugin Loader] Caused by: " + e.toString());
 				continue;
 			}
 		}
 	}
