 ...
 public class NexusClassLoader extends URLClassLoader{
 	...
 	private static Logger Log = NexusLog.MakeLogger("ClassLoader");
 	...
 	public NexusClassLoader(URL[] sources){
		...
		this.Parent = (URLClassLoader) this.getClass().getClassLoader();
 		...
 		this.addTransformerExclusion("com.nexus.TestTransformer");
 		
 		this.registerTransformer("com.nexus.TestTransformer");
 		...
 		if(DEBUG_CLASSLOADING_SAVE){
 			...
 			if(temp_folder.exists()){
 				Log.info("DEBUG_CLASSLOADING_SAVE enabled,  but 10 temp directories already exist, clean them and try again.");
 				temp_folder = null;
 			}else{
 				Log.info(String.format("DEBUG_CLASSLOADING_SAVE Enabled, saving all classes to \"%s\"", temp_folder.getAbsolutePath().replace('\\', '/')));
 				temp_folder.mkdirs();
 			}
 		}
 	}
 	...
 	public void registerTransformer(String transformerClassName){
 		try{
 			...
 			if(IClassTransformer.class.isInstance(transformer)){
 				...
 				Log.log(Level.SEVERE, "ASM Transformer %s is invalid! It cannot be cast to IClassTransformer", transformerClassName);
 			}
 		}catch(Exception e){
 			NexusLog.log(Log.getName(), Level.SEVERE, e, "A critical problem occured registering the ASM transformer class %s", transformerClassName);
 		}
 	}
 		...
 		try{
 			...
 			if(urlConnection instanceof JarURLConnection && lastDot > -1){
 				...
 				if(jf != null && jf.getManifest() != null){
 					...
 					if(pkg == null){
 						...
 						if(pkg.isSealed() && !pkg.isSealed(jarUrlConn.getJarFileURL())){
 							NexusLog.log(Log.getName(), Level.SEVERE, "The jar file %s is trying to seal already secured path %s", jf.getName(), pkgname);
 						}else if(isSealed(pkgname, mf)){
 							NexusLog.log(Log.getName(), Level.SEVERE, "The jar file %s has a security seal for path %s, but that path is defined and not secure", jf.getName(), pkgname);
 						}
 					}
 				}
 				jf.close(); // correct?
 				...
 				if(pkg == null){
 					...
 					NexusLog.log(Log.getName(), Level.SEVERE, "The URL %s is defining elements for sealed path %s", urlConnection.getURL(), pkgname);
 				}
 			}
			...
			Log.info(String.format("Class %s Loaded!", name));
			
 			return cl;
 		}catch(Throwable e){
 			this.InvalidClasses.add(name);
 			if(DEBUG_CLASSLOADING){
 				NexusLog.log(Log.getName(), Level.WARNING, e, "Exception encountered attempting classloading of %s", name);
 			}
 	...
 	private byte[] readFully(InputStream stream){
 		try{
 			...
 			Log.log(Level.WARNING, "Problem loading class", t);
 			return new byte[0];
 		}
 	}
 	...
 	private void saveTransformedClass(byte[] data, String transformedName){
 		...
 		try{
 			Log.fine(String.format("Saving transformed class \"%s\" to \"%s\"", transformedName, outFile.getAbsolutePath().replace('\\', '/')));
 			OutputStream output = new FileOutputStream(outFile);
 			output.write(data);
 			output.close();
 		}catch(IOException ex){
 			Log.log(Level.WARNING, "Could not save transformed class \"%s\"", transformedName);
 			ex.printStackTrace();
 		}
 	}
 	...
 	public byte[] getClassBytes(String name) throws IOException{
 		...
 		try{
 			...
 			if(classResource == null){
 				if(DEBUG_CLASSLOADING){
 					Log.finest(String.format("Failed to find class resource %s", name.replace('.', '/').concat(".class")));
 				}
 				return null;
 			}
 			classStream = classResource.openStream();
 			if(DEBUG_CLASSLOADING){
 				Log.finest(String.format("Loading class %s from resource %s", name, classResource.toString()));
 			}
