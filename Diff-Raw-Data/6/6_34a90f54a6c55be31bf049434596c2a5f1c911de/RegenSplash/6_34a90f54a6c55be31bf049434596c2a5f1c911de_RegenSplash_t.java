 ...
 public class RegenSplash extends AutomationBase
 {
 	...
 	public boolean run()
 	{
		...
		"&& ruby " + SCRIPT_NAME + " \\\"" + tocURL + "\\\"" + (this.product != null && this.product.length() == 0 ? " \\\"" + this.product + "\\\" " : " ") +
 		...
 		"&& echo -------------------" +
 		"&& echo version_packages.txt " +
 		"&& echo -------------------" +
 		"&& cat \\\"" + getTmpDirectory(randomInt) + "/version_packages.txt\\\" " +
 		"&& echo -------------------" +
 		"&& echo product_packages.txt " +
 		"&& echo -------------------" +
 		"&& cat \\\"" + getTmpDirectory(randomInt) + "/product_packages.txt\\\"";
 		...
 		final String[] environment = new String[]	{ PASSWORD_ENV_VARIABLE_NAME + "=" + this.getPassword(), "CVSEDITOR=/opt/automation-interface/cvs/dummyeditor.sh" };
