 ...
 public class DiskIndex
 {
 	 ...
 	 */
 	public static DiskIndex createIndex( IDocumentManager documentManager,
 			String name, int sonNumber, IDiskArray keyList, boolean isSorted )
 			throws IOException, DataException
 	{
 		assert sonNumber <= 127;
 		assert keyList.size( ) > 0;
 		DiskIndex indexTree = new DiskIndex( documentManager,
 				name,
 				sonNumber,
 				keyList,
 				isSorted );
 	 ...
 	 */
 	public static DiskIndex createIndex( IDocumentManager documentManager,
 			String name, IDiskArray keyList, boolean isSorted )
 			throws IOException, DataException
 	{
 	 ...
 	 * @throws DataException 
 	 */
 	public static DiskIndex loadIndex( IDocumentManager documentManager,
 			String name ) throws IOException, DataException
 	{
 	 ...
 	 * @throws DataException 
 	 */
 	DiskIndex( IDocumentManager documentManager, String name, int granularity,
 	 ...
 	 * @throws DataException 
 	 */
 	DiskIndex( IDocumentManager documentManager, String name )
 			throws IOException, DataException
 	{
 		this.name = name;
 		this.documentManager = documentManager;
 	...
 	private void loadFromDisk( ) throws IOException, DataException
 	{
 		...
 		{
 			throw new DataException( ResourceConstants.OLAPFILE_FORMAT_INVALID,
 					name );
 		}
 	}
 	 ...
 	 * @throws DataException 
 	 */
 	private void produce( IDiskArray keyList, boolean isSorted )
 			throws IOException, DataException
 	{
 		...
 		while ( sonStartOffset != null && sonStartOffset.size( ) > 1 )
 		{
 			...
 			sonStartOffset = writeNonLeafNode( sortedKeyArray,
 					sonStartOffset,
 					numberOfLevel,
 					sonLevelTotalNumber );
 			numberOfLevel++;
 ...
 //		documentObject.skipBytes( 6 );
 			...
 			throws IOException, DataException
 	{
 		...
 		for ( int i = 0; i < startOffset.size( ); i++ )
 		{
 			...
 			{
 				documentObject.writeByte( sonLevelTotalNumber
 						- ( startOffset.size( ) - 1 ) * degree );
 	...
 	public IndexKey findFirst( Object[] key ) throws IOException, DataException
 	{
 		...
 		{
 			throw new DataException( ResourceConstants.OLAPFILE_DATA_ERROR,
 					name );
 		}
 	...
 	{
 		if ( CompareUtil.compare( node.minKeyValue, key ) > 0
 				|| CompareUtil.compare( node.maxKeyValue, key ) < 0 )
 		{
 			return false;
 		}
 		else
 	...
 	public IDiskArray find( ISelection[] selections ) throws IOException, DataException
 	{
 		...
 		boolean find = false;
 		NodeSelection nodeSelection = new NodeSelection( currentNode,
 				selections.length );
 		...
 		for ( int i = 0; i < numberOfLevel - 1; i++ )
 		{
 			...
 			{
 				processNonLeafNode( (NodeSelection) validNodeList.get( j ),
 						selections,
 						validSonNode );
 			}
 	...
 	{
 		if ( node.maxKeyValue == null
 				|| node.minKeyValue == null
 	 ...
 	 * @throws IOException
 	 */
 	private void processNonLeafNode( NodeSelection nodeSelection,
 			ISelection[] selections, IDiskArray resultList ) throws IOException
 	{
 		NonLeafNode tempNode = null;
 	...
 	{
 		if ( selection.getMin( ) != null
 				&& CompareUtil.compare( node.maxKeyValue, selection.getMin( ) ) < 0 )
 		{
 			return false;
 		}
 		if ( selection.getMax( ) != null
 				&& CompareUtil.compare( node.minKeyValue, selection.getMax( ) ) > 0 )
 		{
 			return false;
 		}
 		return true;
 ...
 class NodeSelectionCreator implements IStructureCreator
 {
 	...
 	public IStructure createInstance( Object[] fields )
 	{
 		...
 		NodeSelection nodeSelection = new NodeSelection( node,
 				selections.length( ) );
 		for ( int i = 0; i < selections.length( ); i++ )
 		{
