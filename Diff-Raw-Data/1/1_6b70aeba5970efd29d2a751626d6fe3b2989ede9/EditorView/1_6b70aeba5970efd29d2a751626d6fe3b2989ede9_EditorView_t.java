 ...
 public class EditorView extends JFrame implements Observer
 {
     ...
     public EditorView()
     {
         ...
         this.addWindowListener(new WindowListener(){
             ...
             public void windowClosing(WindowEvent e){
                 commands.get(CmdCloseAll.class).execute(new Context());
             }
         ...
         if(obs instanceof Session){
             ...
             while(mailbox.hasMail()){
                 ...
                 if(m.getMailType() == MailType.UNSAVED_CHANGES){
                     ...
                     unsavedChanges(m.getBuf());
                 }else if(m.getMailType() == MailType.WELL_FORMED_ERROR){
                     tabs.setSelectedComponent(m.getBuf());
                     wellFormedDialog(false);
                 }else if(m.getMailType() == MailType.WELL_FORMED_SUCCESS){
                     tabs.setSelectedComponent(m.getBuf());
                     wellFormedDialog(true);
                 }
             }
         }
     ...
     public void unsavedChanges(HTMLBuffer buf){
         final JFrame frame = new JFrame();
         final HTMLBuffer buff = buf;
         frame.setLocationRelativeTo(this);
         frame.setDefaultCloseOperation(DISPOSE_ON_CLOSE);
         frame.setLayout(new GridLayout(2,1));
         frame.add(new JLabel("Unsaved changes for "+buf.getFile().getName()));
         JPanel buttons = new JPanel(new GridLayout(1,3));
         JButton cancel = new JButton("Cancel");
         ...
         yes.addActionListener(new ActionListener(){
             public void actionPerformed(ActionEvent e){
                 commands.get(CmdSave.class).execute(new Context());
                 frame.dispose();
             }
         });
         no.addActionListener(new ActionListener(){
             public void actionPerformed(ActionEvent e){
                 Context c = new Context();
                 c.setBuffer(buff);
                 commands.get(CmdForceClose.class).execute(c);
                 frame.dispose();
             }
         });
         cancel.addActionListener(new ActionListener(){
