 ...
 public class MyXMLHandler implements DocHandler {
 	...
 	private CASTableCellValue cellValueElement;
 	...
 	private void endCASSessionElement(String eName) {
 		...
 		default:
 			...
 			Application.debug("unknown cas session mode:" + constMode);
 		}
 	...
 	private void processCellPairList() {
 		...
 		try {
 			...
 			CASTable table = app.getCasView().getConsoleTable();
 			...
 			table.setFocusAtRow(table.getRowCount()-1, CASPara.contCol);
 		} catch (Exception e) {
 			cellPairList.clear();
 			e.printStackTrace();
 	...
 	private void startGeoGebraElement(String eName, LinkedHashMap attrs) {
 		if (eName.equals("euclidianView")) {
 			...
 			Application.debug("unknown tag in <geogebra>: " + eName);
 		}
 	}
 	...
 	private void startMacroElement(String eName, LinkedHashMap attrs) {
 		if (eName.equals("macroInput")) {
 			...
 			Application.debug("unknown tag in <macro>: " + eName);
 		}
 	}
 	...
 	private void startEuclidianViewElement(String eName, LinkedHashMap attrs) {
 		...
 		default:
 			Application.debug("unknown tag in <euclidianView>: " + eName);
 		}
 	...
 	private void startCASViewElement(String eName, LinkedHashMap attrs) {
 		boolean ok = true;
 		CASView ev = app.getCasView();
 
 		switch (eName.charAt(0)) {
 		case 's':
 			if (eName.equals("size")) {
 				ok = handleCASSize(ev, attrs);
 				break;
 			}
 
 		default:
 			Application.debug("unknown tag in <casView>: " + eName);
 		}
 
 		if (!ok)
 			Application.debug("error in <casView>: " + eName);
 	...
 	private boolean handleSpreadsheetSize(SpreadsheetView spreadsheet,
 			LinkedHashMap attrs) {
 		if (app.isApplet())
 			return true;
 	...
 	private boolean handleCASSize(CASView casView, LinkedHashMap attrs) {
 		if (app.isApplet())
 			return true;
 		...
 		try {
 			...
 			casView.setPreferredSize(new Dimension(width, height));
 			return true;
 		} catch (Exception e) {
 			return false;
 		}
 	...
 	private void startGUIElement(String eName, LinkedHashMap attrs) {
 		...
 		default:
 			Application.debug("unknown tag in <gui>: " + eName);
 		}
 	...
 	private void handleConstruction(LinkedHashMap attrs) {
 		try {
 			...
 			Application.debug("error in <construction>");
 		}
 	}
 	...
 	private void initMacro(LinkedHashMap attrs) {
 		try {
 			...
 			Application.debug("error in <macro>");
 		}
 	}
 	...
 	private void startCASSessionElement(String eName, LinkedHashMap attrs) {
 		...
 		case MODE_CAS_SESSION:
 			if (eName.equals("cellPair")) {
 				casSessionMode = MODE_CAS_CELL_PAIR;
 				cellValueElement = new CASTableCellValue();
 			} else {
 				Application.debug("unknown tag in <cellPair>: " + eName);
 			}
 			break;
 	...
 	private void startCellOutputElement(String eName, LinkedHashMap attrs) {
 		...
 		case 'c':
 			if (eName.equals("color")) {
 				...
 				if (ok) {
 					...
 					cellPairList.add(cellValueElement);
 		...
 		default:
 			Application.debug("unknown tag in <outputCell>: " + eName);
 		}
 
 		if (!ok)
 			Application.debug("error in <outputCell>: " + eName);
 	...
 	private void startCellInputElement(String eName, LinkedHashMap attrs) {
 		...
 		default:
 			Application.debug("unknown tag in <inputCell>: " + eName);
 		}
 	...
 	private void startConstructionElement(String eName, LinkedHashMap attrs) {
 		...
 		default:
 			Application.debug("unknown construction mode:" + constMode);
 		}
 	}
 	...
 	private GeoElement getGeoElement(LinkedHashMap attrs) {
 		...
 		if (label == null || type == null) {
 			Application.debug("attributes missing in <element>");
 			return geo;
 		}
 	...
 	private void startGeoElement(String eName, LinkedHashMap attrs) {
 		...
 		default:
 			Application.debug("unknown tag in <element>: " + eName);
 		}
 	...
 	private boolean handleObjColor(LinkedHashMap attrs) {
 		...
 		if (red != null && green != null && blue != null)
 			try {
 				...
 				Application.debug("Error loading Dynamic Colors");
 			}
 	...
 	private boolean handleEqnStyle(LinkedHashMap attrs) {
 		...
 		if (geo.isGeoLine()) {
 			...
 			if (style.equals("implicit")) {
 				...
 				Application.debug("unknown style for line in <eqnStyle>: "
 						+ style);
 				return false;
 			}
 		}
 		// conic
 			...
 			if (style.equals("implicit")) {
 				...
 				Application.debug("unknown style for conic in <eqnStyle>: "
 						+ style);
 				return false;
 			}
 		} else {
 			Application.debug("wrong element type for <eqnStyle>: "
 					+ geo.getClass());
 			return false;
 		}
 		return true;
 	...
 	private boolean handleInputExpression(LinkedHashMap attrs) {
 		try {
 			cellValueElement.setCommand((String) attrs.get("value"));
 			return true;
 		} catch (Exception e) {
 			return false;
 		}
 	...
 	private boolean handleOutExpression(LinkedHashMap attrs) {
 		try {
 			cellValueElement.setOutput((String) attrs.get("value"));
 			cellValueElement.setOutputAreaInclude(true);
 			return true;
 		} catch (Exception e) {
 			return false;
 		}
 	...
 	private void startCommandElement(String eName, LinkedHashMap attrs) {
 		...
 		if (eName.equals("input")) {
 			...
 			Application.debug("unknown tag in <command>: " + eName);
 			...
 			if (attrs.size() != cmdOutput.length) {
 				Application
 						.debug("error in <output>: wrong number of labels for command "
 								+ cmdName);
 				Application.debug("   cmdOutput.length = " + cmdOutput.length
 						+ ", labels = " + attrs.size());
 				return false;
 			}
 	...
 	private void startExpressionElement(String eName, LinkedHashMap attrs) {
 		...
 		try {
 			...
 			if (result != null && label != null && result.length == 1) {
 				...
 				Application.debug("error in <expression>: " + exp + ", label: "
 						+ label);
 			}
