 ...
 public class Player extends Entity {
 	...
 	private void checkBorders(int keycode) {
		...
		if (getX() <= 1 && keycode == Keyboard.KEY_A) {
 			
 			setScene(getSceneX()-1, getSceneY());
 			World.setCurrentMap(getSceneX(), getSceneY());
 			setX(Main.SCREEN_PLAYABLE_WIDTH);
 			
		} else if (getX() >= Main.SCREEN_PLAYABLE_WIDTH-getW()-5 && keycode == Keyboard.KEY_D) {
 			
 			setScene(getSceneX()+1, getSceneY());
 			World.setCurrentMap(getSceneX(), getSceneY());
 			setX(-Main.gridSize);
 			
		} else if (getY() <= 1 && keycode == Keyboard.KEY_W) {
 			
 			setScene(getSceneX(), getSceneY()+1);
 			World.setCurrentMap(getSceneX(), getSceneY());
 			setY(Main.SCREEN_PLAYABLE_HEIGHT);
 			
		} else if (getY() >= Main.SCREEN_PLAYABLE_HEIGHT-getH()-2 && keycode == Keyboard.KEY_S) {
