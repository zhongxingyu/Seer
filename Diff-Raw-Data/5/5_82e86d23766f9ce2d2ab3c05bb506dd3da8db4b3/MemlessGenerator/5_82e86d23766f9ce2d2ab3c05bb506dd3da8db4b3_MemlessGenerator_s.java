 ...
 public class MemlessGenerator {
 	...
 	public static void main(String[] args) throws Exception
     {
 		...
 		for (int i = 1; i < args.length; i++) {
 			try {
 				process(output, args[i], enumHelper, config);
 			} catch (Exception e) {
 	...
 	private static void process(File output, MemlessParser parser, GsonEnumHelper enumHelper, GeneratorConfiguration config) throws IOException, Exception {
 		...
 		for (ProtobufEnum curEnum : parser.getEnums()) {
 			String curEnumData = generateEnum(curEnum, enumHelper, config);
 			if (parser.getOuterClassName() != null) {
 	...
 	private static String generateMessage(ProtobufMessage curMessage, String outerClassName, GeneratorConfiguration config, GsonEnumHelper enumHelper) {
 		...
 		for (ProtobufEnum curEnum : curMessage.getEnums()) {
 			result.append(generateEnum(curEnum, enumHelper, config));
 		}
 		result.append("}\n");
 		return result.toString();
 	}
 	...
 	private static String generateEnum(ProtobufEnum pEnum, GsonEnumHelper enumHelper, GeneratorConfiguration config) {
 		StringBuilder result = new StringBuilder();
 		result.append("public enum ");
 		result.append(pEnum.getName());
 
         String enumInterface = config.getEnumImplementsInterface();
         if (enumInterface != null && !enumInterface.isEmpty())
         {
         	result.append(" implements ").append(enumInterface);
         }
         result.append(" { \n");
