 ...
 public class DFSServer extends DFSComponent {
   ...
   public class Checkout {
     ...
     public DFSFilename getCachePath() {
       ...
       if (transaction == null)
         throw new IllegalStateException("Non-transactioned checkout has no cache entry!");
 
       System.err.println("getCP: file=" + file + ", txn = " + transaction);
     ...
     public DFSFilename getCheckInPath() {
       if (!file.getPath().startsWith("/tx"))
         return file;
 
      int pathIndex = file.getPath().indexOf('/', 2);
       String newPath = file.getPath().substring(pathIndex);
       return new DFSFilename(DFSFilename.kPrefixString +
                              file.getOwningServer() +
                              newPath);
     }
     ...
     public String getData() {
       ...
       if (getParent() == null) {
         throw new IllegalStateException("No parent or entry for file version " + version + "!");
       }
     ...
     public void commit(int commitTime) {
       System.err.println("Committing checkout " + file + " @ ver " + version);
       PersistentStorageCache.CacheEntry entry = cache.get(getCachePath());
       if (entry != null &&
           !entry.getState().equals(PersistentStorageCache.CacheState.INVALID)) {
         cache.rename(getCachePath(), getCheckInPath());
         version.commit(commitTime);
       } else {
         PersistentStorageCache.CacheEntry checkInEntry = cache.get(getCheckInPath());
         System.err.println("Ver: " + version.nextVersion() + " vs " + checkInEntry.getVersion());
         if (version.nextVersion().compareTo(checkInEntry.getVersion()) > 0) {
           System.err.println("setting cache");
           cache.set(checkInEntry.getKey(), version.nextVersion(), getData(), PersistentStorageCache.kNotOwned, exists());
         }
       }
     }
   ...
   public class FileState {
     ...
     public FileState(DFSFilename file, PersistentStorageCache cache) {
       ...
       System.err.println("Instantiating FS, file=" + file + ", ver=" + entry.getVersion());
     ...
     public Errors commit(FileVersion ver, int commitTime) {
       System.err.println("FS Committing ver " + ver + " @ " + commitTime);
       Checkout commit = checkouts.remove(ver);
       if (commit == null)
         return Errors.VersionNotFound;
 
       System.err.println("Found commit!");
 ...
 /*    OwnershipRequestWrapper req = transferOwnerRequests.get(filename);
   ...
   private void commitTransaction(TransactionId id) {
     ...
     try {
       ...
       for (DFSFilename f : t.checkouts.keySet()) {
         ...
         System.err.println("Commit is ok, writing...");
   ...
   private Errors createFile(TransactionId id,
     ...
     if (latestCommit == null) {
       ...
       newVersion = new FileVersion(id, 0, 1);
     } else {
       if (fs.checkouts.get(latestCommit).entry.exists)
         return Errors.FileAlreadyExists;
 
       newVersion = new FileVersion(id, latestCommit.getVersion(), 1);
     }
     ...
     newCheckout = new Checkout(transactions.get(id), file, newVersion, newOwner != PersistentStorageCache.kNotOwned);
     ...
     System.err.println("Sending response; " + sdMsg);
