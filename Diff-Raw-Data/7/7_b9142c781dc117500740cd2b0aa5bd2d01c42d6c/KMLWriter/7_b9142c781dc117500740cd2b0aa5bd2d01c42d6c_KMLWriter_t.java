 ...
 public class KMLWriter extends OutputStreamWriter {
     private static final Logger LOGGER = Logger.getLogger(KMLWriter.class.getPackage().getName());
     ...
     public void writeFeaturesAsRaster(final FeatureCollection features, final MapLayer layer,
         final int order) throws IOException, AbortedException {
     ...
     public void writeFeaturesAsVectors(final FeatureCollection features, final MapLayer layer)
      ...
      */
     public void startFolder(String name, String description)
         throws IOException {
         write("<Folder>");
     ...
     protected void writePlaceMarkPoint(Geometry geom, GeometryTransformer trans)
         throws IOException, TransformerException {
         final Coordinate[] coordinates = getCentroid(geom).getCoordinates();
         write("<Point><coordinates>" + coordinates[0].x + "," + coordinates[0].y + ","
             + coordinates[0].z + "</coordinates></Point>");
     }
     ...
     protected boolean isMultiPart(Geometry geom) {
         ...
         return (geomClass.equals(MultiPolygon.class) || geomClass.equals(MultiPoint.class)
         || geomClass.equals(MultiLineString.class));
     }
                     ...
                     try {
                        ...
                        startDocument(id, layer.getTitle());
                         ...
                         for (Iterator it = ruleList.iterator(); it.hasNext();) {
                             Rule r = (Rule) it.next();
                             LOGGER.finer(new StringBuffer("applying rule: ").append(r.toString())
                                                                             .toString());
                             ...
                             if ((filter == null) || filter.evaluate(feature)) {
                                 ...
                                 processVectorSymbolizers(feature, symbolizers, scaleRange,
                                     featureLabel);
                             }
                         }
                         ...
                         // we have written out the style, so now lets write out the geometry
                         ...
                         //e.printStackTrace();
                         LOGGER.warning(new StringBuffer("KML transform for feature failed ").append(
     ...
     private void processStylersRaster(final FeatureCollection features,
         final FeatureTypeStyle[] featureStylers, final MapLayer layer, final int order)
         throws IOException, IllegalAttributeException {
                 ...
                 try {
                     ...
                     for (Iterator it = ruleList.iterator(); it.hasNext();) {
                         ...
                         if ((filter == null) || filter.evaluate(feature)) {
                             ...
                             processRasterSymbolizers(feature, symbolizers, order);
                             layerCounter++;
                         }
                     }
     ...
     /**
      * Sorts the rules into "If" rules and "Else" rules.
      * The rules are sorted into their respective lists.
      * @param rules
      * @param ruleList
      * @param elseRuleList
     ...
     private void processVectorSymbolizers(final Feature feature, final Symbolizer[] symbolizers,
         Range scaleRange, StringBuffer featureLabel) throws IOException, TransformerException {
     ...
     *
      ...
      */
     private void processRasterSymbolizers(final Feature feature, final Symbolizer[] symbolizers,
         final int order) throws IOException, TransformerException {
         ...
         write(new StringBuffer("<href>layer_").append(order).append(".png</href>")
                                               ...
                                               .append("</LatLonBox>").append("</GroundOverlay>")
                                               .toString());
     }
     ...
     private void writeStyle(final Style2D style, final String id, Symbolizer sym)
         ...
         if (style instanceof PolygonStyle2D && sym instanceof PolygonSymbolizer) {
             ...
             styleString.append(
                 "<Icon><href>root://icons/palette-3.png</href><x>224</x><w>32</w><h>32</h></Icon>");
             styleString.append("</IconStyle>");
                 ...
                 if ((marks.length > 0) && (marks[0] != null)) {
                     ...
                     if (p instanceof Color) {
                         styleString.append("#").append(intToHex(opacity))
                                    .append(colorToHex((Color) p)); //transparancy needs to come from the opacity value.
                     } else {
                         styleString.append("#ffaaaaaa"); //should not occure in normal parsing
                     }
                 } else {
