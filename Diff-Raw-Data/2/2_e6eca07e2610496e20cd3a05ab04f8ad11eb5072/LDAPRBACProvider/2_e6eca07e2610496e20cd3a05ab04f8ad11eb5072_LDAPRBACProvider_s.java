 ...
 class LDAPRBACProvider implements RBACProvider, LDAPProperties {
 	...
 	static Map<String, String> readProperties(String keyRoot, Map source)
 		...
 		for (int i = 0; true; i++) {
 			...
 			if (front.length() == 0 || back.length() == 0)
 				throw new IllegalArgumentException(
 						"Format is not reconized! key: " + key + " value: "
 								+ value);
 			if (result == null)
 	...
 	class LDAPRBACQuery implements RBACQuery, LDAPProperties {
 		...
 		private Set<String> _multipleUserOU;
 		private Set<String> _multipleRoleOU;
 		...
 		public String[] assignedUsers(String role)
 			...
 			try {
 				...
 				if (_userRoles != null) {
 					...
 					found = _engine.queryRelations(role, _roleBase, _roleId,
 							_userBase, _userRoles, checkRole, list);
 
 				} else {
 					// _roleUsers!=null, foreign key on the role
 					found = _engine.queryFilteredFields(role, _roleBase,
 							_roleId, _roleUsers, _userBase, list);
 				}
 		...
 		public String[] authorizedUsers(String role)
 			...
 			try {
 				...
 				short found;
 				if (_roleAscen != null)
 					found = _engine.queryInheritRelation(_roleBase, _roleId,
 							_roleAscen, list);
 				else
 					found = _engine.queryInheritReference(_roleBase, _roleId,
 							_roleDescen, list);
 				if (found == LDAPQueryEngine.SUBJECT_NOT_FOUND)
 					throw new RoleNotFoundException("Role, " + role
 							+ ", is not found!");
 				...
 				if (_userRoles != null) {
 					// foreign key on the user
 					_engine.queryRelations(list, _roleBase, _roleId, _userBase,
 							_userRoles, result);
 				} else { // if (_roleUsers!=null)
 					// foreign key on the role
 					_engine.queryFilteredFields(list, _roleBase, _roleId,
 							_roleUsers, _userBase, result);
 				}
 				return prefix(result);
 			} catch (NamingException ne) {
 				if (LOG.isInfoEnabled())
 		...
 		public String[] assignedRoles(String user)
 			...
 			ArrayList<String> list = new ArrayList<String>();
 			Iterator<String> mmou = _multipleUserOU.iterator();
 			String fuser = "";
 			String userBaseHit = null;
 			while (mmou.hasNext()) {
 				String userBase = mmou.next();
 				try {
 					fuser = _engine.searchUser(user, _userId, userBase);
 					if (!"".equals(fuser)) {
 						userBaseHit = userBase;
 						break;
 					}
 				} catch (Exception e) {
 			...
 			try {
 				...
 				if (_userRoles != null) {
 					...
 					result = _engine.queryFields(user, userBaseHit, _userId,
 							_userRoles, list);
 					...
 					while (mrou.hasNext()) {
 						result = _engine.queryRelations(fuser, mrou.next(),
 								_roleUsers, list);
 					}
 				} else {
 					// TODO: check this query
 			...
 			try {
 				...
 				while (mmou.hasNext()) {
 					...
 					if (_userAllroles != null) {
 						...
 						result = _engine.queryFields(user, mmou.next(),
 								_userId, _userAllroles, list);
 					} else if (_userRoles != null) {
 						result = _engine.queryFields(user, mmou.next(),
 								_userId, _userRoles, list);
 					} else {
 						boolean checkUser = true;
 						result = _engine
 								.queryRelations(user, mmou.next(), _userId,
 										_roleBase, _roleUsers, checkUser, list);
 					}
 				...
 				if (_roleAscen != null)
 					_engine.queryInheritReference(_roleBase, _roleId,
 							_roleAscen, list);
 				else
 					_engine.queryInheritRelation(_roleBase, _roleId,
 							_roleDescen, list);
 		 ...
 		 */
 		public String[] topRoles(String realm) throws RBACException,
 				RemoteException {
 			...
 			try {
 				...
 				if (_roleAscen == null && _roleDescen == null) {
 					...
 					_engine.queryExistRelation(_roleBase, _roleId, _roleDescen,
 							_roleBase, descen);
          ...
          */
 		/*
 		 * public String[] allRoles( String realm ) throws RBACException,
 		 * RemoteException { if (!_realm.equals(realm)) throw new
 		 * RBACException("Unsupported realm, "+realm);
 		 * 
 		 * try { ArrayList list = new ArrayList(); // return all roles
 		 * _engine.queryExtent(_roleBase, _roleId, list);
 		 * 
 		 * return prefix(list); } catch ( NamingException ne ) { if
 		 * (LOG.isInfoEnabled()) LOG.info(ne.getMessage(),ne); throw new
 		 * RBACException(ne); } }
 		 */
 		...
 		// public String searchUser(String user, String userBase) throws
 		// UserNotFoundException, NamingException {
 		//
 		...
 		//            
 		// NamingEnumeration<SearchResult> answer = rootCtx.search(lookup,
 		// filter, ctls);
 		// while (answer.hasMore()) {
 		// SearchResult sr = (SearchResult) answer.next();
 		// if(LOG.isDebugEnabled()) {
 		// LOG.debug("User search on"+this._userBase+" returned:" +
 		...
 		/**
 		 * @see org.intalio.tempo.security.rbac.RBACQuery#roleOperationsOnObject(java.lang.String,
 		 *      java.lang.String)
 		 */
 		public String[] roleOperationsOnObject(String role, String object)
 			...
 			try {
 				...
 				if (_permRoles != null) {
 					...
 					_engine.queryRelations(roles, _roleBase, _roleId,
 							_permBase, _permRoles, perms);
 				} else { // if (_rolePerms!=null)
 					_engine.queryFields(roles, _roleBase, _roleId, _rolePerms,
 							perms);
 				}
 				...
 				TreeSet<String> result = new TreeSet<String>();
 				_engine.queryFields(perms, _permBase, _permId, _permObjects,
 						result);
 		...
 		public String[] userOperationsOnObject(String user, String object)
 			...
 			try {
 				...
 				short found;
 				if (_userRoles != null) {
 					found = _engine.queryFields(user, _userBase, _userId,
 							_userRoles, roles);
 				} else {
 					boolean checkUser = true;
 					found = _engine.queryRelations(user, _userBase, _userId,
 							_roleBase, _roleUsers, checkUser, roles);
 				}
 				if (found == LDAPQueryEngine.SUBJECT_NOT_FOUND)
 					throw new RoleNotFoundException("User, " + user
 							+ ", is not found!");
 				if (found == LDAPQueryEngine.RELATION_NOT_FOUND)
 					return EMPTY_STRINGS;
 		...
 		public Property[] userProperties(String user)
 			...
 			try {
 				...
 				_engine.queryProperties(user, _userBase, _userId, _userProp,
 						result);
 
 				return (Property[]) result.values().toArray(
 						new Property[result.size()]);
 			} catch (NamingException ne) {
 				if (LOG.isInfoEnabled())
 					LOG.info(ne.getMessage(), ne);
 				throw new RBACException(ne);
 		...
 		public Property[] roleProperties(String role)
 			...
 			try {
 				HashMap<String, Property> result = new HashMap<String, Property>();
 				_engine.queryProperties(role, _roleBase, _roleId, _roleProp,
 						result);
