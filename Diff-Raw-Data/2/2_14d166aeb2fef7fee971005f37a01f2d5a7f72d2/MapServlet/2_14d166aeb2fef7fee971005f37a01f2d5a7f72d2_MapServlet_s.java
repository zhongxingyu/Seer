 /*
  * Copyright (2005) Schibsted Sk AS
  * 
  */
 ...
 public class MapServlet extends HttpServlet {
     ...
     //globale konstanter. Hvor br disse settes?? xml fil.
     ...
     final static int imgWidth = 350;//bildestrrelse i pixler, bredde
    final static int imgHeigth = 500;//bildestrrelse i pixler, hyde
     ...
     private String authenticate() throws RemoteException, ServiceException{
         //hvordan sjekke om vi allerede har en valid token? sende eksisterende token med i requesten? Kjr auth.validateToken(token);
     ...
     throws ServletException, IOException {
         ...
         if(!vMapPoints.isEmpty()){
             /*
              * - sjekke om requesten kommer fra sksiden eller navigering i kartet.
              */
              ...
              * - hent zoomlevel. Mest aktuelt ved navigering, men kan ogs vre innsendt med ett sketreff.
              * - sjekk om det er et eller flere punkt
             */
             MapEnvelope me = new MapEnvelope();
             if(action.compareToIgnoreCase("viewone") == 0){//enkelt bedriftstreff. M beregne envelope utifra ett pkt, zoomlevel og bildestrrelse
                 MapPoint mp = (MapPoint) vMapPoints.get(0);
                 me = coordHelper.makeEnvelope(mp, zoomLevel1, imgWidth, imgHeigth);
             }
             else if(action.compareToIgnoreCase("viewmany") == 0){//enkelt bedriftstreff. M beregne envelope utifra ett pkt, zoomlevel og bildestrrelse
