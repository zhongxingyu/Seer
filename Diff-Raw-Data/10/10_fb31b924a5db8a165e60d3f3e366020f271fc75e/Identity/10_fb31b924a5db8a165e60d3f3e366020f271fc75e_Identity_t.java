 					...
 					createKey.addSelectionListener(new SelectionListener() {
 						...
 						public void widgetSelected(SelectionEvent e) {
 							if (radio_RSA.getSelection()){
 								...
 								iMgr.saveRSAKeyToKeystore(Identity.this.identityName, password1.getText(), null, bi_rsaN, bi_rsaP, bi_rsaQ, bi_rsaE, bi_rsaD);
 								...
 								iMgr.saveMpRSAKeyToKeystore(Identity.this.identityName, password1.getText(), null, validCount, bi_ExtrsaN, bi_ExtrsaP, bi_ExtrsaQ, bi_ExtrsaR, bi_ExtrsaS, bi_ExtrsaT, bi_ExtrsaE, bi_ExtrsaD);
 						...
 						public void widgetSelected(SelectionEvent e) {
 							...
 							if (isPubKey){
								...
								if (selectedKey_Keydata.getText().contains(":")){
									String part1 = selectedKey_Keydata.getText().substring(selectedKey_Keydata.getText().indexOf("Bit - ")+6);
									keyAlgorithm = part1.substring(0, selectedKey_Keydata.getText().indexOf(' ')).trim();
								}else{
									keyAlgorithm = selectedKey_Keydata.getText().substring(selectedKey_Keydata.getText().lastIndexOf('-')+1).trim();
								}
 					...
 					reconstructKey.addSelectionListener(new SelectionListener() {
 						...
 						public void widgetSelected(SelectionEvent e) {
 							...
 							if (allTableItems.length < 5){
 								...
 								iMgr.saveRSAKeyToKeystore(keyToAttack.getText().substring(0, keyToAttack.getText().indexOf('-')-1), "1234", identityName, rec_N, rec_p, rec_q, rec_e, rec_d);
 								...
 								case 5: {
 									...
 									iMgr.saveMpRSAKeyToKeystore(keyToAttack.getText().substring(0, keyToAttack.getText().indexOf('-')-1), "1234", identityName, 3, rec_N, rec_p, rec_q, rec_r, BigInteger.ZERO, BigInteger.ZERO, rec_e, rec_d);
 									break;
 								}
 								case 6: {
 									...
 									iMgr.saveMpRSAKeyToKeystore(keyToAttack.getText().substring(0, keyToAttack.getText().indexOf('-')-1), "1234", identityName, 4, rec_N, rec_p, rec_q, rec_r, rec_s, BigInteger.ZERO, rec_e, rec_d);
 									break;
 								}
 								case 7: {
 									BigInteger rec_s = new BigInteger(allTableItems[3].getText(1));
 									...
 									iMgr.saveMpRSAKeyToKeystore(keyToAttack.getText().substring(0, keyToAttack.getText().indexOf('-')-1), "1234", identityName, 5, rec_N, rec_p, rec_q, rec_r, rec_s, rec_t, rec_e, rec_d);
 									break;
 								}
 								}
