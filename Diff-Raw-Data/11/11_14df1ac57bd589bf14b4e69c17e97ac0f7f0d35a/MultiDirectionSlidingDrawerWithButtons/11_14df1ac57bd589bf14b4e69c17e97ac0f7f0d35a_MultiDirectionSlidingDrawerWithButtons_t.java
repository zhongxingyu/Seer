  ...
  * 
  * Modifications by: Vladimir Hudnitsky
  */
 
 package v.hudnitsky.demo.slider.widget;
 ...
 import v.hudnitsky.demo.slider.R;
 
 
 public class MultiDirectionSlidingDrawerWithButtons extends ViewGroup {
     ...
     private int mAnimationPositionButtons;
      ...
      */
     public MultiDirectionSlidingDrawerWithButtons(Context context, AttributeSet attrs) {
      ...
      */
     public MultiDirectionSlidingDrawerWithButtons(Context context, AttributeSet attrs, int defStyle) {
         super(context, attrs, defStyle);
         TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MultiDirectionSlidingDrawerWithButtons, defStyle, 0);
 
         int orientation = a.getInt(R.styleable.MultiDirectionSlidingDrawerWithButtons_direction, ORIENTATION_BTT);
         mVertical = (orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB);
         mBottomOffset = (int) a.getDimension(R.styleable.MultiDirectionSlidingDrawerWithButtons_bottomOffset, 0.0f);
         mTopOffset = (int) a.getDimension(R.styleable.MultiDirectionSlidingDrawerWithButtons_topOffset, 0.0f);
         mAllowSingleTap = a.getBoolean(R.styleable.MultiDirectionSlidingDrawerWithButtons_allowSingleTap, true);
         mAnimateOnClick = a.getBoolean(R.styleable.MultiDirectionSlidingDrawerWithButtons_animateOnClick, true);
         mInvert = (orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR);
 
         int handleId = a.getResourceId(R.styleable.MultiDirectionSlidingDrawerWithButtons_handle, 0);
         if (handleId == 0) {
             throw new IllegalArgumentException("The handle attribute is required and must refer "
                     + "to a valid child.");
         }
 
         int contentId = a.getResourceId(R.styleable.MultiDirectionSlidingDrawerWithButtons_content, 0);
         if (contentId == 0) {
             throw new IllegalArgumentException("The content attribute is required and must refer "
                     + "to a valid child.");
         }
         int buttonId = a.getResourceId(R.styleable.MultiDirectionSlidingDrawerWithButtons_buttons,0);
 ...
 //        mHandle.setOnClickListener(new DrawerToggler());
         ...
         mButtons = findViewById(mButtonsId);
     }
     ...
     protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
         ...
         measureChild(buttons, widthMeasureSpec, heightMeasureSpec);
     ...
     protected void dispatchDraw(Canvas canvas) {
         ...
         drawChild(canvas, buttons, drawingTime);
     ...
     protected void onLayout(boolean changed, int l, int t, int r, int b) {
         ...
         int buttonsWidth = mButtons.getMeasuredWidth();
         int buttonsHeight = mButtons.getMeasuredHeight();
 
         Log.d(LOG_TAG, "handleHeight: " + buttonsHeight);
         ...
         if (mVertical) {
             ...
             buttonsLeft = (width - buttonsWidth) / 2;
             if (mInvert) {
                 Log.d(LOG_TAG, "content.layout(1)");
                 handleTop = mExpanded ? height - mBottomOffset - handleHeight : mTopOffset;
                 buttonsTop = mExpanded ? height - mBottomOffset - buttonsHeight : mTopOffset;
                 ...
                 buttonsTop = mExpanded ? mTopOffset : height - buttonsHeight + mBottomOffset;
             ...
             buttonsTop = (height - buttonsHeight) / 2;
             if (mInvert) {
                 handleLeft = mExpanded ? width - mBottomOffset - handleWidth: mTopOffset+buttonsWidth;
                 buttonsLeft = mExpanded ? width - mBottomOffset - buttonsWidth : mTopOffset;
                 content.layout(mTopOffset, 0, mTopOffset + content.getMeasuredWidth(), content.getMeasuredHeight());
             } else {
                 handleLeft = mExpanded ? mTopOffset : width - handleWidth - buttonsWidth + mBottomOffset;
                 buttonsLeft = mExpanded ? mTopOffset : width - buttonsWidth + mBottomOffset;
         ...
         if(buttons.getVisibility()==View.VISIBLE){
             ...
             buttons.layout(buttonsLeft, buttonsTop, buttonsLeft + buttonsWidth, buttonsTop + buttonsHeight);
         }else{
             handle.layout(handleLeft, handleTop, handleLeft + handleWidth, handleTop + handleHeight);
         }
         mButtonsHeight = buttons.getHeight();
         mButtonsWidth = buttons.getWidth();
