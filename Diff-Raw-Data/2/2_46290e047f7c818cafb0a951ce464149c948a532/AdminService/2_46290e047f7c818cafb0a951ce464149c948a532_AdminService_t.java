 ...
 public class AdminService extends MobilisService {
 	...
 	private class IQListener implements PacketListener {
 		...
 		public void processPacket( Packet packet ) {
 			...
 			if ( packet instanceof BeanIQAdapter ) {
 				...
 				//alle AdministrationBeans haben die Felder ServiceNamespace + ServiceVersion die fr den Abgleich der Berechtigung ntig sind
 		...
 		private void handleInstallServiceBean( InstallServiceBean inBean ) {
 			...
 			//bei container.install() wird standardmig der defaultAgent der Runtime verwendet. Das fhrt dann bei einem uninstall des Service zum lschen des Runtime XMPP Accounts.
 			//Um manuelles installieren wieder zu ermglichen sind sehr umfangreiche nderungen ntig. Daher wird diese Methode der Dienstinstalltion bis auf weiteres deaktiviert.
 	...
 	private boolean checkServiceModifyPermission(String namespace, int version, String from){
		...
		if(runtimeRoster.getGroup(MobilisManager.securityUserGroup + serviceName+version)!=null){
 		...
 		//wenn gruppe nicht existiert, existiert dienst nicht. keine beschrnkung fr dienstzugriff
 		return true;
 	}
