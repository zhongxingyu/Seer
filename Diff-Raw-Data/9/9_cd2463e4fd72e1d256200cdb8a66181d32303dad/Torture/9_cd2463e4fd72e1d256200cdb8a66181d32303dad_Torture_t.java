 ...
 *@author  M. Ranganathan < mranga@nist.gov >
 *
 *Copyrights: None. This code is in the public domain.
 *
 */
 ...
 implements ParseExceptionListener, TagNames,
 TokenValues, ConfigurationSwitches
 {
 	...
 	class TestLogWriter extends PrintStream {
 		public void println(String stuff) {
 			if (outputResults) super.println();
 		}
 	...
 	throws ParseException 
 	{
 		...
 		if (exceptionTable == null) {
 		   ...
 		   if (e == null) {
 			...
 			// wildcard match.
 			e = (ExpectedException) 
 				exceptionTable.get(messageText.trim());
 		   }
 		   if (e == null) {
 			formatFailureDiagnostic(ex,
 	...
 	throws SAXException
 	{
 		if (name.compareTo(TORTURE) == 0 ) {
 			...
 			if (abortString != null 
 		        ...
 		        outputResults = false;
 			String outputResultsString = 
 				attrs.getValue(OUTPUT_RESULTS);
 			if (outputResultsString != null 
 				&& outputResultsString.compareTo(TRUE) == 0) {
 				outputResults = true;
 			} else outputResults = false;
 	...
 	throws SAXException
 	{
 		if (name.compareTo(TESTCASE) == 0) {
 			...
 			if (failureFlag && abortOnFail) {
 				if (testLogWriter != null)testLogWriter.close();
 			...
 			if (outputResults) testLogWriter.close();
 		}
 	...
 	protected void formatFailureDiagnostic( ParseException ex,
 				String message) {
