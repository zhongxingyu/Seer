 ...
 public class MacroFunctions extends AbstractFunction {
 	...
 	private BigDecimal hasMacro(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 2) { // Second parameter should be token name/id.
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("hasMacro(): You do not have the permission to specify the token.");
 			}
 			token = FindTokenFunctions.findToken(param.get(1).toString(), null);
 			if (token == null) {
 				throw new ParserException("hasMacro(): Unknown Token or Token ID, " + param.get(1).toString());
 			}
 	...
 	private String getMacros(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 0) {
 			...
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("getMacros(): You do not have the permission to specify the token.");
 			}
 			delim = param.get(0).toString();
 			token = FindTokenFunctions.findToken(param.get(1).toString(), null);
 			if (token == null) {
 				throw new ParserException("getMacros(): Unknown Token or Token ID, " + param.get(1).toString());
 			}
 		} else {
 	...
 	private Object getMacroProps(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 1) {
 			...
 			if (token == null) {
 				throw new ParserException("getMacroProps(): Unknown Token or Token ID, " + param.get(2).toString());
 			}
 	...
 	private String getMacroIndexes(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 1) {
 			...
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("getMacroIndexes(): You do not have the permission to specify the token.");
 			}
 			delim = param.get(1).toString();
 			token = FindTokenFunctions.findToken(param.get(2).toString(), null);
 			if (token == null) {
 				throw new ParserException("getMacroIndexes(): Unknown Token or Token ID, " + param.get(2).toString());
 			}
 		} else {
 	...
 	private String getMacroCommand(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 1) {
 			...
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("getMacroCommand(): You do not have the permission to specify the token.");
 			}
 
 			token = FindTokenFunctions.findToken(param.get(1).toString(), null);
 			if (token == null) {
 				throw new ParserException("getMacroCommand(): Unknown Token or Token ID, " + param.get(1).toString());
 			}
 		} else {
 	...
 	private BigDecimal createMacro(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 1) { // Only valid if its a json object
 			...
 			try {
 				...
 				token = FindTokenFunctions.findToken(param.get(1).toString(), null);
 				if (token == null) {
 					throw new ParserException("createMacro(): Unknown Token or Token ID, " + param.get(1).toString());
 				}
 				prop = param.get(0).toString();
 				delim = "json";
 			...
 			if (token == null) {
 				throw new ParserException("createMacro(): Unknown Token or Token ID, " + param.get(4).toString());
 			}
 		} else {
 	...
 	private String setMacroProps(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 2 || param.size() == 3) {
 			...
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("setMacroProps(): You do not have the permission to specify the token.");
 			}
 			token = FindTokenFunctions.findToken(param.get(3).toString(), null);
 			if (token == null) {
 				throw new ParserException("setMacroProps(): Unknown Token or Token ID, " + param.get(3).toString());
 			}
 		} else {
 	...
 	private String setMacroCommand(MapToolVariableResolver res, List<Object> param) throws ParserException {
 		...
 		if (!MapTool.getParser().isMacroTrusted()) {
 			throw new ParserException("setMacroCommand(): You do not have permission to call this function.");
 		}
 		...
 		if (param.size() == 2) {
 			...
 			token = FindTokenFunctions.findToken(param.get(2).toString(), null);
 			if (token == null) {
 				throw new ParserException("setMacroCommand(): Unknown Token or Token ID, " + param.get(3).toString());
 			}
 		} else {
 	...
 	private String removeMacro(MapToolVariableResolver res, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 1) {
 			...
 			token = FindTokenFunctions.findToken(param.get(1).toString(), null);
 			if (token == null) {
 				throw new ParserException("removeMacro(): Unknown Token or Token ID, " + param.get(1).toString());
 			}
 		} else {
 	...
 	private String getMacroGroup(MapToolVariableResolver resolver, List<Object> param) throws ParserException {
 		...
 		if (param.size() == 0) {
 			...
 			if (!MapTool.getParser().isMacroTrusted()) {
 				throw new ParserException("getMacroGroup(): You do not have the permission to specify the token.");
 			}
 			group = param.get(0).toString();
 			delim = param.get(1).toString();
 			token = FindTokenFunctions.findToken(param.get(2).toString(), null);
 			if (token == null) {
 				throw new ParserException("getMacroGroup(): Unknown Token or Token ID, " + param.get(2).toString());
 			}
 		} else {
