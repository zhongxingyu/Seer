 ...
 public class VerSummaryActivity extends SenateActivity {
 	...
 	ArrayList<InvItem> missingItems=new ArrayList<InvItem>();// for saving items which are not allocated to that location
 	...
 	protected void onCreate(Bundle savedInstanceState) {
 		...
 		AllScannedItems = this.getInvArrayListFromJSON( getIntent().getStringArrayListExtra("scannedList")) ;
 		missingItems = this.getInvArrayListFromJSON(getIntent().getStringArrayListExtra("missingList"));
 		newItems = this.getInvArrayListFromJSON(getIntent().getStringArrayListExtra("newItems"));
 		scannedBarcodeNumbers= this.getInvArrayListFromJSON(getIntent().getStringArrayListExtra("scannedBarcodeNumbers"));
 		...
 		InvListViewAdapter  listAdapter1 =  new InvListViewAdapter(this, R.layout.invlist_item, AllScannedItems);   
 		InvListViewAdapter  listAdapter2 =  new InvListViewAdapter(this, R.layout.invlist_item, missingItems);   
 		InvListViewAdapter  listAdapter3 =  new InvListViewAdapter(this, R.layout.invlist_item, newItems);   
 		...
 		if (ar!=null) {
 			for (int x=0;x<ar.size();x++) {
 				...
 				try {
 					...
 					try {
 						curInvItem.setNusenate(jsonObject.getString("nusenate"));
 					}
 					catch (Exception e2) {
 						e2.printStackTrace();
 					}
 					try {
 						curInvItem.setCdcategory(jsonObject.getString("cdcategory"));
 					}
 					catch (Exception e2) {
 						e2.printStackTrace();
 					}	
 					...
 					try {
 						curInvItem.setDecommodityf(jsonObject.getString("decommodityf"));
 					}
 					catch (Exception e2) {
 						e2.printStackTrace();
 					}					
 					...
 					Log.i("System.err", "ERROR CONVERTING FROM ARRAYLIST OF JSON"+x+":"+ar.get(x)); 
 					e.printStackTrace();
 				}
 			}
 		}
  					...
  					AsyncTask<String, String, String> resr1;
 						try {
 							// Get the URL from the properties 
 			 				...
 			 				System.out.println(URL+"/VerificationReports?loc_code="+loc_code+"&barcodes="+barcodeNum);
