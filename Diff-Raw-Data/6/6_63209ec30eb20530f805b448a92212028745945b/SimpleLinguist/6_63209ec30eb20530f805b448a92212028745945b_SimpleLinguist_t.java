  ...
  * assumptions:
  *
  *      1) Zero or  one word per grammar node
  *      2) No fan-in allowed ever
  *      3) No composites (yet)
  *      4) Only Unit, HMMState, and pronunciation states (and the
  *        initial/final grammar state are in the graph (no word, alternative
  ...
  */
 public class SimpleLinguist implements  Linguist {
     ...
     /**
      * Ensures that there is a starting path by adding an empty left
      * context to the strating gstate
      */
 
     // TODO: Currently the SimpleLinguist requires that the initial
     ...
     /**
      * Gets a SentenceHMMStateArc. The arc is drawn from a pool of
      * arcs.
      *
      ...
      *
      */
     private SentenceHMMStateArc getArc(
     ...
     protected class GState {
         ...
         /**
          * Retrieves the set of starting contexts for this node. The
          * starting contexts are the set of Unit[] with a size equal
          * to the maximum right context size.
          *
          * @return the set of starting contexts
          * acrosss nodes. 
          */
             ...
             if (startingContexts == null) {
                 ...
                 if (node.isEmpty()) {
                     ...
                     for (int i = 0; i < arcs.length; i++) {
                         GState gstate = 
                             getGState((GrammarNode) arcs[i].getGrammarNode());
         ...
         /**
          * Retrieves the starting UnitContext for the given
          * pronunciation
          *
          * @param pronunciation the pronunciation
          *
          * @return a UnitContext representing the starting context of
          * the pronunciation
          */
         ...
         /**
          * Retrieves the set of trailing contexts for this node.
          * the trailing contexts are the set of Unit[] with a size equal
          * to the maximum left context size that align with the end of
          * the node
          *
          */
         ...
         /**
          * Visit all of the successor states, and gather their
          * starting contexts into this gstates right context
          */
         ...
         /**
          * Returns the set of succesor arcs for this grammar
          * node. If a successor grammar node has no words 
          * we'll substitute the successors for that node (avoiding loops
          * of course)
          *
         ...
         /**
          * Visit all of the successor states, and push our ending
          * context into the successors left context
          */
         ...
         /**
          * Pushes the given left context into the
          * successor states.  If a successor state is empty,
          * continue to push into this empty states successors
          *
         ...
         /**
          * Adds the given context to the set of left contexts for this
          * state
          *
         ...
         public SentenceHMMState getEntryPoint() {
             ContextPair cp = 
                 ContextPair.get(UnitContext.SILENCE, UnitContext.SILENCE);
         ...
         /**
          * Add the items on the newContexts list to the dest list.
         ...
         /**
          * Collects the right contexts for this node and pushes this
          * nodes ending context into the next next set of nodes.
          */
         ...
         // of starting states.
             ...
             for (Iterator i = leftContexts.iterator(); i.hasNext();) {
                 ...
                 for (Iterator j = 
                         getStartingContexts().iterator(); j.hasNext();) {
                     UnitContext startingContext = (UnitContext) j.next();
                     ContextPair contextPair =
                         ContextPair.get(leftContext, startingContext);
             ...
             if (node.isFinalNode()) {
                 ...
                 for (Iterator i = entryPoints.values().iterator();
                         i.hasNext();) {
                 ...
                 for (Iterator i = entryPoints.keySet().iterator();
                         i.hasNext(); ) {
         ...
         /**
          * Adds the set of empty entry points.  The list of entry
          * points are tagged with a context pair. The context pair
          * represent the left context for the state and the starting
          * contesxt for the state, this allows states to be hooked up
          * properly.  However, we may be transitioning from states
          * that have no right hand context (CI units such as SIL fall
          * into this category). In this case we'd normally have no
          * place to transition to since we add entry points for each
          * starting context.  To make sure that there are entry points
          * for empty contexts if necesary, we go through the list of
          * entry points and find all left contexts that have a right
          * hand context size of zero. These entry points will need an
          * entry point with an empty starting context. These entries
          * are synthesized and added to the the list of entry points.
          */
         private void addEmptyEntryPoints() {
             ...
             for (Iterator i = entryPoints.keySet().iterator(); i.hasNext();) {
                 ...
                 if (needsEmptyVersion(cp)) {
                     ContextPair emptyContextPair =
                         ContextPair.get(cp.getLeftContext(), UnitContext.EMPTY);
         ...
         /**
          * Determines if the context pair needs an empty version. A
          * context pair needs an empty version if the left context 
          * has a max size of zero.
          *
         ...
         private void expandPronunciation(UnitContext leftContext,
             ...
             String pname = "P(" + pronunciation.getWord() + "[" 
                 + leftContext + "," + startingContext +"])";
             PronunciationState ps =
                 new PronunciationState(pname, pronunciation, which);
 
             T("     Expanding " + ps.getPronunciation()
                     + " for lc " + leftContext);
             ...
             for (int i = 0; tail != null && i < fanOutPoint; i++) {
                 tail = attachUnit(ps, tail, units, i,
                         leftContext, UnitContext.EMPTY);
             }
         ...
         /**
          * Attaches the given unit to the given tail, expanding the
          ...
          */
         private SentenceHMMState attachUnit(
                 PronunciationState parent,
             ...
             if (existingState != null) {
                 ...
                 if (unitState.isLast()) {
                     UnitContext nextLeftContext =
                         generateNextLeftContext(leftContext, units[which]);
                     ...
                     if (actualRightContext == UnitContext.SILENCE) {
                         ...
                         SentenceHMMState silExistingState =
                             getExistingState(silUnit);
 
                         if (silExistingState != null) {
                             attachState(tail, silExistingState,
                                 logOne, logOne, logSilenceInsertionProbability);
                         } else {
                             ...
                             ContextPair silCP =
                                 ContextPair.get(UnitContext.SILENCE,
                                         UnitContext.EMPTY);
         ...
         /**
          * Get the left context for a unit based upon the left context
          * size, the entry left context and the current unit.
          *
         ...
         /**
          * Get the right context for a unit based upon the right context
          * size, the exit right context and the current unit.
          *
          * @param units the set of units
          * @param index the index of the current unit
          ...
          */
         private HMMStateState expandHMMTree(
                         UnitState parent, HMMStateState tree) {
         ...
         /**
          * Connect up all of the GStates.  Each state now has a table
          * of exit points. These exit points represent tail states for
          * the node. Each of these tail states is tagged with a
          * ContextPair, that indicates what the left context is (the
          * exiting context) and the right context (the entering
          * context) for the transition.  To connect up a state, the
          * connect does the following:
          *
          *  1) Iterate through all of the grammar successors for this
          *  state
          *
          *  2) Get the 'entry points' for the successor that match
          *  the exit points. 
          *
          *  3) Hook them up.
          * 
          *  Note that for a task with 1000 words this will involve 
          *  checking on the order of 35,000,000 connections  and
         ...
         /**
          * connect all the states in the source list to the states
          * in the destination list
          *
          * @param sourceList the set of source states
          * @param destList the set of destinatin states.
          */
         private void connect(List sourceList, List destList,  
                 float logLangProb) {
         ...
         /**
          * Checks to see if a state that matches the given state
          * already exists
          *
          * @param state the state to check
          *
          * @return true if a state with an identical signature already
          * exists.
          */
 ...
 class UnitContext {
     ...
     /**
      * Gets the unit context for the given units. There is a single
      * unit context for each unit combination.
      *
 ...
 /**
  * A context pair hold a left and starting context. It is used as a
  * hash into the set of starting points for a particular gstate
  */
 class ContextPair {
     ...
     /**
      * Gets the ContextPair for the given set of contexts. This is a
      * factory method. If the ContextPair already exists, return that
      * one, othewise, create it and store it so it can be reused.
      *
