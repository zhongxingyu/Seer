 ...
 public class MainWindow extends JFrame{
 	...
 	public static void main(String[] args) {
 		try {
 			if ((!new File("Default.theme").exists()) || (!new File("GameData").exists()))
 			{
 				JOptionPane.showMessageDialog(null, "Default.theme or GameData folder not found in current directory: " + new File(".").getAbsolutePath() + " Make sure Zauber has been correctly " +
 						"extracted.", "Missing File Error", 0);
 				System.exit(0);
 			}
 ...
 class GamePanel extends JPanel implements KeyListener{
 	...
 	public void createTutorial()
 	{
 		...
 		gamedata.getAnimPanel().makeTooltips();
 	}
 	...
 	public void loadGame() {
 		...
 		File f = new File("GameData/Saves/test.test");
 		f.mkdirs();
 		f.delete();
 	...
 	public void addOptions()
 	{
 		...
 		comboBox.setModel(new DefaultComboBoxModel(new String[] {"300x300", "450x450", "600x600", "750x750", "900x900"}));
 		comboBox.setSelectedIndex(((gamedata.getResolution()/gamedata.getPixelSize())/10)-2);
 		...
 		btnApply.addActionListener(new ActionListener(){
 			public void actionPerformed(ActionEvent e) {
 				gamedata.setResolution(((comboBox.getSelectedIndex()+2)*10)*gamedata.getPixelSize());
 ...
 //		gridBagLayout.rowWeights = new double[]{0.0, 0.0, 1.0, Double.MIN_VALUE};
 		...
 		JLabel lblSo = new JLabel("Amazing Music - Deceased Superior Technician - http://www.nosoapradio.us/");
 		GridBagConstraints gbc_lblSo = new GridBagConstraints();
 		gbc_lblSo.insets = new Insets(0, 0, 5, 0);
 		gbc_lblSo.gridx = 0;
 		gbc_lblSo.gridy = 5;
 		add(lblSo, gbc_lblSo);
 		...
 		gbc_lblEverythingElseMe.gridy = 6;
 	...
 	public void keyPressed(KeyEvent e) {
 		...
 		else if ((e.getKeyCode() == KeyEvent.VK_TAB) && (!screen.tabKey))
 		{
 ...
 class AnimationPanel extends JPanel {
 	...
 	public static final int VSLOW = 250;
 	public static final int SLOW = 250;
 	public static final int NORMAL = 150;
 	public static final int FAST = 100;
 	public static final int VFAST = 50;
 	
 	private int animSpeed = AnimationPanel.NORMAL;
 	...
 	public void paintComponent(Graphics g)
 	{
 		...
 		if (drawMode == 0)
 		{
 			...
 			g2d.drawImage(getAnimationImage(gamedata.getPlayer().getCurrentArmour().getSpritesheet(gamedata.getPlayer().getCurrentForm().getFemale()).getImage()),
 					(this.getWidth()/2)-25, 10, null);
 			g2d.drawImage(getAnimationImage(gamedata.getPlayer().getCurrentWeapon().getSpritesheet(gamedata.getPlayer().getCurrentForm().getFemale()).getImage()),
 					(this.getWidth()/2)-25, 10, null);
 	...
 	public void stopPlayingSpecificAnimation()
 	{
 		this.setDrawMode(0);
 	}
 ...
 class AnimationThread extends Thread
 {
 	...
 	public void run()
 	{
 		...
 		while(true)
 		{
 			...
 			if (panel.getAnimstage() != 0)
 			{
 				if ((!panel.getGamedata().getScreen().upKey) ||
 						(!panel.getGamedata().getScreen().downKey) ||
 						(!panel.getGamedata().getScreen().leftKey) ||
 						(!panel.getGamedata().getScreen().rightKey))
 				{
 					panel.setPlayAnim(false);
 				}
 ...
 @SuppressWarnings("serial")
 class ScreenPanel extends JPanel implements MouseListener, MouseMotionListener, ActionListener{
 	...
 	public void paintComponent(Graphics g)
 	{
 		...
 		g2d.drawImage(screenImage, 0, 0, this.getWidth(), this.getHeight(), screenpos[0], screenpos[1], screenpos[0]+gamedata.getResolution(), screenpos[1]+gamedata.getResolution(), null);
 	...
 	private void box() {
 		...
 		g2d.setColor(Color.GREEN);
 		g2d.drawRect(targetpos[0]-1, targetpos[1]+3,
 	...
 	public void drawScreen()
 	{
 		...
 		if (selectionbox) {
 			...
 			g2d.drawRect(selectionpos[0]-1, selectionpos[1]+3,
 					gamedata.getPixelSize(), gamedata.getPixelSize());
 		}
 	...
 	public void addKeyListener()
 	{
 		TimedKeyListener kl = new TimedKeyListener() {
 		      ...
 		      public void keyPressed(KeyEvent e) {
 				...
 				else if ((e.getKeyCode() == KeyEvent.VK_TAB) && (!gamedata.isDialogue()) && (!tabKey))
 				{
 					selectionbox = false;
 					TaskManager.clearTasks();
 					for (Actor actor : gamedata.getActors())
 		      ...
 		      public void releaseOtherKeys(int direction)
 		        {
 		      ...
 		      public void keyReleased(KeyEvent evt) {
 		        ...
 		        if (getReleased()) {
 		        	...
 		        	if (evt.getKeyCode() == KeyEvent.VK_RIGHT) {
 						...
 						escKey = false;
 					} else if (evt.getKeyCode() == KeyEvent.VK_BACK_QUOTE) {
 						graveKey = false;
 ...
 //	}
 	...
 	long start = 0;
 	public void keyPressedOld(KeyEvent e) {
 //		int direction = -1;
 //
 //		if ((e.getKeyCode() == KeyEvent.VK_ESCAPE) && (!gamedata.isDialogue()) && (!escKey))
 //		{
 ...
 //			gamedata.getAnimPanel().setPlayAnim(false);
 	...
 	@Override
 	public void mouseMoved(MouseEvent e) {
 		if ((!gamedata.isDialogue()) && (!gamedata.isTurn())) {
 			double xspacing = (double) this.getWidth()
 					/ gamedata.getResolution();
 			double yspacing = (double) this.getHeight()
 					/ gamedata.getResolution();
 	...
 	public void actionPerformed(ActionEvent e) {
 		...
 		if ((direction > -1) && (!gamedata.isDialogue()) && (ctrlKey)){
 			gamedata.getPlayer().setDirection(direction);
 		}
 		else if ((direction > -1) && (!gamedata.isDialogue()) && (!gamedata.isTurn())){
 			gamedata.stopAll();
 			selectionbox = false;
 			final int finaldirection = direction;
 ...
 class MovementThread extends Thread
 {
 	...
 	public void run()
 	{
 		...
 		while (true) {
 			...
 			if ((direction > -1) && (move)) {
 				gamedata.getPlayer().setDirection(direction);
 				gamedata.getPlayer().moveActor(direction);
 				if ((gamedata.getPlayer().nextpos[0] != gamedata.getPlayer().getPos()[0]) || (gamedata.getPlayer().nextpos[1] != gamedata.getPlayer().getPos()[1]))
 					gamedata.startTurn(1);
 				else
 					gamedata.getScreen().selectionbox = false;
 ...
 //			}
 			...
 			sleepTime = gamedata.getMovementSpeed() - (System.currentTimeMillis() - sT);
