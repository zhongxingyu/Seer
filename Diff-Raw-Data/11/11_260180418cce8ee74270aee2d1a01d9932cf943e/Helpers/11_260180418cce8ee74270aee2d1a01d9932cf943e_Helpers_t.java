 ...
 public class Helpers {
    ...
    * @return commands leading to destination or null if no way found
    */
   public static Pair<List<Command>, WayCoordinateSimple> moveToFallableCell(State s, Cell goal, final int N) {
     WayCoordinateSimple[][] m = new WayCoordinateSimple[s.board.width][s.board.height];
     PriorityQueue<WayCoordinateSimple> p = new PriorityQueue<WayCoordinateSimple>();
     WayCoordinateSimple wc = new WayCoordinateSimple(s.robotCol, s.robotRow, 0, null);
     ...
     while (wc != null) {
       ...
       if (m[wc.col][wc.row] == null) { // not already better way
         ...
         //TODO check if position can fall
         if (c == goal) { //target cell found
           Cell below = s.board.get(wc.col, wc.row - 1);
           boolean stop = false;
           switch (below) {
           case Earth:
           case Lambda:
     ...
     return Pair.create(Arrays.asList(cmds), end);    
   }
 
   
   /**
    * Moves to the given target row in the shortest way.
    * But does not consider rock movement. 
    * 
    * @param s starting state
    * @param destRow destination row
    * @return commands leading to destination or null if no wa yfound
    */
   public static List<Command> moveToRow(State s, int destRow) {
     
     WayCoordinateRow[][] m = new WayCoordinateRow[s.board.width][s.board.height];
     PriorityQueue<WayCoordinateRow> p = new PriorityQueue<WayCoordinateRow>();
     WayCoordinateRow wc = new WayCoordinateRow(s.robotCol, s.robotRow, destRow, 0, null);
      
     
     while (wc != null && (wc.row != destRow)) {
       if (m[wc.col][wc.row] == null || m[wc.col][wc.row].steps > wc.steps) { // not already better way
         ...
         if (c == Cell.Earth || c == Cell.Empty || c == Cell.Lambda || c == Cell.Robot || c == Cell.Razor) { // may move there
           ...
           // generate follow up moves
           p.add(new WayCoordinateRow(wc.col - 1, wc.row, destRow, wc.steps + 1, Command.Left));
           p.add(new WayCoordinateRow(wc.col + 1, wc.row, destRow, wc.steps + 1, Command.Right));
           p.add(new WayCoordinateRow(wc.col, wc.row + 1, destRow, wc.steps + 1, Command.Up));
           if (s.board.get(wc.col, wc.row + 1) != Cell.Rock)
             p.add(new WayCoordinateRow(wc.col, wc.row - 1, destRow, wc.steps + 1, Command.Down));
