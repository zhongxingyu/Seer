   ...
   implements Dauerauftrag
 {
   ...
   protected void insertCheck() throws ApplicationException
   {
       ...
       // BUGZILLA 197
       if (getBetrag() == 0.0)
         throw new ApplicationException(i18n.tr("Bitte geben Sie einen gltigen Betrag ein."));
 
       if (getTurnus() == null)
 				throw new ApplicationException(i18n.tr("Bitte whlen Sie einen Zahlungsturnus aus"));
 
 			if (getErsteZahlung() == null)
 				throw new ApplicationException(i18n.tr("Bitte geben Sie ein Datum fr die erste Zahlung an"));
 			...
 			if (!isActive())
 			{
 				...
 				if (getErsteZahlung().before(today))
 					throw new ApplicationException(i18n.tr("Bitte whlen Sie fr die erste Zahlung ein Datum in der Zukunft"));
 			}
      ...
      // BUGZILLA 371
			if (getLetzteZahlung() != null && getLetzteZahlung().before(getErsteZahlung()))
 				throw new ApplicationException(i18n.tr("Bei Angabe eines Datum fr die letzte Zahlung muss dieses nach der ersten Zahlung liegen"));
 		}
 		catch (RemoteException e)
 		{
 			Logger.error("error while insert check in DauerAuftrag",e);
 			throw new ApplicationException(i18n.tr("Fehler bei der Prfung des Dauerauftrags"));
 		}
     super.insertCheck();
   }
