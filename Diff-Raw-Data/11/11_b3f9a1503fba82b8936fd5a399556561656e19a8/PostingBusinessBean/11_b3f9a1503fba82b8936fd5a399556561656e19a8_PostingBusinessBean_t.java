 ...
 * $Id: PostingBusinessBean.java,v 1.42 2003/11/21 14:05:24 roar Exp $
 ...
 public class PostingBusinessBean extends com.idega.business.IBOServiceBean implements PostingBusiness  {
 	...
 	*/
 	...
 	/**
 	 * Hmta konteringsinformation
 	 * Retrieves accounting default information. Matching on the main rules like 
 	 * Verksamhet, Regspectyp, Bolagstyp, kommuntill.hrighet is done via primary keys
 	 *  
 	...
 	public PostingParameters getPostingParameter(Date date, int act_id, 
 		...
 		try {
 			...
 			throw new PostingParametersException(KEY_ERROR_POST_NOT_FOUND, "Skt parameter hittades ej");
 		} catch (RemoteException e) {
 			return null;
 		} catch (FinderException e) {
 			return null;
 	...
 	public void savePostingParameter(String sppID,
 			...
 			if (	((schoolYear1ID.compareTo("0") != 0) && 
 					...
 					( schoolYear2ID.compareTo("0") != 0))) {
 				throw new PostingParametersException(KEY_ERROR_POST_PARAM_SCHOOL_YEARS, "Ange skolrsintervall!");			
 			}
 			
 			if (periodeFrom == null || periodeTo == null) {
 				throw new PostingParametersException(KEY_ERROR_POST_PARAM_DATE_NULL, "Datum mste fyllas i!");			
 			}
 
 			if (schoolYear1ID != null && schoolYear1ID != null) {
 				if (Integer.parseInt(schoolYear1ID) > Integer.parseInt(schoolYear2ID)) {
 					throw new PostingParametersException(KEY_ERROR_POST_PARAM_SCHOOL_YEAR_ORDER, "Fel ordning bland skolren!");			
 				}
 			}
 			if(periodeFrom == null || periodeTo == null) {
 			...
 			if (periodeFrom.after(periodeTo)) {
 				throw new PostingParametersException(KEY_ERROR_POST_PARAM_DATE_ORDER, "Frn datum kan ej vara senare n tom datum!");			
 			}
