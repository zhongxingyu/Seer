 ...
 public class UserManagement implements CommandHandler, CommandHandlerFactory {
 	...
 	private Reply doSITE_ADDIP(BaseFtpConnection conn)
 		...
 		try {
 			myUser = conn.getGlobalContext().getUserManager().getUserByName(
 					args[0]);
 	...
 	private Reply doSITE_ADDUSER(BaseFtpConnection conn)
 		...
 		try {
 			...
 			newUser = conn.getGlobalContext().getUserManager().create(
 					newUsername);
 			...
 			if (newGroup != null) {
 				...
 				logger.info("'" + conn.getUserNull().getName() + "' added '"
 			...
 			if (conn.getUserNull().isGroupAdmin()
 				...
 				if (ratio == 0F) {
 					...
 					if (usedleechslots >= conn.getUserNull().getKeyedMap()
 						...
 						return new Reply(452, conn
 								.jprintf(UserManagement.class,
 	...
 	private Reply doSITE_CHGRP(BaseFtpConnection conn) throws ReplyException,
 		...
 		try {
 			...
 			return new Reply(452, "IO error reading user: " + e.getMessage());
 		}
 		...
 		for (int i = 1; i < args.length; i++) {
 			...
 			try {
 				...
 				logger.info("'" + conn.getUserNull().getName() + "' removed '"
 	...
 	private Reply doSITE_CHPASS(BaseFtpConnection conn)
 		...
 		try {
 			...
 			return new Reply(452, "Error reading userfile: " + e.getMessage());
 		}
 	}
 	...
 	private Reply doSITE_DELUSER(BaseFtpConnection conn) throws ReplyException,
 		...
 		logger.info("'" + conn.getUserNull().getName() + "' deleted user '"
 	...
 	private Reply doSITE_GINFO(BaseFtpConnection conn)
 		...
 		ResourceBundle bundle = ResourceBundle.getBundle(UserManagement.class
 				.getName());
 		ReplacerEnvironment env = new ReplacerEnvironment();
 		env.add("group", group);
 		env.add("sp", " ");
 	...
 	private Reply doSITE_GIVE(BaseFtpConnection conn)
 		...
 		if (!conn.getGlobalContext().getConfig().checkPermission("give",
 				conn.getUserNull())) {
 			return Reply.RESPONSE_530_ACCESS_DENIED;
 		}
 		...
 		try {
 			...
 			return new Reply(452, "The string " + amt
 		...
 		return new Reply(200, "OK, gave " + Bytes.formatBytes(credits)
 				+ " of your credits to " + myUser.getName());
 	}
 
 	private Reply doSITE_GROUP(BaseFtpConnection conn)
 			throws ImproperUsageException {
 		FtpRequest request = conn.getRequest();
 	...
 	private Reply doSITE_KICK(BaseFtpConnection conn)
 		...
 		ArrayList<BaseFtpConnection> conns = new ArrayList<BaseFtpConnection>(
 				conn.getGlobalContext().getConnectionManager().getConnections());
 	...
 	private Reply doSITE_SEEN(BaseFtpConnection conn)
 		...
 		try {
 			user = conn.getGlobalContext().getUserManager().getUserByName(
 					request.getArgument());
 		} catch (NoSuchUserException e) {
 			return new Reply(452, e.getMessage());
 		} catch (UserFileException e) {
 		...
 		return new Reply(200, "User was last seen: "
 	...
 	private Reply doSITE_TAGLINE(BaseFtpConnection conn) throws ReplyException,
 		...
 		try {
			...
			conn.getUserNull().getKeyedMap().setObject(UserManagement.TAGLINE,
					request.getArgument());
 	...
 	private Reply doSITE_TAKE(BaseFtpConnection conn)
 		...
 		if (!conn.getGlobalContext().getConfig().checkPermission("take",
 				conn.getUserNull())) {
 			return Reply.RESPONSE_530_ACCESS_DENIED;
 		}
 		...
 		return new Reply(200, "OK, removed " + credits + "b from "
 				+ myUser.getName() + ".");
 	}
 	...
 	private Reply doSITE_USER(BaseFtpConnection conn) throws ReplyException,
 		...
 		response.addComment(BaseFtpConnection.jprintf(UserManagement.class,
 				"user", null, myUser));
 		return response;
 	}
 	...
 	private Reply doSITE_BAN(BaseFtpConnection conn)
 			throws ImproperUsageException {
 		FtpRequest request = conn.getRequest();
 		...
 		if (st.hasMoreTokens()) {
 			...
 			banMsg = "Banned by " + conn.getUserNull().getName() + " for "
 					+ banTime + "m";
 		}
 	...
 	private Reply doSITE_UNBAN(BaseFtpConnection conn)
 			throws ImproperUsageException {
 		FtpRequest request = conn.getRequest();
 		...
 		for (User user : myUsers) {
 			...
 			if (timeleft > 0) {
 				...
 				response.addComment(BaseFtpConnection.jprintf(
 						UserManagement.class, "bans", env, user));
 			}
 		}
 	...
 	public Reply execute(BaseFtpConnection conn) throws ReplyException,
 		...
 		throw UnhandledCommandException.create(UserManagement.class, conn
 				.getRequest());
 	}
