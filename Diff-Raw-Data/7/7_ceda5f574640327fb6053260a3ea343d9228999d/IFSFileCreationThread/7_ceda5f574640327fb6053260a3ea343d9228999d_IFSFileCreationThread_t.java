 ...
 public class IFSFileCreationThread extends Thread {
 	...
 	private void createPaymentFilesExcel(Collection data, String fileName, String headerText, boolean doublePosting) throws IOException {
 		if (data != null && !data.isEmpty()) {
 			...
 			sheet.createRow(rowNumber += 2).createCell(row.getFirstCellNum()).setCellValue(numberOfRecords + " bokfringsposter,   Kreditbelopp totalt:  - " + getNumberFormat().format(totalAmount) + ",   Debetbelopp totalt: " + getNumberFormat().format(totalAmount));
 			saveExcelWorkBook(fileName, wb);
 		}
 	}
 	...
 	private void createInvoiceSigningFilesExcel(String fileName, String headerText, boolean signingFooter) throws IOException, IDOException {
 		...
 		row = sheet.createRow(rowNumber++);
 		row.createCell(cellNumber++).setCellValue("Innevarande mnad");
 		row.createCell(cellNumber).setCellValue("Fregende mnad");
 		row = sheet.createRow(rowNumber++);
 	...
 	private void createPaymentSigningFilesExcel(Collection data, String fileName, String headerText, boolean signingFooter) throws IOException, FinderException {
 		if (data != null && !data.isEmpty()) {
 			...
 			if (!signingFooter) {
 				...
 				cell.setCellValue("Summa frn egna kommunala anordnare " + getNumberFormat().format(getInCommuneSum()));
 				row = sheet.createRow(rowNumber++);
 				cell = row.createCell(cellNumber);
 				cell.setCellValue("Summa frn vriga anordnare " + getNumberFormat().format(totalAmount - getInCommuneSum()));
 				row = sheet.createRow(rowNumber++);
 				cell = row.createCell(cellNumber);
 			}
 	...
 	private void createSigningFooter(HSSFSheet sheet, short rowNumber) {
 		...
 		rowNumber = createSigningFooterDetail(sheet, rowNumber += 5, cellNumber -= 2, "Behrighetsattest");
 	}
