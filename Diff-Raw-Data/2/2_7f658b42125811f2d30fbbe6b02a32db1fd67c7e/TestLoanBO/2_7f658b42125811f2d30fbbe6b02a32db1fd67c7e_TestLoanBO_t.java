 ...
 public class TestLoanBO extends MifosTestCase {
 	...
 	private List<FeeView> feeViews = new ArrayList<FeeView>();
 	...
 	public void testPeriodicFeeWithHoliday() throws Exception {
 		...
 		try {
 			...
 			for (AccountActionDateEntity accountActionDateEntity : accountBO.getAccountActionDates()) {
 				LoanScheduleEntity loanScheduleEntity = (LoanScheduleEntity) accountActionDateEntity;
 				assertEquals(expectedDates.get(i++), loanScheduleEntity.getActionDate());
 				assertEquals(new Money("100"), loanScheduleEntity.getTotalFees());
 			}
 
 			assertEquals(intialTotalFeeAmount.add(new Money("600.0")), loanBO.getLoanSummary().getOriginalFees());
 		} finally {
 			// make sure that we don't leave any persistent changes that could
 			// affect subsequent tests
 			DateUtils.setCurrentTime(null);
 	...
 	private AccountBO getLoanAccount(Date startDate, AccountState state) throws MeetingException {
 	...
 	private void createInitialCustomers(Date meetingStartDate) throws MeetingException {
 		Calendar cal = Calendar.getInstance();
 		cal.setTime(meetingStartDate);
 		// create a meeting based on meetingStartDate rather than the
 		// current system date
 		MeetingBO meeting = TestObjectFactory.createMeeting(new MeetingBO(
 				WeekDay.getWeekDay(cal.get(Calendar.DAY_OF_WEEK)), EVERY_WEEK,
 				meetingStartDate, CUSTOMER_MEETING, "place"));
 		center = TestObjectFactory.createCenter("Center", meeting);
 		group = TestObjectFactory.createGroupUnderCenter("Group",
 				CustomerStatus.GROUP_ACTIVE, center);
 	}
 	
 	private HolidayBO createHoliday(Date holidayDate) throws PersistenceException,
 	...
 	private void deleteHoliday(HolidayBO holiday) throws PersistenceException {
 	...
 	public void testWaiveMiscPenaltyAfterPayment() throws Exception {
 		...
 		for (AccountActionDateEntity accountActionDateEntity : loan
 			...
 			if (loanScheduleEntity.getInstallmentId()
 					.equals(Short.valueOf("1"))) {
 				assertEquals(loanScheduleEntity.getMiscPenalty(), new Money(
 						"200"));
 				assertEquals(loanScheduleEntity.getMiscPenaltyPaid(),
 						new Money());
 			}
 		}
 		assertEquals(loan.getLoanSummary().getOriginalPenalty(), new Money(
 				"200"));
 	 ...
 	 * TODO: re-enable this test when loan schedule adjustments correctly handle removing fees.
 	 */
 	public void xxtestSuccessRemoveFees() throws Exception {
 		...
 		for (AccountFeesEntity accountFeesEntity : accountFeesEntitySet) {
 			assertEquals(accountFeesEntity.getFeeStatusAsEnum(),
 					FeeStatus.INACTIVE);
 		}
 		...
 		for (AccountActionDateEntity accountActionDate : accountBO
 			...
 			if (accountActionDate.getInstallmentId().equals(Short.valueOf("1")))
 				...
 				assertEquals(new Money("111.3"), loanScheduleEntity
 						.getTotalDueWithFees());
 			else assertEquals(new Money("112.0"), loanScheduleEntity
 					.getTotalDueWithFees());
 		}
 	...
 	*/
 	 ...
 	 * <p/>
 	 * TODO: This test code needs to be refactored! By creating the loan with a set of terms,
 		...
 		try {
 			loan = LoanBO.createLoan(TestUtils.makeUser(), loanOffering,
 					customer, state, new Money(currency, "300.0"), Short
 							.valueOf("6"), meetingDates.get(0), false, 0.0,
 	...
 	public static LoanBO createIndividualLoanAccount(String globalNum,
 		...
 		MifosCurrency currency = TestObjectFactory.getCurrency();
 		try {
 			loan = LoanBO.createIndividualLoan(TestUtils.makeUser(),
 					loanOfering, customer, state, new Money(currency, "300.0"),
 					Short.valueOf("6"), meetingDates.get(0), false, false, 0.0,
 					(short) 0, new FundBO(), new ArrayList<FeeView>(), null);
 		}
 		catch (ApplicationException e) {
 			throw new RuntimeException(e);
 	...
 	public void testUpdateLoanForLogging() throws ApplicationException,
 		...
 		assertEquals(EntityType.LOAN, auditLogList.get(0).getEntityTypeAsEnum());
 	...
 	public void testMakeEarlyRepaymentForCurrentDateLiesBetweenInstallmentDates()
 		...
 		assertEquals(accountBO.getAccountState().getId(), Short
 				.valueOf(AccountStates.LOANACC_OBLIGATIONSMET));
 	...
 	public void testMakeEarlyRepaymentOnPartiallyPaidAccount() throws Exception {
 		...
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("160"), accountBO
 		...
 		UserContext uc = TestUtils.makeUser();
 		Money totalRepaymentAmount = loanBO.getTotalEarlyRepayAmount();
 		assertEquals(new Money("300.2"), totalRepaymentAmount);
 		loanBO.makeEarlyRepayment(loanBO.getTotalEarlyRepayAmount(), null,
 				null, "1", uc.getId());
 		...
 		for (AccountTrxnEntity accountTrxnEntity : accountPaymentEntity
 			...
 			if (accountTrxnEntity.getInstallmentId().equals(Short.valueOf("6"))) {
 				LoanTrxnDetailEntity loanTrxnDetailEntity = (LoanTrxnDetailEntity) accountTrxnEntity;
 				assertEquals(new Money("46.0"), accountTrxnEntity.getAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 						.getInterestAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 						.getMiscFeeAmount());
 				...
 				assertEquals(new Money("0.2"), loanTrxnDetailEntity
 						.getInterestAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 						.getMiscFeeAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 				...
 				LoanTrxnDetailEntity loanTrxnDetailEntity = (LoanTrxnDetailEntity) accountTrxnEntity;
 				assertEquals(new Money("50.8"), accountTrxnEntity.getAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 						.getInterestAmount());
 				assertEquals(new Money(), loanTrxnDetailEntity
 						.getMiscFeeAmount());
 	...
 	public void testApplyRoundingAfterAddMiscFee() throws Exception {
 		...
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 		 ...
 		 FeeBO periodicFee = TestObjectFactory.createPeriodicAmountFee(
 				"Periodic Fee", FeeCategory.LOAN, "33", RecurrenceType.WEEKLY,
 				Short.valueOf("1"));
 		feeViewList.add(new FeeView(userContext, periodicFee));
 		...
 		*/
 		...
 		for (AccountActionDateEntity accountActionDateEntity : accountBO
 			...
 			if (loanScheduleEntity.getInstallmentId()
 				...
 				assertEquals(new Money("0.0"), loanScheduleEntity.getMiscFeeDue());
 				assertEquals(new Money("93.0"), loanScheduleEntity.getTotalFeeDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getInterestDue());
 			} else if (loanScheduleEntity.getInstallmentId().equals(Short.valueOf("2"))){
 				//periodic fee applies every 2nd week so no payments in even weeks
 				assertEquals(new Money("50.0"), loanScheduleEntity.getPrincipalDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getMiscFeeDue());
 				assertEquals(new Money("66.0"), loanScheduleEntity.getTotalFeeDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getInterestDue());
 		...
 		for (AccountActionDateEntity accountActionDateEntity : accountBO
 			...
 			if (loanScheduleEntity.getInstallmentId()
 					.equals(Short.valueOf("1"))){
 				assertEquals(new Money("50.0"), loanScheduleEntity.getPrincipal());
 				assertEquals(new Money("10.0"), loanScheduleEntity.getMiscFeeDue());
 				assertEquals(new Money("93.0"), loanScheduleEntity.getTotalFeeDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getInterestDue());
 				assertEquals(new Money("2.0"), loanScheduleEntity.getMiscPenaltyDue());
 			} else if (loanScheduleEntity.getInstallmentId().equals(Short.valueOf("2"))){
 				//periodic fee applies every  week so double fee payment (66) due
 				assertEquals(new Money("50.0"), loanScheduleEntity.getPrincipalDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getMiscFeeDue());
 				assertEquals(new Money("66.0"), loanScheduleEntity.getTotalFeeDue());
 				assertEquals(new Money("0.0"), loanScheduleEntity.getInterestDue());
 	...
 	public void testMakeEarlyRepaymentOnPartiallyPaidPricipal()
 		...
 		UserContext uc = TestUtils.makeUser();
 		Money totalRepaymentAmount = loanBO.getTotalEarlyRepayAmount();
 		assertEquals(new Money("280.1"), totalRepaymentAmount);
 		loanBO.makeEarlyRepayment(loanBO.getTotalEarlyRepayAmount(), null,
 				null, "1", uc.getId());
 	...
 	public void testDisburseLoanWithFeeAtDisbursement() throws Exception {
 		...
 		accountBO = getLoanAccountWithMiscFeeAndPenalty(
 				AccountState.LOAN_APPROVED, startDate, 1, miscFee, miscPenalty);
 	...
 	public void testDisbursalLoanWithInterestDeductedAtDisbursal()
 		...
 		for (AccountTrxnEntity transaction : accountTrxns) {
 			finTransaction += transaction.getFinancialTransactions().size();
 			...
 			if (accountAction == AccountActionTypes.FEE_REPAYMENT) {
 				...
 				Set<FeesTrxnDetailEntity> feesTrxnDetails = ((LoanTrxnDetailEntity) transaction)
 						.getFeesTrxnDetails();
 				assertEquals(2, feesTrxnDetails.size());
 				fail("This doesn't actually happen.  "
 						+ "Is it a copy-paste from a test which doesn't apply here?");
 			}
 ...
 */
 	...
 	public void testDisbursalLoanRegeneteRepaymentSchedule()
 		...
 		Date startDate = new Date(System.currentTimeMillis());
 		accountBO = getLoanAccountWithMiscFeeAndPenalty(
 				AccountState.LOAN_APPROVED, startDate, 3, new Money("20"),
 				new Money("30"));
 ...
 */
 	...
 	public void testWaivePenaltyChargeDue() throws Exception {
 		...
 		LoanActivityEntity loanActivityEntity = (LoanActivityEntity) objectList
 				.get(0);
 		assertEquals(LoanConstants.PENALTY_WAIVED, loanActivityEntity
 				.getComments());
 	...
 	public void testWaiveFeeChargeOverDue() throws Exception {
 		...
 		for (AccountActionDateEntity accountAction : loanBO
 			...
 			for (AccountFeesActionDetailEntity accountFeesActionDetailEntity : accountActionDateEntity
 				...
 				else assertEquals(new Money("100"),
 						accountFeesActionDetailEntity.getFeeAmount());
 			}
 		}
 	...
 	public void testRegenerateFutureInstallments() throws Exception {
 		...
 		for (AccountActionDateEntity actionDateEntity : accountBO
 			...
 			if (actionDateEntity.getInstallmentId() < 3)
 				assertEquals(DateUtils.getCurrentPlusWeeksDateWithoutTimeStamp(actionDateEntity.getInstallmentId() - 1),
 						DateUtils.getDateWithoutTimeStamp(actionDateEntity.getActionDate()));
 			else
 				assertEquals(DateUtils.getDateWithoutTimeStamp(meetingDates.get(actionDateEntity.getInstallmentId() - 1)), 
 						DateUtils.getDateWithoutTimeStamp(actionDateEntity.getActionDate()));
 		}
 	}
 	...
 	public void testRegenerateFutureWhenDayScheduleChanges() throws Exception {
 		...
 		MeetingRecurrenceEntity recurrence = meeting.getMeetingDetails()
 	...
 	public void testGetTotalRepayAmountForCustomerPerfHistory()
 		...
 		/*customerPerformanceHistory = ((LoanBO) accountBO).getCustomer().getPerformanceHistory();		
 	...
 	public void testDisbursalLoanForCustomerPerfHistory() throws Exception {
 		Date startDate = new Date(System.currentTimeMillis());
 		accountBO = getLoanAccountWithPerformanceHistory(
 				AccountState.LOAN_APPROVED, startDate, 3);
 	 ...
 	 * changes to the loan schedule. Note the use of method getLoanAccount(), which
 	 * radically alters the terms of the loan, unbeknownst to the reader of this code.
 	 * Here's a trace of this bizarre code:
 	 * <ul>
 	 *     <li> getLoanAccount() calls
 	 *     <li> TestObjectFactory.createLoanAccount(), which calls
 	 *     <li> TestLoanBO.createLoanAccount(). This method initiall creates the loan with terms:
 	 *     <ul>
 	 *         <li> Loan amount 0
 	 ...
 	 *         <li> no fees
 	 *     </ul>
 	 *     <li> The method then goes on to alter the loan by directly manipulating instance variables:
 	 *     <ul>
 	 *         <li> Add a periodic amount fee of 100 to each installment
 	 *         <li> Change each installment's principal to 100 and interest to 12.0 (but the
 	 *              loan amount is still 300, although the principal due is now 600!)
 	 *     </ul>
 	 * </ul>
 	 * This leaves the loan in a badly inconsistent state, causing the (second) call to
 	 * applyRounding() to break. This test code needs to be refactored!
 	 * <p>
 	 * This test now breaks. However, rounding with miscellaneous fees is fully covered by
 	 * tests in TestLoanRedoDisbursal.java, so it is disabled.
 	 */
 	public void xxtestRoundInstallments() throws AccountException,
 			SystemException {
 		accountBO = getLoanAccount();
 		TestObjectFactory.flushandCloseSession();
 		accountBO = TestObjectFactory.getObject(AccountBO.class, accountBO
 ...
 //		product.updateLoanOfferingSameForAllLoan(product);
 	...
 	/*
 	 * TODO: turn this back on when interest deducted at disbursement 
 	 * are re-enabled.
 	 *
 	...
 	public void testGracePeriodForInterestDedAtDisb()
 			throws NumberFormatException, AccountException, Exception {
 		createInitialCustomers();
 		LoanOfferingBO loanOffering = createLoanOffering(false, true);
 		LoanOfferingInstallmentRange eligibleInstallmentRange = loanOffering
 				.getEligibleInstallmentSameForAllLoan();
 ...
 */
 	...
 	/*public void testBuildLoanWithInvalidDisbDate()
 	 ...
 	 try {
 	 new LoanBO(TestUtils.makeUser(), loanOffering, group,
 	...
 	*/
 	...
 	/*
 	 * TODO: Add this back in when principal due in last installment is supported again.
 	 *
 	...
 	/*
 	 * TODO: turn this back on when principal due in last installment
 	 * and interest deducted at disbursement are re-enabled.
 	 *
 	public void testCreateLoanForPrincipalAtLastInstAndIntDedAtDisb()
 			throws NumberFormatException, AccountException, Exception {
 		createInitialCustomers();
 		LoanOfferingBO loanOffering = createLoanOffering(true, true);
 		List<FeeView> feeViews = getFeeViews();
 		boolean isInterestDedAtDisb = true;
 		Short noOfinstallments = (short) 6;
 ...
 */
 	public void testLoanScheduleRoundingWithMiscFee() throws Exception {
 		...
 		LoanBO loan = createButDontRetrieveLoanAccount(loanOffering, false, null, noOfinstallments, 0.0, new Money("300.0"));
 		...
 		for (AccountActionDateEntity accountActionDate : loan
 			...
 			if (loanScheduleEntity.getInstallmentId() != 6) {
 				...
 				assertEquals(
 						...
 						new Money("49.0"), loanScheduleEntity
 								.getTotalDueWithFees());
 			}
 		}
 	}
 	...
 	/**
 	 * TODO: Re-enable this method when rounding code can handle mid-stream changes to the loan that
 	 * require re-applying applyRounding(). (method LoanBO.applyCharge() does. When this method sets
 	 * payment status to PAID, applyRounding applies only to unpaid installments, which it cannot
 	 * currently handle.)
 	 * <p/>
 	 * Note: this method should not directly set status to PAID, as it leaves the loan in an 
 	 * inconsistent state. The the status is paid, but the installment's state is inconsistent --
 	 * the "paidX" instance variable still have value 0.
 	 */
 ...
 */
 	...
 	public void testCreateLoanAccountWithInterestDeductedAtDisbursment()
 		...
 		accountBO = LoanBO.createLoan(TestUtils.makeUser(), loanOffering,
 				group, AccountState.LOAN_ACTIVE_IN_GOOD_STANDING, new Money(
 						"300.0"), Short.valueOf("6"), startDate, true, 1.2,
 				(short) 0, new FundBO(), feeViewList, null);
 		new TestObjectPersistence().persist(accountBO);
 ...
 */
 	...
 	public void testCreateNormalLoanAccount() throws Exception {
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				"Loan", ApplicableTo.GROUPS, startDate, PrdStatus.LOAN_ACTIVE,
 				300.0, 1.2, 3, InterestType.FLAT, center
 						.getCustomerMeeting().getMeeting());
 		UserContext userContext = TestUtils.makeUser();
 		userContext.setLocaleId(null);
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14), "50.8", "0.2", fees2,
 				paymentsArray[0]);
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 6), "46.0", "0.0",
 				fees1, paymentsArray[5]);
 		...
 		assertEquals(new Money("300.0"), loanSummaryEntity
 				.getOriginalPrincipal());
 		assertEquals(new Money("1.0"), loanSummaryEntity.getOriginalInterest());
 		assertEquals(new Money("490.0"), loanSummaryEntity.getOriginalFees());
 		assertEquals(new Money("0.0"), loanSummaryEntity.getOriginalPenalty());
 	}
 	...
 	public void testCreateNormalLoanAccountWithPricipalOnlyGrace()
 		...
 		group = TestObjectFactory.createGroupUnderCenter("Group",
 				CustomerStatus.GROUP_ACTIVE, center);
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				"Loan", "Loan".substring(0, 1), ApplicableTo.GROUPS, new Date(
 						System.currentTimeMillis()), PrdStatus.LOAN_ACTIVE,
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14), "0.0", "1.0", fees2,
 				paymentsArray[0]);
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 6), "56.8", "-0.8",
 				fees1, paymentsArray[5]);
		...
		long sampleTime = new DateMidnight(2009, 11, 25).getMillis();
 		...
 		for (AccountFeesEntity accountFeesEntity : accountBO.getAccountFees()) {
 			if (accountFeesEntity.getFees().getFeeName().equals("Upfront Fee")) {
 				...
 				assertEquals(new Money("91.08"), accountFeesEntity
 						.getAccountFeeAmount());
 				assertEquals(new Double("30.0"), accountFeesEntity
 						.getFeeAmount());
 			}
 					...
 					"First Repayment Fee")) {
 				assertEquals(new Money("1.44"), accountFeesEntity
 						.getAccountFeeAmount());
 ...
 //		assertEquals(new Money("3.6"), loanSummaryEntity.getOriginalInterest());
 		assertEquals(new Money("4.5"), loanSummaryEntity.getOriginalInterest());
 //		assertEquals(new Money("1252.5"), loanSummaryEntity.getOriginalFees());
 		assertEquals(new Money("1252.58"), loanSummaryEntity.getOriginalFees());
 		assertEquals(new Money("0.0"), loanSummaryEntity.getOriginalPenalty());
 	}
 	...
 	public void testDisburseLoanWithAllTypeOfFees() throws Exception {
 		...
 		else disbursementDate = new GregorianCalendar(year, month + 1, day);
 		UserContext userContext = TestUtils.makeUser();
 		userContext.setLocaleId(null);
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 		FeeBO upfrontFee = TestObjectFactory.createOneTimeRateFee(
 		...
 		else disbursementDate = new GregorianCalendar(year, month + 2, day);
 	...
 	public void testUpdateLoanFailure() throws Exception {
 		...
 		try {
 			...
 			else assertEquals("errors.invalidDisbursementDate", expected
 					.getKey());
 	...
 	/*
 	 * TODO: fee_rounding 0.3 is expected as the onetime fee,
 	 * but 0.27 is returned.  Should this be rounded or not?
 ...
 */
 	public void testApplyPaymentForFullPayment() throws Exception {
 		...
 		assertEquals(new Money("212.0"), ((LoanBO) accountBO)
 				.getTotalPaymentDue());
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("212"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 	...
 	public void testApplyPaymentForPartialPayment() throws Exception {
 		...
 		assertEquals(new Money("212.0"), ((LoanBO) accountBO)
 				.getTotalPaymentDue());
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("200"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 	...
 	public void testApplyPaymentForFuturePayment() throws Exception {
 		...
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("312"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 	...
 	public void testApplyPaymentForCompletePayment() throws Exception {
 		...
 		assertEquals(new Money("212.0"), nextInstallment.getTotalDueWithFees());
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("1272"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 		...
 		assertEquals(AccountState.LOAN_CLOSED_OBLIGATIONS_MET, accountBO
 				.getState());
 	}
 	...
 	public void testApplyPaymentForPaymentGretaterThanTotalDue()
 		...
 		try {
 			accountBO.applyPaymentWithPersist(TestObjectFactory
 					.getLoanAccountPaymentData(null, new Money("1300"),
 							accountBO.getCustomer(), accountBO.getPersonnel(),
 							"432423", (short) 1, new Date(System
 	...
 	public void testFeeForMultiplePayments() throws Exception {
 		...
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("10"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 		...
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("30"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 	...
 	public void testLoanPerfHistoryForUndisbursedLoans() throws Exception {
 		...
 		((LoanScheduleEntity) accountActionDate2)
 				.setActionDate(offSetCurrentDate(14));
 	...
 	public void testFeeForMultiplePaymentsIncludingCompletePayment()
 		...
 		assertEquals(new Money("202.0"), ((LoanBO) accountBO)
 				.getTotalPaymentDue());
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("202"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 	...
 	public void testRemoveFeeForPartiallyPaidFeesAccount() throws Exception {
 		...
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("60"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, startDate, startDate));
 		HibernateUtil.commitTransaction();
 		...
 		for (AccountFeesEntity accountFeesEntity : accountBO.getAccountFees()) {
 			assertEquals(FeeStatus.INACTIVE, accountFeesEntity
 					.getFeeStatusAsEnum());
 	...
 	public void testApplyChargeForPartiallyPaidFeesAccount() throws Exception {
 		...
 		for (AccountFeesEntity accountFeesEntity : accountBO.getAccountFees()) {
 			assertEquals(FeeStatus.ACTIVE, accountFeesEntity
 					.getFeeStatusAsEnum());
 	...
 	public void testPartialPaymentForPrincipalGrace() throws Exception {
 		...
 		assertEquals(new Money("112.0"), ((LoanBO) accountBO)
 				.getTotalPaymentDue());
 		accountBO.applyPaymentWithPersist(TestObjectFactory
 				.getLoanAccountPaymentData(null, new Money("100"), accountBO
 						.getCustomer(), accountBO.getPersonnel(), "432423",
 						(short) 1, new Date(System.currentTimeMillis()),
 						new Date(System.currentTimeMillis())));
 		accountBO = saveAndFetch(accountBO);
 		...
 		assertEquals(Integer.valueOf(0), ((LoanBO) accountBO)
 				.getPerformanceHistory().getNoOfPayments());
 		assertFalse(((LoanBO) accountBO).getAccountActionDate((short) 1)
 				.isPaid());
 	}
 
 	public void testGetDaysInArrears() {
 		java.sql.Date lastWeekDate = setDate(
 				new GregorianCalendar().DAY_OF_MONTH, -14);
 
 		java.sql.Date twoWeeksBeforeDate = setDate(
 				new GregorianCalendar().DAY_OF_MONTH, -21);
 		accountBO = getLoanAccount();
 		for (AccountActionDateEntity installment : accountBO
 				.getAccountActionDates()) {
 			if (installment.getInstallmentId().intValue() == 1) {
 	...
 	public void testGetDaysWithoutPaymentWhendaysLessThanLateness()
 			throws PersistenceException {
 		int daysLessThanLateness = -7;
 		java.sql.Date lastWeekDate = setDate(
 				new GregorianCalendar().DAY_OF_MONTH, daysLessThanLateness);
 		accountBO = getLoanAccount();
 		for (AccountActionDateEntity installment : accountBO
 				.getAccountActionDates()) {
 			((LoanScheduleEntity) installment).setActionDate(lastWeekDate);
 		...
 		accountBO = TestObjectFactory.getObject(AccountBO.class, accountBO
 				.getAccountId());
 		assertEquals(0, ((LoanBO) accountBO).getDaysWithoutPayment());
 
 	}
 
 	public void testGetDaysWithoutPaymentWhendaysMoreThanLateness()
 			throws PersistenceException {
 		int daysMoreThanLateness = -21;
 		java.sql.Date twoWeeksBeforeDate = setDate(
 				new GregorianCalendar().DAY_OF_MONTH, daysMoreThanLateness);
 		accountBO = getLoanAccount();
 		...
 		accountBO = TestObjectFactory.getObject(AccountBO.class, accountBO
 				.getAccountId());
 		assertEquals(21, ((LoanBO) accountBO).getDaysWithoutPayment());
 	...
 	public void testGetTotalInterestAmountInArrearsAndOutsideLateness()
 			throws PersistenceException {
 	...
 	public void testGetTotalPrincipalAmountInArrearsAndOutsideLateness()
 			throws PersistenceException {
 		java.sql.Date lastWeekDate = setDate(
 				new GregorianCalendar().WEEK_OF_MONTH, -1);
 		java.sql.Date twoWeeksBeforeDate = setDate(
 	...
 	public void testGetNetOfSaving() throws Exception {
 		...
 		assertEquals(loanAccount.getRemainingPrincipalAmount().subtract(
 				loanAccount.getCustomer().getSavingsBalance()), loanAccount
 				.getNetOfSaving());
 	}
 	...
 	public void testCreateLoanAccountWithDecliningInterestNoGracePeriod()
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				...
 				PrdStatus.LOAN_ACTIVE, 300.0, 12.0, (short) 3,
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 1), "50.3", "0.7",
 				fees0, paymentsArray[0]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 2), "50.4", "0.6",
 				fees0, paymentsArray[1]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 3), "50.5", "0.5",
 				fees0, paymentsArray[2]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 4), "50.6", "0.4",
 				fees0, paymentsArray[3]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 5), "50.7", "0.3",
 				fees0, paymentsArray[4]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 6), "47.5", "0.5",
 	 ...
 	 */
 	/*
 	private void assertOneInstallmentFee (Money expected, LoanScheduleEntity installment) {
 		...
 		for (AccountFeesActionDetailEntity detail : actionDetails) {
 			assertEquals (expected, detail.getFeeAmount());
 		}
 	}
 	
 	private void assertInstallmentDetails (LoanScheduleEntity installment, Double principal,
 			Double interest, Double accountFee, Double miscFee, Double miscPenalty) {
 		assertEquals(new Money(principal.toString()), installment.getPrincipalDue());
 		assertEquals(new Money(interest.toString()), installment.getInterestDue());
 		assertEquals(new Money(miscFee.toString()), installment.getMiscFeeDue());
 		assertEquals(new Money(miscPenalty.toString()), installment.getMiscPenaltyDue());
 		assertOneInstallmentFee(new Money(accountFee.toString()), installment);
 	}
 	*/
 	public void testCreateLoanAccountWithDecliningInterestWithFeesNoGracePeriod()
 	throws NumberFormatException, PropertyNotFoundException,
 	SystemException, ApplicationException {
 		Date startDate = new Date(System.currentTimeMillis());
 		...
 		FeeBO fee = TestObjectFactory.createPeriodicRateFee("Periodic Rate Fee", FeeCategory.LOAN, 
 				new Double("9.0"), FeeFormula.INTEREST, RecurrenceType.WEEKLY,
 				new Short("1"), userContext, meeting);
 		...
 		LoanBO loan = (LoanBO) accountBO;
 		loan.applyCharge(Short.valueOf(AccountConstants.MISC_FEES), (double) 2.0);
 		loan.applyCharge(Short.valueOf(AccountConstants.MISC_PENALTY), (double) 10.0);
 ...
 HashMap fees0 = new HashMap();
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 1), "50.0", "0.7",
 				fees0, paymentsArray[0]);
 		assertOneInstallmentFee(new Money("0.3"), paymentsArray[0]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 2), "50.1", "0.6",
 				fees0, paymentsArray[1]);
 		assertOneInstallmentFee(new Money("0.3"), paymentsArray[1]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 3), "50.2", "0.5",
 				fees0, paymentsArray[2]);
 		assertOneInstallmentFee(new Money("0.3"), paymentsArray[2]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 4), "50.3", "0.4",
 				fees0, paymentsArray[3]);
 		assertOneInstallmentFee(new Money("0.3"), paymentsArray[3]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 5), "50.4", "0.3",
 				fees0, paymentsArray[4]);
 		assertOneInstallmentFee(new Money("0.3"), paymentsArray[4]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(7 * 6), "49.0", "0.1",
 				fees0, paymentsArray[5]);
 		assertOneInstallmentFee(new Money("-0.1"), paymentsArray[5]);
 	...
 	public void testCreateLoanAccountWithZeroFlatInterest()
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				"Loan", "L", ApplicableTo.GROUPS, startDate,
 				PrdStatus.LOAN_ACTIVE, 300.0, 0.0, (short) 3,
 				InterestType.FLAT, false, false, center
 						.getCustomerMeeting().getMeeting(), GraceType.NONE,
 				"1", "1");
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 1), "50.0", "0.0",
 				fees0, paymentsArray[0]);
 		
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 2), "50.0", "0.0",
 				fees0, paymentsArray[1]);
 		
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 3), "50.0", "0.0",
 				fees0, paymentsArray[2]);
 		
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 4), "50.0", "0.0",
 				fees0, paymentsArray[3]);
 		
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 5), "50.0", "0.0",
 				fees0, paymentsArray[4]);
 		
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 6), "50.0", "0.0",
 				fees0, paymentsArray[5]);
 	...
 	public void testCreateLoanAccountWithZeroDecliningInterest()
 			throws NumberFormatException, PropertyNotFoundException,
 			SystemException, ApplicationException {
 		Date startDate = new Date(System.currentTimeMillis());
 	...
 	public void testCreateLoanAccountWithDecliningInterestGraceAllRepayments()
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				...
 				12.0, (short) 3, InterestType.DECLINING, center
 						.getCustomerMeeting().getMeeting());
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 	...
 	public void testCreateLoanAccountWithDecliningInterestGracePrincipalOnly()
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				"Loan", "L", ApplicableTo.GROUPS, new Date(System
 						.currentTimeMillis()), PrdStatus.LOAN_ACTIVE, 300.0,
 				12.0, (short) 3, InterestType.DECLINING, false, false, center
 						.getCustomerMeeting().getMeeting(),
 				GraceType.PRINCIPALONLYGRACE, "1", "1");
 		List<FeeView> feeViewList = new ArrayList<FeeView>();
 		...
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 1), "0.0", "1.4",
 				fees0, paymentsArray[0]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 2), "0.0", "1.4",
 				fees0, paymentsArray[1]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 3), "74.5", "1.4",
 				fees0, paymentsArray[2]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 4), "74.8", "1.1",
 				fees0, paymentsArray[3]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 5), "75.2", "0.7",
 				fees0, paymentsArray[4]);
 
 		checkLoanScheduleEntity(incrementCurrentDate(14 * 6), "75.5", "0.3",
 				fees0, paymentsArray[5]);
 ...
 */
 	...
 	public void testCreateLoanAccountWithEqualPrincipalDecliningInterestGraceAllRepayments()
 		...
 		LoanOfferingBO loanOffering = TestObjectFactory.createLoanOffering(
 				"Loan", ApplicableTo.GROUPS, new Date(System
 						.currentTimeMillis()), PrdStatus.LOAN_ACTIVE, 300.0,
 				1.2, (short) 3, InterestType.DECLINING_EPI,
 ...
 */
 	...
 	private LoanBO createAndRetrieveLoanAccount(LoanOfferingBO loanOffering,
 		...
 		//LoanBO loan = LoanBO.createLoan(TestUtils.makeUser(), loanOffering,
 		LoanBO loan = LoanBO.createLoan(userContext, loanOffering,
 	...
 	/**
 	 * Added by Keith -- the original, createAndRetrieveLoanAccount(), when it retrieves
 	 * the loan from the database, would have its userContext set to null --
 	 * this would happen in the test method testLoanScheduleRoundingWithMiscFee().
 	 * This modified method saves the loan but does not replace it with the retrieved
 	 * one before returning it.
 	 */
 	private LoanBO createButDontRetrieveLoanAccount(LoanOfferingBO loanOffering,
 			boolean isInterestDedAtDisb, List<FeeView> feeViews,
 			Short noOfinstallments, Double interestRate, Money loanAmount)
 			throws NumberFormatException, AccountException, SystemException,
 			ApplicationException {
 	...
 	/*
 	 * This method is called by tests that are currently commented out
 	 * waiting for these parameters to be turned back on.  The tests
 	 * that use this method have been marked as TODO 's
 	 */
 	private LoanOfferingBO createLoanOffering(boolean isPrincipalAtLastInst,
 			boolean interestDeductedAtDisbursement) {
 		return createLoanOffering(isPrincipalAtLastInst, interestDeductedAtDisbursement, PrdStatus.LOAN_ACTIVE);
 	}
 	...
 	private LoanOfferingBO createLoanOffering(boolean principalAtLastInst,
 			boolean interestDeductedAtDisbursement, PrdStatus status) {
 		MeetingBO meeting = TestObjectFactory.createMeeting(TestObjectFactory
 				.getNewMeetingForToday(WEEKLY, EVERY_WEEK, CUSTOMER_MEETING));
 		return TestObjectFactory.createLoanOffering("Loan",
 				ApplicableTo.GROUPS, new Date(System.currentTimeMillis()),
 				status, 300.0, 1.2, 3, InterestType.FLAT, interestDeductedAtDisbursement,
 	...
 	public void testIsDisbursementDateLessThanCurrentDateShouldReturnTrueIfDateInPast()
 			throws Exception {
 		assertTrue(LoanBO
 				.isDisbursementDateLessThanCurrentDate(getDateFromToday(-1)));
 	}
 
 	public void testIsDisbursementDateLessThanCurrentDateShouldReturnFalseIfDateInFuture()
 			throws Exception {
 		assertFalse(LoanBO
 				.isDisbursementDateLessThanCurrentDate(getDateFromToday(1)));
 	}
 
 	public void testIsDisbursementDateLessThanCurrentDateShouldReturnFalseIfDateIsToday()
 			throws Exception {
 		assertFalse(LoanBO
 				.isDisbursementDateLessThanCurrentDate(getCurrentDateWithoutTimeStamp()));
 	}
 
 	public void testIsDisbursementDateAfterProductStartDateShouldReturnFalseIfProductStartDateIsLaterThanDisbursement()
 			throws Exception {
 		assertFalse(LoanBO.isDisbursementDateAfterProductStartDate(
 				currentDate(), new LoanOfferingBO() {
 					@Override
 	...
 	public void testIsDisbursementDateAfterProductStartDateShouldReturnTrueIfProductStartDateIsEarlierThanDisbursement()
 			throws Exception {
 		assertTrue(LoanBO.isDisbursementDateAfterProductStartDate(
 				currentDate(), new LoanOfferingBO() {
 					@Override
 					public Date getStartDate() {
 						return getDateFromToday(-1);
