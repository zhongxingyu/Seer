 ...
 public class FacebookConnect extends Plugin {
 	...
 	private final String CLASS = FacebookConnect.class.getName();
 	...
 	public PluginResult postOnWall(final JSONArray args, String callbackId) throws JSONException {
 		JSONObject params = args.getJSONObject(0);
 		
 		Log.d(CLASS, "Posting to wall: " + args.toString());
 		...
 		try {
 			...
 			response = _facebook.request("me/feed", parameters, "POST");
 			Log.d(CLASS, "got response: " + response);
 		...
 		Log.d(CLASS, "post to wall::result " + response.toString());
 	 ...
 	 * @throws IOException
 	 */
 	public PluginResult login(final JSONArray args, final String callbackId) throws JSONException, MalformedURLException,
 	        IOException {
 	 ...
 	 * @throws IOException
 	 */
 	public PluginResult requestWithGraphPath(final JSONArray args, final String callbackId) throws JSONException,
 	 ...
 	 * @throws IOException
 	 */
 	public PluginResult dialog(final JSONArray args, final String callbackId) throws JSONException, FileNotFoundException,
 		...
 		Runnable runnable = new Runnable() {
 			public void run() {
 				me.getFacebook().dialog(me.cordova.getActivity().getApplicationContext(), method, options,
 				        new RegularDialogListener(me, callbackId));
 			};
 		};
 		pluginResult.setKeepCallback(true);
 		this.cordova.getActivity().runOnUiThread(runnable);
 	 ...
 	 * @throws IOException
 	 */
 	public PluginResult logout(final JSONArray args, final String callbackId) throws JSONException, MalformedURLException,
 	        IOException {
 	...
 	public void onActivityResult(int requestCode, int resultCode, Intent data) {
 		...
 		// this.webView.sendJavascript("window.alert('test')"); //@todo not
 		// working :(
 	}
