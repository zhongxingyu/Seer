 ...
 public class CommandHandler {
     ...
     public void debug(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
         if (args.hasFlag('s')) {
             ...
             Messaging.send(sender, ChatColor.YELLOW + (dB.showStackTraces ? "Denizen dBugger is now showing caught " +
                     "exception stack traces." : "Denizen dBugger is now hiding caught stacktraces."));
 
         } if (args.hasFlag('c')) {
             if (!dB.showDebug) dB.toggle();
             dB.showColor = !dB.showColor;
             Messaging.send(sender, ChatColor.YELLOW + (dB.showColor ? "Denizen dBugger is now showing color."
             ...
             Messaging.send(sender, ChatColor.YELLOW + (dB.showScriptBuilder ? "Denizen dBugger is now logging the " +
             ...
             dB.record = !dB.record;
             dB.Recording = new StringBuilder();
             Messaging.send(sender, ChatColor.YELLOW + (dB.record ? "Denizen dBugger is now recording. Use /denizen " +
             ...
             Messaging.send(sender, ChatColor.YELLOW + "Denizen dBugger filter now: " + dB.filter.toString());
 
         } else if (args.getFlags().isEmpty()) {
             dB.toggle();
             Messaging.send(sender, ChatColor.YELLOW + "Denizen dBugger is now: "
                     + (dB.showDebug ? ChatColor.GREEN + "ENABLED" : ChatColor.RED + "DISABLED"));
         }
