 ...
 public final class PluginLoader {
 	...
 	private static void loadPlugin(File file) {
 		...
 		for (PluginMetadata.Annotation metadata : metadatas) {
 			try {
 				...
 				clazz = ensureBackwardsCompatibility(clazz);
 	...
 	private static Class<?> ensureBackwardsCompatibility(Class<?> clazz) {
		...
		ClassVisitor remapper = new RemappingClassAdapter(writer, new SimpleRemapper("spaceshooters/api/plugins/PluginMetadataAnnotation", "spaceshooters/api/plugins/PluginMetadata$Annotation"));
 		reader.accept(remapper, ClassReader.EXPAND_FRAMES);
 		return PluginClassLoader.loadCompatible(clazz.getCanonicalName(), writer.toByteArray());
 	}
