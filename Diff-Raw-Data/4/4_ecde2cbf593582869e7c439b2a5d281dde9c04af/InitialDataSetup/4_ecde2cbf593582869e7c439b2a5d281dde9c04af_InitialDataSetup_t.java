 		...
 		this.transactionTemplate.execute(new TransactionCallback<Void>() {
 			...
 			public Void doInTransaction(TransactionStatus status) {
 				...
 				// Create account for JD and admin
 				{
 					...
 					johnDoe = InitialDataSetup.this.accountBuilder.address("Brussels", "1000", "Nieuwstraat", "1", "A")
 							.credentials("jd", "secret").name("John", "Doe")
 							.roleWithPermissions(roleCustomer, permissionCreateOrders).build();
 
 					InitialDataSetup.this.accountBuilder.address("Antwerp", "2000", "Meir", "1", "A")
 							.credentials("admin", "secret").name("Super", "User")
 							.roleWithPermissions(roleAdmin, permissionAddBooks, permissionAddCategories).build();
 
 					InitialDataSetup.this.accountBuilder.address("Gent", "9000", "Abdijlaan", "1", "A")
 							.credentials("author", "secret").name("Some", "Author")
