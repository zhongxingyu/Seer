 package org.bpelunit.test.datasources;
 ...
 import org.bpelunit.framework.control.datasource.VelocityDataSource;
 ...
 public class VelocityDataSourceTest {
 	...
 	public void invalidRowsAreReported() throws Exception {
 		...
 		try {
 			ds.initializeContext(ctx, 4);
 			fail("A DataSourceException was expected.");
 		} catch (DataSourceException ex) {
 			assertTrue("The exception message should indicate that an "
 					+ "invalid row index was used.", ex.getMessage().contains(
 					"row"));
 		}
 	}
 	...
 	public void nestedListLiteralsWork() throws Exception {
 		...
 		final String[][] expectedRows = new String[][]{
 				new String[]{},
 				new String[]{"A"},
 				new String[]{"A","B"},
 				new String[]{"A","B","C"}
 		...
 		for (int iRow = 0; iRow < ds.getNumberOfRows(); ++iRow) {
 			...
 			ArrayList actualLines = (ArrayList)ctx.get("lines");
