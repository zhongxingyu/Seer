  ...
  * class.
  * 
 * @version 1.2 $Revision: 1.11 $ $Date: 2007-09-17 17:08:08 $
  * 
 ...
 class DialogFilter implements ServerRequestInterface, ServerResponseInterface {
 			...
 			if (transaction != null && transaction.isInviteTransaction()) {
 				...
 				if (dialog == null) {
 					...
 					if (!dialog.handleAck(transaction)) {
 						...
 						/*
 						 * try { sipStack.addTransaction(transaction); } catch
 						 * (IOException ex) { // should never happen. }
 						 */
 	...
 	public void processResponse(SIPResponse response,
 		...
 		if (transaction == null) {
 			...
 			ResponseEvent sipEvent = new ResponseEvent(sipProvider,
 	...
 	public void processResponse(SIPResponse sipResponse,
 		...
 		if ( this.transactionChannel != null) {
			...
			// Workaround for asterisk bug.
 			if (originalToTag == null && sipResponse.getToTag() != null && sipResponse.getCSeq().getMethod().equals(Request.CANCEL)) {
				sipStack.getLogWriter().logDebug("CANCEL has a to tag while original Request does not have to tag -- stripping the spurious tag. ");
 			...
 			if (sipProvider.isAutomaticDialogSupportEnabled()) {
 				if (this.transactionChannel != null) {
 					if (sipDialog == null) {
 						...
 						sipDialog = new SIPDialog(this.transactionChannel,
 								sipResponse);
 		...
 		ResponseEvent responseEvent = new javax.sip.ResponseEvent(sipProvider,
 				(ClientTransaction) transaction, sipDialog,
