 ...
 public class GIFImageWriter extends ImageWriter {
   ...
   static final String               STANDARD_METADATA_NAME = IIOMetadataFormatImpl.standardMetadataFormatName;
 
   static final String               STREAM_METADATA_NAME   = GIFWritableStreamMetadata.NATIVE_FORMAT_NAME;
 
   static final String               IMAGE_METADATA_NAME    = GIFWritableImageMetadata.NATIVE_FORMAT_NAME;
   ...
   /**
    * The number of bits represented by the value which should be a legal length
    * for a color table.
    */
   ...
   private static byte[] createColorTable(ColorModel colorModel, SampleModel sampleModel) {
     ...
     if (colorModel instanceof IndexColorModel) {
       ...
       /**
        * The GIF image format assumes that size of image palette is power of
        * two. We will use closest larger power of two as size of color table.
        */
       ...
       /**
        * fill tail of color component arrays by replica of first color in order
        * to avoid appearance of extra colors in the color table
        */
   ...
   /**
    * According do GIF specification size of clor table (palette here) must be in
    * range from 2 to 256 and must be power of 2.
    */
   ...
   /**
    * Merges <code>inData</code> into <code>outData</code>. The supplied metadata
    * format name is attempted first and failing that the standard metadata
    * format name is attempted.
    */
   ...
   /**
    * Creates a default stream metadata object and merges in the supplied
    * metadata.
    */
   ...
   public IIOMetadata convertImageMetadata(IIOMetadata inData,
     ...
     GIFWritableImageMetadata im = (GIFWritableImageMetadata) getDefaultImageMetadata(imageType,
                                                                                      param);
   ...
   public void prepareWriteSequence(IIOMetadata streamMetadata) throws IOException {
     ...
     if (streamMetadata == null) {
       ...
       convertMetadata(STREAM_METADATA_NAME, streamMetadata, theStreamMetadata);
     }
   ...
   public void write(IIOMetadata sm, IIOImage iioimage, ImageWriteParam p) throws IOException {
     ...
     if (sm == null) {
       streamMetadata = (GIFWritableStreamMetadata) getDefaultStreamMetadata(p);
     } else {
       streamMetadata = (GIFWritableStreamMetadata) convertStreamMetadata(sm, p);
     }
   ...
   public void writeToSequence(IIOImage image, ImageWriteParam param) throws IOException {
     ...
     write(!wroteSequenceHeader, false, theStreamMetadata, image, param);
   ...
   /**
    * Writes any extension blocks, the Image Descriptor, the image data, and
    * optionally the header (Signature and Logical Screen Descriptor) and trailer
    * (Block Terminator).
    * 
    ...
    *           greater than 8.
    * @throws IllegalArgumentException if the color component size is greater
    *           than 8.
   ...
   private void write(boolean writeHeader,
     ...
     if (iioimage.getMetadata() != null) {
       ...
       if (imageMetadata.localColorTable == null) {
         ...
         if (colorModel instanceof IndexColorModel) {
           ...
           /*
            * NB: transparentColorFlag might have not beed reset for greyscale
            * images but explicitly reseting it here is potentially not right
            * thing to do until we have way to find whether current value was
     ...
     if (writeHeader) {
       ...
       if (streamMetadata.colorResolution == GIFMetadata.UNDEFINED_INTEGER_VALUE) {
         streamMetadata.colorResolution = colorModel != null ? colorModel.getComponentSize()[0]
     ...
     writeImage(iioimage.getRenderedImage(),
                imageMetadata,
                p,
                globalColorTable,
                sourceBounds,
   ...
   private void writeImage(RenderedImage image,
                           ...
                           Dimension destSize) throws IOException {
     ColorModel colorModel = image.getColorModel();
     SampleModel sampleModel = image.getSampleModel();
     ...
     if (imageMetadata == null) {
       ...
       imageMetadata = (GIFWritableImageMetadata) getDefaultImageMetadata(new ImageTypeSpecifier(image),
                                                                          param);
     ...
     int bitsPerPixel = getNumBits(imageMetadata.localColorTable == null ? (globalColorTable == null ? sampleModel.getSampleSize(0)
   ...
   private void writeRows(RenderedImage image,
                          ...
                          int dh,
                          int numRowsWritten,
                          int progressReportRowPeriod) throws IOException {
     LOG.debug("Writing unoptimized");
 
     int[] sbuf = new int[sw];
     byte[] dbuf = new byte[dw];
 
     Raster raster = image.getNumXTiles() == 1 && image.getNumYTiles() == 1 ? image.getTile(0, 0)
                                                                           : image.getData();
   ...
   private void writeRasterData(RenderedImage image,
     ...
     boolean isOptimizedCase = periodX == 1 && periodY == 1
         && sampleModel instanceof ComponentSampleModel && image.getNumXTiles() == 1
   ...
   private void writeImageDescriptor(int imageLeftPosition,
                                     ...
                                     boolean sortFlag,
                                     int bitsPerPixel,
                                     byte[] localColorTable) throws IOException {
