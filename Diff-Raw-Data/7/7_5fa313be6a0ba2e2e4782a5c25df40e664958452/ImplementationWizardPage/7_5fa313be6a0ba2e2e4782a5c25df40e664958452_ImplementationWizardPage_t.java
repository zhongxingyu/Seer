...
import org.bonitasoft.studio.common.jface.databinding.validator.InputLengthValidator;
  ...
  *
  */
 public class ImplementationWizardPage extends NewTypeWizardPage implements ISelectionChangedListener {
     ...
     public ImplementationWizardPage(ConnectorImplementation implementation,List<ConnectorImplementation> existingImpl,List<ConnectorDefinition> definitions,SourceRepositoryStore sourceStore,String pageTitle,String pageDescription,DefinitionResourceProvider messageProvider) {
         super(true,ImplementationWizardPage.class.getName());
         setTitle(pageTitle);
         ...
         try {
             IPackageFragmentRoot root = RepositoryManager.getInstance().getCurrentRepository().getJavaProject().findPackageFragmentRoot(sourceStore.getResource().getFullPath());
     ...
     public void createControl(Composite parent) {
         ...
         final Label definitionLabel = new Label(mainComposite, SWT.NONE);
         definitionLabel.setText(Messages.definition+" *");
         definitionLabel.setLayoutData(GridDataFactory.fillDefaults().align(SWT.END, SWT.CENTER).create()) ;
 
         final ComboViewer definitionViewer = new ComboViewer(mainComposite, SWT.BORDER | SWT.READ_ONLY) ;
         definitionViewer.getCombo().setLayoutData(GridDataFactory.fillDefaults().grab(true, false).span(2, 1).create()) ;
 
         definitionViewer.setContentProvider(new ArrayContentProvider()) ;
 
         definitionViewer.setLabelProvider(new ConnectorDefinitionTreeLabelProvider(messageProvider){
             @Override
         ...
         definitionViewer.setInput(definitions) ;
 
         String id = implementation.getDefinitionId() ;
         String version = implementation.getDefinitionVersion() ;
         Collection<ConnectorDefinition> definitions = (Collection<ConnectorDefinition>) definitionViewer.getInput() ;
         for(ConnectorDefinition def : definitions){
             if(def.getId().equals(id) && def.getVersion().equals(version)){
                 definitionViewer.setSelection(new StructuredSelection(def)) ;
                 break ;
             }
         }
         ...
         final IViewerObservableValue observeSingleSelection = ViewersObservables.observeSingleSelection(definitionViewer);
         ...
         context.bindValue(observeSingleSelection, EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__DEFINITION_VERSION),defVersionStrategy,null) ;
         ...
         idStrategy.setBeforeSetValidator(new IValidator() {
             ...
             @Override
             public IStatus validate(Object value) {
                 if(!FileUtil.isValidName(NamingUtils.toConnectorImplementationFilename(value.toString(), implementation.getImplementationVersion(), true))){
         ...
         context.bindValue(observableIdText, EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__IMPLEMENTATION_ID),idStrategy,null) ;
         context.bindValue(observableVersionText, EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__IMPLEMENTATION_VERSION),versionStrategy,null) ;
        ...
        context.bindValue(SWTObservables.observeText(descriptionText, SWT.Modify), EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__DESCRIPTION),descStrategy,null) ;
                 ...
                 if(className != null && !className.isEmpty()){
                     ...
                     for(ConnectorImplementation impl : existingImpl){
                         if(qualifiedClassName.equals(impl.getImplementationClassname())){
                             return ValidationStatus.error(qualifiedClassName +" "+ Messages.bind(Messages.alreadyExistsForAnotherImplementation,NamingUtils.toConnectorImplementationFilename(impl.getImplementationId(), impl.getImplementationVersion(), false)));
                         }
                     }
                 }
                 return ValidationStatus.ok();
         ...
         definitionViewer.addSelectionChangedListener(new ISelectionChangedListener() {
             ...
             public void selectionChanged(SelectionChangedEvent event) {
                 ConnectorDefinition def = (ConnectorDefinition) ((IStructuredSelection) event.getSelection()).getFirstElement();
                 if(idText.getText() == null || idText.getText().isEmpty()){
                     idText.setText(def.getId()+"-impl");
                 }
                 if(observeClassText.getValue() == null || observeClassText.getValue().toString().isEmpty()){
                     String id = def.getId();
                     ...
                     observeClassText.setValue(className);
                 }
                 if(observePackageText.getValue() == null || observePackageText.getValue().toString().isEmpty()){
     ...
     protected void createClassAndPackageName(Composite mainComposite) {
         ...
         classTargetToModel.setBeforeSetValidator(new IValidator() {
             ...
             @Override
             public IStatus validate(Object value) {
                 if(implementation.getImplementationClassname() == null || implementation.getImplementationClassname().toString().indexOf(".") == -1){
         ...
         context.bindValue(observeClassText, EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__IMPLEMENTATION_CLASSNAME),classTargetToModel,classModelToTarget) ;
         context.bindValue(observePackageText, EMFObservables.observeValue(implementation, ConnectorImplementationPackage.Literals.CONNECTOR_IMPLEMENTATION__IMPLEMENTATION_CLASSNAME),packageTargetToModel,packageModelToTarget) ;
         ...
         browsePackagesButton.addListener(SWT.Selection, new Listener() {
             ...
             public void handleEvent(Event event) {
                 IPackageFragment selectedPackage = ImplementationWizardPage.this.choosePackage();
     ...
     protected void createDependenciesViewer(Composite mainComposite) {
         ...
         context.bindValue(ViewersObservables.observeInput(viewer), EMFObservables.observeValue(implementation.getJarDependencies(), ConnectorImplementationPackage.Literals.JAR_DEPENDENCIES__JAR_DEPENDENCY)) ;
