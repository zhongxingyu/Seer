 ...
 public class AdminCmdWorker {
     ...
     public AdminCmdWorker(int sVersion) {
         ...
         materialsColors.put(Material.WOOL, new String[]{
                     "White", "Orange", "Magenta", "LightBlue", "Yellow",
                     "LimeGreen", "Pink", "Gray", "LightGray", "Cyan",
                     "Purple", "Blue", "Brown", "Green", "Red", "Black"
                 });
         materialsColors.put(Material.INK_SACK, new String[]{
                     "Black", "Red", "Green", "Brown", "Blue", "Purple",
                     "Cyan", "LightGray", "Gray", "Pink", "LimeGreen",
                     "Yellow", "LightBlue", "Magenta", "Orange", "White"
                 });
         materialsColors.put(Material.LOG, new String[]{
                     "Oak", "Pine", "Birch"
                 });
         materialsColors.put(Material.STEP, new String[]{
     ...
     private boolean tpP2P(String nFrom, String nTo) {
         ...
         if (pTo == null) {
             player.sendMessage(ChatColor.RED + "Player " + ChatColor.WHITE + nTo + ChatColor.RED + " not found!");
             found = false;
         }
         if (found) {
             if (serverVersion >= 600)
                 pFrom.teleport(pTo);
             else
                 pFrom.teleportTo(pTo);
             player.sendMessage("Succefully teleported " + ChatColor.BLUE + pFrom.getName() + ChatColor.WHITE + " to " + ChatColor.GREEN + pTo.getName());
         }
         return true;
     }
     ...
     public boolean playerLocation(String[] name) {
         ...
         if (name.length == 0) {
             ...
             try {
                 ...
                 player.sendMessage(ChatColor.RED + "Player " + ChatColor.WHITE + name[0] + ChatColor.RED + " not found!");
                 return true;
             }
     ...
     public boolean itemGive(String[] args) {
         ...
         if (!target.getName().equals(player.getName())) {
             target.sendMessage(ChatColor.RED + "[" + player.getName() + "]" + ChatColor.WHITE + " send you " + ChatColor.GOLD + cnt + " " + m.name());
             player.sendMessage(ChatColor.RED + "Added " + ChatColor.GOLD + cnt + " " + m.name() + " to " + ChatColor.WHITE + target.getName() + "'s inventory");
         } else
             player.sendMessage(ChatColor.RED + "Added " + ChatColor.GOLD + cnt + " " + m.name() + " to " + ChatColor.WHITE + "your inventory");
     ...
     public boolean itemColor(String color) {
         ...
         // error?
         if (value < 0) {
             player.sendMessage(ChatColor.RED + "Color " + ChatColor.WHITE + color
