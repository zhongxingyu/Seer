 ...
 public class ScrabbleMap {
 	...
 	public List<BrickList> addBricks(BrickList aBrickList){
 		...
 		ScrabbleLogger.log( "> found word: " + word.toWord() + " - words.size: " + words.size(), LogLevel.DEBUG );
 	...
 	public boolean removeBricks(BrickList aBrickList) {
 		...
 		if( orientation == OrientationEnum.HORIZONTAL ){
 			return removeBricksHoriontal( aBrickList );
 		}
 		else if( orientation == OrientationEnum.VERTICAL ){
 			return removeBricksVertical( aBrickList );
 		}
 		return false;
 	}
 	...
 	protected boolean isBrickPlaceable(int aRow, int aCol, Brick aBrick){
 		...
 		boolean B = (A && map[aRow][aCol].getLetter() == aBrick.getLetter() && aBrick.getWeight() < 0);
 		return ( !A || B );
 	}
 	 ...
 	 * @param aRow Row to place first letter
 	 * @param aCol Column to place first letter
 	 * @param aLength Length of the word (to check for first placed word)
 	 * @param aOrientation Orentation to place the word
