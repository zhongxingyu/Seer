 ...
 public class Main {
 	...
 	// TODO changed to maps for easier access to specific values
 	...
 	private void initialiseSolution() {
 		...
 		murderScene = ((Map<String, Room>) board.getAllRooms()).get((int) (Math
 	...
 	private void initialiseRooms() {
 		...
 		for (String r : ROOM) {
			...
			board.addRoom(new Room(r, temp.get(rand)));
 			temp.remove(rand);
 		}
 		// add doors to rooms
 		for (Tile tile : board.getDoors()) {
 			board.getRoom(tile.getRoom()).addDoors(tile);
 		}
 	}
 	...
 	public void startMainGameLoop() throws NumberFormatException, IOException {
 		...
 		while (winner == null) {
 			...
 			if (startLocation instanceof Room) {
 				winner = offerRoomOptions(currentPlayer, currentToken,
 						(Room) startLocation);
 	...
 	private Player offerRoomOptions(Player currentPlayer,
 			Character currentToken, Room room) throws IOException {
 	...
 	private Player useSecretPassage(Player currentPlayer,
			Character character, Room startLocation, String passageWay) {
 		Set<Room> rooms = (Set<Room>) board.getAllRooms();
 		...
 		while (dieRoll > 0) {
 			...
 			if (board.validLocation(x, y)) {
 				...
 				if (toMoveTile.getType() == '.') {
 					...
 					dieRoll--;
 					if (toMoveTile.getType() == 'E') {
 						board.enterRoom(currentLocation, toMoveTile);
 					}
 				} else {
