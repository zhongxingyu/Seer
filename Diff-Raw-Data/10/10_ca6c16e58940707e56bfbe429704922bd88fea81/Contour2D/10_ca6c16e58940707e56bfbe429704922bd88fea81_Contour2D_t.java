 ...
 */
 public class Contour2D {
   ...
   public static void contour(float g[], int nr, int nc, float interval,
                              ...
                              int maxv1, int[] numv1, float vx2[][],
                              float vy2[][], float[][] vz2, int maxv2,
                              int[] numv2, float vx3[][], float vy3[][],
                              float[][] vz3, int maxv3, int[] numv3,
                              float vx4[][], float vy4[][], float[][] vz4,
                              int maxv4, int[] numv4, byte[][] auxValues,
                              byte[][] auxLevels1, byte[][] auxLevels2,
                              byte[][] auxLevels3, boolean[] swap,
                              boolean fill, float[][] tri, byte[][] tri_color,
                              float[][][] grd_normals, float[][] tri_normals,
                              byte[][] interval_colors, float[][][][] lbl_vv,
                              byte[][][][] lbl_cc, float[][][] lbl_loc,
                              double scale_ratio, double label_size,
                              byte[] labelColor, Gridded3DSet spatial_set)
           throws VisADException {
 ...
 /* DRM 1999-05-18, CTR 29 Jul 1999: values could be null */
     ...
     if (values != null && 
       ...
       byte[][] tmpColors =
         new byte[interval_colors.length][interval_colors[0].length];
       for (int colorIdx = 0; colorIdx < interval_colors[0].length;
               colorIdx++) {
         for (int rgbIdx = 0; rgbIdx < interval_colors.length; rgbIdx++) {
           tmpColors[rgbIdx][indices[colorIdx]] =
             interval_colors[rgbIdx][colorIdx];
         }
       }
       interval_colors = tmpColors;
 
 
       //JDM: Clip the myvals array to only use values within the range of the data
       int minIdx = 0;
       ...
       if (debug) {
         System.err.println(
           "min: " + minValue + " max:" + maxValue + " idx:" + minIdx + "-" +
           maxIdx + " new size:" + newSize);
 ...
 */
 ...
 //    Trace.startTrace();
     ...
     visad.util.Trace.call1(
       "Contour2d.loop",
       " nrm=" + nrm + " ncm=" + ncm + " naux=" + naux + " myvals.length=" +
       myvals.length);
 ...
 */
         ...
         if (gMax < minLevelValue || gMin > maxLevelValue) {
           ...
           // some inside the box
           //JDM: Instead of iterating through the whole list just do a binarySearch
           /*
           for (int i = 0; i < myvals.length; i++)            {
           ...
           if (hi < 0) hi = (-hi) - 1;
           if (hi >= myvals.length) hi = myvals.length - 1;
 ...
 /*
 if (!any && numc > 0) {
   System.out.println("gMin = " + gMin + " gMax = " + gMax + " gAvg = " + gAvg);
 ...
 */
         ...
         for (il = 0; il < numc; il++) {
           if ((low + il) >= myvals.length) {
             System.err.println(
               "bad range: myvals.length=" + myvals + " il=" + il + " low=" +
               low + " high=" + hi);
           }
           gg = myvals[low + il];
               ...
               if (naux > 0) {
                 ...
                 for (int i = 0; i < naux; i++) {
                   t = (int)((1.0f - ratioba) * ((auxa[i] < 0)
                           ? ((float)auxa[i]) + 256.0f
                           : ((float)auxa[i])) + ratioba * ((auxb[i] < 0)
                           ? ((float)auxb[i]) + 256.0f
                           : ((float)auxb[i])));
                   auxLevels[i][numv] = (byte)((t < 0)
                           ? 0
                           : ((t > 255)
                              ? -1
                              : ((t < 128)
                   ...
                   t = (int)((1.0f - ratioca) * ((auxa[i] < 0)
                           ? ((float)auxa[i]) + 256.0f
                           : ((float)auxa[i])) + ratioca * ((auxc[i] < 0)
                           ? ((float)auxc[i]) + 256.0f
                           : ((float)auxc[i])));
                   auxLevels[i][numv + 1] = (byte)((t < 0)
                           ? 0
                           : ((t > 255)
                              ? -1
                              : ((t < 128)
 ...
 */
               ...
               if (((gba) < 0
                    ? -(gba)
                    : (gba)) < 0.0000001) vx[numv] = xx;
               else vx[numv] = xx + xd * (gg - ga) / gba;
               vy[numv] = yy;
               numv++;
               if (((gdb) < 0
                    ? -(gdb)
                    : (gdb)) < 0.0000001) vy[numv] = yy;
               else vy[numv] = yy + yd * (gg - gb) / gdb;
               vx[numv] = xx + xd;
               numv++;
               if (fill) {
                 o_flags[ir][ic][n_lines[ir][ic]] = (byte)ii;
 ...
 */
               ...
               if (((gca) < 0
                    ? -(gca)
                    : (gca)) < 0.0000001) vy[numv] = yy;
               else vy[numv] = yy + yd * (gg - ga) / gca;
               vx[numv] = xx;
               numv++;
               if (((gdb) < 0
                    ? -(gdb)
 ...
 */
               ...
               if (((gdc) < 0
                    ? -(gdc)
                    : (gdc)) < 0.0000001) vx[numv] = xx;
               else vx[numv] = xx + xd * (gg - gc) / gdc;
               vy[numv] = yy + yd;
               numv++;
               if (fill) {
                 o_flags[ir][ic][n_lines[ir][ic]] = (byte)ii;
 ...
 */
     ...
     //    System.err.println ("ii:" + ii1 + " " +ii2 + " " +ii3 + " " +ii4 + " " +ii5 + " " +ii6);
 ...
 /**-------------------  Color Fill -------------------------*/
     if (fill) {
       fillGridBox(
         g, n_lines, vx, vy, xd, xdd, yd, ydd, nr, nrm, nc, ncm, ctrLow, tri,
         tri_color, o_flags, myvals, color_bin, grd_normals, tri_normals);
 ...
 //---TDR, build Contour Strips
     ...
     if (contourDifficulty == Contour2D.EASY) {
       ...
       for (int kk = 0; kk < ctrSet.qSet.length; kk++) {
         ctrSet.qSet[kk].getArrays(
           vx, vy, auxLevels, vx_tmp, vy_tmp, vz_tmp, aux_tmp, spatial_set);
   ...
   private static void fillGridBox(float[] g, short[][] n_lines, float[] vx,
                                   float[] vy, float xd, float xdd, float yd,
                                   float ydd, int nr, int nrm, int nc,
                                   int ncm, short[][] ctrLow, float[][] tri,
                                   byte[][] tri_color, byte[][][] o_flags,
     ...
     for (int ir = 0; ir < nrm; ir++) {
       ...
       for (int ic = 0; ic < ncm; ic++) {
         ...
         fillGridBox(
           new float[] {ga, gb, gc, gd}, n_lines[ir][ic], vx, vy, xx, yy, xd,
           yd, ic, ir, ctrLow[ir][ic], tri, t_idx, tri_color, numv[0],
           o_flags[ir][ic], values, color_bin, grd_normals, n_idx,
           tri_normals);
       }
     }
   }
   ...
   private static void fillGridBox(float[] corners, int numc, float[] vx,
                                   float[] vy, float xx, float yy, float xd,
                                   float yd, int nc, int nr, short ctrLow,
                                   float[][] tri, int[] t_idx,
                                   byte[][] tri_color, int numv,
     ...
     for (int iii = 0; iii < o_flags.length; iii++) {
       if (o_flags[iii] > 32) {
         fillCaseSix(
           xx, yy, xd, yd, v_idx, dir, o_flags, ctrLow, vx, vy, nc, nr,
           crnr_color, crnr_out, tri, t_idx, color_bin, tri_color,
     ...
     if (o_flag == 1 || o_flag == 4 || o_flag == 2 || o_flag == 7) {
       ...
       if (opp) {
         fillToOppCorner(
           xx, yy, xd, yd, v_idx, o_flag, cnt_tri, dir, vx, vy, nc, nr,
           crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals, n_idx,
           tri_normals, closed);
       }
       else {
         fillToNearCorner(
       ...
       if (right) flag = -1;
       fillToSide(
         xx, yy, xd, yd, v_idx, o_flag, flag, cnt_tri, dir, vx, vy, nc, nr,
         crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals, n_idx,
         tri_normals, closed);
     }
     ...
     for (il = 1; il < numc; il++) {
       ...
       if (o_flags[o_idx] != last_o) //- contour line case change
       {
         ...
         fillToLast(
           xx, yy, xd, yd, nc, nr, vv1, vv2, vv1_last, vv2_last, tri, cnt_tri,
           t_idx, tri_color, color_bin, cc, color_length, grd_normals, n_idx,
           tri_normals);
 
         byte[] b_arg2 = new byte[2];
         getBoxSide(
           vv[0], vv[1], xx, xd, yy, yd, 0, dir, o_flags[o_idx], b_arg);
         getBoxSide(
           vv_last[0], vv_last[1], xx, xd, yy, yd, 0, dir, last_o, b_arg2);
         ...
         for (int kk = 0; kk < 2; kk++) { //- close off corners
           ...
           if (side != last_s) {
             if ((side == 0 && last_s == 3) || (side == 3 && last_s == 0)) { //- case 1
               fillToNearCorner(
                 xx, yy, xd, yd, 0, (byte)1, cnt_tri, 1,
                 new float[] {vv[0][kk],
                              vv_last[0][kk]}, new float[] {vv[1][kk],
                       vv_last[1][kk]}, nc, nr, crnr_color, crnr_out, tri,
                                        t_idx, tri_color, grd_normals, n_idx,
                                        tri_normals, closed);
             }
             if ((side == 0 && last_s == 1) || (side == 1 && last_s == 0)) { //- case 2
               fillToNearCorner(
                 xx, yy, xd, yd, 0, (byte)2, cnt_tri, 1,
                 new float[] {vv[0][kk],
                              vv_last[0][kk]}, new float[] {vv[1][kk],
                       vv_last[1][kk]}, nc, nr, crnr_color, crnr_out, tri,
                                        t_idx, tri_color, grd_normals, n_idx,
             ...
             if ((side == 2 && last_s == 3) || (side == 3 && last_s == 2)) { //- case 4
               fillToNearCorner(
                 xx, yy, xd, yd, 0, (byte)4, cnt_tri, 1,
                 new float[] {vv[0][kk],
                              vv_last[0][kk]}, new float[] {vv[1][kk],
                       vv_last[1][kk]}, nc, nr, crnr_color, crnr_out, tri,
                                        t_idx, tri_color, grd_normals, n_idx,
             ...
             if ((side == 2 && last_s == 1) || (side == 1 && last_s == 2)) { //- case 7
               fillToNearCorner(
                 xx, yy, xd, yd, 0, (byte)7, cnt_tri, 1,
                 new float[] {vv[0][kk],
                              vv_last[0][kk]}, new float[] {vv[1][kk],
                       vv_last[1][kk]}, nc, nr, crnr_color, crnr_out, tri,
                                        t_idx, tri_color, grd_normals, n_idx,
             ...
             if ((side == 2 && last_s == 0) || (side == 0 && last_s == 2)) { //- case 5
               if (side == 0) {
                 vvx[0] = vv[0][kk];
                 vvy[0] = vv[1][kk];
                 vvx[1] = vv_last[0][kk];
               ...
               fillToSide(
                 xx, yy, xd, yd, 0, (byte)5, flag, cnt_tri, 1, vvx, vvy, nc,
                 nr, crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals,
                 n_idx, tri_normals, closed);
             }
             if ((side == 1 && last_s == 3) || (side == 3 && last_s == 1)) { //- case 3
               if (side == 3) {
                 vvx[0] = vv[0][kk];
                 vvy[0] = vv[1][kk];
                 vvx[1] = vv_last[0][kk];
               ...
               fillToSide(
                 xx, yy, xd, yd, 0, (byte)3, flag, cnt_tri, 1, vvx, vvy, nc,
                 nr, crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals,
                 n_idx, tri_normals, closed);
             }
           }
     ...
     if ((last_o == 1) || (last_o == 2) || (last_o == 4) || (last_o == 7)) {
       ...
       if (flag_set > 0) {
         fillToOppCorner(
           xx, yy, xd, yd, v_idx, last_o, cnt_tri, dir, vx, vy, nc, nr,
           crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals, n_idx,
           tri_normals, closed);
       }
       else {
       ...
       fillToSide(
         xx, yy, xd, yd, v_idx, last_o, flag, cnt_tri, dir, vx, vy, nc, nr,
         crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals, n_idx,
         tri_normals, closed);
     }
     else if (last_o == 5) {
   ...
   private static void getBoxSide(float[] vx, float[] vy, float xx, float xd,
                                  float yy, float yd, int v_idx, int dir,
                                  byte o_flag, byte[] side) {
     /*
   ...
   private static void interpNormals(float vx, float vy, float xx, float yy,
     ...
     switch (side) {
       case 0:
         nn[0] = ((grd_normals[nc][nr + 1][0] - grd_normals[nc][nr][0]) /
                  xd) * dx + grd_normals[nc][nr][0];
         nn[1] = ((grd_normals[nc][nr + 1][1] - grd_normals[nc][nr][1]) /
                  xd) * dx + grd_normals[nc][nr][1];
         nn[2] = ((grd_normals[nc][nr + 1][2] - grd_normals[nc][nr][2]) /
                  xd) * dx + grd_normals[nc][nr][2];
         break;
       case 3:
         nn[0] = ((grd_normals[nc + 1][nr][0] - grd_normals[nc][nr][0]) /
                  yd) * dy + grd_normals[nc][nr][0];
         nn[1] = ((grd_normals[nc + 1][nr][1] - grd_normals[nc][nr][1]) /
                  yd) * dy + grd_normals[nc][nr][1];
         nn[2] = ((grd_normals[nc + 1][nr][2] - grd_normals[nc][nr][2]) /
                  yd) * dy + grd_normals[nc][nr][2];
         break;
       case 1:
         nn[0] =
           ((grd_normals[nc + 1][nr + 1][0] - grd_normals[nc][nr + 1][0]) /
            yd) * dy + grd_normals[nc][nr + 1][0];
         nn[1] =
           ((grd_normals[nc + 1][nr + 1][1] - grd_normals[nc][nr + 1][1]) /
            yd) * dy + grd_normals[nc][nr + 1][1];
         nn[2] =
           ((grd_normals[nc + 1][nr + 1][2] - grd_normals[nc][nr + 1][2]) /
            yd) * dy + grd_normals[nc][nr + 1][2];
         break;
       case 2:
         nn[0] =
           ((grd_normals[nc + 1][nr + 1][0] - grd_normals[nc + 1][nr][0]) /
            xd) * dx + grd_normals[nc + 1][nr][0];
         nn[1] =
           ((grd_normals[nc + 1][nr + 1][1] - grd_normals[nc + 1][nr][1]) /
            xd) * dx + grd_normals[nc + 1][nr][1];
         nn[2] =
           ((grd_normals[nc + 1][nr + 1][2] - grd_normals[nc + 1][nr][2]) /
            xd) * dx + grd_normals[nc + 1][nr][2];
         break;
       default:
         System.out.println("interpNormals, bad side: " + side);
     }
     //- re-normalize
     float mag = (float)Math.sqrt(nn[0] * nn[0] + nn[1] * nn[1] +
                                  nn[2] * nn[2]);
     nn[0] /= mag;
     nn[1] /= mag;
     nn[2] /= mag;
   ...
   private static void fillToLast(float xx, float yy, float xd, float yd,
                                  int nc, int nr, float[] vv1, float[] vv2,
                                  float[] vv1_last, float[] vv2_last,
                                  float[][] tri, int[] cnt_tri, int[] t_idx,
                                  byte[][] tri_color, byte[][] color_bin,
                                  int cc, int color_length,
                                  float[][][] grd_normals, int[] n_idx,
                                  float[] tri_normals) {
     int tt = t_idx[0];
     ...
     tri[1][tt] = vv1[1];
     interpNormals(
       tri[0][tt], tri[1][tt], xx, yy, nc, nr, xd, yd, grd_normals, n_idx,
       tri_normals);
     tt++;
   ...
   private static void fillCaseSix(float xx, float yy, float xd, float yd,
                                   int v_idx, int dir, byte[] o_flags,
                                   short ctrLow, float[] vx, float[] vy,
                                   int nc, int nr, byte[][] crnr_color,
                                   boolean[] crnr_out, float[][] tri,
                                   int[] t_idx, byte[][] color_bin,
                                   byte[][] tri_color, int color_length,
                                   float[][][] grd_normals, int[] n_idx,
     ...
     for (int kk = 0; kk < cases.length; kk++) {
       ...
       if (nn == 0) {
         ...
         fillToNearCorner(
           xx, yy, xd, yd, 0, (byte)cases[kk], cnt_tri, dir, vvx, vvy, nc, nr,
           crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals, n_idx,
           tri_normals, closed);
         ...
         if (cornerFirst) {
           fillToNearCorner(
             xx, yy, xd, yd, 0, (byte)cases[kk], cnt_tri, dir, vvx, vvy, nc,
             nr, crnr_color, crnr_out, tri, t_idx, tri_color, grd_normals,
             n_idx, tri_normals, closed);
         }
         else {
         ...
         for (il = 1; il < nn; il++) {
           ...
           fillToLast(
             xx, yy, xd, yd, nc, nr, vvv1, vvv2, vvv1_last, vvv2_last, tri,
             cnt_tri, t_idx, tri_color, color_bin, clr_idx[il], color_length,
             grd_normals, n_idx, tri_normals);
 
           if (!cornerFirst && il == (nn - 1)) {
             fillToNearCorner(
               xx, yy, xd, yd, idx, (byte)cases[kk], cnt_tri, dir, vvx, vvy,
               nc, nr, crnr_color, crnr_out, tri, t_idx, tri_color,
     ...
     for (int kk = 0; kk < edge_points[0].length; kk++) {
       ...
       //- renormalize normal
       float mag = (float)Math.sqrt(nrm[0] * nrm[0] + nrm[1] * nrm[1] +
                                    nrm[2] * nrm[2]);
       nrm[0] /= mag;
       nrm[1] /= mag;
       nrm[2] /= mag;
   ...
   private static void fillToNearCorner(float xx, float yy, float xd,
                                        float yd, int v_idx, byte o_flag,
                                        int[] cnt, int dir, float[] vx,
                                        float[] vy, int nc, int nr,
                                        byte[][] crnr_color,
                                        boolean[] crnr_out, float[][] tri,
                                        int[] t_idx, byte[][] tri_color,
                                        float[][][] grd_normals, int[] n_idx,
                                        float[] tri_normals, int[] closed) {
     float cx = 0;
     float cy = 0;
   ...
   private static void fillToOppCorner(float xx, float yy, float xd, float yd,
                                       int v_idx, byte o_flag, int[] cnt,
                                       int dir, float[] vx, float[] vy,
                                       int nc, int nr, byte[][] crnr_color,
                                       boolean[] crnr_out, float[][] tri,
                                       int[] t_idx, byte[][] tri_color,
                                       float[][][] grd_normals, int[] n_idx,
   ...
   private static void fillToSide(float xx, float yy, float xd, float yd,
                                  int v_idx, byte o_flag, int flag, int[] cnt,
                                  int dir, float[] vx, float[] vy, int nc,
                                  int nr, byte[][] crnr_color,
                                  boolean[] crnr_out, float[][] tri,
                                  int[] t_idx, byte[][] tri_color,
                                  float[][][] grd_normals, int[] n_idx,
                                  float[] tri_normals, int[] closed) {
 ...
 class ContourQuadSet {
   ...
   ContourQuadSet(int nr, int nc, int lev_idx, ContourStripSet css) {
     ...
     for (int j=0; j<ny; j++) {
       for (int i=0; i<nx; i++) {
       qarray[j][i] = makeContourQuad(j,i);
       }
       }*/
   }
   ...
   public void getArrays(float[] vx, float[] vy, byte[][] auxLevels,
                         float[][] vx1, float[][] vy1, float[][] vz1,
                         byte[][] colors, Gridded3DSet spatial_set)
           throws VisADException {
 ...
 class ContourQuad {
   ...
   public int[][][] getWorkArrays(int leny, int lenx) {
     ...
     return new int[][][] {
       subgrid, subgrid2, markgrid, markgrid2
     };
   }
   ...
   public void get() {
     ...
     if (vert_indices_save == null) {
       ...
       System.arraycopy(
         vert_indices, 0, vert_indices_save, 0, vert_indices.length);
     }
     else {
       System.arraycopy(
         vert_indices_save, 0, vert_indices, 0, vert_indices.length);
     }
 ...
 */
 ...
 class ContourStripSet {
   ...
   ContourStripSet(int size, float[] levels, boolean[] swap,
     ...
     for (int kk = 0; kk < n_levs; kk++) {
       ...
       plot.plotdigits(
         levels[kk], 0f, 0f, fac * 1, fac * 1, 400, new boolean[] {false,
               false, false});
   ...
   void getLineColorArrays(float[] vx, float[] vy, byte[][] colors,
                           ...
                           byte[][][] out_bb, float[][][][] out_vvL,
                           byte[][][][] out_bbL, float[][][] out_loc,
                           boolean[] dashed) {
 ...
 //          locL[kk]);
     ...
     for (int tt = 0; tt < 2; tt++) {
       ...
       for (int mm = 0; mm < n_strips; mm++) {
         if (la[mm][tt] != null) {
           System.arraycopy(
             la[mm][tt][0], 0, out_vv[tt][0], cnt, la[mm][tt][0].length);
           System.arraycopy(
             la[mm][tt][1], 0, out_vv[tt][1], cnt, la[mm][tt][1].length);
           System.arraycopy(
             la[mm][tt][2], 0, out_vv[tt][2], cnt, la[mm][tt][1].length);
         ...
         for (int mm = 0; mm < n_strips; mm++) {
           if (ca[mm][tt] != null) {
             for (int cc = 0; cc < clr_dim; cc++) {
               System.arraycopy(
                 ca[mm][tt][cc], 0, out_bb[tt][cc], cnt,
                 ca[mm][tt][cc].length);
             }
   ...
   void makeContourStrips(float[] vx, float[] vy) {
     for (int kk = 0; kk < n_levs; kk++) {
       ...
       for (int j = 0; j < ny; j++) {
         for (int i = 0; i < nx; i++) {
           if (qSet[kk].qarray[j][i] == null) continue;
 ...
 class ContourStrip {
   ...
   void getInterpolatedLabeledColorArray(float[] vx, float[] vy,
     ...
     processLineArrays(
       interp_vv, interp_bb, labelColor, out_vv, out_colors, out_vvL,
       out_colorsL, lbl_loc);
   }
   ...
   private void processLineArrays(float[][] vv_grid, byte[][] bb,
     ...
     //if ((lev_idx & 1) == 1 || css.n_levs == 1) {
       ...
       if (rotate) {
         ...
         try {
           norm = cg3d.getNormals(new float[][] {
             {vv_grid[0][loc]}, {vv_grid[1][loc]}
           });
         }
         catch (VisADException e) {
           System.out.println(e.getMessage());
         ...
         for (int kk = 0; kk < plot.NumVerts; kk++) {
           lbl_dcoords[0][kk] = vx_tmp[kk] * ctr_u[0] +
                                vyB_tmp[kk] * norm_x_ctr[0];
           lbl_dcoords[1][kk] = vx_tmp[kk] * ctr_u[1] +
                                vyB_tmp[kk] * norm_x_ctr[1];
