 ...
 public class Matrix {
 	...
 	public float getDeterminant() {
 		...
 		int j = 0;
 		for (int i = 0; i < n; i++) {
 			value += parity(i, j) * getValue(i, j)
 					* this.removeRow(j).removeColumn(i).getDeterminant();
 		}
 	...
 	public Matrix getInverse() {
		...
		for (int i = 0; i < n; i++) {
			if (abs(echelonForm.getValue(i, i) - 1f) > 0.1) {
				System.err.println("Matrix not invertible. Returning null.");
				return null;
			}
		}
