 ...
 public class DFSClient extends DFSComponent {
   ...
   public class DFSFile {
     ...
     Map<Flags,ErrorCode> completedSyncRequests;
     ...
     public DFSFile(DFSFilename name) {
       ...
       this.completedSyncRequests = new HashMap<Flags,ErrorCode>(3);
   ...
   protected ErrorCode perform(DFSCommands.Command cmd) {
     ...
     if (e.isTransient()) {
       ...
       if (f.completedSyncRequests.containsKey(cmd.getCacheSyncFlags())) {
         ...
         e = f.completedSyncRequests.get(cmd.getCacheSyncFlags());
         cmd.complete(e);
       }
     } else {
       cmd.complete(e);
     ...
     if (cache.get(cmd.getLocalFile()).isDirty()) {
       Callback dirtyCb = getMaybeFlushDirtyEntryCb();
       dirtyCb.setParams(new Object[]{cmd.getLocalFile(),
                                      cache.get(cmd.getLocalFile()).getVersion()});
   ...
   protected int performTransactionCommit(boolean transferOwnership) {
     ...
     while (it.hasNext()) {
       ...
       if (cachedFile.isDirty() &&
         ...
         transaction.addFile(
           new DFSFilename(cachedFile.getKey().getOwningServer(),
                           cachedFile.getKey().getPath().substring(
                             cachedFile.getKey().getPath().indexOf("-") + 1)),
           cachedFile);
       }
     }
 
     ProposalValue p = new TransactionProposal(transaction);
     // TODO: What to do here is propose returns null?
     parent.paxos.propose(p);
   ...
   protected void serviceBlockedRequests() {
     if (serverAddr < 0) {
       System.err.println("Not servicing any blocked requests because no server " +
                          "is available");
       return;
     }
 
     while (blockedRequests.size() > 0) {
       Object rq = blockedRequests.peek();
   ...
   protected void sendTransactionRequest() {
     ...
     if (serverAddr < 0) {
       System.err.println("No server to service transaction request; deferring!");
       return;
     }
    ...
    * requests are present, sends the transaction request.
    */
   ...
   /**
    * Set the designated proposer to a new node id.
    */
   public void setNewLeader(int newDesignatedProposer) {
     System.err.println("setting new leader: " + serverAddr + " -> " + newDesignatedProposer);
     // Cancel any open transactions.
     Object r = blockedRequests.peek();
     while ((r instanceof TransactionRequest) &&
            ((TransactionRequest) r).type.equals(TransactionFlags.Commit)) {
       completeOutstandingTransactionRequest(ErrorCode.NoTransaction);
     }
     invalidateAllFiles();
     currentTxId = null;
 
     serverAddr = newDesignatedProposer;
     serviceBlockedRequests();
   }
   ...
   public void invalidateAllFiles() {
     ...
     while (it.hasNext()) {
      ...
      if (!file.getPath().matches("[0-9]+:[0-9]+-"))
         continue;
       ...
       DFSFile f = files.get(file);
       if (f != null)
         f.completedSyncRequests.clear();
   ...
   private void handleSyncDataMessage(SyncDataMessage msg) {
     ...
     DFSFilename localFile = new DFSFilename(fname.getOwningServer(),
                                             msg.getTxId() + "-" + fname.getPath());
     DFSFile file = getFile(fname);
 
     // Set the cache to the freshly-received copy of the file.
     System.err.println("Set " + localFile + " ver " + msg.getVersion());
     cache.set(localFile, msg.getVersion(), msg.getData(),
               msg.getOwner(), msg.exists());
 
     if (msg.getFlags().isSet(SyncFlags.TransferOwnership))
       cache.takeOwnership(localFile, msg.getVersion());
 
     PersistentStorageCache.CacheEntry cachedItem = cache.get(localFile);
     ...
     if (msg.getData() == null && msg.exists()) {
       ...
       if (msg.getFlags().equals(file.pendingSyncRequests.peek())) {
         ...
         file.completedSyncRequests.put(file.pendingSyncRequests.poll(),
                                        ErrorCode.Success);
   ...
   private void handleTransactionMessage(TransactionMessage msg) {
     ...
     if (txFlags.isSet(TransactionFlags.Confirm)) {
       ...
       if (txReq.type.equals(TransactionFlags.Commit)) {
         // Shouldn't we not get here? Returning just in case.
         return;
       }
   ...
   protected void failPacket(int seqno, ErrorCode err) {
     ...
     file.completedSyncRequests.put(id.flags, ErrorCode.NetworkTimeout);
   ...
   /**
    * Boilerplate for transaction callback from server.
    */
   public Callback getServerTransactionCb() {
     Method method;
     Callback cb;
     try {
       String[] paramTypes = { "ErrorCode", "Proposal" };
       method = Callback.getMethod("serverTransactionCb", this, paramTypes);
       return new Callback(method, this, null);
     } catch (Exception e) {
       System.err.println("Client: Error when instantiating server transaction cb: ");
       e.printStackTrace();
       return null;
     }
   }
   ...
   /**
    * Transaction callback from server.
    */
   public void serverTransactionCb(ErrorCode e, Proposal p) {
     ProposalValue v = p.getValue();
     TransactionProposal tp = (TransactionProposal) v;
     ...
     case Success:
       ...
       if (blockedRequests.size() == 0 ||
           !(blockedRequests.peek() instanceof TransactionRequest)) {
         System.err.println("Node " + this.addr + " received a transaction " +
                            "confirmation message, but no transaction is " +
                            "active atm; dropping message...");
         return;
       }
 
 
       if (v == null || !(v instanceof TransactionProposal)) {
         System.err.println("Node " + this.addr + " Received an invalid " +
     ...
     case VersionNotFound:
       System.err.println("Node " + this.addr + " received a status code of " +
                          e.getId() + " in response to transaction id: " + t.id);
       break;
     default:
       System.err.println("Node " + this.addr + " received an invalid status " +
                          "code in response to transaction id: " + t.id);
     }
   }
    ...
    *
    * @return a Callback with no params set to the network delivery cb.
    */
