 ...
 public abstract class Component implements ConstrainedVisual {
 	...
 	public final class StyleDictionary implements Dictionary<String, Object>,
 		...
 		/**
 		 * Stores the supplied value for the specified style.<br/>
 		 * <br/>
 		 * 
 		 ...
 		 * new value differs from the old value or not.<br/>
 		 * <br/>
 		 * 
 		 * This behaviour may change in the future so should not be relied upon.
 		 * 
 		 * @param key
 		 *            Style whose value will be overwritten
 		 * @param value
 		 *            Value to be stored
 	...
 	private static class ComponentListenerList extends
 			WTKListenerList<ComponentListener> implements ComponentListener {
 		@Override
 	...
 	private static class ComponentStateListenerList extends
 			WTKListenerList<ComponentStateListener> implements
 	...
 	private static class ComponentStyleListenerList extends
 			WTKListenerList<ComponentStyleListener> implements
 	...
 	private static class ComponentMouseListenerList extends
 			WTKListenerList<ComponentMouseListener> implements
 	...
 	private static class ComponentKeyListenerList extends
 			WTKListenerList<ComponentKeyListener> implements
 	...
 	private static class ComponentDataListenerList extends
 			WTKListenerList<ComponentDataListener> implements
 	...
 	private static class ComponentClassListenerList extends
 			WTKListenerList<ComponentClassListener> implements
 	...
 	protected void setSkin(Skin skin) {
 		...
 		for (Class<?> styleType : styleTypes) {
 			Map<String, ?> stylesLocal = typedStyles
 	...
 	protected void installSkin(Class<? extends Component> componentClass) {
 		...
 		Class<? extends org.apache.pivot.wtk.Skin> skinClass = theme
 				.getSkinClass((Class<? extends Component>) type);
 
 		while (skinClass == null && type != componentClass
 		...
 		if (this.preferredWidth == -1) {
 			if (height == -1) {
 				...
 				if (preferredSize != null && preferredSize.height == height) {
 					...
 					preferredWidthLocal = widthLimits.constrain(skin
 		...
 		if (this.preferredHeight == -1) {
 			if (width == -1) {
 				...
 				if (preferredSize != null && preferredSize.width == width) {
 					...
 					preferredHeightLocal = heightLimits.constrain(skin
 	...
 	public Dimensions getPreferredSize() {
 		if (preferredSize == null) {
 			...
 			if (preferredWidth == -1 && preferredHeight == -1) {
 				...
 				preferredSizeLocal = new Dimensions(
 			...
 			int preferredWidthLocal = widthLimits
 					.constrain(preferredSizeLocal.width);
 			int preferredHeightLocal = heightLimits
 					.constrain(preferredSizeLocal.height);
 
 			if (preferredSizeLocal.width > preferredWidthLocal) {
 				preferredHeightLocal = heightLimits.constrain(skin
 						.getPreferredHeight(preferredWidthLocal));
 			}
 
 			if (preferredSizeLocal.height > preferredHeightLocal) {
 				preferredWidthLocal = widthLimits.constrain(skin
 						.getPreferredWidth(preferredHeightLocal));
 			}
 
 			this.preferredSize = new Dimensions(preferredWidthLocal,
 					preferredHeightLocal);
 		}
 	 ...
 	 */
 	public Point mapPointToAncestor(final Container ancestor, int xArgument,
 			int yArgument) {
 		if (ancestor == null) {
 	 ...
 	 *         if the component is not a descendant of the specified ancestor.
 	 */
 	public Point mapPointFromAncestor(final Container ancestor, int xArgument,
 			int yArgument) {
 		if (ancestor == null) {
 			throw new IllegalArgumentException("ancestor is null");
 		}
 	 ...
 	 *         not part of the component hierarchy.
 	 */
 	public Bounds getVisibleArea(int xArgument, int yArgument, int width,
 			int height) {
 		...
 		while (component != null && component.isVisible()) {
 			...
 			if (component instanceof Display) {
 				visibleArea = new Bounds(left - xOffset, top - yOffset, right
 						- left + 1, bottom - top + 1);
 			}
 	 ...
 	 * @param height
 	 */
 	public void scrollAreaToVisible(int xArgument, int yArgument, int width,
 			int height) {
 		int xArgumentMutable = xArgument;
 		int yArgumentMutable = yArgument;
 		...
 		while (component != null) {
 			if (component instanceof Viewport) {
 				...
 				try {
 					...
 					xArgumentMutable = Math.max(xArgumentMutable,
 							viewportBounds.x);
 					widthMutable = Math
 							.min(widthMutable, Math.max(viewportBounds.width
 									- (xArgumentMutable - viewportBounds.x), 0));
 					...
 					if (deltaY != 0) {
 						...
 						scrollTop = Math
 								.min(Math.max(scrollTop + deltaY, 0),
 										Math.max(viewHeight
 					...
 					yArgumentMutable = Math.max(yArgumentMutable,
 							viewportBounds.y);
 					heightMutable = Math
 							.min(heightMutable, Math.max(viewportBounds.height
 									- (yArgumentMutable - viewportBounds.y), 0));
 	 ...
 	 * @param height
 	 */
 	public final void repaint(int xArgument, int yArgument, int width,
 			int height) {
 	 ...
 	 * @param immediate
 	 */
 	public void repaint(int xArgument, int yArgument, int width, int height,
 			boolean immediate) {
 		Container.assertEventDispatchThread(this);
 		if (parent != null) {
 			int xArgumentMutable = xArgument;
 			...
 			widthMutable = Math.min(right, getWidth() - 1) - xArgumentMutable
 					+ 1;
 			heightMutable = Math.min(bottom, getHeight() - 1)
 					- yArgumentMutable + 1;
 
 			if (widthMutable > 0 && heightMutable > 0) {
 				// Notify the parent that the region needs updating
 				parent.repaint(xArgumentMutable + this.x, yArgumentMutable
 						+ this.y, widthMutable, heightMutable, immediate);
 				...
 				for (Decorator decorator : decorators) {
 					...
 					if (!transform.isIdentity()) {
 						...
 						Rectangle area = new Rectangle(xArgumentMutable,
 								yArgumentMutable, widthMutable, heightMutable);
 						Shape transformedShape = transform
 								.createTransformedShape(area);
 						Bounds tranformedBounds = new Bounds(
 								transformedShape.getBounds());
 
 						// Limit the transformed area to the decorator's bounds
 						tranformedBounds = tranformedBounds.intersect(decorator
 								.getBounds(this));
 
 						// Add the bounded area to the repaint region
 						parent.repaint(tranformedBounds.x + this.x,
 								tranformedBounds.y + this.y,
 								tranformedBounds.width,
 								tranformedBounds.height, immediate);
 					}
 				}
 			}
 		}
 	...
 	public boolean requestFocus() {
 		if (isFocusable() && getDisplay() != null) {
 			setFocusedComponent(this);
 
 			ApplicationContext.DisplayHost displayHost = getDisplay()
 					.getDisplayHost();
 	...
 	public void reenterMouseFull() {
		...
		if (location == null) {
			System.err
					.println("reenterMouseFull, display.getMouseLocation() is null ");
			return;
		}
 	...
 	protected boolean mouseMove(int xArgument, int yArgument) {
 		...
 		if (isEnabled()) {
 			...
 			triggerTooltipCallback = ApplicationContext.scheduleCallback(
 					new Runnable() {
 						@Override
 						public void run() {
 							Point mouseLocationLocal = getMouseLocation();
 							componentTooltipListeners.tooltipTriggered(
 									Component.this, mouseLocationLocal.x,
 									mouseLocationLocal.y);
 						}
 					}, tooltipDelay);
 
 			consumed = componentMouseListeners.mouseMove(this, xArgument,
 					yArgument);
 		}
 	...
 	protected boolean mouseDown(Mouse.Button button, int xArgument,
 			int yArgument) {
 		boolean consumed = false;
 		...
 		if (isEnabled()) {
 			...
 			consumed = componentMouseButtonListeners.mouseDown(this, button,
 					xArgument, yArgument);
 		}
 	...
 	protected boolean mouseUp(Mouse.Button button, int xArgument, int yArgument) {
 		...
 		if (isEnabled()) {
 			consumed = componentMouseButtonListeners.mouseUp(this, button,
 					xArgument, yArgument);
 		}
 	...
 	protected boolean mouseClick(Mouse.Button button, int xArgument,
 			int yArgument, int count) {
 		boolean consumed = false;
 
 		if (isEnabled()) {
 			consumed = componentMouseButtonListeners.mouseClick(this, button,
 					xArgument, yArgument, count);
 		}
 	...
 	protected boolean mouseWheel(Mouse.ScrollType scrollType, int scrollAmount,
 		...
 		if (isEnabled()) {
 			consumed = componentMouseWheelListeners.mouseWheel(this,
 					scrollType, scrollAmount, wheelRotation, xArgument,
 					yArgument);
 		}
 	...
 	protected static final void indexBoundsCheck(String indexName, int index,
 			int min, int max) throws IndexOutOfBoundsException {
 		if (max < min) {
 			throw new IllegalArgumentException("max (" + max + ") < " + "min ("
 					+ min + ")");
 		}
 		if (index < min) {
 			throw new IndexOutOfBoundsException(indexName + ": index (" + index
 					+ ") < min (" + min + ")");
 		}
 		if (index > max) {
 			throw new IndexOutOfBoundsException(indexName + ": index (" + index
