 ...
 public abstract class AbstractBaseUeberweisungControl extends AbstractTransferControl
 {
   ...
   public Input getComment() throws RemoteException
     ...
     Terminable t = (Terminable) getTransfer();
 		if (t.ausgefuehrt())
 		{
 			comment.setValue(i18n.tr("Der Auftrag wurde ausgefhrt"));
 		}
 		else if (t.ueberfaellig())
 		{
 			comment.setValue(i18n.tr("Der Auftrag ist berfllig"));
 		}
 		return comment;
 	}
 			...
 			public void handleEvent(Event event) {
 				...
 				try {
 					...
 					if (choosen.before(new Date()))
 						getComment().setValue(i18n.tr("Der Auftrag ist berfllig."));
 				}
   ...
   public synchronized boolean handleStore()
   {
 		try
 		{
 			...
 			if (bu.ausgefuehrt())
 			{
 				GUI.getStatusBar().setErrorText(i18n.tr("Der Auftrag wurde bereits ausgefhrt und kann daher nicht gendert werden"));
 				return false;
 			}
