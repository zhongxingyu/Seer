...
import java.beans.*;
 ...
 public class FileImport extends JPanel implements ActionListener {
     ...
     tableNameLbl= new Label("Table Name");
     //  tableNameTxt.setVisible(false);
     infoPnl=new JPanel();
     ...
     // Create show button
     showButton = new JButton("Choose File");
     ...
     if (e.getSource() == showButton){ //click "choose file"
         ...
         if(theFile != null) {
             ...
             _session.getMessageHandler().showMessage("File:\n\"" + filename + "\" is opened");
     ...
     else if (e.getSource() == readButton){ //read file
         if ( read_file()){
     ...
     else { //create and insert data
         if (createTable()) {
         if(insertData()){
          _session.getMessageHandler().showMessage("All insertions are done!");
         }
         }
     }
     }
