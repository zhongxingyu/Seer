 ...
 //			}
 	...
 	public boolean visit(MethodInvocation node) {
 		...
 		/**
 		 * convert "Thread.sleep(mills)" to "Concurrent.Thread.sleep(mills);"
 		 */
 		...
 		if(methodName.equals("sleep")||methodName.equals("Thread.sleep")){
 			...
 			if (binding != null && binding instanceof IMethodBinding) {
 				...
 				if(className.equals("java.lang.Thread")){
 					//add "toString()" to avoid conflicting with package prefix
 					isThread = true;
 					buffer.append("Concurrent.Thread.sleep(");
 					visitList(args, ", ", 0, node.resolveMethodBinding().getParameterTypes().length - 1);
 					Expression arg = (Expression) args.get(args.size() - 1);
 
 					visitList(args, ", ", node.resolveMethodBinding().getParameterTypes().length - 1, size);
 					buffer.append(")");
 					return false;
 				}
 			}
