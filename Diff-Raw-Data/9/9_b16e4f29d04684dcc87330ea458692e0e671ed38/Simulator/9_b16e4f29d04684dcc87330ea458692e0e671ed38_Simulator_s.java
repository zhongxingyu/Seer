 ...
 import evaluation.simulator.statistics.EvaluationType;
 import evaluation.simulator.statistics.GeneralStatistics;
 ...
 public class Simulator extends GMixTool {
 	...
 	private static void saveConfigToDisk(ExperimentConfig ep) {
 		...
 		for (String property: settings.getPropertiesObject().stringPropertyNames())
 			setup += property + " = " +settings.getProperty(property) + "\n";
 		
 		Util.writeToFile(	description + setup, 
 							Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart 
 							+ "-configShort.txt");
 		
 		Util.writeToFile(	description + setup, 
 							Paths.SIM_OUTPUT_FOLDER_PATH +"last-configShort.txt");
 	...
 	private static void saveResultsToDisk(BigDecimal[][][][] results, ExperimentConfig ep) {
 		...
 		// write files
 		contentForFileExplainedResultsTxt += "\n";
 		Util.writeToFile(contentForFileExplainedResultsTxt, Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart + "-explained-results.txt");
 		Util.writeToFile(contentForFileExplainedResultsTxt, Paths.SIM_OUTPUT_FOLDER_PATH +"last-explained-results.txt");
 		for (String measurand:measurands) {
 			Util.writeToFile(contentForResultsTxts.get(measurand), Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart +"-results-" +measurand +".txt");
 			Util.writeToFile(contentForResultsTxts.get(measurand), Paths.SIM_OUTPUT_FOLDER_PATH +"last-results-" +measurand +".txt");
 		}
