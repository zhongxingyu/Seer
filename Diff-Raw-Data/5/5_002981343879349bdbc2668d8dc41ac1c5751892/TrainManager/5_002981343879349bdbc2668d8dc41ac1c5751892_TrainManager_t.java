 ...
 public class TrainManager {
 	...
 	public void removeCorsa(int idCorsa) {
 		...
 		TypedQuery<IstanzaTreno> query = em.createQuery(
 				"select i from IstanzaTreno i where corsa=" + idCorsa,
 				IstanzaTreno.class);
 		List<IstanzaTreno> istanze = query.getResultList();
 		for (int i = 0; i < istanze.size(); i++) {
 			removeIstanza(istanze.get(i).getId());
 		}
 		em.remove(c);
 		em.getTransaction().commit();
 		em.close();
 	...
 	public void addStazioneToTratta(int idTratta, String stazione, int distanza)
 		...
 		Tratta t = em.find(Tratta.class, idTratta);
 		StazioneTratta ultimaStazione = t.getStazioniTratta().get(t.getStazioniTratta().size()-1);
 		
 		if(distanza > ultimaStazione.getDistanza()){
 			throw new TrainException("Non puoi inserire una nuova stazione di arrivo!");
 		}
 	...
 	public void removeStazioneFromTratta(int idTratta, String stazione) throws TrainException {
 		EntityManager em = HibernateUtil.getEntityManagerFactory()
 				.createEntityManager();
 		...
 		if(st.getDistanza()==ultimaStazione.getDistanza()){
 			throw new TrainException("Non puoi eliminare l'ultima stazione di una tratta!");
 		}
 		if(st.getDistanza()==0){
 			throw new TrainException("Non puoi eliminare la prima stazione di una tratta!");
 		}
 	...
 	public void removeFermataFromCorsa(int idCorsa, int idStazioneTratta)
 		...
 		try {
			...
			List<IstanzaTreno> i = queryIstanza.getResultList();
 	...
 	public Tratta createTratta(SortedMap<Integer, Stazione> map, String nome)
 		...
 		if (!map.containsKey(0)) {
 			throw new TrainException(
 					"Non hai impostato una stazione di partenza!");
 		}
 	...
 	public Corsa createCorsa(Tratta tratta, Tipologia tipo,
 		...
 		for (int i = 1; i < tratta.getStazioniTratta().size(); i++) {
 			if (ultimaStazione.getDistanza() < tratta.getStazioniTratta()
 					.get(i).getDistanza()) {
 	...
 	public IstanzaTreno createIstanza(Corsa corsa, Treno treno, Date data)
 		...
 		StazioneTratta ultimaStazione = corsa.getTratta().getStazioniTratta().get(0);
 		for(int i=1; i< corsa.getTratta().getStazioniTratta().size();i++){
 			if (ultimaStazione.getDistanza() < corsa.getTratta().getStazioniTratta().get(i).getDistanza()){
 				ultimaStazione=corsa.getTratta().getStazioniTratta().get(i);
 			}
 		}
 		if (corsa.getFermataArrivo().getStazioneTratta().getDistanza() != ultimaStazione.getDistanza()) {
 			throw new TrainException(
 	...
 	public void aggiornaNumPostiDisponibili(Posto posto) {
 		EntityManager em = HibernateUtil.getEntityManagerFactory()
 				.createEntityManager();
 		em.getTransaction().begin();
 		IstanzaTrenoPrenotabile i = em.find(IstanzaTrenoPrenotabile.class,
 				posto.getIstanzaTreno().getId());
 		i.numPostiDisponibiliPerClasse.put(posto.getClasse(),
 				i.numPostiDisponibiliPerClasse.get(posto.getClasse()) + 1);
 		em.merge(i);
 		em.getTransaction().commit();
 		em.close();
 	}
 	...
 	public void removePrenotazione(int idPrenotazione) {
 		...
 		for (int i = 0; i < p.getPostiPrenotati().size(); i++) {
 			Posto posto = em.find(Posto.class, p.getPostiPrenotati().get(i)
 					.getId());
 			posto.cancellaPrenotazione();
 		}
 		em.remove(p);
 		em.getTransaction().commit();
 		em.close();
 	...
 	public void updateIstanza(int idIstanza, Date nuovaData) throws TrainException{
 		EntityManager em = HibernateUtil.getEntityManagerFactory()
 				.createEntityManager();
 		em.getTransaction().begin();
 		IstanzaTreno i = em.find(IstanzaTreno.class, idIstanza);
 		...
 		List<Treno> treniUsati= getTreniUsati(nuovaData);
 	        for(int j=0; j< treniUsati.size(); j++){
         		if(i.getTreno().getId()==treniUsati.get(j).getId()){
         			throw new TrainException("Non puoi impostare questa data in quanto in questa data il treno selezionato e' occupato!");
         		}
        		}	
 		em.getTransaction().commit();
 		em.close();
 	...
 	public List<Prenotazione> getPrenotazioni(Utente u) {
 		EntityManager em = HibernateUtil.getEntityManagerFactory()
 				.createEntityManager();
 
 		TypedQuery<Prenotazione> query = em.createQuery(
 				"select p from Prenotazione p where utente=" + u.getId(), Prenotazione.class);
 	...
 	public List<Treno> getTreniUsati(Date data) {
 		...
 		TypedQuery<IstanzaTreno> query = em.createQuery(
 				"select i from IstanzaTreno i where data='" + data + "'",
 				IstanzaTreno.class);
