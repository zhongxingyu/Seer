 ...
 public class RoundedMoneyTest {
 	...
 	public void testOfCurrencyUnitBigDecimal() {
 		RoundedMoney m = RoundedMoney.of(MonetaryCurrencies.getCurrency("EUR"),
 				TEN);
 	...
 	@Test
 	public void testPow() {
 		RoundedMoney m = RoundedMoney.of("CHF", new BigDecimal("23.23"));
 		for (int p = 0; p < 10; p++) {
 			assertEquals(
 					RoundedMoney.of("CHF", m.getNumber(BigDecimal.class).pow(p)
 		...
 		for (RoundedMoney m : moneys) {
 			for (int p = -10; p < 10; p++) {
 				assertEquals(
 						...
 						m.with(m.getNumber(BigDecimal.class).scaleByPowerOfTen(
 								p)),
 	...
 	public void testWithCurrencyUnitNumber() {
 		...
 		RoundedMoney[] moneys2 = new RoundedMoney[] {
 				s.with(MonetaryCurrencies.getCurrency("CHF"), 100),
 				s.with(MonetaryCurrencies.getCurrency("USD"), 34242344),
 				s.with(MonetaryCurrencies.getCurrency("EUR"), new BigDecimal(
 						"23123213.435")),
 				s.with(MonetaryCurrencies.getCurrency("USS"), new BigDecimal(
 						"-23123213.435")),
 	...
 	/**
 	 * Test method for
 	 * {@link org.javamoney.moneta.RoundedMoney#getNumber(java.lang.Class)}.
 	 */
 	@Test
