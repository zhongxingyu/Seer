 ...
 public class BrowseMethods {
 	...
 	/**
 	 * setImportantHeaders setzt Header fr den HTTP-Request
 	 * 
 	 * @param RequestMethod
 	 *            RequestObject.METHOD_POST oder RequestObject.METHOD_GET
 	 * @param domain
 	 *            wichtig fr CookieHandler
 	 */
 	...
 	/**
 	 * HauptMethode der BrowseMethods: ldt die Seite und gibt sie als
 	 * AnswerObject wieder zurck
 	 * 
 	 * @param requestInfo
 	...
 	public AnswerObject browse(RequestObject requestInfo) {
 		...
 		if (Integer.parseInt(Build.VERSION.SDK) < 9) {
 			// Notwendig, da sonst die Verbindung bei lteren Systemen nicht
 			// klappt
 		...
 		try {
 			...
 			if (realURL != null) {
 				...
 				if (RequestMethod == "POST") {
 					// Output bei POST-bertragung aktivierten
 					HTTPConnection.setDoOutput(true);
 				}
 				// Ursprnglichen Request setzen
