 ...
 * @version $Id: RangeSet.java,v 1.5 2003/08/10 20:27:06 desruisseaux Exp $
 ...
 public class RangeSet extends AbstractSet implements SortedSet, Cloneable, Serializable {
     ...
     /**
      * Le type des donnes de l'intervalle.  Il s'agit du type
      * qui sera spcifi aux objets {@link Range} reprsentant
      * un intervalle.
      */
     ...
     /**
      * Le type des donnes utilis dans le tableau <code>array</code>.
      * Il s'agira souvent du mme type que <code>type</code>, sauf si
      ...
      * tableau de type <code>Comparable[]</code>. Les lments de ce tableau doivent
      ...
      * t plus efficace d'utiliser une variable spare  (pour ne pas tre oblig
      * d'agrandir ce tableau  chaque ajout d'un intervalle), mais malheureusement
      * le J2SE 1.4 ne nous fournit pas de mthode <code>Arrays.binarySearch</code>
      * qui nous permettent de spcifier les limites du tableau  (voir RFE #4306897
      *  http://developer.java.sun.com/developer/bugParade/bugs/4306897.html).
      */
     private Object array;
 
     /**
      * Compte le nombre de modifications apportes au tableau des intervalles.
      * Ce comptage sert  vrifier si une modification survient pendant qu'un
      * itrateur balayait les intervalles.
      */
     ...
     public boolean add(Comparable lower, Comparable upper) throws IllegalArgumentException {
         ...
         if (i0 < 0) {
             /*
              * Si le dbut de la plage ne correspond pas  une des dates en
              * mmoire, il faudra l'insrer  quelque part dans le tableau.
              * Si la date tombe dans une des plages dj existantes (si son
              * index est impair), on tend la date de dbut pour prendre le
              * dbut de la plage. Visuellement, on fait:
              *
              *   0   1     2      3     4   5    6     7
              *   #####     ########     #####    #######
              *             <---^           ^
             ...
             if (((i0=~i0) & 1) != 0) { // Attention: c'est ~ et non -
                 ...
                 /*
                  * Si la date de dbut ne tombe pas dans une plage dj
                  * existante, il faut tendre la valeur de dbut qui se
                 ...
                 if (i0!=Array.getLength(array) && (i1=binarySearch(upper))!= ~i0) {
                      ...
                      * est  insrer  la fin du tableau. Dans ce cas, on
                      * n'a qu' agrandir le tableau et crire les valeurs
                      * directement  la fin. Ce traitement est ncessaire
                      * pour eviter les 'ArrayIndexOutOfBoundsException'.
                      * Un autre cas particulier se produit si la nouvelle
                      * plage est  entirement  comprise entre deux plages
                      * dj existantes.  Le mme code ci-dessous insrera
         ...
         /*
          * A ce stade, on est certain que 'i0' est pair et pointe vers le dbut
          * de la plage dans le tableau. Fait maintenant le traitement pour 'i1'.
          */
         if (i1 < 0) {
             /*
              * Si la date de fin tombe dans une des plages dj existantes
              * (si son index est impair), on l'tend pour pendre la fin de
              * la plage trouve dans le tableau. Visuellement, on fait:
              *
              *   0   1     2      3     4   5    6     7
              *   #####     ########     #####    #######
              *             ^             ^-->
             ...
             if (((i1=~i1) & 1) != 0) { // Attention: c'est ~ et non -
                 ...
                 /*
                  * Si la date de fin ne tombe pas dans une plage dj
     ...
     public boolean remove(Comparable lower, Comparable upper) throws IllegalArgumentException {
         ...
         if (i0 < 0) {
             if (((i0 = ~i0) & 1) != 0) { // Attention: c'est ~ et non -
                 /*
                  * Si le dbut de la plage ne correspond pas  une des dates en mmoire,
                  * il faudra faire un trou  quelque part dans le tableau. Si la date tombe
                  * dans une des plages dj existantes (si son index est impair), on change
         ...
         if (i1 < 0) {
             /*
              * Si la date de fin tombe dans une des plages dj existantes
              * (si son index est impair), on change la date de dbut de la
             ...
             if (((i1 = ~i1) & 1) != 0) { // Attention: c'est ~ et non -
                 ...
                 /*
                  * Si la date de fin ne tombe pas dans une plage dj existante, il
                  * faudra (plus tard) supprimer les ventuelles plages qui le prcde.
                  *
                  *   0   1     2      3        4     5        6         7
                  *   #####     ########        #######        ###########
                  *                    ^                  ^
         ...
         /*
          * A ce stade, on est certain que 'i1' est pair et pointe vers la dbut
          * de la plage dans le tableau. On va maintenant supprimer tout ce qui
          * se trouve entre 'i0' et 'i1',  l'exclusion de 'i0' et 'i1'.
          */
         assert (i0 & 1) != 0 : i0;
     ...
     /**
      * Retourne l'index de l'lment <code>value</code> dans le tableau <code>array</code>.
      * Cette mthode interprte le tableau <code>array</code> comme un tableau d'un des types
      * intrinsques du Java, et appelle la mthode <code>Arrays.binarySearch</code> approprie.
      *
