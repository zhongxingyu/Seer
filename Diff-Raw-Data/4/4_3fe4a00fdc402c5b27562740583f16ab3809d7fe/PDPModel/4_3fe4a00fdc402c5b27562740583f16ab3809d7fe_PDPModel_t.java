 ...
 public class PDPModel implements Model<PDPObject>, TickListener, ModelReceiver {
   ...
   protected void doDeliver(Vehicle vehicle, Parcel parcel, long time) {
     synchronized (this) {
       ...
       eventDispatcher.dispatchEvent(new PDPModelEvent(
           PDPModelEventType.END_DELIVERY, this, time, parcel, vehicle));
     }
   }
   ...
   public boolean register(PDPObject element) {
     synchronized (this) {
       if (element.getType() == PDPType.PARCEL) {
         ...
         eventDispatcher.dispatchEvent(new PDPModelEvent(
             PDPModelEventType.NEW_PARCEL, this, currentTime, p, null));
         ...
         if (element.getType() == PDPType.VEHICLE) {
           ...
           eventDispatcher.dispatchEvent(new PDPModelEvent(
               PDPModelEventType.NEW_VEHICLE, this, currentTime, null, v));
         }
       }
    ...
    * previously started {@link Action}. By calling this method before executing
    * other actions, time consistency is enforced since any pending actions will
    ...
    * no pending {@link Action}s nothing will happen.
   ...
   abstract class VehicleParcelAction implements Action, VehicleParcelActionInfo {
