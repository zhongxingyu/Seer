 ...
 public class AliquotAction extends SecureAction
 {
 			...
 			HttpServletRequest request, HttpServletResponse response) throws Exception
 	{
 		...
 		try
 		{
 			...
 			{
 				aliquotForm.setNoOfAliquots(AppUtility.isValidCount(aliquotForm.getNoOfAliquots(),errors));
 	...
 	{
 		final Map<String, String> aliquotMap = aliquotForm.getAliquotMap();
 ...
 //		final String containerKey = "Specimen:" + counter + "_StorageContainer_id";
 		...
 		if (!containerMap.isEmpty())
 		{
 			...
 			for (int containerCtr = 0; containerCtr < containerId.length; containerCtr++)
 			{
 				...
 				if (flag && containerNvb.getValue().equals(containerName))
 				{
 					flag = false;
 				}
 				if (flag)
 	...
 	private void writeResponse(HttpServletRequest request, HttpServletResponse response,
 			final Map<String, String> aliquotMap)
 	{
 ...
 //			"from edu.wustl.catissuecore.domain.CollectionProtocol as collectionProtocol where collectionProtocol.id ="+ cpId;
 			...
 			Map<String, String> tempAliquotMap = new HashMap<String, String>();
 					...
 					|| aliquotForm.getButtonClicked().equalsIgnoreCase("checkbox"))
 			{
 				...
 				tempAliquotMap = (HashMap<String, String>) request.getSession().getAttribute("tempAliquotMap");
 				final String label = tempAliquotMap.get(Constants.SYSTEM_LABEL);
 				final String barcode = tempAliquotMap.get(Constants.SYSTEM_BARCODE);
 				...
 				final String aliquotcount = tempAliquotMap.get("aliquotcount");
 				if (!aliquotcount.trim().equalsIgnoreCase(aliquotForm.getNoOfAliquots().trim()))
 				{
 					arePropsChanged = true;
 				}
 
 				final String qtyperaliquot = tempAliquotMap.get("quantityperaliquot");
 						...
 						&& aliquotForm.getButtonClicked().equalsIgnoreCase("create"))
 				{
 					...
 					for (int i = 1; i <= noOfAliquots; i++)
 					{
 						...
 						{
 							containerId = aliquotMap.get(containerIdKey);
 						}
 						else if (aliquotMap.get(radioButonKey) != null
 								&& aliquotMap.get(radioButonKey).equals("3"))
 						{
 							final String containerName = aliquotMap.get(containerNameKey
 									+ "_fromMap");
 				...
 				{
 					final Map<String, String> aliquotMap = aliquotForm.getAliquotMap();
 					final Iterator<String> keyIterator = aliquotMap.keySet().iterator();
 					/**
 					...
 					while (keyIterator.hasNext())
 					{
 						...
 						{
 							final String value = aliquotMap.get(key);
 			...
 			{
 				final Map<String, String> map = (Map<String, String>) request.getAttribute("forwardToHashMap");
 ...
 //			aliquotForm.setGenerateLabel(SpecimenUtil.isLblGenOnForCP(parentLabelFormat, deriveLabelFormat, aliqLabelFormat, Constants.ALIQUOT));
 	 ...
 	 */
 	private void populateTempAliqoutMap(final AliquotForm aliquotForm, Map<String, String> tempAliquotMap)
 	{
 			...
 			throws Exception
 	{
 		...
 		else
 		{
 			...
 			if (Constants.PAGE_OF_CREATE_ALIQUOT.equals(pageOf))
 			{
 				...
 				if(form.getNoOfAliquots()!=null)
 				{
 					form.setNoOfAliquots(AppUtility.isValidCount(form.getNoOfAliquots(), err));
 				}
 				if(err.size() < 1)
 				{
 	...
 	{
 		form.setClassName(specimen.getSpecimenClass());
 			...
 			throws Exception
 	{
 		...
 		if(form.getNoOfAliquots()!=null)
 		{
 			form.setNoOfAliquots(AppUtility.isValidCount(form.getNoOfAliquots(), errors));
 		}
 		if(errors.size() < 1)
 		{
 		...
 		final Map<String, String> aliquotMap = form.getAliquotMap();
 	...
 	private void setAliquotMap(AliquotForm form, DAO dao, final int aliquotCount,
 			String distributedQuantity, final Map<String, String> aliquotMap) throws BizLogicException
 	{
 				...
 				if (!xDimensionMap.isEmpty())
 				{
 					...
 					for (int j = 0; j < xDimension.length; j++)
 					{
 						...
 						for (int k = 0; k < yDimensionList.size(); k++)
 						{
 							...
 							form.setNoOfAliquots(AppUtility.isValidCount(form.getNoOfAliquots(), errors1));
 	...
 	private String validate(HttpServletRequest request, AliquotForm form)
 	{
 		...
 		if (!validator.isNumeric(form.getNoOfAliquots(),1))
 		{
 			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("errors.item.format",
 			...
 			String cpQuery)
 	{
 		...
 		Map<String, String> aliquotMap = new HashMap<String, String>();
 		final Map<String, String> dataMap = (Map<String, String>) request.getAttribute("dataMap");
 		...
 		final List<AliquotBean> aliquotBeanList = new ArrayList<AliquotBean>();
