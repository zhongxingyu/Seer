 ...
 public class JspCommands implements CommandMarker {
 	...
 	public void newMvcArtifact(
 		...
 		jspOperations.createManualController(controller, preferredMapping, pathResolver.getFocusedPath(Path.SRC_MAIN_WEBAPP));
 	}
 	...
 	public void view(@CliOption(key = "path", mandatory = true, help = "The path the static view to create in (required, ie '/foo/blah')") final String path,
 		...
 		jspOperations.installView(path, viewName, title, "View", pathResolver.getFocusedPath(Path.SRC_MAIN_WEBAPP));
 	}
 	...
 	public void update(
 		...
 		jspOperations.updateTags(backup, pathResolver.getFocusedPath(Path.SRC_MAIN_WEBAPP));
 	}
 }
