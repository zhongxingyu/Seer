  ...
  * <p>
  * This implementation strives to provide complete compatibility with the
  * standard CVS client, so that operations on locally checked-out files can be
  * carried out by either this library or the standard client without causing the
  * other to fail. Any such failure should be considered a bug in this library.
  * 
  * @author Robert Greig
  */
 public class StandardAdminHandler implements AdminHandler {
      ...
      *            absolute path). Must not end with a slash.
      * @param entry
     ...
     public void updateAdminData(final String localDirectory, String repositoryPath, final Entry entry,
                     final GlobalOptions globalOptions) throws IOException {
         ...
         final File rootFile = new File(CVSdir, "Root"); // NOI18N
         if (!rootFile.exists()) {
             final PrintWriter w = new PrintWriter(new FileWriter(rootFile));
             try {
         ...
         final File repositoryFile = new File(CVSdir, "Repository"); // NOI18N
         if (!repositoryFile.exists()) {
             final PrintWriter w = new PrintWriter(new FileWriter(repositoryFile));
             try {
                 if ((entry != null) && !entry.isDirectory()) {
                     // If there is a file entry, the repository path is for a
                     // file!
                     final int length = entry.getName().length();
      ...
      * exists. Any line containing only a D is deleted from the parent Entries
      * file.
      * 
      * @param CVSdir
      *            the full path to the CVS directory for the directory that has
      *            just been added
      */
         ...
         synchronized (ksEntries) {
             ...
             if (parentCVSEntries != null) {
                 ...
                 try {
                     ...
                     final Entry directoryEntry = new Entry();
     ...
     /**
      * Update the specified Entries file with a given entry This method
      * currently does the following, in order:
      * <OL>
      * <LI>Create a new temporary file, Entries.temp</LI>
      * <LI>Iterate through each line of the original file, checking if it
      * matches the entry of the entry to be updated. If not, simply copy the
      * line otherwise write the new entry and discard the old one</LI>
      ...
      * <LI>Delete Entries.temp2</LI></UL> Note that in the case where the
      * Entries file does not exist, it is simply created and the entry appended
      * immediately. This all ensures that if a failure occurs at any stage, the
      * original Entries file can be retrieved
      * 
      * @param originalFile
      ...
      */
     private void updateEntriesFile(final File originalFile, final Entry entry) throws IOException {
         synchronized (ksEntries) {
             ...
             try {
                 ...
                 writer = new BufferedWriter(new FileWriter(tempFile));
                 String line;
                 // indicates whether we have written the entry that was passed
                 // in
      ...
      */
     public Entry[] getEntriesAsArray(final File directory) throws IOException {
         ...
         synchronized (ksEntries) {
            ...
            final File directory = file.getParentFile();
             ...
             try {
                 ...
                 writer = new BufferedWriter(new FileWriter(tempFile));
                 String line;
                 // allows us to determine whether we need to put in a "D" line.
                 // We do
      ...
      * command-line tool). However, the path stored in that file is relative to
      * the repository path
      * 
      * @param directory
      *            the directory
     ...
     /**
      * Update the Entries file using information in the Entries.Log file (if
      * present). If Entries.Log is not present, this method does nothing.
      * 
      * @param directory
      ...
      */
     private void processEntriesDotLog(final File directory) throws IOException {
         ...
         synchronized (ksEntries) {
             ...
             final List<Entry> additionsList = new LinkedList<Entry>();
             final HashSet<String> removalSet = new HashSet<String>();
             ...
             if ((additionsList.size() > 0) || (removalSet.size() > 0)) {
                 ...
                 try {
                     ...
                     final Iterator<Entry> it = additionsList.iterator();
                     while (it.hasNext()) {
     ...
     public String getStickyTagForDirectory(final File directory) {
         BufferedReader reader = null;
         final File tagFile = new File(directory, "CVS/Tag"); // NOI18N
 
         try {
             reader = new BufferedReader(new FileReader(tagFile));
             final String tag = reader.readLine();
             return tag;
      ...
      */
     private static File seekEntries(final File folder) {
         synchronized (ksEntries) {
             final File entries = new File(folder, "CVS/Entries"); // NOI18N
             if (entries.exists()) {
                 return entries;
             } else {
                 final File backup = new File(folder, "CVS/Entries.Backup"); // NOI18N
                 if (backup.exists()) {
                     try {
                         if (t9yBeforeRename != null) {
                             t9yBeforeRename.run();
     ...
     private static File seekEntries(final String folder) {
         return seekEntries(new File(folder));
     }
 
     static void t9yBeforeRenameSync(final Runnable run) {
         t9yBeforeRename = run;
     }
 }
