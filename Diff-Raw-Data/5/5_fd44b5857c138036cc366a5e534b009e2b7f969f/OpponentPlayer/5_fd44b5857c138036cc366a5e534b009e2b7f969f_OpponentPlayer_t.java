 ...
 public class OpponentPlayer extends AbstractCardPlayer {
 	 ...
 	 * @see de.jskat.ai.mjl.CardPlayer#playNextCard(jskat.share.CardList,
 	 *      de.jskat.ai.mjl.TrickInfo)
 	 */
 	...
 	private int findFirstInitial(PlayerKnowledge knowledge) {
 		...
 		if (knowledge.getDeclarer() == Player.MIDDLE_HAND) {
 			...
 			if (cards.get(cards.getFirstIndexOfSuit(maxSuit)).getRank() == Rank.ACE)
 				return cards.getFirstIndexOfSuit(maxSuit);
 			...
 			if (cards.get(cards.getFirstIndexOfSuit(minSuit)).getRank() == Rank.ACE)
 				return cards.getFirstIndexOfSuit(minSuit);
