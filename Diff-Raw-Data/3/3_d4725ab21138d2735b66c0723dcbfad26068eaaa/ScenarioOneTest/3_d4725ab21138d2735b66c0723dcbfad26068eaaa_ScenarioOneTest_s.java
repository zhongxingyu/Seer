 package org.rhq.plugins.mock.jboss.test;
 ...
 import org.rhq.plugins.mock.jboss.ResourceCache;
 ...
 public class ScenarioOneTest {
     ...
     public void inventory() throws Exception {
         ...
         for (Resource server : platform.getChildResources()) {
             ...
             assert childResources.size() == SERVER_SERVICE_COUNTS.get(serverName) : "Services not correctly loaded for "
                 + serverName
                 + ". Expected "
                 + SERVER_SERVICE_COUNTS.get(serverName)
                 + " found "
                 + childResources.size();
         }
     }
     ...
     @Test
     public void configurationDefinition() throws Exception {
         ...
         ResourceType noTxDatasourceResourceType = pluginContainer.getPluginManager().getMetadataManager().getType(
             "no-tx-datasource", ResourceCategory.SERVICE);
 
         ConfigurationDefinition noTxConfigurationDefinition = noTxDatasourceResourceType
             .getResourceConfigurationDefinition();
 
         assert noTxConfigurationDefinition != null : "Configuration definition could not be read for no-tx-datasource";
 
         assert noTxDatasourceResourceType.getResourceConfigurationDefinition().getDefaultTemplate() != null : "Default template could not be read for no-tx-datasource";
     }
     ...
     public void readConfiguration() throws Exception {
         ...
         assert configuration.getSimple("connectionUrl").getStringValue().equals(
             "jdbc:oracle:thin:@dev33.qa.atl.jboss.com:1521:jon") : "Property connectionUrl was incorrect";
     ...
     //@Test
     public void createServiceViaConfiguration() throws Exception {
         PluginContainer pc = PluginContainer.getInstance();
         ConfigurationAgentService configService = pc.getConfigurationAgentService();
         ...
         ResourceType dsType = pc.getPluginManager().getMetadataManager().getType("no-tx-datasource",
             ResourceCategory.SERVICE);
 
         Configuration config = new Configuration();
         config.put(new PropertySimple("jndiName", "New DS"));
 
         CreateResourceRequest request = new CreateResourceRequest(1, jonServer.getId(), "createdResource", dsType
             .getName(), dsType.getPlugin(), null, config);
         ...
         assert newDsAvailability.getAvailabilityType() == null : "Availability was not supposed to have been set for newly created service: "
             + newDsAvailability;
     ...
     //@Test
     public void deleteCreatedResource() throws Exception {
         // Create Resource
         PluginContainer pc = PluginContainer.getInstance();
         ResourceFactoryAgentService resourceFactoryAgentService = pc.getResourceFactoryAgentService();
     ...
     public void metrics() throws Exception {
         ...
         Set<MeasurementData> values = measurementService.getRealTimeMeasurementValue(eventQueue.getId(),
             DataType.MEASUREMENT, "consumer-count");
     ...
     public void operations() throws Exception {
         ...
         assert operationCallback.getLastJobId().equals("0") : "Incorrect job ID returned for success case. Expected: 0  Received: "
             + operationCallback.getLastJobId();
         ...
         String successMessage = (result == null) ? "ok" : result.getSimpleProperties().values().iterator().next()
             .getStringValue();
 
         assert successMessage != null : "Null result message received from success case";
         assert successMessage.endsWith("started correctly.") : "Incorrect result message received for success case: "
             + successMessage;
         ...
         assert operationCallback.getLastJobId().equals("1") : "Incorrect job ID returned for failure case. Expected: 1  Received: "
             + operationCallback.getLastJobId();
         ...
         assert failureException.getMessage().startsWith("Error occurred") : "Incorrect message received for failure case: "
         ...
         assert operationCallback.getLastJobId().equals("2") : "Incorrect job ID returned for time out case. Expected 2  Received: "
             + operationCallback.getLastJobId();
