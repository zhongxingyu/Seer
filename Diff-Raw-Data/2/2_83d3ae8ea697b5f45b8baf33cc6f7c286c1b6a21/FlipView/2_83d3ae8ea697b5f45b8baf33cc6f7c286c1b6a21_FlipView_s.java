 ...
 public class FlipView extends FrameLayout {
     ...
     private static final int FLIP_DISTANCE_PER_PAGE = 180; //for normalizing width/height
 	...
 	/**
 	 * will draw a shade if flipping on the previous(top/left) half 
 	 * and a shine if flipping on the next(bottom/right) half
 	 * 
 	 * @param canvas
 	 */
 	...
 	private View viewForPage(int page){
 		...
 		// check if view needed is in active views, if so order to front and return this view
 		View v = getActiveView(page);
 		if(v != null){
 			return v;
 		}
 
 		// get(and remove) a convertview with correct viewtype from recycled views
 		...
 		// insert that view into active views pushing the least used active view into recycled views
 	 ...
 	 * @return
 	 * The duration for a flip, bigger deltaFlipDistance = longer duration. 
 	 * The increase if duration gets smaller for bigger values of deltaFlipDistance.
 	 */
 	 ...
 	 * 
 	 * @param adapter
 	 * a regular ListAdapter, not all methods if the list adapter are used by the flipview
 	 * 
 	 */
 	...
 	/**
 	 * Hint that there is a next page
 	 * will do nothing if there is no next page
 	 * 
 	 * @param once
 	 * if true, only peak once. else peak until user interacts with view 
 	 */
 	...
 	/**
 	 * Hint that there is a previous page
 	 * will do nothing if there is no previous page
 	 * 
 	 * @param once
 	 * if true, only peak once. else peak until user interacts with view 
 	 */
 	 ...
 	 * 
 	 * @return
 	 * true if the view is flipping vertically, can only be set via xml attribute "orientation"
 	 */
