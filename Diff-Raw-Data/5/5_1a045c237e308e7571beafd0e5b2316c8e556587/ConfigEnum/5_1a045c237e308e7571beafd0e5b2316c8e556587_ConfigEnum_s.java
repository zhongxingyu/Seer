 ...
 public enum ConfigEnum {
 	...
 	G_NEWYVEL(
 			"fly.gliding.newYvelocity",
 			-0.5D,
 			"New velocity to slower the fall"),
 	G_VELCHECK(
 	...
 	JQMSG(
 			"useJoinQuitMsg",
 			true,
 			"To activate the change of the Join/Quit message\n"
 	...
 	SCALE_TIMEOUT(
 			"timeOutScale",
 			60,
 			"When using the parameter -t in power commands (god, fly, etc ...)\n"
 					+ "Configure the scale used for the time (here 60 sec).\n"
 					+ "It mean if you type /god -t 5 you will be god for 5 minutes"),
 	TIME_STOP(
 			"timeInSecBeforeStop",
 			0,
 			"Time before stopping the server when using the AdminCmd command Stop."),
 	TP_DIFF_WORLD(
 			"tpInDiffWorld",
 			true,
 			"Allow users to teleport to other user that are in a different world.\n"
 	...
 	ARMOR_KIT_OVERRIDE(
 			"armorKitOverride",
 			true,
 			"When using an armored kit, if this options is set to true,"
 					+ " it will override what the player is wearing to replace it by the armor of the kit."),
 	USE_PREFIX(
 			"usePSfix",
 			true,
 			"Use the prefix and the suffix when displaying names"),
 	DEGG_KILL_RADIUS(
 			"egg.killer.defaultRadiusInBlock",
 			15,
 			"When the Killer Egg is used, this radius will be used if no radius is set in the command."),
 	MAXEGG_KILL_RADIUS(
 			"egg.killer.maxRadiusInBlock",
 			30,
 			"When the Killer Egg is used, this radius will be the max radius possible for the command."),
 	EGG_BLOCK_TIMEOUT(
 			"egg.block.Timeout",
 			5,
 			"How much time the new blocks stay (in sec) when using the BlockEgg. Put 0 for never change back."),
 	DEGG_BLOCK_RADIUS(
 			"egg.block.defaultRadiusInBlock",
 			8,
 			"Default freeze radius when using the Block"),
 	MAXEGG_BLOCK_RADIUS(
 			"egg.block.maxRadiusInBlock",
 			12,
 			"Maximum radius for the BlockEgg"),
 	EGG_FREEZE_TIMEOUT(
 			"egg.freezer.Timeout",
 			5,
 			"How much time the ice stay (in sec) when using the FreezerEgg. Put 0 for never change back."),
 	DEGG_FREEZE_RADIUS(
 			"egg.freezer.defaultRadiusInBlock",
 			8,
 			"Default freeze radius when using the FreezerEgg"),
 	MAXEGG_FREEZE_RADIUS(
 			"egg.freezer.maxRadiusInBlock",
 			12,
 			"Maximum radius for the FreezergEgg"),
 	DEGG_EX_RADIUS(
 			"egg.explosion.defaultRadiusInBlock",
 			8,
 			"Default freeze radius when using the ExplosionEgg"),
 	MAXEGG_EX_RADIUS(
 			"egg.explosion.maxRadiusInBlock",
 			12,
 			"Maximum radius for the ExplosionEgg"),
 	EGG_HIGH_TIMEOUT(
 			"egg.high.Timeout",
 			20,
 			"How much time (in sec) the high effect least."),
 	DEGG_HIGH_RADIUS(
 			"egg.high.defaultRadiusInBlock",
 			5,
 			"Default radius when using the HighEgg"),
 	MAXEGG_HIGH_RADIUS(
 			"egg.high.maxRadiusInBlock",
 			12,
 			"Maximum radius for the HighEgg"),
 	IMPORT_BAN_TXT(
 			"importBannedPlayersTXT",
 			false,
 			"If set to true, this will import any player which does not exist in the banned.yml, afterwards it will be set to false!"),
 	SUPER_BLACKLIST(
 			"supBlacklist",
 			false,
 			"With this mode, the users that don't have the admincmd.spec.noblacklist can't pickup/drop/use any blacklisted items."),
 	EDIT_SIGN(
 			"editSignRightClick",
 			true,
 	...
 	LISTER_ITEMS(
 			...
 			8,
 			"How many items are displayed per page in banlist/mutelist etc ..."),
 	IMPORT_ESSENTIALS(
 			"importFrom.essentials",
 			true,
 			"If set to true AdminCmd will look for Essential datas the next time the server starts, import them if they exist and deactivate this feature."),
 	POWERS_OFF(
 			"turnPowersOff",
 			false,
 			"If set to true all powers(god etc.) are removed from a player when he leaves the game."),
 	DEATH_MSG(
 			"deathMessage",
 			true,
 	...
 	DATA_WRAPPER(
 			...
 			"DON'T TOUCH !\n"
					...
					+ "How the player data, world data are saved. Possible options : mysql,yml."),
 	CONVERT_INTO(
 			"convertInto",
 			"yml",
			"By setting this parameter, you tell AdminCmd that it have to convert the data to the new format. Possible format : mysql,yml.\n Mysql need the host,user,database,password to be set"
