 ...
 public class WorldMap implements Serializable {
 	...
 	private String nameLocation(Random mapRand){
 	...
 	private void convertToMapCoords(LocationNode node){
 		...
 		newY = ((node.MAP_YPOS/1.4) + 320) + ((node.MAP_XPOS -600)/2.4);
 		 
 		/* removed arcane, obtuse algorithm.  yay.
 		 *if (node.getRank() == 0) {
 			newY = (-1 * node.MAP_YPOS/1.2) + 60 + ((this.MAX_RANK - node.getRank()) * (480.0/ Math.pow(this.MAX_RANK,1))) ;			
 		}
 		else if (node.getRank() < (.05 * (1.0 * this.MAX_RANK))) {
 			newY = (-1 * node.MAP_YPOS/1.2) + 50 + ((this.MAX_RANK - node.getRank()) * (480.0/ Math.pow(this.MAX_RANK,1))) ;	
 		} else if (node.getRank() < (.25 * (1.0 * this.MAX_RANK))){
 			newY = (-1 * node.MAP_YPOS/1.6) + 115 + ((this.MAX_RANK - node.getRank()) * (480.0/ Math.pow(this.MAX_RANK,1.05))) ;	
