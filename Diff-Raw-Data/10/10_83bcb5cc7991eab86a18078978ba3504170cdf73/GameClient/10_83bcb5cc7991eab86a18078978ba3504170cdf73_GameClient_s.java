 ...
 public class GameClient extends SimpleApplication implements AnalogListener
 {
     ...
     public void simpleUpdate(float tpf)
     {
         if(timeToPlay && timeToPlayCount < 2)
         {
             ...
             else if(timeToPlayCount == 1)
             {
                 ...
                 try
                 {
                     connect("planetlab2.eecs.umich.edu", 8000, 6001);
                     connected = true;
                 }
                 catch(Exception e)
                 {
         ...
         Iterator it = playerLocations.entrySet().iterator();
         while(it.hasNext())
         {
             Map.Entry pairs = (Map.Entry)it.next();
 
             Spatial pNode = players.get(pairs.getKey());
             Vector3f pos = ((PlayerData)pairs.getValue()).position;
             Quaternion rot = ((PlayerData)pairs.getValue()).rotation;
             rot = rot.inverse();
         ...
         it = playerRemovals.iterator();
         while(it.hasNext())
         {
             NodeHandle id = (NodeHandle)it.next();      
     ...
     public void setPlayerYPos()
     {
         ...
         float h = terrain.getHeightmapHeight(new Vector2f(pt.x, pt.z)) * ts.y + tt.y;
         playerNode.setLocalTranslation(pt.x, h, pt.z);
     }
     
     GameNode gamenode;
     boolean connected = false;
     public void connect(String address, int bootPort, int thisPort) throws Exception
     {
         ...
         try
         {
             ...
             // launch our node!
             gamenode = new GameNode(thisPort, bootaddress, env, this);
         }
         catch (Exception e)
         {
     ...
     public Region getPlayersRegion(Vector3f pos)
     {
     ...
     public synchronized void removePlayer(NodeHandle id)
     { 
     ...
     int numPlayers = 0;
     public synchronized void updatePlayer(NodeHandle id, Vector3f location, Quaternion rotation)
     {
         if(!players.containsKey(id))
         {
             numPlayers++;
     ...
     public void sendChatMessage(String message)
     {
         if(connected)
         {
             gamenode.app.sendChatMessage(message);
         }
     }
     ...
     public void play(String name, String ip)
     {
         playerName = name;
         timeToPlay = true;
     }
     ...
     public void onAnalog(String name, float value, float tpf)
     {
         ...
         if(positionChanged)
         {
             Region newRegion = getPlayersRegion(this.getPlayerPosition());
