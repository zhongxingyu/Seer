         ...
         while(breakwhile)
         {
             ...
             else if(read.split(" ")[1].equalsIgnoreCase("PRIVMSG"))
             {
                 if(read.split(" ")[3].startsWith(":."))
                    ...
                    instance.getServer().getPluginManager().callEvent(new IRCUserMessageChannelEvent(read.split(" ")[0], instance.getMessage(read), new IRCChannel(read.split(" ")[2], new IRCServer(this))));
             }
             instance.debug(read);
         }
     }
