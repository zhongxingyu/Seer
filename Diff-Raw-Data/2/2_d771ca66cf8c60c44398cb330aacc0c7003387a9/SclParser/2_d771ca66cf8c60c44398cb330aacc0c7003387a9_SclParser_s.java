 ...
 final class SclParser {
 				...
 				for (int j = 0; j < servicesElements.getLength(); j++) {
 					if (servicesElements.item(i).getNodeName().equals("ReportSettings")) {
 						...
 						if (resvTmsAttribute != null) {
 							useResvTmsAttributes = resvTmsAttribute.getNodeValue().equals("true");
 						}
 					}
 				}
 			}
 	...
 	private ServerModel createServerModel(Node serverXMLNode) throws SclParseException {
 		...
 		for (LnSubDef rcbDef : rcbDefs) {
 			Rcb rcb = createReportControl(rcbDef.logicalNode, rcbDef.defXmlNode);
 
 			if (rcb instanceof Urcb) {
 				rcbDef.logicalNode.addUrcb((Urcb) rcb);
 			}
 			else {
 				rcbDef.logicalNode.addBrcb((Brcb) rcb);
 			}
 		}
 	...
 	private LogicalNode createNewLogicalNode(Node lnXmlNode, String parentRef) throws SclParseException {
 		...
 		// look for DataSet, ReportControl etc. definitions
 		for (int i = 0; i < lnXmlNode.getChildNodes().getLength(); i++) {
 			Node childNode = lnXmlNode.getChildNodes().item(i);
 			if ("DataSet".equals(childNode.getNodeName())) {
 				dataSetDefs.add(new LnSubDef(childNode, lNode));
 																		...
 																		// "LogControl".equals(childNode.getNodeName()))
 																		// {
 				rcbDefs.add(new LnSubDef(childNode, lNode));
 			}
 		}
 		return lNode;
 	}
 	...
 	private DataSet createDataSet(ServerModel serverModel, LogicalNode lNode, Node dsXmlNode) throws SclParseException {
 		...
 		for (int i = 0; i < dsXmlNode.getChildNodes().getLength(); i++) {
 			...
 			if ("FCDA".equals(fcdaXmlNode.getNodeName())) {
 				...
 				if (lnInst == null) {
 					throw new SclParseException("Required attribute \"lnInst\" not found in FCDA!");
 				}
 				if (lnClass == null) {
 	...
 	private Rcb createReportControl(LogicalNode lnode, Node xmlNode) throws SclParseException {
 		...
 		BdaTriggerConditions trigOps = new BdaTriggerConditions(new ObjectReference(lnode.getReference().toString()
 				+ ".TrgOps"));
 		BdaOptFlds optFields = new BdaOptFlds(new ObjectReference(lnode.getReference().toString() + ".OptFlds"));
 				...
 				if (attributes != null) {
 					for (int j = 0; j < attributes.getLength(); j++) {
 						...
 						if (nodeName.equals("dchg")) {
 							trigOps.setDataChange(node.getNodeValue().equals("true"));
 						}
 						else if (nodeName.equals("qchg")) {
 							trigOps.setQualityChange(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("dupd")) {
 							trigOps.setDataUpdate(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("period")) {
 							trigOps.setIntegrity(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("gi")) {
 							trigOps.setGeneralInterrogation(node.getNodeValue().equals("true"));
 						}
 					}
 				}
 			}
 				...
 				if (attributes != null) {
 					for (int j = 0; j < attributes.getLength(); j++) {
 						...
 						if (nodeName.equals("seqNum")) {
 							optFields.setSequenceNumber(node.getNodeValue().equals("true"));
 						}
 						else if (nodeName.equals("timeStamp")) {
 							optFields.setReportTimestamp(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("reasonCode")) {
 							optFields.setReasonForInclusion(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("dataSet")) {
 							optFields.setDataSetName(node.getNodeValue().equals("true"));
 							...
 							optFields.setBufferOverflow(node.getNodeValue().equals("true"));
 
 						}
 						else if (nodeName.equals("entryID")) {
 							optFields.setEntryId(node.getNodeValue().equals("true"));
 		...
 		BdaVisibleString rptId = new BdaVisibleString(new ObjectReference(reportObjRef.toString() + ".RptID"), null,
 				"", 129, false, false);
 		...
 		children.add(new BdaBoolean(new ObjectReference(reportObjRef.toString() + ".RptEna"), null, "", false, false));
 
 		if (fc == Fc.RP) {
 			children.add(new BdaBoolean(new ObjectReference(reportObjRef.toString() + ".Resv"), fc, "", false, false));
 		}
 
 		BdaVisibleString datSet = new BdaVisibleString(new ObjectReference(reportObjRef.toString() + ".DatSet"), null,
 				"", 129, false, false);
 
 		attribute = xmlNode.getAttributes().getNamedItem("datSet");
 		if (attribute != null) {
 			String nodeValue = attribute.getNodeValue();
 			String dataSetName = lnode.getReference() + "." + nodeValue;
 			datSet.setValue(dataSetName.getBytes());
 		}
 		children.add(datSet);
 
 		BdaInt32U confRef = new BdaInt32U(new ObjectReference(reportObjRef.toString() + ".ConfRev"), null, "", false,
 				false);
 		...
 		BdaInt32U bufTm = new BdaInt32U(new ObjectReference(reportObjRef.toString() + ".BufTm"), null, "", false, false);
 		...
 		children.add(new BdaInt8U(new ObjectReference(reportObjRef.toString() + ".SqNum"), null, "", false, false));
 
 		children.add(trigOps);
 
 		BdaInt32U intgPd = new BdaInt32U(new ObjectReference(reportObjRef.toString() + ".IntgPd"), null, "", false,
 				false);
 		...
 		children.add(new BdaBoolean(new ObjectReference(reportObjRef.toString() + ".GI"), null, "", false, false));
 
 		Rcb rcb = null;
 
 		if (fc == Fc.BR) {
 
 			children.add(new BdaBoolean(new ObjectReference(reportObjRef.toString() + ".PurgeBuf"), null, "", false,
 					false));
 
 			children.add(new BdaOctetString(new ObjectReference(reportObjRef.toString() + ".EntryID"), null, "", 8,
 					false, false));
 
 			children.add(new BdaEntryTime(new ObjectReference(reportObjRef.toString() + ".TimeOfEntry"), null, "",
 					false, false));
 	...
 	private Array createArrayOfDataAttributes(String ref, Da dataAttribute, Node iXmlNode) throws SclParseException {
 		...
 		for (int i = 0; i < size; i++) {
 			...
 			arrayItems.add(createDataAttribute(ref + '(' + i + ')', fc, dataAttribute, iXmlNode,
 					dataAttribute.isDchg(), dataAttribute.isDupd()));
 		}
 	...
 	private FcModelNode createDataAttribute(String ref, Fc fc, AbstractDataAttribute dattr, Node iXmlNode,
 			boolean dchg, boolean dupd) throws SclParseException {
 		...
 		if (!bType.equals("Struct")) {
 			...
 			if (bType.equals("BOOLEAN")) {
 				...
 				return new BdaQuality(new ObjectReference(ref), fc, sAddr, dchg, dupd);
 			}
