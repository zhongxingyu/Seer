  ...
  * Compiled statically to improve CPU & memory performance.
  */
 public class XmcdRegEx {
      ...
      * Handles number formats that end with L on some xmcd files and other oddities.
      */
    public static Pattern TRACK_FRAME_OFFSET = Pattern.compile("^#\\s*(\\d*).*$", Pattern.MULTILINE);
     /** Group 1 will have disc length in seconds */
     public static Pattern DISC_LENGTH = Pattern.compile("^#\\s*Disc length:\\s*(\\d+)", Pattern.MULTILINE);
     /** Group 1 will have revision # */
     public static Pattern REVISION = Pattern.compile("^#\\s*Revision:\\s*(\\d+)", Pattern.MULTILINE);
     /**
      * Group 1 will have FreeDB disc ID.
      * Additional IDs are ignored since this functionality is deprecated.
      */
     public static Pattern DISC_ID = Pattern.compile("^DISCID=([a-f0-9]{8})", Pattern.MULTILINE);
     ...
     /** Matches the track variant, e.g. " (Radio Edit)" */
     public static Pattern TRACK_VARIANT = Pattern.compile("\\s*[(\\[].*?[)\\]]\\s*$");
     /** Matches the xmcd splitter */
     public static Pattern SPLITTER = Pattern.compile("\\s+/\\s+");
 }
