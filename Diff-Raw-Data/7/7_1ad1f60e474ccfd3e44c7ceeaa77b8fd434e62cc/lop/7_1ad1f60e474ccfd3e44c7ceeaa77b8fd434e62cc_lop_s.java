 ...
 *	$Date: 2006/11/03 11:14:38 $
 ...
 public class lop extends ej {
 	...
 	private String op = "*"; // il y en faut bien un par dfaut
 	...
 	public lop(Atom[] args)	{
 		...
 		// premier argument dfini l'oprateur
 		if (args.length > 0)
 			setOp(new Atom[] { args[0] });
 		
 		// arguments suivants dfinissent les valeurs par dfaut
 		if (args.length > 1)
 	...
 	private float[] generateValFromArgs(Atom[] a) {
 		float[] tmp = new float[a.length - 1]; // 1 de moins car le premier est le nom de l'oprateur
 	...
 	private void calcule() {
 		...
 		else if (scalarmode == false || (a.length > 1 && b.length > 1))		// si c'est pas scalarmode ou que les deux entres ont reues des listes
 		...
 		// else : scalarmode est true mais l'une des deux listes n'est pas dfinie
 	}
