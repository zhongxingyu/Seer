 ...
 public class Jailer {
    ...
    public void writeEntities(String sqlScriptFile, final ScriptType scriptType, final Set<Table> progress, StatementExecutor statementExecutor) throws Exception {
     ...
     private synchronized void runstats(StatementExecutor statementExecutor) throws Exception {
         if (lastRunstats * 2 <= entityGraph.getTotalRowcount()) {
     ...
     private static void export(String extractionModelFileName, String scriptFile, String deleteScriptFileName, String driverClassName, String dbUrl, String dbUser, String dbPassword, boolean explain, int threads) throws Exception {
         ...
         final Jailer exportTool = new Jailer(threads);
         
         exportTool.appendCommentHeader("generated by Jailer at " + new Date() + " from " + getUsername());
         Set<Table> totalProgress = new HashSet<Table>();
         Set<Table> subjects = new HashSet<Table>();
         boolean firstTask = true;
         for (ExtractionModel.ExtractionTask task: extractionModel.getTasks()) {
             exportTool.appendCommentHeader("");
             exportTool.appendCommentHeader("extraction model:  " + task.subject.getName() + " where " + task.condition + " (" + extractionModelFileName + ")");
             exportTool.appendCommentHeader("database URL:      " + dbUrl);
             exportTool.appendCommentHeader("database user:     " + dbUser);
             ...
             exportTool.setEntityGraph(graph);
             exportTool.setDataModel(task.dataModel);
             Set<Table> progress = exportTool.exportInitialData(task.subject);
             progress.addAll(exportTool.export(task.subject, task.condition, progress, task.limit));
         ...
         if (explain) {
             ExplainTool.explain(entityGraph, exportTool.readInitialDataTables(), statementExecutor, exportTool.datamodel);
         }
         
         totalProgress = exportTool.datamodel.normalize(totalProgress);
         subjects = exportTool.datamodel.normalize(subjects);
         ...
         exportTool.setEntityGraph(entityGraph);
         exportTool.writeEntities(scriptFile, ScriptType.INSERT, totalProgress, statementExecutor);
         entityGraph.delete();
         
         if (deleteScriptFileName != null) {
             exportTool.setEntityGraph(exportedEntities);
             exportTool.deleteEntities(subjects, totalProgress, statementExecutor, CommandLineParser.getInstance().getTabuTables(exportTool.datamodel));
             exportTool.datamodel.transpose();
             exportTool.writeEntities(deleteScriptFileName, ScriptType.DELETE, totalProgress, statementExecutor);
