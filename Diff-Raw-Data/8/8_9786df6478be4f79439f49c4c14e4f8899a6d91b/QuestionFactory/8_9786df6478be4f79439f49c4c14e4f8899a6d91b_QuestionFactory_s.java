 ...
 public class QuestionFactory {
     ...
     public MatchQuestion matchAlbumsToGenreQuestionType() throws DALException {
         ...
         for (IAlbumDescriptor album: albums) {
             final String title = String.format("%s, by %s", album.getTitle(), album.getArtistName());
             matches.add(new Pair<String, String>(title, album.getGenre()));
         }
     ...
     public MatchQuestion matchAlbumsToYearQuestionType() throws DALException {
         ...
         for (IAlbumDescriptor album: albums) {
             final String title = String.format("%s, by %s", album.getTitle(), album.getArtistName());
             final String year = Integer.toString(album.getYear());
             matches.add(new Pair<String, String>(title, year));
         }
         MatchAnswer answer = new MatchAnswer(matches);
         return new MatchQuestion(question, answer);
     }
     ...
     public MatchQuestion matchTrackToAlbumQuestionType() throws DALException {
         ...
         Collection<Pair<String,String>> matches = new HashSet<Pair<String,String>>(4);
         for (IAlbumDescriptor album: albums) {
             matches.add(new Pair<String, String>(String.format("%s, by %s", album.getTitle(), album.getArtistName()),
     ...
     public MatchQuestion matchTrackToArtistQuestionType() throws DALException {
         ...
         for (ITrackDescriptor track: tracks) {
             matches.add(new Pair<String, String>(String.format("%s, in album %s", track.getTitle(),
                     track.getAlbumName()),
                     track.getArtistName()));
         }
         MatchAnswer answer = new MatchAnswer(matches);
     ...
     public MultipleChoiceQuestion multipleAlbumHasNumberOfTracksQuestionType() throws DALException {
         ...
         for (IAlbumDescriptor album: albums) {
             ...
             answers.add(new MultipleChoiceAnswer(String.format("%s, by %s", album.getTitle(),
                     album.getArtistName())));
         }
         String formattedQuestion = String.format(
                 question, Integer.toString(databaseLayer.getAlbumTrackDescriptors(winningAlbum).size()));
         final MultipleChoiceAnswer winningAnswer = answers.get(0);
         Collections.shuffle(answers);
         return new MultipleChoiceQuestion(formattedQuestion, winningAnswer, answers);
     ...
     public OrderedQuestion sortAlbumsByYearQuestionType() throws DALException {
         ...
         for (IAlbumDescriptor album: albums) {
             final String year = Integer.toString(album.getYear());
             final String title = String.format("%s, by %s", album.getTitle(), album.getArtistName());
             matches.add(new Pair<String, String>(year, title));
         }
     ...
     public OrderedQuestion sortTracksByLengthQuestionType() throws DALException {
         ...
         Collection<Pair<String,String>> matches = new HashSet<Pair<String,String>>(4);
         for (ITrackDescriptor track: tracks) {
            final String length = Integer.toString(track.getLengthInSeconds());
             final String title = String.format("%s, by %s, in album %s",
                     track.getTitle(), track.getArtistName(), track.getAlbumName());
             matches.add(new Pair<String, String>(title, length));
         }
         OrderedAnswer answer = new OrderedAnswer(matches);
         return new OrderedQuestion(question, answer);
     }
     ...
     public OrderedQuestion sortTrackByOrderInAlbumQuestionType() throws DALException {
         String question = "Sort the following tracks by their order in album %s, by %s:";
         IAlbumDescriptor selectedAlbum = getAlbumWithAtleastFourTracks();
         Set<ITrackDescriptor> tracks = new HashSet<ITrackDescriptor>(4);
         this.populateTracksSetWithRandomTracksFromAlbum(tracks, selectedAlbum);
         Collection<Pair<String,String>> matches = new HashSet<Pair<String,String>>(4);
         for (ITrackDescriptor track: tracks) {
             matches.add(new Pair<String, String>(Integer.toString(track.getTrackNum()), track.getTitle()));
         }
         OrderedAnswer answer = new OrderedAnswer(matches);
         String formattedQuestion = String.format(question, selectedAlbum.getTitle(), selectedAlbum.getArtistName());
     ...
     private Collection<IAlbumDescriptor> getRandomAlbums(int number) throws DALException {
         ...
         ITableStatistics stats = this.databaseLayer.getAlbumTableStatistics();
         int expected = guessHowManyIdsAreNeeded(number, stats);
         int iterations=0;
         while (collection.size() < number) {
             ...
             Set<Integer> randomIds = getSetRandomIds(expected, stats, usedIds);
     ...
     private Collection<IArtistDescriptor> getRandomArtists(int number) throws DALException {
         ...
         Set<Integer> usedIds = new HashSet<Integer>();
         ITableStatistics stats = this.databaseLayer.getArtistTableStatistics();
         int expected = guessHowManyIdsAreNeeded(number, stats);
         int iterations=0;
         while (collection.size() < number) {
             iterations+=1;
     ...
     private Set<Integer> getSetRandomIds(int number, ITableStatistics stats, Set<Integer> blacklist) {
         Random random = new Random();
         Set<Integer> randomIds = new HashSet<Integer>(number);
         while (randomIds.size() < number) {
             Integer randomNumber = new Integer(stats.getMinId() + random.nextInt(stats.getMaxId() - stats.getMinId() + 1));
     ...
     private int guessHowManyIdsAreNeeded(int actual, ITableStatistics stats) {
         int expected = actual;
         float numerator = stats.getMaxId() - stats.getMinId() +1;
         float denumerator = stats.getExtimatedRowCount();
         float factor = numerator/denumerator;
         if (factor>1) {
             expected = (((int)factor)+1)*actual;
             expected++; // We preffer this instead of rounding the number.
         }
         return expected;
     }
     ...
     private Set<Integer> getSetOfRandomNumbers(int maxId, int count) {
         ...
         while (randomIds.size() < count) {
             randomIds.add(random.nextInt(maxId*1000) % maxId);
         }
         return randomIds;
     }
     ...
     private Collection<String> getThreeTrackNamesFromArtist(IArtistDescriptor artist) throws DALException {
         ...
         while (selectedtracks.size() < 3) {
             Set<Integer> indexes = this.getSetOfRandomNumbers(tracks.size(), 3);
             for (Integer index: indexes) {
                 selectedtracks.add(tracks.get(index.intValue()).getTitle());
             }
         }
         return selectedtracks;
     }
