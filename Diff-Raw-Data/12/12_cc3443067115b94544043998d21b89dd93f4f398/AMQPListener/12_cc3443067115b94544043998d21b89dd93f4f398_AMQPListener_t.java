 ...
 */
 ...
 /**
  * The revamped AMQP Transport listener implementation. Creates {@link ServiceTaskManager} instances
  ...
  * AMQPListner on the axis2.xml, and this provides a way to reuse common configuration between
  ...
  * {@see AMQPConstants.DEFAULT_CONFAC_NAME}
  * <p>
  ...
  * <p>
  * All Destinations / AMQP Administered objects used MUST be pre-created or already available 
  */
 public class AMQPListener extends AbstractTransportListenerEx<AMQPEndpoint> implements ManagementSupport,
     TransportErrorSource {
 
     public static final String TRANSPORT_NAME = "amqp";
 
     /** The AMQPConnectionFactoryManager which centralizes the management of defined factories */
     ...
     protected void startEndpoint(AMQPEndpoint endpoint) throws AxisFault {
         ...
         for (int i=0; i<3; i++) {
             if (stm.getActiveTaskCount() > 0) {
                 log.info("Started to listen on destination : " + stm.getDestinationName() +
                     " of type " + AMQPUtils.getDestinationTypeAsString(stm.getDestinationType()) +
         ...
         log.warn("Polling tasks on destination : " + stm.getDestinationName() +
             " of type " + AMQPUtils.getDestinationTypeAsString(stm.getDestinationType()) +
     ...
     protected void stopEndpoint(AMQPEndpoint endpoint) {
         ...
         if (log.isDebugEnabled()) {
             log.debug("Stopping listening on destination : " + stm.getDestinationName() +
                 " for service : " + stm.getServiceName());
         }
      ...
      * @return the AMQPConnectionFactory to be used, or null if reference is invalid
      */
