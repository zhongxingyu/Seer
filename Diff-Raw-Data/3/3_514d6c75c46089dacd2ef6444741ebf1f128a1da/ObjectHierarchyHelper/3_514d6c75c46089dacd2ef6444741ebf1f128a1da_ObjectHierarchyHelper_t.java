 ...
 public class ObjectHierarchyHelper {
     ...
     private static final String           TAG                = ObjectHierarchyHelper.class
                                                                      .getSimpleName();
     ...
     public void calculateInterfaceIndices() {
         buildConflictGraph();
         Log.debug(TAG, "Built interface conflict graph containing " + conflictGraph.size()
                 + " interfaces");
         int lastStartingNumber = colorConflictGraph();
         Log.debug(TAG, "Colored graph with highest starting index " + lastStartingNumber);
         verifyConflictGraph();
         Log.debug(TAG, "Verified correctness of coloring for " + conflictGraph.size()
                 + " interfaces");
     ...
     private void processNodeForConflictGraph(GraphNode node) {
         ...
         for (XmlvmResource interfaceResource : getInterfacesRecursive(node.getResource()
                 .getFullName())) {
         ...
         for (ColoredGraphNode connectFromNode : conflictingInterfaces) {
             for (ColoredGraphNode connectToNode : conflictingInterfaces) {
                 if (!connectFromNode.getResource().getFullName()
                         .equals(connectToNode.getResource().getFullName())) {
     ...
     public void verifyConflictGraph() {
         for (ColoredGraphNode node : conflictGraph.values()) {
             if (node.getResource().getMethods().size() != node.getColors().size()) {
                 throw new RuntimeException("Not every method in interface "
                         + node.getResource().getFullName() + " is assigned a color!");
             }
     ...
     public int getInterfaceIndex(XmlvmResource iface, XmlvmMethod m) {
         ...
         Log.debug(TAG, "Interface index for " + iface.getFullName() + " " + m.getName() + "("
                 + parameterList + ") = " + interfaceIndex);
