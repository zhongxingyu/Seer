 ...
 public class DFSServer extends DFSComponent {
   ...
   private void addLiveTx(TransactionMessage msg) {
     Transaction t = new Transaction(msg.getTxId());
 
     transactions.put(msg.getTxId(), t); 
   }
   ...
   */
   private void updateCheckout(DFSFilename file, TransactionId txId, boolean writePermit) {
     ...
     Checkout c = new Checkout(t, sendVersion, writePermit); //client cannot write, it's a readReq
 ...
 /*    OwnershipRequestWrapper req = transferOwnerRequests.get(filename);
   ...
   private void handleSyncRequest(Integer from, SyncRequestMessage msg) {
    ...
    SyncDataMessage sdMsg = buildSyncDataMessage(filename, flags);
