 ...
 public class BsonParser extends ParserBase {
     ...
     public enum Feature {
         /**
          * Honors the document length field when parsing, useful for when reading from streams that may contain other
          * content after the document that will be read by something else.
          */
    ...
    private de.undercouch.bson4jackson.io.CountingInputStream _counter;
      ...
      * @param jsonFeatures bit flag composed of bits that indicate which {@link com.fasterxml.jackson.core.JsonParser.Feature}s
      *                     are enabled.
      * @param bsonFeatures bit flag composed of bits that indicate which {@link Feature}s are enabled.
     ...
     public BsonParser(IOContext ctxt, int jsonFeatures, int bsonFeatures, InputStream in) {
         ...
         if (!isEnabled(Feature.HONOR_DOCUMENT_LENGTH)) {
             ...
             _counter = new de.undercouch.bson4jackson.io.CountingInputStream(in);
     ...
     /**
      * Can be called when a new embedded document is found. Reads the document's header and creates a new context on the
      * stack.
      *
     ...
     /**
      * Converts a BSON regex pattern string to a combined value of Java flags that can be used in {@link
     ...
     /**
      * Reads a string that consists of a integer denoting the number of bytes, the bytes (including a terminating 0
      * byte)
      *
      * @return the string
      * @throws IOException if the string could not be read
