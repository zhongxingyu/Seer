 ...
 public class WeeklyAsteroids extends Activity {
 	...
 	ArrayList<Double> asteroidSizeList = new ArrayList<Double>(); 
 
 	private static final String WEEKLYASTEROIDS_TAG = "Weekly_Asteroids";
 	private ListView listView1;
 	private Double approxDiameter;
 	...
 	public void onCreate(Bundle savedInstanceState) {
 		...
 		db.openDB();
 		Cursor c = db.getUpcomingAsteroids();
 		if (c.moveToFirst()) {
 			do {
 		...
 		for(int i = 0; i<asteroidList.size(); i++){
 			...
 			asteroidSize = asteroidSizeDecider(asteroidSizeList.get(i));
 			asteroid_data[i] = new Asteroids(asteroidSize, asteroidList.get(i), false);	
 		}
 	...
 	//decide which asteroid image goes into listview based on diameter 
 	public int asteroidSizeDecider(Double approxDiameter){
 		
 		int asteroidImg;
 		
 		if (approxDiameter <= 100){
 			asteroidImg = R.drawable.mini_icon_asteroid_small;
 		}
 		else if (approxDiameter > 100 && approxDiameter < 300){
 			asteroidImg = R.drawable.mini_icon_asteroid_medium;
 		}
 		else {
 			asteroidImg = R.drawable.mini_icon_asteroid_large;
 		}
 		return asteroidImg;
 	}
