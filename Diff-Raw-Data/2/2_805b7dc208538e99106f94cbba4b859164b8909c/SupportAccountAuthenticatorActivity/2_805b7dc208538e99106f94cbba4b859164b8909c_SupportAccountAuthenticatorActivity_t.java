  ...
  * AbstractAccountAuthenticator. If the AbstractAccountAuthenticator needs to use an activity to
  * handle the request then it can have the activity extend AccountAuthenticatorActivity. The
  * AbstractAccountAuthenticator passes in the response to the intent using the following:
  * 
  * <pre>
  ...
  * {@link #setAccountAuthenticatorResult(android.os.Bundle)}. This result will be sent as the result
  * of the request when the activity finishes. If this is never set or if it is set to null then
  * error {@link AccountManager#ERROR_CODE_CANCELED} will be called on the response.
  * 
