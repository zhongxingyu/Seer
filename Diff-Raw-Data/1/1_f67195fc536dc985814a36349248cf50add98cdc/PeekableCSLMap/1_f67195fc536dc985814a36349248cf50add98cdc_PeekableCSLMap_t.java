 /*
  * @(#)PeekableCSLMap.java	1.8 07/03/13
  *
  * Copyright 2006 Sun Microsystems, Inc. All rights reserved.
  * SUN PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
  */
 
 package edu.stanford.ppl.util;
...
import java.util.AbstractMap.SimpleImmutableEntry;
 ...
 public class PeekableCSLMap<K,V> extends AbstractMap<K,V>
     ...
     public interface PeekIterator<K,V> extends Iterator<K> {
         V peekValue();
     }
      ...
      * clear, readObject. and PeekableCSLSet.clone.
     ...
     private static final
         AtomicReferenceFieldUpdater<PeekableCSLMap, HeadIndex>
         headUpdater = AtomicReferenceFieldUpdater.newUpdater
         (PeekableCSLMap.class, HeadIndex.class, "head");
      ...
      */
     public PeekableCSLMap(Comparator<? super K> comparator) {
      ...
      *         or values are null
      */
     public PeekableCSLMap(Map<? extends K, ? extends V> m) {
         this.comparator = null;
         initialize();
         putAll(m);
     }
      ...
      */
     public PeekableCSLMap(SortedMap<K, ? extends V> m) {
     ...
     /**
      * Returns a shallow copy of this <tt>PeekableCSLMap</tt>
      * instance. (The keys and values themselves are not cloned.)
      *
      * @return a shallow copy of this map
      */
     public PeekableCSLMap<K,V> clone() {
         PeekableCSLMap<K,V> clone = null;
         try {
             clone = (PeekableCSLMap<K,V>) super.clone();
 	...
 	V nextValue;
         ...
         public void remove() {
             ...
             PeekableCSLMap.this.remove(l.key);
 	    lastReturned = null;
         }
     ...
     final class KeyIterator extends Iter<K> implements PeekIterator<K,V> {
         public K next() {
             Node<K,V> n = next;
             advance();
             return n.key;
     ...
     // Factory methods for iterators needed by PeekableCSLSet etc
     ...
     static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
         ...
         public Iterator<E> iterator() {
             if (m instanceof PeekableCSLMap)
                 return ((PeekableCSLMap<E,Object>)m).keyIterator();
             else
                 return ((PeekableCSLMap.SubMap<E,Object>)m).keyIterator();
         }
         public boolean equals(Object o) {
             if (o == this)
                 return true;
     ...
     static final class Values<E> extends AbstractCollection<E> {
         ...
         public Iterator<E> iterator() {
             if (m instanceof PeekableCSLMap)
                 return ((PeekableCSLMap<Object,E>)m).valueIterator();
             else
     ...
     static final class EntrySet<K1,V1> extends AbstractSet<Entry<K1,V1>> {
         ...
         public Iterator<Entry<K1,V1>> iterator() {
             if (m instanceof PeekableCSLMap)
                 return ((PeekableCSLMap<K1,V1>)m).entryIterator();
             else
     ...
     /**
      * Submaps returned by {@link PeekableCSLMap} submap operations
     ...
     static final class SubMap<K,V> extends AbstractMap<K,V>
         ...
         /** Underlying map */
         private final PeekableCSLMap<K,V> m;
          ...
          */
         private boolean isBeforeEnd(PeekableCSLMap.Node<K,V> n) {
             if (n == null)
                 return false;
             if (hi == null)
                 return true;
          ...
          */
         private PeekableCSLMap.Node<K,V> loNode() {
             if (lo == null)
                 return m.findFirst();
             else if (loInclusive)
          ...
          * most usages need to check bounds
          */
         private PeekableCSLMap.Node<K,V> hiNode() {
             if (hi == null)
                 return m.findLast();
             else if (hiInclusive)
          ...
          */
         private K lowestKey() {
             PeekableCSLMap.Node<K,V> n = loNode();
             if (isBeforeEnd(n))
                 return n.key;
             else
                 throw new NoSuchElementException();
         ...
         private K highestKey() {
             PeekableCSLMap.Node<K,V> n = hiNode();
             if (n != null) {
                 K last = n.key;
                 if (inBounds(last))
                     return last;
         ...
         /**
          * Submap version of PeekableCSLMap.getNearEntry
          */
         ...
         public int size() {
             long count = 0;
             for (PeekableCSLMap.Node<K,V> n = loNode();
                  isBeforeEnd(n);
                  n = n.next) {
                 if (n.getValidValue() != null)
                     ++count;
         ...
         final class SubMapKeyIterator extends SubMapIter<K> implements PeekIterator<K,V> {
             public K next() {
                 Node<K,V> n = next;
                 advance();
                 return n.key;
