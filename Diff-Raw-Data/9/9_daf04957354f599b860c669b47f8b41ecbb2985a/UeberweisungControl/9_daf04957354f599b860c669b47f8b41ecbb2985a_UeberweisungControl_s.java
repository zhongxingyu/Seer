 ...
 public class UeberweisungControl extends AbstractControl {
   ...
   public Konto getKonto() throws RemoteException
 	...
 	public Part getUeberweisungListe() throws RemoteException
 	{
       ...
       public void format(TableItem item) {
		...
		table.addMenu(i18n.tr("Neue berweisung"), new UeberweisungCreate());
     table.addMenu(i18n.tr("Jetzt ausfhren"), new UeberweisungExecute());
 		table.addMenu(i18n.tr("Duplizieren"), new UeberweisungDuplicate());
 
 		table.addColumn(i18n.tr("Konto"),"konto_id");
 		table.addColumn(i18n.tr("Kto. des Empfngers"),"empfaenger_konto");
 		table.addColumn(i18n.tr("BLZ des Empfngers"),"empfaenger_blz");
 		table.addColumn(i18n.tr("Name des Empfngers"),"empfaenger_name");
       ...
       public String format(Object o) {
 				try {
 					...
 					return i == 1 ? i18n.tr("ausgefhrt") : i18n.tr("offen");
 				}
 				catch (Exception e) {}
 				return ""+o;
       }
   ...
   public Input getComment() throws RemoteException
 	{
 		...
 		if (u.ausgefuehrt())
 		{
 			comment.setValue(i18n.tr("Die berweisung wurde bereits ausgefhrt"));
 		}
 		else if (u.ueberfaellig())
 		{
 			comment.setValue(i18n.tr("Die berweisung ist berfllig"));
 		}
 		return comment;
 	}
   ...
   public Input getKontoAuswahl() throws RemoteException
 	{
 		...
 		if (getUeberweisung().ausgefuehrt())
 		{
 			...
 			GUI.getView().setSuccessText(i18n.tr("Die berweisung wurde bereits ausgefhrt und kann daher nicht mehr gendert werden."));
 		}
 		return kontoAuswahl;
 	}
   ...
   public Input getEmpfaengerKonto() throws RemoteException
 	{
 		...
 		d.addColumn(i18n.tr("Kontonummer"),"kontonummer");
 		d.addColumn(i18n.tr("BLZ"),"blz");
 		d.setTitle(i18n.tr("Auswahl des Empfngers"));
 			...
 			public void handleEvent(Event event) {
 				...
 				try {
 					...
 					if (choosen.before(new Date()))
 						getComment().setValue(i18n.tr("Die berweisung ist berfllig."));
 				}
 				catch (RemoteException e) {/*ignore*/}
 			}
 		});
   ...
   public synchronized void handleDelete() {
 		try {
 			...
 			d.setText(i18n.tr("Wollen Sie die berweisung wirklich lschen?"));
 			if (!((Boolean) d.open()).booleanValue())
 				return;
 			getUeberweisung().delete();
 			GUI.getStatusBar().setSuccessText(i18n.tr("berweisung gelscht."));
 		}
 		catch (Exception e)
 		{
 			Logger.error("error while deleting ueberweisung",e);
 			GUI.getStatusBar().setErrorText(i18n.tr("Fehler beim Lschen der berweisung."));
 		}
   }
   	...
   	try {
   		...
   		getUeberweisung().setZweck((String)getZweck().getValue());
 			...
 			if (store.booleanValue())
 			{
 				...
 				{
 					YesNoDialog d = new YesNoDialog(YesNoDialog.POSITION_CENTER);
 					d.setTitle(i18n.tr("Empfnger existiert"));
 					d.setText(i18n.tr("Ein Empfnger mit dieser Kontonummer und BLZ existiert bereits. " +
 							"Mchten Sie den Empfnger dennoch zum Adressbuch hinzufgen?"));
   ...
   public synchronized void handleExecute()
 	{
 		...
 		try {
 			if (getUeberweisung().ausgefuehrt())
 			{
 				GUI.getView().setErrorText(i18n.tr("Die berweisung wurde bereits ausgefhrt."));
 				return;
 			}
 		...
 		catch (Exception e)
 		{
 			...
 			GUI.getView().setErrorText(i18n.tr("Fehler beim Prfen der berweisung"));
 		}
 
 
 		GUI.getStatusBar().startProgress();
 		GUI.getStatusBar().setSuccessText(i18n.tr("Fhre berweisung aus..."));
       	...
       	public void run() {
       		try {
       			getUeberweisung().execute();
 						GUI.getStatusBar().setSuccessText(i18n.tr("...berweisung erfolgreich ausgefhrt."));
       			disableAll();
       		}
       		catch (ApplicationException e)
       		{
       		...
       		catch (Throwable t)
       		{
       			...
       			errorText.append("Fehler beim Ausfhren der berweisung.");
       			throw new RuntimeException(); // wir wollen nicht warten, bis sich die GUI ausgekaest hat, raus hier!
       		}
       	}
       });
   ...
   private class KontoListener implements Listener
 	{
 		...
 		public void handleEvent(Event event) {
 			...
 			catch (RemoteException er)
 			{
 				...
 				GUI.getStatusBar().setErrorText(i18n.tr("Fehler bei Ermittlung der Whrung"));
 			}
 		}
 	}
     ...
     public void handleEvent(Event event) {
     	if (event == null)
 			...
 			catch (RemoteException er)
 			{
 				...
 				GUI.getStatusBar().setErrorText(i18n.tr("Fehler bei der Auswahl des Empfngers"));
     	}
     }
 	}
 }
