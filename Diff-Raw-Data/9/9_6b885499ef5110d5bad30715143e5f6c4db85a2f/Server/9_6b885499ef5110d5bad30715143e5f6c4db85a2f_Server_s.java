 ...
 public class Server implements NetServer {
     ...
     private class ConHan implements ConnectionHandler {
         ...
         private Command switchGET_WEEK(Command comm) {
             ...
             Meeting[] meetings = srv.db.getMeetingsInWeek(calendarId, weekNr);
         ...
         private Command internalSetReply(Command comm, boolean accept) {
             Command cmd = new Command();
            MeetingParticipant meetPar = srv.db.getMeetingParticipant(cmd.intarg1, user.getId());
             if (meetPar == null) {
                 cmd.command = Commands.ERROR;
                 return cmd;
             }
             if (accept) {
                 ...
                 if (srv.db.createCalendarHasMeetingEntry(cmd.intarg1, cmd.intarg2))
                     cmd.command = Commands.OK;
                 else
                     cmd.command = Commands.ERROR;
             } else
                 meetPar.setReplyStatus(ReplyStatus.NO);
             if (srv.db.saveMeetingParticipant(meetPar) && (cmd.command == Commands.OK))
                 cmd.command = Commands.OK;
             else
                 cmd.command = Commands.ERROR;
             return cmd;
         ...
         private Command switchADD_MEETING(Command comm) {
             ...
             if (srv.db.userOwnsCalendar(user.getId(), comm.intarg1)) {
                 if (srv.db.createMeeting(meeting, comm.intarg1, user.getId())) {
                     ...
                     cmd.command = Commands.ERROR;
                     cmd.strarg1 = "failed in createMeeting";
                 }
             } else {
                 cmd.command = Commands.ERROR;
                 cmd.strarg1 = "failed in userOwnsCalendar";
             }
             return cmd;
         }
         ...
         private Command switchGET_UNINVITED(Command comm) {
             Command cmd = new Command(Commands.MEETINGPARTICIPANTS);
             User[] uninvitedUsers = srv.db.getNotInvited(comm.intarg1);
         ...
         private Command switchINVITE(Command comm) {
             Command cmd = new Command();
             String username = cmd.strarg1;
             int meetingId = cmd.intarg1;
             int userId = srv.db.getUserByName(username);
             if (srv.db.createUserHasMeetingEntry(userId, meetingId, ReplyStatus.WAITING))
                 cmd.command = Commands.OK;
             else
                 cmd.command = Commands.ERROR;
             return cmd;
