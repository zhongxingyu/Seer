 ...
 public class Folder extends LinearLayout implements DragSource, View.OnClickListener,
     ...
     private static final String TAG = "ChaOSLauncher.Folder";
     ...
     private static final int SORT_USAGE = 3;
    ...
    private LinearLayout mFolderFooter;
     ...
     private void showPopup(View v) {
         ...
         popup.getMenu().getItem(mInfo.sortType).setChecked(true);
         popup.show();
     }
     ...
     PopupMenu.OnMenuItemClickListener mMenuItemClickListener = new PopupMenu.OnMenuItemClickListener() {
         ...
         public boolean onMenuItemClick(MenuItem menuItem) {
             switch (menuItem.getItemId()) {
                 case R.id.folder_sort_none:
                     mInfo.sortType = SORT_NONE;
                     break;
                 case R.id.folder_sort_ascending:
                     orderAscending(null);
                     mInfo.sortType = SORT_ASCENDING;
                     break;
                 case R.id.folder_sort_descending:
                     orderDescending(null);
                     mInfo.sortType = SORT_DESCENDING;
                     break;
                 case R.id.folder_sort_usage:
                     orderByUsage(null);
                     mInfo.sortType = SORT_USAGE;
                     break;
             }
             LauncherModel.updateItemInDatabase(mLauncher, (ItemInfo)mInfo);
             return true;
         }
     };
     ...
     private void orderAscending(ArrayList<View> list) {
         if (list == null) {
         ...
         for (int i = 0; i < list.size(); i++) {
             ...
             order.put(info.title.toString(), v);
         }
 
         ArrayList<View> orderedList = new ArrayList<View>();
         Set<String> set = order.keySet();
         for (String s : set) {
             orderedList.add(order.get(s));
         }
 
         arrangeChildren(orderedList);
     }
 
     private void orderDescending(ArrayList<View> list) {
         if (list == null) {
             list = getItemsInReadingOrder();
         }
     ...
     private void orderByUsage(ArrayList<View> list) {
         if (list == null) {
             list = getItemsInReadingOrder();
         }
 
         Map<Integer, List<View>> ordered = new TreeMap<Integer, List<View>>(Collections.reverseOrder());
         for (int i = 0; i < list.size(); i++) {
             View v = list.get(i);
             ShortcutInfo info = (ShortcutInfo) v.getTag();
             Integer launchCount = info.launchCount;
             List<View> subList = null;
             if (!ordered.containsKey(launchCount)) {
                 subList = new ArrayList<View>();
                 ordered.put(launchCount, subList);
             } else
                 subList = ordered.get(launchCount);
             subList.add(v);
         }
 
         ArrayList<View> orderedList = new ArrayList<View>();
         Set<Integer> set = ordered.keySet();
         for (Integer i : set) {
             List<View> views = ordered.get(i);
             for (View v : views)
                 orderedList.add(v);
         }
