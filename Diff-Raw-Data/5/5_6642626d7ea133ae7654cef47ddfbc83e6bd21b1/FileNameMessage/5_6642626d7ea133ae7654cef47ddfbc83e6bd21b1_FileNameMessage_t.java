 ...
 public class FileNameMessage extends DFSMessage {
   ...
   protected FileNameMessage(MessageWireType messageType, String fileName) {
     ...
     this.version = new FileVersion(new TransactionId(0,0), 0, 0);
   }
 
   protected FileNameMessage(MessageWireType messageType, String fileName, FileVersion version) {
   ...
   protected FileNameMessage(MessageWireType messageType, String fileName, FileVersion version, TransactionId txId) {
     this(messageType, fileName, version);
     this.txId = txId;
   }
   ...
   protected byte[] createPackedHeader(int numAdditionalBytes) {
     ...
     byte[] packedMsg = new byte[3 + fileNameBytes.length + numAdditionalBytes + packedVersion.length];
   ...
   protected int unpackFileNameHeader(byte[] msg) {
    ...
    System.arraycopy(msg,3+fileNameSize, versionByte, 0, versionByte.length);
 
   version = new FileVersion();
    version.unpack(versionByte, 0);

    return fileNameSize + 3 + versionByte.length;
   }
