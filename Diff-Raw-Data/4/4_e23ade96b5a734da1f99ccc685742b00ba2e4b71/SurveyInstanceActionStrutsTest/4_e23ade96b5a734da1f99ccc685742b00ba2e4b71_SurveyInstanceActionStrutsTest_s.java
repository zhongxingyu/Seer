 ...
 import org.mifos.customers.surveys.business.SurveyIntegrationTest;
 ...
 public class SurveyInstanceActionStrutsTest extends MifosMockStrutsTestCase {
     ...
     public void testSurveyValidation() throws Exception {
        ...
        Assert.assertEquals(SurveyInstanceAction.getBusinessObjectName(survey.getAppliesToAsEnum(), globalNum),
         ...
         // addRequestParameter("value(response_" + question4Id + ")",
         // Integer.toString(choice1.getChoiceId()));
         setRequestPathInfo("/surveyInstanceAction");
         addRequestParameter("method", "preview");
         actionPerform();
         String[] expectedErrors = { "errors.SurveyValidator.MISSING", "errors.DateValidator.DATE_FORMAT",
     ...
     public void testGetPPISurvey() throws Exception {
         ...
         for (SurveyQuestion question : ppiSurvey.getQuestions()) {
             addRequestParameter("value(response_" + question.getSurveyQuestionId() + ")", question.getQuestion()
                     .getChoices().get(0).getChoiceId()
                     + "");
         }
         addRequestParameter("method", "preview");
         actionPerform();
     ...
     private LoanBO createLoan() throws Exception {
         ...
         return TestObjectFactory.createLoanAccount("12345", customer, AccountState.LOAN_APPROVED, new Date(),
                 loanOffering);
     }
