...
import com.vmware.aurora.buildinfo.BuildInfo;
 ...
 /**
  * <code>Configuration</code> is a util class for accessing configurations of CMS. <br>
  *
  * @since   0.4.2
  * @version 0.4.2
  * @author Gene Zhang
 ...
 public class Configuration {
     ...
     private static org.apache.commons.configuration.Configuration init() {
         ...
         String propertyFilePrefix = System.getProperty("PROPERTY_FILE_PREFIX", "aurora-cms");
         String propertyFileName = propertyFilePrefix + ".properties";
         String transientPropertyFileName = propertyFilePrefix + "-transient.properties";
         String devPropertyFileName = propertyFilePrefix + "-dev.properties";
 
         try {
             logger.info("Reading properties file " + propertyFileName);
             config = CommonUtil.GetPropertiesConfiguration(propertyFileName);
         } catch (ConfigurationException ex) {
             // error out if the configuration file is not there
             String message = "Failed to load cms configuration file.";
             logger.fatal(message, ex);
             throw AuroraException.APP_INIT_ERROR(ex, message);
         }
 
         try {
            logger.info("Reading transient properties file " + transientPropertyFileName);
            org.apache.commons.configuration.Configuration tranCfg = CommonUtil.GetPropertiesConfiguration(transientPropertyFileName);
            Iterator<?> keys = tranCfg.getKeys();
            while (keys.hasNext()) {
               String key = (String)keys.next();
               config.setProperty(key, tranCfg.getProperty(key));
            }
         } catch (ConfigurationException ex) {
         ...
         if (true) {
            try {
               logger.info("Reading dev properties file " + devPropertyFileName);
               org.apache.commons.configuration.Configuration devCfg = CommonUtil.GetPropertiesConfiguration(devPropertyFileName);
               Iterator<?> keys = devCfg.getKeys();
               while (keys.hasNext()) {
                  String key = (String)keys.next();
                  config.setProperty(key, devCfg.getProperty(key));
               }
            } catch (ConfigurationException ex) {
               logger.info("Failed to load cms dev configuration properties file.");
            }
         }
         logConfig(config);
         return config;
    ...
    private static void logConfig(org.apache.commons.configuration.Configuration config) {
     ...
     public enum BootstrapUsage { DISALLOWED, ALLOWED, FINALIZED; }
     private static BootstrapUsage bootstrapMode = BootstrapUsage.DISALLOWED;
 
     /**
      * Set the CMS instance identifier which will be returned by getCmsInstanceId().
      * Can be called only once per process during initialization. This variant is
      * used only during system setup (VirtualCenterResourceManagement.initializeSystem);
      * otherwise setCmsInstanceId is preferred.
      */
     ...
     /**
      * Set the CMS instance identifier which will be returned by getCmsInstanceId().
      * Can be called only once per process during initialization.
      * @param id Instance ID retrieved from database.
      */
     ...
     /**
      * Gets the fqdn of cms server. If it's not presented, the method tries to retrieve
      * the ip address of first network interface and reports it as the fqdn.
      *
