 ...
 public class QuestionFactory {
     ...
     public MultipleChoiceQuestion multipleCountAlbumsByArtistQuestionType() throws DALException {
         ...
         for (int i=0; i<4; i++) {
            ...
            answers.add(new MultipleChoiceAnswer(
                    Integer.toString(databaseLayer.getArtistAlbumDescriptors(artist).size())));
         }
        String formattedQuestion = String.format(question, winningArtist.getName());
         return new MultipleChoiceQuestion(formattedQuestion, answers.get(0), answers);
     }
     ...
     public OrderedQuestion sortAlbumsByYearQuestionType() throws DALException {
         ...
         Collection<IAlbumDescriptor> albums = getFourDifferentAlbumsByDifferentYears();
         Collection<Pair<Integer,String>> matches = new HashSet<Pair<Integer,String>>(4);
         int index = 0;
         for (IAlbumDescriptor album: albums) {
             matches.add(new Pair<Integer, String>(new Integer(album.getYear()), album.getTitle()));
             index++;
         }
         OrderedAnswer answer = new OrderedAnswer(matches);
         return new OrderedQuestion(question, answer, this.shufflePairs(matches));
     }
 
     private Collection<Pair<Integer, String>> shufflePairs(Collection<Pair<Integer, String>> matches) {
         Collection<Pair<Integer,String>> shuffledItems = new HashSet<Pair<Integer,String>>(4);
         List<Integer> leftItems = new ArrayList<Integer>(4);
         List<String> rightItems = new ArrayList<String>(4);
         for (Pair<Integer, String> pair: matches) {
             leftItems.add(pair.fst);
             rightItems.add(pair.snd);
         }
         Collections.shuffle(leftItems);
         Collections.shuffle(rightItems);
         for (int i=0; i<4; i++) {
             shuffledItems.add(new Pair<Integer, String>(leftItems.get(i), rightItems.get(i)));
         }
     ...
     public OrderedQuestion sortTracksByLengthQuestionType() throws DALException {
         ...
         int index = 0;
         for (ITrackDescriptor track: tracks) {
             matches.add(new Pair<Integer, String>(index, Integer.toString(track.getLengthInSeconds())));
             index++;
         }
         OrderedAnswer answer = new OrderedAnswer(matches);
         return new OrderedQuestion(question, answer, this.shufflePairs(matches));
     ...
     public OrderedQuestion sortTrackByOrderInAlbumQuestionType() throws DALException {
         ...
         for (ITrackDescriptor track: sortedTrackListByTrackNumber) {
             matches.add(new Pair<Integer, String>(index, track.getTitle()));
             index++;
         }
         OrderedAnswer answer = new OrderedAnswer(matches);
        return new OrderedQuestion(question, answer, this.shufflePairs(matches));
