 ...
 import jeevlet.representation.BlobRepresentation;
 import jeevlet.representation.JDomRepresentation;
 import jeevlet.representation.OutputStreamRepresentation;
 ...
 public class JeevletServiceManager {
 	...
 	public void loadProfiles(String file, String nodeId) throws Exception {
 		String webInfPath = Jeeves.Path.WEBINF;
 		if (nodeId != null && !nodeId.equals(""))
 			webInfPath = "WEB-INF-" + nodeId + "/";
 
		profilMan = new ProfileManager(null, null, appPath + webInfPath + file);
 	}
 	...
 	private Service buildService(String pack, Element clas) throws Exception {
 		...
 		if (name == null)
 			throw new IllegalArgumentException(
 					"Missing 'name' attrib in 'class' element");
 		...
 		service.init(
 				appPath,
 				new ServiceConfig(clas
 						.getChildren(ConfigFile.Class.Child.PARAM)));
 	...
 	private OutputPage buildOutputPage(String pack, Element output)
 			throws Exception {
 		OutputPage outPage = new OutputPage();
 
 		outPage.setStyleSheet(output
 				.getAttributeValue(ConfigFile.Output.Attr.SHEET));
 		outPage.setForward(output
 				.getAttributeValue(ConfigFile.Output.Attr.FORWARD));
 		outPage.setTestCondition(output
 				.getAttributeValue(ConfigFile.Output.Attr.TEST));
 		...
 		String contType = output
 				.getAttributeValue(ConfigFile.Output.Attr.CONTENT_TYPE);
 	...
 	private GuiService getGuiService(String pack, Element elem)
 			throws Exception {
 		...
 		throw new IllegalArgumentException("Unknown GUI element : "
 				+ Xml.getString(elem));
 	}
 	...
 	private ErrorPage buildErrorPage(Element err) throws Exception {
 		...
 		errPage.setStyleSheet(err
 				.getAttributeValue(ConfigFile.Error.Attr.SHEET));
 		errPage.setTestCondition(err
 				.getAttributeValue(ConfigFile.Error.Attr.ID));
 
 		// --- set content type
 
 		String contType = err
 				.getAttributeValue(ConfigFile.Error.Attr.CONTENT_TYPE);
 		...
 		// -- set status code
 		int statusCode;
 		String strStatusCode = err
 				.getAttributeValue(ConfigFile.Error.Attr.STATUS_CODE);
 	...
 	public void dispatch(JeevletServiceRequest req, UserSession session) {
 		...
 		try {
 			while (true) {
 				...
 				String forward = dispatchOutput(req, context, response,
 						outPage, srvInfo.isCacheSet());
 	...
 	private void handleError(JeevletServiceRequest req, Element response,
 			ServiceContext context, ServiceInfo srvInfo, Throwable e) {
 		...
 		try {
 			...
 			if (input == InputMethod.SOAP || output == OutputMethod.SOAP) {
 				...
 				error.setAttribute("encodingStyle",
 						"http://www.geonetwork.org/encoding/error",
 						SOAPUtil.NAMESPACE_ENV);
 
 				boolean sender = (code < 500);
 				String message = error.getChildText("class") + " : "
 						+ error.getChildText("message");
 
 				// req.write(SOAPUtil.embedExc(error, sender, id, message));
 
 				// FIXME JDomRepresentation avec MediaType pour
 				// application/soap+xml, encodage utf8 et cache http
 				MediaType mt = new MediaType("application/soap+xml");
 				JDomRepresentation rep = new JDomRepresentation(
 						SOAPUtil.embedExc(error, sender, id, message), mt,
 						cache);
 				req.setRepresentation(rep);
 			}
 
 			else if (input == InputMethod.XML || output == OutputMethod.XML
 					|| srvInfo == null) {
 				req.setStatusCode(code);
 				// req.beginStream("application/soap+xml; charset=UTF-8",
 				// cache);
 				// req.write(error);
 
 				// JDomRepresentation MediaType application/soap+xml - encodage
 				// utf8
 				JDomRepresentation rep = new JDomRepresentation(error,
 						new MediaType("application/xml"), cache);
 				req.setRepresentation(rep);
 			}
 				...
 				ErrorPage errPage = (srvInfo != null) ? srvInfo
 						.findErrorPage(id) : null;
 				...
 				try {
 					...
 					// JDomRepresentation avec encodage utf8 et cache http
 					JDomRepresentation rep = new JDomRepresentation(error,
 							cache);
 					req.setRepresentation(rep);
 				}
 	...
 	private String dispatchOutput(JeevletServiceRequest req,
 			ServiceContext context, Element response, OutputPage outPage,
 		...
 		if (outPage == null) {
 			...
 			if (response == null)
 				warning("Response is null and there is no output page for : "
 						+ req.getService());
 			else {
 				info("     -> writing xml for : " + req.getService());
 
 				// --- this logging is usefull for xml services that are called
 				// by javascript code
 				...
 				if (in == InputMethod.SOAP || out == OutputMethod.SOAP) {
 					...
 					// application/soap+xml, encodage utf8 et cache http
 					MediaType mt = new MediaType("application/soap+xml");
 					JDomRepresentation rep = new JDomRepresentation(response,
 							mt, cache);
 					req.setRepresentation(rep);
 		...
 		// --- FILE output
 			...
 			if (outPage.getContentType().equals("application/pdf")
 				...
 				Element rootElem = new Element(Jeeves.Elem.ROOT).addContent(
 						guiElem).addContent(response);
 				...
 				if (!new File(styleSheet).exists())
 					error(" -> stylesheet not found on disk, aborting : "
 							+ styleSheet);
 				else {
 					...
 					try {
 						...
 						String file = Xml.transformFOP(uploadDir, rootElem,
 								styleSheet);
 						response = BinaryFile.encode(200, file, "document.pdf",
 								true);
 						autoDeleting = true;
 					} catch (Exception e) {
 						error(" -> exception during XSL/FO transformation for : "
 								+ req.getService());
 			...
 			// BinaryFile.getContentDisposition(response);
 			// String contentLength = BinaryFile.getContentLength(response);
 			//
 			// int cl = (contentLength == null) ? -1 :
 			...
 			// FIXME FileRepresentation encodage utf8, contentDisposition,
 			// contentLength et cache http
 			String path = response.getAttributeValue("path");
 			File f = new File(path);
 			FileRepresentation rep = new FileRepresentation(f, new MediaType(
 					contentType));
 			rep.setAutoDeleting(autoDeleting);
 			req.setRepresentation(rep);
 		}
 		...
 		// --- BLOB output
 			...
 			// FIXME Representation contentLength, contentDisposition, cache
 			// http
 			BlobRepresentation rep = new BlobRepresentation(response,
 					new MediaType(contentType), cache);
 			req.setRepresentation(rep);
 		...
 		// --- HTML/XML output
 			...
 			if (req.hasDebug()) {
 				// req.beginStream("application/xml; UTF-8", cache);
 				// req.write(rootElem);
 				JDomRepresentation rep = new JDomRepresentation(rootElem, cache);
 				req.setRepresentation(rep);
 			} else {
 				// --- do an XSL transformation
 				...
 				if (!new File(styleSheet).exists())
 					...
 					try {
 						...
 						// --- then we set the content-type and output the
 						// result
 						...
 						OutputStreamRepresentation rep = new OutputStreamRepresentation(
 								baos, new MediaType(outPage.getContentType()),
 								cache);
 						req.setRepresentation(rep);
 
 					} catch (Exception e) {
 						error("   -> exception during transformation for : "
 								+ req.getService());
 						error("   ->  (C) stylesheet : " + styleSheet);
 						error("   ->  (C) message    : " + e.getMessage());
 						error("   ->  (C) exception  : "
 	...
 	private void dispatchError(JeevletServiceRequest req,
 			ServiceContext context, Element response, ErrorPage outPage,
 			boolean cache) throws Exception {
 		...
 		if (req.hasDebug()) {
 			...
 			try {
 				...
 				Xml.transform(rootElem, styleSheet, baos);
 				OutputStreamRepresentation rep = new OutputStreamRepresentation(
 						baos, new MediaType(outPage.getContentType()), cache);
 				req.setRepresentation(rep);
 			} catch (Exception e) {
 				e.printStackTrace();
 			}
 	...
 	private Element getError(JeevletServiceRequest req, Throwable t,
 			Element response) {
 		Element params = new Element(Jeeves.Elem.REQUEST).addContent(
 				new Element("language").setText(req.getLanguage())).addContent(
 				new Element("service").setText(req.getService()));
 	...
 	private void addPrefixes(Element root, String lang, String service) {
 		...
 		root.addContent(new Element(Jeeves.Elem.LOC_URL).setText(baseUrl
 				+ "/loc/" + lang));
 		root.addContent(new Element(Jeeves.Elem.BASE_SERVICE).setText(baseUrl
 				+ "/" + Jeeves.Prefix.SERVICE));
 		root.addContent(new Element(Jeeves.Elem.LOC_SERVICE).setText(baseUrl
 				+ "/" + Jeeves.Prefix.SERVICE + "/" + lang));
 	}
