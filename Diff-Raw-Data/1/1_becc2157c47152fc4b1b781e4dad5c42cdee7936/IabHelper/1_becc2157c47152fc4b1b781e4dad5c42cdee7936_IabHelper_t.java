  ...
  * automatic signature verification. After instantiating, you must perform setup
  * in order to start using the object. To perform setup, call the
  * {@link #startSetup} method and provide a listener; that listener will be
  * notified when setup is complete, after which (and not before) you may call
  * other methods. After setup is complete, you will typically want to request an
  * inventory of owned items and subscriptions. See {@link #queryInventory},
  * {@link #queryInventoryAsync} and related methods. When you are done with this
  * object, don't forget to call {@link #dispose} to ensure proper cleanup. This
  * object holds a binding to the in-app billing service, which will leak unless
  * you dispose of it correctly. If you created the object on an Activity's
  * onCreate method, then the recommended place to dispose of it is the
  * Activity's onDestroy method. A note about threading: When using this object
  * from a background thread, you may call the blocking versions of methods; when
  * using from a UI thread, call only the asynchronous versions and handle the
  * results via callbacks. Also, notice that you can only call one asynchronous
  * operation at a time; attempting to start a second asynchronous operation
  * while the first one has not yet completed will result in an exception being
  * thrown.
  * 
 ...
 public class IabHelper {
      ...
      *            This is used for verification of purchase signatures. You can
      *            find your app's base64-encoded public key in your
      *            application's page on Google Play Developer Console. Note that
      *            this is NOT your "developer public key".
      */
      ...
      *            with the purchase data when the purchase completes. This extra
      *            data will be permanently bound to that purchase and will
      *            always be returned when the purchase is queried.
      */
 ...
 /**
      ...
      *            on, regardless of ownership. Ignored if null or if
