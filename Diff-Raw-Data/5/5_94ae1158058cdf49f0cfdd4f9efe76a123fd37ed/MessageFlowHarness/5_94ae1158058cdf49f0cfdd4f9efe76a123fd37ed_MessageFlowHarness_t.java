 ...
 /** Made these static in the super class - EI - breaks my code. will see why later .*/
         ...
         catch ( Throwable exc )
         {
             throw new TckInternalError(
                 "An exception occurred while initialising RI" );
         }
         // super.getFactories();
     ...
     {
         //Source SipUri
         SipURI srcSipURI = addressFactory.createSipURI(
             null,
             srcProvider.getSipStack().getIPAddress()
             );
         srcSipURI.setPort( srcProvider.getListeningPoint().getPort() );
         srcSipURI.setTransportParam( srcProvider.getListeningPoint().
                                      getTransport() );
 
         //Destination SipURI
         SipURI dstSipURI = addressFactory.createSipURI(
             null,
             dstProvider.getSipStack().getIPAddress()
             );
         dstSipURI.setPort( dstProvider.getListeningPoint().getPort() );
         dstSipURI.setTransportParam( dstProvider.getListeningPoint().
                                      getTransport() );
         //CallId
         ...
         ViaHeader viaHeader = headerFactory.createViaHeader(
             ...
             Long.toString(System.currentTimeMillis())//branch id
         );
         via.add(viaHeader);
         MaxForwardsHeader maxForwards = headerFactory.createMaxForwardsHeader(3);
 
         Request request =
             messageFactory.createRequest( dstSipURI,
                                           method,
                                           callId,
                                           cSeq,
                                           from,
              ...
              && content != null )
         {
             ContentTypeHeader contentTypeHdr =
                 headerFactory.createContentTypeHeader( contentType,
                 contentSubType );
     ...
     protected Request createRiInviteRequest( String contentType,
                                                 String contentSubType,
                                                 Object content ) throws TckInternalError
     {
         try
         {
             return createRequest( Request.INVITE, riAddressFactory,
                                   riHeaderFactory,
                                   riMessageFactory, riSipProvider,
                                   tiSipProvider, contentType,
     ...
     protected Request createTiInviteRequest( String contentType,
                                                 String contentSubType,
                                                 Object content ) throws TiUnexpectedError
     {
         try
         {
             return createRequest( Request.INVITE, tiAddressFactory,
                                   tiHeaderFactory,
                                   tiMessageFactory, tiSipProvider,
                                   riSipProvider,
                                   contentType,
                                   contentSubType, content );
         }
      ...
      * @throws TckInternalError if anything should gou wrong.
      */
     protected Request createRiRegisterRequest( )
      ...
      * @throws TiUnexpectedError if anything should gou wrong.
      */
     protected Request createTiRegisterRequest( )
     ...
     */
 	...
 	ContactHeader contact = tiHeaderFactory.createContactHeader();
        	SipURI srcSipURI = tiAddressFactory.createSipURI(
           	   null, tiSipProvider.getSipStack().getIPAddress()
         );
         srcSipURI.setPort( tiSipProvider.getListeningPoint().getPort() );
         srcSipURI.setTransportParam( tiSipProvider.getListeningPoint().
                                      getTransport() );
     ...
     */
 	...
 	ContactHeader contact = riHeaderFactory.createContactHeader();
        	SipURI srcSipURI = tiAddressFactory.createSipURI(
           	   null, riSipProvider.getSipStack().getIPAddress()
         );
         srcSipURI.setPort( riSipProvider.getListeningPoint().getPort() );
         srcSipURI.setTransportParam( riSipProvider.getListeningPoint().
                                      getTransport() );
	...
	//properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL","32");
 	...
 	properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL","32");
 	properties.setProperty("gov.nist.javax.sip.DEBUG_LOG","tiDebugLog.txt");
 	properties.setProperty("gov.nist.javax.sip.SERVER_LOG",
 				"tiMessageLog.txt");
