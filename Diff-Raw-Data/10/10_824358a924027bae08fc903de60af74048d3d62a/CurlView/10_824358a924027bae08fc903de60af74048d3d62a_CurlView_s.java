 ...
 public class CurlView extends GLSurfaceView implements View.OnTouchListener,
 	...
 	public void onScale(float scaleFactor) {
 		...
 		Log.d(TAG, "Scale to factor: " + this.scaleFactor);
 		this.requestRender();
 	}
 
 	float scaleFactor = 1f;
 	private boolean curlStarted = false;
 	private float curlStartX = 0;
 	private boolean isScaled = false;
 	private PointF translateStartP = new PointF();
 	...
 	public boolean onTouch(View view, MotionEvent me) {
 		...
 		case MotionEvent.ACTION_DOWN: {
 			if (isScaled) {
 				translateStartP.set(me.getX(), me.getY());
 				this.mRenderer.setTranslateStartP(translateStartP);
 			}
 			if (!isScaled) {
 				curlStartX = me.getX();
 			}
 		...
 		case MotionEvent.ACTION_MOVE: {
 			...
 			if (!curlStarted && !isScaling) {
 				if (((me.getX() - curlStartX) > 2) && mCurrentIndex > 0) {
 					// Log.d(TAG, "right");
 					// direction is right
 					mDragStartPos.x = rightRect.left;
 					startCurl(CURL_LEFT);
 	...
 	public CurlRenderer getmRenderer() {
 		return mRenderer;
 	}
 
 	public void setmRenderer(CurlRenderer mRenderer) {
