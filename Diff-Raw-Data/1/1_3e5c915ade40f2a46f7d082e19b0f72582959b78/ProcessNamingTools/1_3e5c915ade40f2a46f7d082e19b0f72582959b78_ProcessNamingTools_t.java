 ...
 public class ProcessNamingTools {
 	...
 	public ProcessNamingTools(EditingDomain editingDomain){
 		...
 		this.diagramStore = (DiagramRepositoryStore) RepositoryManager.getInstance().getRepositoryStore(DiagramRepositoryStore.class);
 	}
 	...
 	public void changeProcessNameAndVersion(final AbstractProcess process,final String name, final String version) {
 		...
 		if(process instanceof MainProcess){
			editingDomain.getCommandStack().execute(SetCommand.create(editingDomain, process, ProcessPackage.Literals.ABSTRACT_PROCESS__AUTHOR, System.getProperty("user.name", "Unknown")));
 	...
 	public void proceedForPools(final Element pool,final String newPoolName,final String oldPoolName,final String oldVersion,final String newVersion) {
