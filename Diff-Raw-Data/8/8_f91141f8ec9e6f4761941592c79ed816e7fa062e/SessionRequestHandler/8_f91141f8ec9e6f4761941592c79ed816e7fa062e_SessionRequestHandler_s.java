 ...
 * $Id: SessionRequestHandler.java,v 1.8 2009-02-19 05:39:25 bhavnab Exp $
 ...
 public class SessionRequestHandler implements RequestHandler {
     ...
     private Response processRequest(Request req,
         ...
         SessionResponse sres = new SessionResponse(sreq.getRequestID(), sreq
                 .getMethodID());
         ...
         try {
             ...
             SessionService.sessionDebug.error(
                     "SessionRequestHandler encounterd exception", e);
     ...
     private SessionResponse processSessionRequest(SessionRequest req,
         ...
         try {
             ...
             case SessionRequest.GetSessionCount:
                  ...
                  */
                 if (requesterSession
                         .getProperty(Session.TOKEN_RESTRICTION_PROP) != null) {
             ...
             case SessionRequest.DestroySession:
                 ...
                 if (!sessionService.isSessionFailoverEnabled()) {
                     ...
                     if (SessionService.getUseInternalRequestRouting()) {
                         ...
                         if (!sessionService.isLocalServer(hostServerID)) {
                             try {
                                 return forward(Session
                                         .getSessionServiceURL(hostServerID),
                                         req);
                                 ...
                                 if (!sessionService.checkServerUp(hostServerID))
                                 {
                                     ...
                                     if (retryHostServerID.equals(hostServerID))
                                     {
                                         ...
                                         // case
                                         if (!sessionService.isLocalServer(
                                                 retryHostServerID)) 
                                         {
                                             return 
                                               forward(
                         ...
                         // we check that both OpenSSO session and
                         // HTTP session cookies were enclosed in the request
                         // If they were then LB must have routed to the proper
                         // server
                         // instance and we must treat it as a session recovery
                         // case
                         // If any of the cookies missing or do not match the sid
                         // in
                         // the message we assume that request was misrouted and
                         // correct it
                         // by forwarding via LB with all cookies enclosed
 
                         String isSessionCookie = CookieUtils
                                 .getCookieValueFromReq(servletRequest, Session
                                         .getCookieName());
                         String httpCookie = CookieUtils.getCookieValueFromReq(
                                 servletRequest, SessionService
                                         .getHttpSessionTrackingCookieName());
 
                         if (!sessionService.isSessionPresent(sid)
                                 && (isSessionCookie == null
                                         || !isSessionCookie.equals(sid
                                                 .toString())
                                         || httpCookie == null || !httpCookie
                                         .equals(sid.getTail()))) {
                             return forward(Session.getSessionServiceURL(sid),
                                     req);
                         }
                     }
                     /*
             ...
             switch (req.getMethodID()) {
             case SessionRequest.GetSession:
                 res.addSessionInfo(sessionService.getSessionInfo(sid, req
                         .getResetFlag()));
                 break;
             ...
             case SessionRequest.DestroySession:
                 sessionService.destroySession(requesterSession, new SessionID(
                         req.getDestroySessionID()));
                 break;
             ...
             case SessionRequest.AddSessionListener:
                 sessionService
                         .addSessionListener(sid, req.getNotificationURL());
                 break;
             ...
             case SessionRequest.AddSessionListenerOnAllSessions:
                 ...
                 if (getEnableAddListenerOnAllSessions()) {
                     sessionService.addSessionListenerOnAllSessions(
                             requesterSession, req.getNotificationURL());
                 }
                 break;
