 ...
 public class PhonemiseDenglish
 {
 		...
 		*/
 	 	...
 	 	* @param currentResult
 	 	* @param allowOtherLanguage whether to allow component words from other language in compound analysis
 		...
 		private String compoundAnalysis(Word word, Result currentResult, boolean allowOtherLanguage) {
 			...
 			for (int i=word.getToBePhonemised().length()-2; i >= 2; i--) { //-3!!! >= 3!!!
 				...
 				getEndingsAndAffixes("noun_genitive_accusative_and_plural_endings");//should be 's' and 'n' for german
 ...
 //					Or does it help if we cut off a Fuge?
 					if (restPhon == null) {
 						...
 						if (helper != null && helper.length == 2) {
 							...
 							if (restPhon == null) restPhon = compoundAnalysis(new Word(rest2), currentResult, allowOtherLanguage);
 						}
 					}
 					...
 					if (restPhon == null) restPhon = compoundAnalysis(new Word(rest), currentResult, allowOtherLanguage);
 		 ...
 		 * @return a two-item String array. First string is the trannscnscription of the
 		 * Fuge found, second is the suffix after the Fuge was removed. Returns
 		 * null if no Fuge was found.
 		 */
 		...
 		* @param toBePhonemised the input word
 		* @param endings endings from language specific ending list
 		* @return when valid flection ending is found, returns a string array of
 		* two elements (first is stem, second is ending); else, returns null.
 		*/
 				...
 				if (word.getOtherLanguageBaseForm() == null) {
 					for (int j = 1; j<3; j++) {//chop off 1-3 chars from end of word
 						...
 						if (knowEnBaseForm(wordMinusFlectionEnding.substring(0, wordMinusFlectionEnding.length()-j))) {
 							...
 							&& !(isShortParticipleBaseEnding(wordMinusFlectionEndingUltimateChar, participleBaseShort) && //to force that sth like 'cha>tt<en' is correctly 
 							isShortParticipleBaseEnding(wordMinusFlectionEndingPenultimateChar, participleBaseShort))) {//processed in geminate clause
 		...
 		* @param enBaseForm: the english infinitive as found in english lexicon
 				...
 				if (word.getFlectionEnding() != null) {
 					if (endingTranscriptionLookup(word.getFlectionEnding()) != null) {
 						...
 						&& !(word.getCouldBeParticiple())) {
 							...
 							if (word.getCouldBeParticiple() && isShortSuperlative(word.getFlectionEnding()) && word.getExtraSyll()) {//i.e. downgeloadetsten
 								result = newEnTranscription+flectionFugeTrans
 								+participleBaseShortEndingTrans+endingTranscription;
 								logger.debug("(1)");
 							} else if (word.getCouldBeParticiple() && word.getCouldBeParticipleInBaseForm() && word.getExtraSyll()) {//scrollet or downloadet
 								...
 								+participleBaseShortEndingTrans+endingTranscription;
 								logger.debug("(4)");
 							} else if (word.getCouldBeParticiple() && isShortSuperlative(word.getFlectionEnding())) {//i.e. gescrolltstem
 								result = otherLanguageTranscription
 								+participleBaseShortEndingTrans+endingTranscription;
 								logger.debug("(5)");
 								...
 								if (word.getIsVerbalGerund()) {
 									...
 									if (isShortSuperlative(word.getFlectionEnding())) {
 										...
 										if (word.getExtraSyll()) {//means: word ends on 't' or 'd'
 											...
 											if (endingContainsVowel(word.getFlectionEnding()) && (!(endingBeginsWithVowel(word.getFlectionEnding())))) {
 												result = otherLanguageTranscription+"-"+endingTranscription;
 											} else {
 												if (endingContainsVowel(word.getFlectionEnding()) && endingBeginsWithVowel(word.getFlectionEnding())) {
 													result = newEnTranscription+endingTranscription;
 		...
 		*/
 		...
 		/**
 		 * Checks if input string is a gerund - means: input ends with 'end'
 		 * Sets global var isVerbalGerund which is important for
 		...
 		**/
 		...
 		private String prefixLexiconLookup(String s) {
 			 ...
 			 * @param Description key of the affixes/endings you want to get, i.e. superlative_short
