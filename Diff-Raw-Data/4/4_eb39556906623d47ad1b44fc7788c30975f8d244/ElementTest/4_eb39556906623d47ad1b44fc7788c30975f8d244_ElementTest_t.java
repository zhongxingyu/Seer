 ...
 public class ElementTest extends TestCase
 {
 	...
 	private void doTestAction( LabelHandle handle ) throws SemanticException,
 			ScriptException
 	{
 		...
 		assertEquals( DesignChoiceConstants.ACTION_FORMAT_TYPE_HTML, action
 				.getFormatType( ) );
 	...
 	private void doTestReportElement( IReportElement element )
 	...
 	public void testDataBinding( ) throws ScriptException, SemanticException
 	{
 		...
 		ComputedColumn column = StructureFactory.newComputedColumn(
 				tableHandle, "column1" );//$NON-NLS-1$
 		column.setExpression( "row[\"123\"]" );//$NON-NLS-1$
 		tableHandle.addColumnBinding( column, true );
 
 		ComputedColumn column2 = StructureFactory.newComputedColumn(
 				tableHandle, "column2" );//$NON-NLS-1$
 	...
 	/**
 	 * Tests addColumnBinding , addHighLightRule , addHideRule ,
 	...
 	public void testAddFunction( ) throws SemanticException, ScriptException
 	{
 		...
 		IFilterCondition filter = StructureScriptAPIFactory
 	...
 	public void testFilterCondition( ) throws SemanticException,
 			ScriptException
 	{
 		TableHandle tableHandle = factory.newTableItem( "table", 3 );//$NON-NLS-1$
 		designHandle.getBody( ).add( tableHandle );
 	...
 	public void testHideRule( ) throws SemanticException, ScriptException
 	{
 		...
 		HideRuleHandle hideHandle = (HideRuleHandle) tableHandle
 				.getPropertyHandle( "visibility" ).addItem( hide );//$NON-NLS-1$
 	...
 	{
 		DataSourceHandle dataSourceHandle = factory.newOdaDataSource(
 				DATA_SOURCE, null );
 	...
 	public void testDynamicText( ) throws ScriptException
 	{
 		...
 		assertEquals( DesignChoiceConstants.TEXT_CONTENT_TYPE_HTML, dynamicText
 				.getContentType( ) );
 	...
 	public void testImage( ) throws ScriptException
 	{
 		...
 		assertEquals( DesignChoiceConstants.IMAGE_SIZE_SCALE_TO_ITEM, image
 				.getSize( ) );
 	...
 	public void testStyle( ) throws ScriptException
 	{
 		...
 		style
 				.setBackgroundRepeat( DesignChoiceConstants.BACKGROUND_REPEAT_REPEAT );
 		assertEquals( DesignChoiceConstants.BACKGROUND_REPEAT_REPEAT, style
 				.getBackgroundRepeat( ) );
 		...
 		assertEquals( DesignChoiceConstants.LINE_STYLE_DOTTED, style
 				.getBorderLeftStyle( ) );
 		...
 		assertEquals( DesignChoiceConstants.LINE_STYLE_DASHED, style
 				.getBorderRightStyle( ) );
 		...
 		assertEquals( DesignChoiceConstants.LINE_STYLE_DOUBLE, style
 				.getBorderTopStyle( ) );
 		...
 		assertEquals( DesignChoiceConstants.DATETIEM_FORMAT_TYPE_MUDIUM_DATE,
 				style.getDateTimeFormatCategory( ) );
 		...
 		assertEquals( DesignChoiceConstants.FONT_FAMILY_SANS_SERIF, style
 				.getFontFamily( ) );
 		...
 		assertEquals( DesignChoiceConstants.NUMBER_FORMAT_TYPE_CURRENCY, style
 				.getNumberFormat( ) );
 
 		style
 				.setNumberFormatCategory( DesignChoiceConstants.NUMBER_FORMAT_TYPE_PERCENT );
 		assertEquals( DesignChoiceConstants.NUMBER_FORMAT_TYPE_PERCENT, style
 		...
 		style
 				.setStringFormatCategory( DesignChoiceConstants.STRING_FORMAT_TYPE_PHONE_NUMBER );
 		assertEquals( DesignChoiceConstants.STRING_FORMAT_TYPE_PHONE_NUMBER,
 				style.getStringFormatCategory( ) );
 
 		style.setTextAlign( DesignChoiceConstants.TEXT_ALIGN_RIGHT );
 		assertEquals( DesignChoiceConstants.TEXT_ALIGN_RIGHT, style
 				.getTextAlign( ) );
 		...
 		assertEquals( DesignChoiceConstants.TEXT_LINE_THROUGH_LINE_THROUGH,
 				style.getTextLineThrough( ) );
 
 		style.setTextOverline( DesignChoiceConstants.TEXT_OVERLINE_OVERLINE );
 		assertEquals( DesignChoiceConstants.TEXT_OVERLINE_OVERLINE, style
 				.getTextOverline( ) );
 
 		style.setTextTransform( DesignChoiceConstants.TRANSFORM_LOWERCASE );
 		assertEquals( DesignChoiceConstants.TRANSFORM_LOWERCASE, style
 				.getTextTransform( ) );
 
 		style.setTextUnderline( DesignChoiceConstants.TEXT_UNDERLINE_UNDERLINE );
 		assertEquals( DesignChoiceConstants.TEXT_UNDERLINE_UNDERLINE, style
 				.getTextUnderline( ) );
 
 		style.setVerticalAlign( DesignChoiceConstants.VERTICAL_ALIGN_TOP );
 		assertEquals( DesignChoiceConstants.VERTICAL_ALIGN_TOP, style
 				.getVerticalAlign( ) );
 
 		style.setWhiteSpace( DesignChoiceConstants.WHITE_SPACE_NOWRAP );
 		assertEquals( DesignChoiceConstants.WHITE_SPACE_NOWRAP, style
 				.getWhiteSpace( ) );
 	...
 	public void testTextItem( ) throws ScriptException
 	{
 		...
 		assertEquals( DesignChoiceConstants.TEXT_CONTENT_TYPE_HTML, text
 				.getContentType( ) );
 	}
