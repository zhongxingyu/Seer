 ...
 public final class Utils {
 	...
 	/**
 	 * Check first for a particular property in the System Properties, so that the
 	 * -Dprop="value" command line arg mechanism can be used to override values
 	 * defined in the passed in property file. This is especially useful for
 	 * defining the marc.source property to define which file to operate on, in a
 	 * shell script loop.
 	 * 
 	 * @param props
 	 ...
 	 * @returns String - value stored for that property (or null if it doesn't
 	 *          exist)
 	 */
 	 ...
 	 * @returns String - value stored for that property (or the if it doesn't
 	 *          exist)
 	 */
	...
	public static synchronized String cleanDate(final String date) {
 	 ...
 	 *          Pattern class). Note that the regular expression should NOT have
 	 *          '$' at the end. (e.g. " *[,/;:]" replaces any commas, slashes,
 	 *          semicolons or colons at the end of the string, and these chars may
 	 *          optionally be preceded by a space)
 	...
 	/**
 	 * Removes trailing characters indicated in regular expression, PLUS trailing
 	 * period if it is preceded by its regular expression.
 	 * 
 	 * @param origStr
 	 *          String to clean
 	 * @param trailingCharsRegEx
 	...
 	/**
 	 * Remove the characters per the regular expression if they are at the end of
 	 * the string.
 	 * 
 	 * @param origStr
 	 *          string to be cleaned
 	 * @param charsToReplaceRegEx
 	 *          - a regular expression of the trailing string/chars to be removed
 	 *          e.g. " *([,/;:])" meaning last character is a comma, slash,
 	 *          semicolon, colon, possibly preceded by one or more spaces.
 	 ...
 	 * then: if "displayRawIfMissing" is a key in the map, then the raw field
 	 * value is used. if "displayRawIfMissing" is not a key in the map, and the
 	 * allowDefault param is set to true, then if the map contains "__DEFAULT" as
 	 * a key, the value of "__DEFAULT" in the map is used; if allowDefault is true
 	 * and there is neither "displayRawIfMissing" nor "__DEFAULT", as a key in the
 	 * map, then if the map contains an empty key, the map value of the empty key
 	 ...
 	 * @param separator
 	 *          Separation character to put between
 	...
 	/**
 	 * returns true if the 3 letter language code is for a right to left language
 	 * (one written in arabic or hebrew characters)
 	 * 
 	 * @param langcode
 	 * @return
 	 */
 	public final static boolean isRightToLeftLanguage(String langcode) {
 		...
 		// hebrew characters
 				langcode.equals("heb") || langcode.equals("yid") || langcode.equals("lad") || langcode.equals("jpr") || langcode.equals("jrb"))
 			return true;
 		else
 			return false;
 	}
 	 ...
 	 * 
 	 * @param str
 	 * @return if an unescaped open parenthesis occurs within this object, return
 	 *         the index of the first open paren; -1 if no unescaped open paren.
 	 */
 	...
 	/**
 	 * return the index within this string of the first occurrence of a comma that
 	 * isn't escaped with a backslash.
 	 * 
 	 * @param str
 	...
 	public static Set<String> returnValidISBNs(Set<String> candidates) {
 		// NOTE 1: last digit of ISBN is a check digit and may be "X"
 		...
 		while (iter.hasNext()) {
 			...
 			else if (p10.matcher(value).matches() && !p13any.matcher(value).matches()) isbnSet.add(value.substring(0, 10));
 		}
 		return isbnSet;
 	}
 
 	/**
 	 * For each occurrence of a marc field in the tags list, extract all subfield
 	 * data from the field, place it in a single string (individual subfield data
 	 * separated by spaces) and add the string to the result set.
 	 */
 	 ...
 	 * 
 	 * @param df
 	 *          - DataField of interest
 	 * @param code
 	 *          - code of subfield of interest
 	...
 	/**
 	 * given a latin letter with a diacritic, return the latin letter without the
 	 * diacritic. Shamelessly stolen from UnicodeCharUtil class of
 	 * UnicodeNormalizeFilter by Bob Haschart
 	 */
 	public static char foldDiacriticLatinChar(char c) {
 		switch (c) {
 		case 0x0181:
 			return (0x0042); // LATIN CAPITAL LETTER B WITH HOOK -> LATIN CAPITAL
 												// LETTER B
 		case 0x0182:
 			return (0x0042); // LATIN CAPITAL LETTER B WITH TOPBAR -> LATIN CAPITAL
 												// LETTER B
 		case 0x0187:
 			return (0x0043); // LATIN CAPITAL LETTER C WITH HOOK -> LATIN CAPITAL
 												// LETTER C
 		case 0x0110:
 			return (0x0044); // LATIN CAPITAL LETTER D WITH STROKE -> LATIN CAPITAL
 												// LETTER D
 		case 0x018A:
 			return (0x0044); // LATIN CAPITAL LETTER D WITH HOOK -> LATIN CAPITAL
 												// LETTER D
 		case 0x018B:
 			return (0x0044); // LATIN CAPITAL LETTER D WITH TOPBAR -> LATIN CAPITAL
 												// LETTER D
 		case 0x0191:
 			return (0x0046); // LATIN CAPITAL LETTER F WITH HOOK -> LATIN CAPITAL
 												// LETTER F
 		case 0x0193:
 			return (0x0047); // LATIN CAPITAL LETTER G WITH HOOK -> LATIN CAPITAL
 												// LETTER G
 		case 0x01E4:
 			return (0x0047); // LATIN CAPITAL LETTER G WITH STROKE -> LATIN CAPITAL
 												// LETTER G
 		case 0x0126:
 			return (0x0048); // LATIN CAPITAL LETTER H WITH STROKE -> LATIN CAPITAL
 												// LETTER H
 		case 0x0197:
 			return (0x0049); // LATIN CAPITAL LETTER I WITH STROKE -> LATIN CAPITAL
 												// LETTER I
 		case 0x0198:
 			return (0x004B); // LATIN CAPITAL LETTER K WITH HOOK -> LATIN CAPITAL
 												// LETTER K
 		case 0x0141:
 			return (0x004C); // LATIN CAPITAL LETTER L WITH STROKE -> LATIN CAPITAL
 												// LETTER L
 		case 0x019D:
 			return (0x004E); // LATIN CAPITAL LETTER N WITH LEFT HOOK -> LATIN CAPITAL
 												// LETTER N
 		case 0x0220:
 			return (0x004E); // LATIN CAPITAL LETTER N WITH LONG RIGHT LEG -> LATIN
 												// CAPITAL LETTER N
 		case 0x00D8:
 			return (0x004F); // LATIN CAPITAL LETTER O WITH STROKE -> LATIN CAPITAL
 												// LETTER O
 		case 0x019F:
 			return (0x004F); // LATIN CAPITAL LETTER O WITH MIDDLE TILDE -> LATIN
 												// CAPITAL LETTER O
 		case 0x01FE:
 			return (0x004F); // LATIN CAPITAL LETTER O WITH STROKE AND ACUTE -> LATIN
 												// CAPITAL LETTER O
 		case 0x01A4:
 			return (0x0050); // LATIN CAPITAL LETTER P WITH HOOK -> LATIN CAPITAL
 												// LETTER P
 		case 0x0166:
 			return (0x0054); // LATIN CAPITAL LETTER T WITH STROKE -> LATIN CAPITAL
 												// LETTER T
 		case 0x01AC:
 			return (0x0054); // LATIN CAPITAL LETTER T WITH HOOK -> LATIN CAPITAL
 												// LETTER T
 		case 0x01AE:
 			return (0x0054); // LATIN CAPITAL LETTER T WITH RETROFLEX HOOK -> LATIN
 												// CAPITAL LETTER T
 		case 0x01B2:
 			return (0x0056); // LATIN CAPITAL LETTER V WITH HOOK -> LATIN CAPITAL
 												// LETTER V
 		case 0x01B3:
 			return (0x0059); // LATIN CAPITAL LETTER Y WITH HOOK -> LATIN CAPITAL
 												// LETTER Y
 		case 0x01B5:
 			return (0x005A); // LATIN CAPITAL LETTER Z WITH STROKE -> LATIN CAPITAL
 												// LETTER Z
 		case 0x0224:
 			return (0x005A); // LATIN CAPITAL LETTER Z WITH HOOK -> LATIN CAPITAL
 												// LETTER Z
 		case 0x0180:
 			return (0x0062); // LATIN SMALL LETTER B WITH STROKE -> LATIN SMALL LETTER
 												// B
 		case 0x0183:
 			return (0x0062); // LATIN SMALL LETTER B WITH TOPBAR -> LATIN SMALL LETTER
 												// B
 		case 0x0253:
 		...
 		case 0x0111:
 			return (0x0064); // LATIN SMALL LETTER D WITH STROKE -> LATIN SMALL LETTER
 												// D
 		case 0x018C:
 			return (0x0064); // LATIN SMALL LETTER D WITH TOPBAR -> LATIN SMALL LETTER
 												// D
 		case 0x0221:
 		...
 		case 0x01E5:
 			return (0x0067); // LATIN SMALL LETTER G WITH STROKE -> LATIN SMALL LETTER
 												// G
 		case 0x0260:
 			return (0x0067); // LATIN SMALL LETTER G WITH HOOK -> LATIN SMALL LETTER G
 		case 0x0127:
 			return (0x0068); // LATIN SMALL LETTER H WITH STROKE -> LATIN SMALL LETTER
 												// H
 		case 0x0266:
 			return (0x0068); // LATIN SMALL LETTER H WITH HOOK -> LATIN SMALL LETTER H
 		case 0x0268:
 			return (0x0069); // LATIN SMALL LETTER I WITH STROKE -> LATIN SMALL LETTER
 												// I
 		case 0x029D:
 			return (0x006A); // LATIN SMALL LETTER J WITH CROSSED-TAIL -> LATIN SMALL
 												// LETTER J
 		case 0x0199:
 			return (0x006B); // LATIN SMALL LETTER K WITH HOOK -> LATIN SMALL LETTER K
 		case 0x0142:
 			return (0x006C); // LATIN SMALL LETTER L WITH STROKE -> LATIN SMALL LETTER
 												// L
 		case 0x019A:
 		...
 		case 0x026B:
 			return (0x006C); // LATIN SMALL LETTER L WITH MIDDLE TILDE -> LATIN SMALL
 												// LETTER L
 		case 0x026C:
 			return (0x006C); // LATIN SMALL LETTER L WITH BELT -> LATIN SMALL LETTER L
 		case 0x026D:
 			return (0x006C); // LATIN SMALL LETTER L WITH RETROFLEX HOOK -> LATIN
 												// SMALL LETTER L
 		case 0x0271:
 			return (0x006D); // LATIN SMALL LETTER M WITH HOOK -> LATIN SMALL LETTER M
 		case 0x019E:
 			return (0x006E); // LATIN SMALL LETTER N WITH LONG RIGHT LEG -> LATIN
 												// SMALL LETTER N
 		case 0x0235:
 			return (0x006E); // LATIN SMALL LETTER N WITH CURL -> LATIN SMALL LETTER N
 		case 0x0272:
 			return (0x006E); // LATIN SMALL LETTER N WITH LEFT HOOK -> LATIN SMALL
 												// LETTER N
 		case 0x0273:
 			return (0x006E); // LATIN SMALL LETTER N WITH RETROFLEX HOOK -> LATIN
 												// SMALL LETTER N
 		case 0x00F8:
 			return (0x006F); // LATIN SMALL LETTER O WITH STROKE -> LATIN SMALL LETTER
 												// O
 		case 0x01FF:
 			return (0x006F); // LATIN SMALL LETTER O WITH STROKE AND ACUTE -> LATIN
 												// SMALL LETTER O
 		case 0x01A5:
 		...
 		case 0x027C:
 			return (0x0072); // LATIN SMALL LETTER R WITH LONG LEG -> LATIN SMALL
 												// LETTER R
 		case 0x027D:
 		...
 		case 0x0167:
 			return (0x0074); // LATIN SMALL LETTER T WITH STROKE -> LATIN SMALL LETTER
 												// T
 		case 0x01AB:
 			return (0x0074); // LATIN SMALL LETTER T WITH PALATAL HOOK -> LATIN SMALL
 												// LETTER T
 		case 0x01AD:
 		...
 		case 0x0288:
 			return (0x0074); // LATIN SMALL LETTER T WITH RETROFLEX HOOK -> LATIN
 												// SMALL LETTER T
 		case 0x028B:
 		...
 		case 0x01B6:
 			return (0x007A); // LATIN SMALL LETTER Z WITH STROKE -> LATIN SMALL LETTER
 												// Z
 		case 0x0225:
 			return (0x007A); // LATIN SMALL LETTER Z WITH HOOK -> LATIN SMALL LETTER Z
 		case 0x0290:
 			return (0x007A); // LATIN SMALL LETTER Z WITH RETROFLEX HOOK -> LATIN
 												// SMALL LETTER Z
 		case 0x0291:
