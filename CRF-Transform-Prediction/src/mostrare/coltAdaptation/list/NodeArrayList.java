...
import mostrare.coltAdaptation.function.NodeProcedure;
import mostrare.tree.Node;
import cern.colt.list.AbstractList;

/**
 * Resizable list holding <code>Node</code> elements; implemented with arrays. This class is an
 * adaptation of <code>cern.colt.list.ObjectArrayList</code> of the colt API.
 * 
 * @author missi
 */
public class NodeArrayList extends AbstractList
{
	/**
	 * 
	 */
	...
	protected boolean			defaultInc			= true;
	 ...
	 */
	public NodeArrayList(Node[] elements)
	{
		elements(elements);
	}
	 ...
	 */
	public NodeArrayList(int initialCapacity)
	{
		this(new Node[initialCapacity]);
		size = 0;
	}

	public void setDefaultInc(boolean defaultInc)
	{
	 ...
	 */
	public void addAllOfFromTo(NodeArrayList other, int from, int to)
	{
	 ...
	 */
	public void beforeInsert(int index, Node element)
	{
	 ...
	 *                index is out of range (<code>index &lt; 0 || index &gt; size()</code>).
	 */
	public void beforeInsertAllOfFromTo(int index, NodeArrayList other, int from, int to)
	{
	 ...
	 * multiple elements equal to the specified Node, there is no guarantee which instance will be
	 * found.
	 * 
	 * @param key
	 ...
	 */
	public int binarySearch(Node key)
	{
	 ...
	 * @see java.util.Arrays
	 */
	public int binarySearchFromTo(Node key, int from, int to)
	{
		int low = from;
		int high = to;
		...
		{
			int mid = (low + high) / 2;
			Node midVal = elements[mid];
			int cmp = ((Comparable<Node>) midVal).compareTo(key);
	 ...
	 * an infinite loop. If the receiver contains multiple elements equal to the specified Node,
	 ...
	 */
	public int binarySearchFromTo(Node key, int from, int to, java.util.Comparator comparator)
	{
	 ...
	 * elements, but do not share the same array to index them; So modifying an Node in the copy
	 * modifies the Node in the receiver and vice versa; However, structurally modifying the copy
	 * (for example changing its size, setting other Nodes at indexes, etc.) does not affect the
	...
	public Object clone()
	{
		NodeArrayList v = (NodeArrayList) super.clone();
		v.elements = (Node[]) elements.clone();
		return v;
	}
	 ...
	 */
	public boolean contains(Node elem, boolean testForEquality)
	{
	 ...
	 * the same array to index them; So modifying an Node in the copy modifies the Node in the
	 * receiver and vice versa; However, structurally modifying the copy (for example changing its
	 * size, setting other Nodes at indexes, etc.) does not affect the receiver and vice versa.
	 * 
	 * @return a copy of the receiver.
	 */
	public NodeArrayList copy()
	{
		return (NodeArrayList) clone();
	}
	 ...
	 */
	public void delete(Node element, boolean testForEquality)
	{
	 ...
	 */
	public NodeArrayList elements(Node[] elements)
	{
	...
	{
		elements = mostrare.coltAdaptation.ArraysForNode.ensureCapacity(elements, minCapacity,defaultInc);
	}

	/**
	 * Compares the specified Node with the receiver for equality. Returns true if and only if the
	 * specified Node is also an NodeArrayList, both lists have the same size, and all corresponding
	 ...
	 * 
	 * @param otherObj
	 *            the Node to be compared for equality with the receiver.
	 * @return true if the specified Node is equal to the receiver.
	 */
	 ...
	 * @return true if the specified Node is equal to the receiver.
	 */
	public boolean equals(Node otherObj, boolean testForEquality)
	{ // delta
		if (!(otherObj instanceof NodeArrayList))
		{
			return false;
		}
		if (this == otherObj)
		...
		if (otherObj == null)
			return false;
		NodeArrayList other = (NodeArrayList) otherObj;
		if (elements == other.elements())
			return true;
		if (size != other.size())
			return false;

		Node[] otherElements = other.elements();
		Node[] theElements = elements;
		if (!testForEquality)
		{
			for (int i = size; --i >= 0;)
			{
	 ...
	 */
	public void fillFromToWith(int from, int to, Node val)
	{
	 ...
	 */
	public boolean forEach(NodeProcedure procedure)
	{
		Node[] theElements = elements;
		int theSize = size;
	 ...
	 *            index of element to return.
	 */
	public Node getQuick(int index)
	{
		return elements[index];
	}
	 ...
	 */
	public int indexOf(Node element, boolean testForEquality)
	{
	 ...
	 *                to&gt;=size())</code>).
	 */
	public int indexOfFromTo(Node element, int from, int to, boolean testForEquality)
	{
		if (size == 0)
			return -1;
	 ...
	 *         <code>-1</code> if the element is not found.
	 */
	public int lastIndexOf(Node element, boolean testForEquality)
	{
	 ...
	 *                index is out of range (<code>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</code>).
	 */
	public int lastIndexOfFromTo(Node element, int from, int to, boolean testForEquality)
	{
		if (size == 0)
			return -1;
		checkRangeFromTo(from, to, size);
	 ...
	 * @see Comparator
	 */
	public void mergeSortFromTo(int from, int to, java.util.Comparator<Node> c)
	{
		if (size == 0)
			return;
		checkRangeFromTo(from, to, size);
	 ...
	 *                index is out of range (<code>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</code>).
	 */
	public NodeArrayList partFromTo(int from, int to)
	{
		if (size == 0)
			return new NodeArrayList(0);

		checkRangeFromTo(from, to, size);

		Node[] part = new Node[to - from + 1];
		System.arraycopy(elements, from, part, 0, to - from + 1);
		return new NodeArrayList(part);
	}
	 ...
	 */
	public boolean removeAll(NodeArrayList other, boolean testForEquality)
	{
		if (other.size == 0)
	 ...
	 */
	public void replaceFromToWithFrom(int from, int to, NodeArrayList other, int otherFrom)
	{
		int length = to - from + 1;
		if (length > 0)
		{
	 ...
	 * </pre>
	 */
	public void replaceFromToWithFromTo(int from, int to, NodeArrayList other, int otherFrom,
			int otherTo)
	{
	...
	public void replaceFromWith(int from, java.util.Collection other)
	{
		...
		for (int i = 0; i < limit; i++)
			elements[index++] = (Node) e.next(); // delta
	}
	 ...
	 * @return <code>true</code> if the receiver changed as a result of the call.
	 */
	public boolean retainAll(NodeArrayList other, boolean testForEquality)
	{
		if (other.size == 0)
		{
			if (size == 0)
	 ...
	 *            element to be stored at the specified position.
	 */
	public void setQuick(int index, Node element)
	{
		elements[index] = element;
	}
	 ...
	 */
	public NodeArrayList times(int times)
	{
		NodeArrayList newList = new NodeArrayList(times * size);
		for (int i = times; --i >= 0;)
		{
	 ...
	 */
	public Node[] toArray(Node array[])
	{
		if (array.length < size)
			array = (Node[]) java.lang.reflect.Array.newInstance(array.getClass()
	...
	public java.util.ArrayList toList()
	{
		int mySize = size();
		Node[] theElements = elements;
		java.util.ArrayList<Node> list = new java.util.ArrayList<Node>(mySize);
	...
	{
		elements = mostrare.coltAdaptation.ArraysForNode.trimToCapacity(elements, size());
	}
}
