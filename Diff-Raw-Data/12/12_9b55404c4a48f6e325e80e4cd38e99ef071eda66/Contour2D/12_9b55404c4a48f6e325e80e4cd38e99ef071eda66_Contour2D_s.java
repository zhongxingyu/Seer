 ...
 */
 public class Contour2D {
  ...
  private static float[][] tri_3 = new float[2][3];
  private static float[][] tri_4 = new float[2][4];
  private static float[][] tri_5 = new float[2][5];
  private static float[][] tri_6 = new float[2][6];
  private static float[][] normals_3 = new float[3][3];
  private static float[][] normals_4 = new float[3][4];
  private static float[][] normals_5 = new float[3][5];
  private static float[][] normals_6 = new float[3][6];
  ...
  public static ContourOutput contour(float g[], int nr, int nc,
    ...
    if (fill) {
      ...
      fillGridBox(g, n_lines, vx, vy, xd, xdd, yd, ydd, nr, nrm, nc, ncm,
          ctrLow, o_flags, myvals, color_bin, grd_normals, triStripBldr);
    ...
    for (int ic = 0; ic < ncm; ic++) {
      ...
      for (int ir = 0; ir < nrm; ir++) {
        ...
        fillGridBox(new float[] { ga, gb, gc, gd }, n_lines[ir][ic], vx, vy,
            ...
            numv[0], o_flags[ir][ic], values, color_bin, grd_normals, triStripBldr);
      }
    }
  }
  ...
  private static void fillGridBox(float[] corners, int numc, float[] vx,
      ...
      short ctrLow, int numv,
      byte[] o_flags, float[] values, byte[][] color_bin,
      float[][][] grd_normals, TriangleStripBuilder triStripBldr) {
    ...
    // If any case 6 (saddle point), handle all contour lines with special logic.
    for (int iii = 0; iii < o_flags.length; iii++) {
      if (o_flags[iii] > 32) {
        fillCaseSix(xx, yy, xd, yd, v_idx, dir, o_flags, ctrLow, vx, vy, nc,
            nr, crnr_color, crnrLevelIdx, crnr_out, color_bin,
            color_length, grd_normals, closed, triStripBldr);
        return;
      }
    }
    ...
    if (o_flag == 1 || o_flag == 4 || o_flag == 2 || o_flag == 7) {
      ...
      if (dist_1 < dist_0)
        opp = true;
      if (opp) {
        fillToOppCorner(xx, yy, xd, yd, v_idx, o_flag, dir, vx, vy,
            nc, nr, crnr_color, crnrLevelIdx, crnr_out, grd_normals, closed, triStripBldr);
      } else {
      ...
      fillToSide(xx, yy, xd, yd, v_idx, o_flag, flag, dir, vx, vy, nc,
          nr, crnr_color, crnrLevelIdx, crnr_out, grd_normals, closed, triStripBldr);
    ...
    // --------------------------------
    il++;
    for (il = 1; il < numc; il++) {  //- iterate over contour line box intersection segments
      v_idx = start + dir * il * 2;
      ...
      {
        //- box side of line segment verticies, 0->v_idx, 1->v_idx->v_idx+1
        byte[] side_s = new byte[2];
 
        //- box side of last line segment verticies, 0->v_idx_last, 1->v_idx->v_idx_last+1
        byte[] last_side_s = new byte[2];
 
        byte[] b_arg = new byte[2];
        boolean flip;
 
        getBoxSide(vx, vy, xx, xd, yy, yd, v_idx, dir, o_flags[o_idx], side_s);
        getBoxSide(vx, vy, xx, xd, yy, yd, v_idx_last, dir, last_o, last_side_s);
        ...
        {
          if (((side_s[0] + last_side_s[0]) & 1) == 1) { // flip so 0idx and 1idx on opposite sides
            flip = false;
          } else {
            flip = true;
          }
        ...
        //- flip only (v_idx, v_idx+1) line segment, not 'last', ie. (v_idx_last, v_idx_last+1)
        if (!flip) {
          vv1[0] = vx[v_idx];
          vv1[1] = vy[v_idx];
          vv2[0] = vx[v_idx + dir];
        ...
        fillCaseChange(xx, yy, xd, yd, nc, nr, vv1, vv2, vv1_last, vv2_last,
           color_bin, cc, color_length, grd_normals, closed, same_side_idx, side_s, last_side_s, triStripBldr);
      }
      else {
        vv1[0] = vx[v_idx];
        vv1[1] = vy[v_idx];
        ...
        fillToLast(xx, yy, xd, yd, nc, nr, vv1, vv2, vv1_last, vv2_last, o_flags[o_idx],
            color_bin, cc, color_length, grd_normals, -1, triStripBldr);
      }
    ...
    if ((last_o == 1) || (last_o == 2) || (last_o == 4) || (last_o == 7)) {
      ...
      if (flag_set > 0) {
        fillToOppCorner(xx, yy, xd, yd, v_idx, last_o, dir, vx, vy,
            nc, nr, crnr_color, crnrLevelIdx, crnr_out, grd_normals, closed, triStripBldr);
      } else {
  ...
  private static void getBoxSide(float[] vx, float[] vy, float xx, float xd,
    ...
    //- check for degenerate corner case, ie both intersection points on a corner
    switch (o_flag) 
    {
      case 1:
        if (side[0] == side[1]) {
  ...
  private static void interpNormals(float[] vx, float[] vy, float xx, float yy,
    ...
    for (int k=0; k<n_verts; k++) {
      interpNormals(vx[k], vy[k], xx, yy, nc, nr, xd, yd, grd_normals, tmp);
   ...
   * @param tri_normals
   */
  private static void fillToLast(float xx, float yy, float xd, float yd, int nc, int nr,
      float[] vv1, float[] vv2, float[] vv1_last, float[] vv2_last, byte kase,
      byte[][] color_bin, int cc, int color_length,
      float[][][] grd_normals, int same_side_idx, TriangleStripBuilder triStripBldr)
  {
    float[][] tri = new float[2][4];
    float[][] normals = new float[3][4];
    byte[] side_s = new byte[2];
    ...
    fillToLast(xx,yy,xd,yd,nc,nr,vv1,vv2,vv1_last,vv2_last,color_bin,cc,color_length,
               grd_normals, same_side_idx, side_s, last_side_s, strp_sides, tri, normals, startIdx, triStripBldr);
 
  }
 
  private static void fillToLast(float xx, float yy, float xd, float yd, int nc, int nr,
      float[] vv1, float[] vv2, float[] vv1_last, float[] vv2_last,
      byte[][] color_bin, int cc, int color_length,
      float[][][] grd_normals, int same_side_idx, byte[] side_s, byte[] last_side_s, byte[] strp_sides, 
      float[][] tri, float[][] normals, int startIdx, TriangleStripBuilder triStripBldr)
  {
    ...
    tri[0][idx] = x0;
    tri[1][idx] = y0;
    interpNormals(tri[0][idx], tri[1][idx], xx, yy, nc, nr, xd, yd, grd_normals, tmp);
  ...
  private static void fillCaseChange(float xx, float yy, float xd, float yd, int nc, int nr,
      float[] vv1, float[] vv2, float[] vv1_last, float[] vv2_last,
      byte[][] color_bin, int cc, int color_length, float[][][] grd_normals, int[] closed, 
      int same_side_idx, byte[] side_s, byte[] last_side_s, TriangleStripBuilder triStripBldr) 
  {
     float[][] tri = null;
     float[][] normals = null;
     byte first_tri_orient = 0;
     ...
     byte[] color = new byte[color_length];
     for (int k=0; k<color_length; k++) color[k] = color_bin[k][cc];
 
        /* no line segments points are on the same box side.  Close off (2) opposite corners */
        if (same_side_idx == -1) {
          tri = new float[2][6];
          ...
          fillToLast(xx, yy, xd, yd, nc, nr, vv1, vv2, vv1_last, vv2_last, color_bin, cc, color_length,
                     grd_normals, same_side_idx, side_s, last_side_s, strp_sides, tri, normals, 1, null);
        ...
        if ((side == 0 && last_s == 3) || (side == 3 && last_s == 0))  kase = 1;
        if ((side == 0 && last_s == 1) || (side == 1 && last_s == 0))  kase = 2;
        if ((side == 2 && last_s == 3) || (side == 3 && last_s == 2))  kase = 4;
        if ((side == 2 && last_s == 1) || (side == 1 && last_s == 2))  kase = 7;
        if ((side == 1 && last_s == 3) || (side == 3 && last_s == 1))  kase = 3;
        if ((side == 2 && last_s == 0) || (side == 0 && last_s == 2))  kase = 5;
 
 
        if (kase == 1 || kase == 2 || kase == 4 || kase == 7) { // close off (1) corner
           byte cornerId = 127;
           tri = new float[2][5];
           normals = new float[3][5];
 
           fillToLast(xx, yy, xd, yd, nc, nr, vv1, vv2, vv1_last, vv2_last, color_bin, cc, color_length,
                      grd_normals, same_side_idx, side_s, last_side_s, strp_sides, tri, normals, 0, null);
           ...
           addCorner(xx, yy, xd, yd, nc, nr, cornerId, grd_normals, closed, 4, tri, normals);
        }
              ...
              addCorner(xx, yy, xd, yd, nc, nr, oppCorners[0], grd_normals, closed, 4, tri, normals);
              addCorner(xx, yy, xd, yd, nc, nr, oppCorners[1], grd_normals, closed, 5, tri, normals);
            }
  ...
  private static void fillCaseSix(float xx, float yy, float xd, float yd,
      ...
      int nc, int nr, byte[][] crnr_color, int[] crnrLevelIdx, boolean[] crnr_out,
      byte[][] color_bin, int color_length, float[][][] grd_normals,
      int[] closed, TriangleStripBuilder triStripBldr) {
    ...
    int cc = ctrLow - 1;
    int cnt = 0;
    int[] cases = { 1, 2, 7, 4 }; // - corner cases, counter-clockwise around box
    ...
    byte[] dum1 = new byte[2];
    byte[] dum2 = new byte[2];
    byte[] dum3 = new byte[4];
 
    /* Letters are the grid corners consistent with previous definition.
       Numbers are indexes into the edge_points array.  Note, if there
       is no corner line segement, the adjacent indexes both refer to
       the corner:  4,3 -> D, 6,5 -> C, 0,7 -> A, 1,2 -> B, otherwise
       they are the contour line segment intersection points.    
    ...
    */
    ...
    /* Iterate through cases, they're all corners, and create the fill
       geometry.  When finished the final edge points are used below
       to fill the remainder of the grid box.
    */
    for (int kk = 0; kk < cases.length; kk++) {
      ...
      if (nn == 0) {
        ...
        fillToNearCorner(xx, yy, xd, yd, 0, (byte) cases[kk], dir,
            vvx, vvy, nc, nr, crnr_color, crnrLevelIdx, crnr_out,
        ...
        for (il = 1; il < nn; il++) {
          ...
          fillToLast(xx, yy, xd, yd, nc, nr, vvv1, vvv2, vvv1_last, vvv2_last, (byte) cases[kk],
              color_bin, clr_idx[il], color_length, grd_normals, -1, triStripBldr);
 
          if (!cornerFirst && il == (nn - 1)) {
            fillToNearCorner(xx, yy, xd, yd, idx, (byte) cases[kk],
                dir, vvx, vvy, nc, nr, crnr_color, crnrLevelIdx, crnr_out,
                grd_normals, closed, triStripBldr);
          }
          if (cornerFirst && il == (nn - 1)) {
    ...
    /* Now use edge_points to create the triangle strip to fill
       the remainder of the grid box, ie. the center region.
    */
    ...
    if (num_corners == 0) {
        ...
        tri = new float[2][6];
        normals = new float[3][6];
        int flag = ((which_corner[0] == 1 && which_corner[1] == 7) || (which_corner[0] == 7 && which_corner[1] == 1)) ? 1 : 4;
        if (flag == 4) {
            tri[0][0] = edge_points[0][6];
            tri[1][0] = edge_points[1][6];
            tri[0][1] = edge_points[0][7];
  ...
  private static void fillToNearCorner(float xx, float yy, float xd, float yd,
      int v_idx, byte o_flag, int dir, float[] vx, float[] vy,
      int nc, int nr, byte[][] crnr_color, int[] crnrLevelIdx, boolean[] crnr_out,
      float[][][] grd_normals, int[] closed, TriangleStripBuilder triStripBldr) 
  {
    float cx = 0;
    float cy = 0;
    int cc = 0;
    ...
    int vidx_0 = 0; // first segment index maps to this in outgoing strip array
    int vidx_1 = 0; // next     "                                       "
    ...
    interpNormals(tri[0][vidx_0], tri[1][vidx_0], xx, yy, nc, nr, xd, yd, grd_normals, tmp);
    ...
    interpNormals(tri[0][vidx_1], tri[1][vidx_1], xx, yy, nc, nr, xd, yd, grd_normals, tmp);
  ...
  private static void fillToSide(float xx, float yy, float xd, float yd,
      int v_idx, byte o_flag, int flag, int dir, float[] vx,
      float[] vy, int nc, int nr, byte[][] crnr_color, int[] crnrLevelIdx, boolean[] crnr_out,
      float[][][] grd_normals, int[] closed, TriangleStripBuilder triStripBldr) {
    ...
    fillToSide(xx, yy, xd, yd, v_idx, o_flag, flag, dir, vx, vy, nc, nr, 
          crnr_color, crnrLevelIdx, crnr_out, grd_normals, closed,
          tri, normals, triStripBldr);
  ...
  private static void fillToSide(float xx, float yy, float xd, float yd,
      int v_idx, byte o_flag, int flag, int dir, float[] vx,
      float[] vy, int nc, int nr, byte[][] crnr_color, int[] crnrLevelIdx, boolean[] crnr_out,
      float[][][] grd_normals, int[] closed, 
      float[][] strpverts, float[][] strpnrmls, TriangleStripBuilder triStripBldr) {
  ...
  private static void addCorner(float xx, float yy, float xd, float yd, int nc, int nr,
      byte cornerID, float[][][] grd_normals, int[] closed,
      int strpIdx, float[][] strpverts, float[][] strpnrmls) {
