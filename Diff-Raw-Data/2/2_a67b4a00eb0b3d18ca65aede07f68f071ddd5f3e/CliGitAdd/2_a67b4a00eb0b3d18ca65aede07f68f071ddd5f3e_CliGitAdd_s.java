  ...
  * 
  * 
  * TO RESOLVE / QUESTIONS 1. How to handle '-i or --interactive' option ? 2. Do we need this option ?`
  * 3. Similarly, '-p' options need interactive access to the git-add process.
  * 
  */
 public class CliGitAdd implements IGitAdd {
   ...
   public GitAddResponse add( String repositoryPath, List<String> options, List<String> fileNames)
     ...
     CheckUtilities.checkStringListArgument(fileNames, "fileNames list");
    ...
    * parameter assigned is not null and is valid.
    * 
    * @param command
    */
   ...
   /**
    * Simple tests to validate the paths and filename given for adding to the repository
    * 
    ...
    * 
    */
   private void doValidityChecks(String directory, String[] args) throws IOException,
     ...
     // Throw exception if git command does not start with "git-add" OR "git
     // add"
     if (!getArg0(args[0]).equals(GIT_ADD) && !getArg0(args[0]).equals(GIT)) {
       throw new IllegalArgumentException("Illegal command name: " + args[0]);
     }
 
     // If the command starts with "git" then the next argument should be
     // "add"
   ...
   public class GitAddParser implements IParser {
     ...
     /**
      * Lines that start with "add" have the second token as the name of the file added by git-added.
      * The file_path added is enclosed in single quotes. I am currently not removing those single
      * quotes as the file_paths that are added may contain files with spaces in it, so it's
      * convenient to use extract those files with single quotes already present. ( Thoughts? )
      * 
      * @param line
      */
