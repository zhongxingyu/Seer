 ...
 */
 ...
 public abstract class Animation3D implements PooledObject {
      ...
      * @param deltaTime
      * @param index The index of the element to update, 0 and upwards
      */
     protected void updateTime(float deltaTime, int index) {
         if (mReverse) {
             mCurrentTime -= deltaTime * mTimeFactor;
     ...
     protected void loopAnimation(int index) {
         ...
         if (!mFinished) {
             if (mPingPong[index]) {
                 if (!mReverse) {
                     mCurrentTime -= mCurrentTime - mTempMaxtime;
                 } else {
     ...
     boolean calcCurrentIndex(float timeDelta) {
        ...
        if ((!mReverse && mCurrentTime > mTempMaxtime) || (mReverse && mCurrentTime < mTempMaxtime)) {
      ...
      * @return True if this animation reached the end, false otherwise. Will return false
      * the time when end was reached, if looping is enabled will return false until next time
      * end is reached.
      */
