 ...
 public class MegaMapManager extends JFrame
 {
 	...
 	{	
 		...
 		viewer.getImagePainter().addMouseListener(new MouseAdapter() 
 		{
 			...
 			public void mouseClicked(MouseEvent e) 
 			{
 		      ...
 		      Position position = new Position(viewer.toDataX(pt.x), viewer.toDataY(pt.y));
 				...
 				{
					if(position.getX() >= Float.parseFloat(((Vector)tableModel.getDataVector().elementAt(i)).elementAt(2).toString()) - 60 * MapViewer.WAYPOINT_RADIUS && position.getX() <= Float.parseFloat(((Vector)tableModel.getDataVector().elementAt(i)).elementAt(2).toString()) + 60 * MapViewer.WAYPOINT_RADIUS && position.getY() >= Float.parseFloat(((Vector)tableModel.getDataVector().elementAt(i)).elementAt(3).toString()) - 60 * MapViewer.WAYPOINT_RADIUS && position.getY() <= Float.parseFloat(((Vector)tableModel.getDataVector().elementAt(i)).elementAt(3).toString()) + 60 * MapViewer.WAYPOINT_RADIUS)
 					{
		...
		JMenuItem menuItem_addGPSWP = new JMenuItem("Add As Waypoint", KeyEvent.VK_A);
		menuItem_connect.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A, ActionEvent.CTRL_MASK));
