 ...
 *******************************************************************************/
 ...
 public class ViewingTest2 extends RDTestCase
 {
 	...
 	public void testMultiUpdate( ) throws Exception
 	{
 		...
 		DataEngineContext deContext4 = newContext( DataEngineContext.MODE_PRESENTATION,
 				fileName );
 	...
 	public void testMultiUpdate2( ) throws Exception
 	{
 		...
 		//add nothing
 		DataEngineContext deContext5 = newContext( DataEngineContext.MODE_UPDATE,
 				fileName,
 				fileName);
 		...
 		//presentation
 		DataEngineContext deContext6 = newContext( DataEngineContext.MODE_PRESENTATION,
 				fileName );
 	...
 	/**
 	 * With filter Gen: filter A Update: filter A, filter B Pre: filter A,
 	 * filter C
 	 * 
 	 * @throws BirtException
 	 */
 	...
 	/**
 	 * With filter Gen: filter A, Update: filter A, filter B, Presentation:
 	...
 	private void updatePreBasicIV( ) throws BirtException
 	{
 		...
 		QueryDefinition qd = newPreIVReportQuery( this.UPDATE_add_filter,
 	...
 	private void preBasicIV( ) throws BirtException
 	{
 		...
 		{
 			// here queryResultID needs to set as the data set
 			QueryDefinition qd = newPreIVReportQuery( this.PRE_add_filter,
 					this.PRE_add_sort,
 					this.PRE_add_group,
 					PRESENTATION );
 			if (!this.usesDetails)
 	 ...
 	 * @throws BirtException
 	 */
 	private void displayPreResult( IQueryResults qr, boolean printGroupInfo,
 			boolean useSkipTo ) throws BirtException
 	{
 		assertTrue( qr.getResultMetaData( ) != null );
 		...
 		do
 		{
 			...
 			{
 				IResultIterator subRi = ri.getSecondaryIterator( subQueryName2,
 						scope );
 				while ( subRi.next( ) )
 				{
 					abc = "      ";
 			...
 			boolean sortNeeded, int groupNeeded, int mode )
 	{
 		...
 		if ( filterNeeded == 4 )
 		{
 			BaseExpression filterExpr = new ConditionalExpression( 
 					"row.SALE_NAME_1", IConditionalExpression.OP_EQ,
 		...
 		if ( filterNeeded == 5 )
 		{
 			ConditionalExpression filterExpr = new ConditionalExpression( "row.AMOUNT_1", IConditionalExpression.OP_TOP_N,"3" );
 			FilterDefinition fd = new FilterDefinition( filterExpr );
 			qd.addFilter( fd );
 		}
 		...
 		if ( groupNeeded == 1 || groupNeeded == 0 )
 		{
 			...
 			if ( this.UPDATE_add_subquery == 2 )
 			{
 				...
 				exprs[1] = new ScriptExpression( "dataSetRow.CITY" );
 				for ( int i = 0; i < subRowExprName2.length; i++ )
 					subqueryDefn.addResultSetExpression( subRowExprName2[i],
 							exprs[i] );
 				subqueryDefn.setApplyOnGroupFlag( true );
