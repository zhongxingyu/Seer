 ...
 /**
  * Analogous to x10.constraint.XTerms. Provides static methods
     ...
     public static XLit makeZero(Type type) {
        ...
        if (type.isIntOrLess()) {
         ...
         if (type.isLongOrLess()) {
             return new CLit(0L, type);
         } else
         if (type.isFloat()) {
             return new CLit(0.0f, type);
         } else
         if (type.isDouble()) {
             return new CLit(0.0, type);
         } else
         if (ts.isObjectOrInterfaceType(type, ts.emptyContext())) {
             return XTerms.NULL;
         } else
         return null;
     }
