 ...
 /**
  * Stateful OCSP fail over to CRL PKIX certificate path checker.
  ...
  * 
  * OCSP is tried first, if ocsp is not available (or any other answer than GOOD or REVOKED) received fails over to CRL
  * 
  * NOTE : indirect CRLs are not supported at the moment (i.e. crls are issued by CA issuing certificate)
  * NOTE1 : scopes of crls are not checked
  *  
	...
	public OCSPCRLPathChecker(X509Certificate rootCACert, Properties props, List<X509Certificate> authorizedOCSPResponderCerts, List<URL> cRLPaths)
 	...
 	throws CertPathValidatorException {
 		...
 		if(!failOverToCRL)
 		{
 			...
 			{
 				//if the OCSPStatusNotGood exception is received it means that the ocsp response was successfully received
 				//and verified, and that status of our certificate is not good. no need to fail over to crl
 				throw new CertPathValidatorException("Responce for queried certificate is not good. Certificate status returned : " + e.getCertStatus());
 			}
 			catch (Exception e) {
 		...
 		if(failOverToCRL)
 		{
 			...
 			//either OCSP url not found, or exception occurred when querying the ocsp 
 	 ...
 	 * 
 	 * if CRL is accessible , and signed by CA issuing x509Cert , and if x509Cert is not revoked returns
 	 * otherwise throws exception
 	 * @param x509Cert - certificate whose revocation status is checked
 	 * @param crlURL - url of the CRL
 	 * @throws SignServerException - if something bad (other than certificate being revoked) happens 
 	 * @throws CRLCertRevokedException  - if certificate is revoked
 	 */
 	private void parseAndVerifyCRL(X509Certificate x509Cert, URL crlURL) throws SignServerException, CRLCertRevokedException {
 		...
 		try {
 			...
 			//could not get crl, nothing more to do
 			log.error("Exception on fetching CRL from url : " + crlURL.toString(), e);
 			throw new SignServerException("Exception on fetching CRL from url : " + crlURL.toString(), e);
 		}
 		
 		try {
 			certCRL.verify(cACert.getPublicKey(), "BC");
 		} catch (Exception e) {
 			msg = "Exception on verifying CRL fetched from url: " + crlURL.toString() + " using CA certificate : " + cACert.getSubject();
 			log.error(msg, e);
 			throw new SignServerException(msg, e);
 		}
 		
 		// now that crl is verified check getThisUpdate < now, getNextUpdate > now
 		// although getNextUpdate is optional RFC 3280 mandates it and does not specify how to interpret the absence of the field
 		if(certCRL.getThisUpdate() != null && (new Date()).compareTo(certCRL.getThisUpdate()) <= 0)
 		{
 			msg = "CRL for certificate : " + x509Cert.getSubject() + " reported thisUpdate as : " + certCRL.getThisUpdate().toString() + " which is later than current date.";
 			log.debug(msg);
 		...
 		{
 			msg = "CRL for certificate : " + x509Cert.getSubject() + " reported nextUpdate as : " + certCRL.getNextUpdate().toString() + " which is earlier than current date.";
 			log.debug(msg);
 			throw new SignServerException(msg);
 		}
 		...
 		if(crlEntry != null)
 		{
 			...
 			if(crlEntry.hasExtensions())
 			{
 				...
 				{
 					//this is tricky, though the certificate is found in CRL it is not revoked, it is activated back from on-hold
 					//so it is actually not revoked !
 					return;
 				}
 				else
 				{
 					//certificate is revoked , append reason code and throw exception so we stop checking
 	...
 	public Object clone() 
 	{
 		try {
 			...
 			clonedOCSPCRLPathChecker = new OCSPCRLPathChecker(rootCACert, this.props, this.authorizedOCSPResponderCerts, this.cRLPaths);
