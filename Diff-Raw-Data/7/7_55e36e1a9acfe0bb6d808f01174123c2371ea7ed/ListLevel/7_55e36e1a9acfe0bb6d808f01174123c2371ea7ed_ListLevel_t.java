 ...
 /**
  *@author previous Apache POI developers.
  *@author Shamal
  *
  */
 public final class ListLevel
 {
   ...
   private byte _nfc;
   private byte _info;
    private static BitField _jc = new BitField(0x03);
     private static BitField _fLegal;
    private static BitField _fNoRestart = new BitField(0x08);
   ...
   public ListLevel(byte[] buf, int offset)
   {
    ...
    _ixchFollow = buf[offset++];
     ...
     // The reserved field is found to be the combination of ilvlRestartLim and grfhic.
     // Therefore skipping the offset increment and getting the mentioned variables. by Shamal
     //offset += LittleEndian.SHORT_SIZE;
     iLvlRestartLim = LittleEndian.getUnsignedByte(buf, offset++);
     grfhic = LittleEndian.getUnsignedByte(buf, offset++);
   ...
   public int getFNoRestart()
   {
 	  return _fNoRestart.getValue(_info);
 	  
   }
 
   public int getIlvlRestartLim()
   {
 	  return iLvlRestartLim;
   }
