 ...
 public class Test2 implements AsynchMessageReceiver, ConsoleListener, Runnable {
     ...
     protected Stack toreview = new Stack(); // terse-stack
     ...
     SimpleDateFormat yesterdayTimeFormat = new SimpleDateFormat("'igr' HH:mm", locale);
 	...
 	try {
 	    ...
 	    return "Mte " + ex1.getErrorStatus() + " (fel " + ex1.getError() + ")";
 	}
 	if (name == null)
 	    return "Person "+n+" (N/A)";
 	if (useAnsiColors)
 	...
 	if (arg.startsWith("m ") || arg.startsWith("p ")) {
 		...
 		consoleWriteLn("%Fel: kunde inte tolka mtesnummer " + ex1.getMessage());
 	    }
 	}
 
 	ConfInfo[] names = foo.lookupName(arg, wantPersons, wantConfs);
 	if (names.length == 0) {
 	    consoleWriteLn("%Fel: hittar inget sdant mte eller sdan person");
 	    return -1;
 	}
 	if (names.length > 1) {
 	...
 	while (messages.size() > 0) {
 		...
 		try {
 		    ...
 		    System.err.println("Det gick inte att utfra get-conf-name: " + ex.getMessage());
 		}
 		if (!useGui) {
 		...
 		if (params[0].intValue() == foo.getMyPerson().getNo()) {
 		    consoleWriteLn("Personligt meddelande frn " + sender + 
 				   " (" + komTimeFormat(m.getArrivalTime()) + "):");
 		} else if (params[0].intValue() == 0) {
 		    consoleWriteLn("Alarmmeddelande frn " + sender + " (" + komTimeFormat(m.getArrivalTime()) + "):");
 		} else {
 		    consoleWriteLn("Meddelande till " + recipient);
 		    consoleWriteLn("frn " + sender +
 				   " (" + komTimeFormat(m.getArrivalTime()) + "):");
 		}
 		consoleWriteLn("");
 	...
 	consoleWriteLn("   -password <lsenord>   Logga in med <lsenord>");
 	...
 	try {
 		...
 		if (server == null) { // user might have pressed ^D, or stdin == /dev/null
 		    consoleWriteLn("Det verkar vara problem med att lsa frn stdin. Det kan hnda att");
 		    consoleWriteLn("Javamiljn fr denna applikation inte har ngon inmatningsterminal.");
 		    consoleWriteLn("Om s r fallet, s br en ny konsoll nu ppnas utav applikationen.");
 		    consoleWriteLn("Om detta inte sker, starta om programmet med parametern \"-gui\", eller");
 		    consoleWriteLn("stt Javasystem-propertyn \"lattekom.use-gui\" till vrdet \"true\".");
 		...
 		while (loginUser == 0 && names.length != 1) {
 			...
 			if (crtReadLine("Vill du skapa ny person med namnet \"" + enteredName + "\"? (j/N)").equals("j")) {
 			    consoleWriteLn("Du kan senare vlja att byta namn, om du vill.");
 			    consoleWriteLn("");
 			    boolean passMatch = false;
 			    String newPassword = null;
 			    while (!passMatch) {
 				newPassword = crtReadLine("Ange ett lsenord: ");
 			    
 				passMatch = newPassword.equals(crtReadLine("Upprepa: "));
 				if (!passMatch) {
 				    consoleWriteLn("Du angav inte samma lsenord bda gngerna. Frsk igen.");
 				}
 			    }
 			    try {
 				...
 				consoleWriteLn("%Fel: det gick inte att skapa ngon person. Felkod " + e1.getError());
 			    }
 		...
 		if (loginUser == 0) {
 		    ...
 		    else loginPassword = crtReadPassword("Lsenord: ");
 	    ...
 	    consoleWriteLn("Inloggad. Vlkommen till LysKOM! Skriv \"?\" och tryck <Enter> fr hjlp.");
 	    consoleWrite("Vnta lite medan jag hmtar information om olsta mten...");
 	    foo.updateUnreads();
 		...
 		if (rc == 0) { // default-action (read next text)
		    ...
		    if (textNo == -1) { // if no unread text in current conference,
 			...
 			if (nextConf == -1) {			    
 			    ...
 			    consoleWriteLn("Det finns inget att lsa.");
 			}
 			else {
 			    int unr = foo.getUnreadCount(nextConf);
 			    String unrS;
 			    if (unr < 1) unrS = "inga olsta";
 			    else if (unr == 1) unrS = "ett olst";
 			    else unrS = unr + " olsta";
 			    consoleWriteLn("Gick till mte: " + confNoToName(nextConf) + " - " + unrS);
 			}
 			rc = 1;
 			continue;
 		    }
 	...
 	if (Boolean.getBoolean("lattekom.show-conf-prompt")) {
 	    curConf = (conf == -1 ? "Ej nrvarande i ngot mte" : "Du r i mte " + confNoToName(conf) + 
 		       " med " + foo.getUnreadCount(conf) + " olsta.") + "\n";
 	}
 	if (!toreview.empty()) {
 	    curConf += "(terse nsta text) ";
 	    setStatus("terser");
 	} else if (!toread.empty()) {
 	    curConf += "(Lsa nsta kommentar) ";
 	    setStatus("Lser kommentarer");
 	} else if (foo.nextUnreadText(false) == -1) {
 	    if (foo.nextUnreadConference(false) == -1) {
 		curConf += "(Slut p inlgg) ";
 		setStatus("Vntar p inlgg");
 	    } else {
 		curConf += "(G till nsta mte) ";
 		if (conf > 0)
 		    setStatus("Har just lst ut ett mte");
 		else
 		    setStatus(randomStatus());
 	    }
 	} else {
 	    curConf += "(Lsa nsta inlgg) ";
 	    setStatus("Lser inlgg");
 	}
     ...
     String[] randomStatus = {
 	"Kliar sig i huvudet.",
 	"Vntar p att himlen ska falla ner.",
 	"Leker pixelleken.",
 	"Funderar.",
 	"r.",
 	"ter banan." };
 	...
 	if (cmd.equals("?")) {
 	    ...
 	    consoleWriteLn("-- vriga kommandon");
 	    consoleWriteLn("\tln                -- lista nyheter");
 	    consoleWriteLn("\tq                 -- avsluta TestKOM");
 	    consoleWriteLn("\trt <text>         -- radera text");
 	    consoleWriteLn("\ts [mtesnamn]     -- skicka meddelande");
 	    consoleWriteLn("\tss <sessionsnr.>  -- visa sessions- och klientinformation");
 	    consoleWriteLn("\tuo                -- uppdatera olsta (typ omstart)");
 	    consoleWriteLn("\tv                 -- lista inloggade aktiva anvndare");
 	    consoleWriteLn("\tk                -- terse (frsta) kommentaren");
 	    consoleWriteLn("\trpc <#> [data]    -- skicka RPC-kommando (svaret ignoreras) (farligt)");
 	    consoleWriteLn("");
 	    consoleWriteLn("  Mtesnamn kan fr det mesta bytas ut mot \"m <nummer>\"");
 	    return 1;
 	}
 	if (cmd.equals("uo")) {
 	    foo.updateUnreads();
 	...
 	if (cmd.equals("v")) {
 	    ...
 	    consoleWriteLn("Listar " + vilka.length + " aktiva anvndare:");
 	    consoleWriteLn("----------------------------------------------------------------");
 		...
 		if (vilka[i].getWorkingConference() != 0) {
 		    consoleWrite(" i mte " + conferenceName);
 		}
 		consoleWriteLn("");
 	...
 	if (cmd.equals("ss")) { // status (fr) session
 	    try {
 		int sessionNo = Integer.parseInt(st.nextToken());
 		SessionInfo si = foo.getStaticSessionInfo(sessionNo);
 		consoleWriteLn("Status fr session " + sessionNo + ":");
 		consoleWriteLn(" Anvndarnamn : " + bytesToString(si.getUsername()));
 		consoleWriteLn(" Vrddator    : " + bytesToString(si.getHostname()));
 		...
 		throw new CmdErrException("Du mste ange ett sessionsnummer");
 	...
 	if (cmd.equals("s")) { // snda meddelande
 	    int confNo = 0;
 	    try {
 	...
 	if (linesSinceLastPrompt > linesPerScreen) {
 	    crtReadLine("(-- tryck Enter fr att fortstta --)", true);
 	}
 	...
 	JFrame pFrame = new JFrame("Lsenord");
 	Container cPane = pFrame.getContentPane();
 	cPane.setLayout(new FlowLayout());
 	cPane.add(new JLabel("Ange lsenord: "));
 	...
 	consoleWriteLn("--- skriv \"!?\" p tom rad fr hjlp ----------------------------------");
 	...
 	while (row != null && !row.equals(".")) {
 		...
 		if (icmd.startsWith("!f")) {
 		    ...
 		    int newConf = parseNameArgs(confs, true, true);
 		    if (newConf > 0) {
 			consoleWriteLn("** Subtraherar " + confNoToName(newConf) + " frn kopiemottagarlista");
 		    ...
 		    int newConf = parseNameArgs(confs, true, true);
 		    if (newConf > 0) {
 			consoleWriteLn("** Subtraherar " + confNoToName(newConf) + " frn mottagarlista");
 			...
 			consoleWriteLn("** Du mste ange ett radnummer.");
 		    }
 		    consoleWriteLn("");
 		    ...
 		    consoleWriteLn("** Kommandon i editorlge:");
 		    consoleWriteLn("  !f <mte>     Flytta texten till <mte>");
 		    consoleWriteLn("  !am <mte>    Addera <mte> som mottagare");
 		    consoleWriteLn("  !ak <mte>    Addera <mte> som kopiemottagare");
 		    consoleWriteLn("  !rr <radnr.>  Radera <radnr.>");
 		    consoleWriteLn("  !sm <mte>    Subtrahera <mte> frn mottagarlista");
 		    consoleWriteLn("  !sk <mte>    Subtrahera <mte> frn kopiemottagarlista");
 		    ...
 		    consoleWriteLn("** Oknt editorkommando");
 		}
 	    } else {
 		if (row.length() > 73) {
 		    consoleWriteLn("** Varning: rad " + (rows.size()+1) + " r ver 73 tecken lng.");
 		}
 		rows.add(row);
 		rowCount++;
 	    }
 	...
 	for (int i=0; i < footnotes.length; i++) {
 	    ...
 	    if (ts != null) {
 		consoleWrite(" av " + confNoToName(ts.getAuthor()) + ", lsa? (J/n) ");
 		if (crtReadLine("").equals("n")) continue;
 		displayText(foo.getText(footnotes[i]), true);
 	    } else {
 		consoleWriteLn("som du inte fr lsa.");
 	    }
