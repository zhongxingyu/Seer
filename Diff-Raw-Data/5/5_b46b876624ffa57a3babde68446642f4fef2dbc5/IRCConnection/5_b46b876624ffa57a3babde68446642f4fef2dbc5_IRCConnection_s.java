 ...
 /**
  * <p>
  * <code>IRCConnection</code> is the core class for Martyr.
  ...
  * 
  * <p>
  * <b>Please read this entirely before using the framework.</b> Or what the
  * heck, try out the example below and see if it works for ya.
  * </p>
  * 
  * <h2>States and State Listeners</h2>
  * <p>
  * <code>IRCConnection</code> is always in one of three states.
  * <code>UNCONNECTED</code>, <code>UNREGISTERED</code> or
  * <code>REGISTERED</code>. It keeps a list of listeners that would like to know
  * when a state change occurs. When a state change occurs each listener in the
  * list, in the order they were added, is notified. If a listener early up on
  * the list causes something to happen that changes the state before your
  * listener gets notified, you will be notified of the state change even though
  * the state has changed. You will be notified again of the new state. That is,
  * state change notifications will always be in order, but they may not always
  * reflect the "current" state.
  * </p>
  * 
  * <h2>Commands and Command Listeners</h2>
  * <p>
  * <code>IRCConnection</code> also keeps a list of listeners for when a command
  * arrives from the server. When a command arrives, it is first parsed into an
  * object. That object is then passed around to all the listeners, again, in
  * order. Commands can be received and the socket closed before the commands are
  * actually send to the listeners, so beware that even though you receive a
  * command, you may not always be guaranteed to have an open socket to send a
  * response back on. A consumer of the command should never modify the command
  * object. If you try to send a command to a closed socket,
  * <code>IRCConnection</code> will silently ignore your command. Commands should
  * always be received in order by all listeners, even if a listener higher up in
  * the list sends a response to the server which elicits a response back from
  * the server before you've been told of the first command.
  * </p>
  * 
  * <h2>Connecting and staying connected</h2>
  * <p>
  * The AutoReconnect class can connect you and will try to stay connected. Using
  * AutoReconnect to connect the first time is recommended, use the
  * <code>go(server,port)</code> method once you are ready to start.
  * </p>
  * 
  * <h2>Registration On The Network</h2>
  * <p>
  * The AutoRegister class can register you automatically on the network.
  * Otherwise, registration is left up to the consumer. Registration should occur
  * any time the state changes to <code>UNREGISTERED</code>. The consumer will
  * know this because it has registered some class as a state observer.
  * </p>
  * 
  * <h2>Auto Response</h2>
  * <p>
  * Some commands, such as Ping require an automatic response. Commands that fall
  * into this category can be handled by the <code>AutoResponder</code> class.
  * For a list of what commands <code>AutoResponder</code> auto responds to, see
  * the source.
  * </p>
  * 
  * <h2>Joining and Staying Joined</h2>
  * <p>
  * You can use the <code>AutoJoin</code> class to join a channel and stay there.
  * <code>AutoJoin</code> will try to re-join if kicked or if the connection is
  * lost and the server re-connects. <code>AutoJoin</code> can be used any time a
  * join is desired. If the server is not connected, it will wait until the
  * server connects. If the server is connected, it will try to join right away.
  * </p>
  * 
  * <h2>Example Usage</h2>
  * <p>
  * You will probably want to at least use the <code>AutoRegister</code> and
  * <code>AutoResponder</code> classes. Example:
  * </p>
  * 
  * <p>
  * Note that in the example, the first line is optional.
  * <code>IRCConnection</code> can be called with its default constructor. See
  * note below about why this is done. <code>IRCConnection</code> will
  * instantiate its own <code>ClientState</code> object if you do not provide
  * one.
  * </p>
  * 
  * <pre>
  ...
  * // disable() method.
  * 
  * AutoRegister autoReg = new AutoRegister(&quot;repp&quot;, &quot;bdamm&quot;, &quot;Ben Damm&quot;, connection);
  ...
  * <h2>Client State</h2>
  * <p>
  * The <code>ClientStateMonitor</code> class tells commands to change the client
  * state when they are received. <code>ClientStateMonitor</code> is
  * automatically added to the command queue before any other command, so that
  * you can be guaranteed that the <code>ClientState</code> is updated before any
  * observer sees a command.
  * </p>
  * 
  * <p>
  * So, how does an application know when a channel has been joined, a user has
  * joined a channel we are already on, etc? How does the application get
  * fine-grained access to client state change info? This is a tricky question,
  * and the current solution is to sublcass the
  * <code>clientstate.ClientState</code> and <code>clientstate.Channel</code>
  * classes with your own, overriding the <code>setXxxxXxxx</code> methods. Each
  * method would call <code>super.setXxxXxxx</code> and then proceed to change
  * the application as required.
  * </p>
  * 
  * <h2>Startup</h2>
  * <p>
  * IRCConnection starts in the <code>UNCONNECTED</code> state and makes no
  * attempt to connect until the <code>connect(...)</code> method is called.
  * </p>
  * 
  * <p>
  * <code>IRCConnection</code> starts a single thread at construction time. This
  * thread simply waits for events. An event is a disconnection request or an
  * incoming message. Events are dealt with by this thread. If connect is called,
  * a second thread is created to listen for input from the server
  * (InputHandler).
  * 
  ...
  * 
  * - A message is an event - A disconnect request is an event, placed on the
  * queue? -- Off I go to do other stuff.
  */
 public class IRCConnection {
 	...
 	/**
 	 * This method exists so that subclasses may perform operations before the
 	 * event thread starts, but overriding this method.
 	 * */
 	...
 	/**
 	 * In the event you want to stop martyr, call this. This asks the event
 	 * thread to finish the current event, then die.
 	 * */
 	public void stop() {
 		eventThread.doStop();
 	}
 	...
 	public void connect(String server, int port) throws IOException {
 		synchronized (connectMonitor) {
 			SChatWrapper.ircLogToChat("IRCConnection: Connecting to " + server
 					+ ":" + port);
 			if (connected) {
 	...
 	/**
 	 * This allows the developer to provide a pre-connected socket, ready for
 	 * use. This is so that any options that the developer wants to set on the
 	 * socket can be set. The server parameter is passed in, rather than using
 	 * the customSocket.getInetAddr() because a DNS lookup may be undesirable.
 	 * Thus, the canonical server name, whatever that is, should be provided.
 	 * This is then passed on to the client state.
 	 * 
 	 * @param customSocket
 	 *            Custom socket that we will connect over
 	 * @param server
 	 *            Server to connect to
 	 * @throws IOException
 	 *             if we could not connect
 		...
 		synchronized (connectMonitor) {
 			if (connected) {
 				throw new IllegalStateException(
 						"Connect requested, but we are already connected!");
 			}
 	 ...
 	 * messages may be processed before a disconnect actually occurs.
 	 * </p>
 	 * <p>
 	 * No errors are possible from the disconnect. If you try to disconnect an
 	 * unconnected socket, your disconnect request will be silently ignored.
 	 * </p>
 	 */
 	 ...
 	 * creates. Default is true, that is, new InputHandler threads are daemon
 	 * threads, although the event thread is always a daemon. The result is that
 	 * as long as there is an active connection, the program will keep running.
 	 * 
 	 * @param daemon
 	 *            Set if we are to be treated like a daemon
 	 */
 	 ...
 	 * 
 	 * @param timeout
 	 *            *2 msec to wait at most for stop.
 	 * 
 	 * */
 	 ...
 	 *             if command is not an OutCommand.
 	 * */
 	...
 	/**
 	 * @param sender
 	 *            sets the class that is responsible for sending commands.
 	 * */
 	 ...
 	 * 
 	 * @param sleepTime
 	 *            Length of time to sleep between commands
 	 * */
 	...
 	/**
 	 * Inserts into the event queue a command that was not directly received
 	 * from the server.
 	 * 
 	 * @param fakeCommand
 	 *            Fake command to inject into incoming queue
 	 * */
 	...
 	/**
 	 * Splits a raw IRC command into three parts, the prefix, identifier, and
 	 * parameters.
 	 * 
 	 * @param wholeString
 	 *            String to be parsed
 	...
 	/**
 	 * Given the three parts of an IRC command, generates an object to represent
 	 * that command.
 	 * 
 	 * @param prefix
 	 *            Prefix of command object
 	 * @param identifier
 	 *            ID of command
 	 * @param params
 	 *            Params of command
 	 ...
 	 * */
 	protected InCommand getCommandObject(String prefix, String identifier,
 			String params) {
 		InCommand command;
 	 ...
 	 * 
 	 * @param wholeString
 	 *            String to be parsed and handled
 	 * */
 	...
 	/**
 	 * Client state (our name, nick, groups, etc). Stored here mainly because
 	 * there isn't anywhere else to stick it.
 	 */
 	private ClientState clientState;
 
 	/**
 	 * Maintains a list of classes observing the state and notifies them when it
 	 * changes.
 	 */
 	...
 	/**
 	 * Command register, contains a list of commands that can be received by the
 	 * server and have matching Command objects.
 	 */
 	...
 	/**
 	 * Setting state prevents setState from recursing in an uncontrolled manner.
 	 */
 	...
 	/**
 	 * Determines the time to sleep every time we send a message. We do this so
 	 * that the server doesn't boot us for flooding.
 	 */
 	...
 	/**
 	 * Unsafe, because this method can only be called by a method that has a
 	 * lock on connectMonitor.
 	 * 
 	 * @param socket
 	...
 	/**
 	 * Does no synchronization on its own. This does not synchronize on any of
 	 * the IRCConnection monitors or objects and returns after making a minimum
 	 * of method calls.
 	 * 
 	...
 	private void doDisconnect() {
 		synchronized (connectMonitor) {
 			...
 			try {
 				...
 				//ALS.alDebugPrint("IRCConnection: Sleeping for a bit ("	+ sleepTime + ")..");
 	 ...
 	 * 
 	 * @param newState
 	 *            New state to set connection to.
 	 */
 	...
 	/**
 	 * Sends the command down the socket, with the required 'CRLF' on the end.
 	 * Waits for a little bit after sending the command so that we don't
