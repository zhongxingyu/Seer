 ...
 public class DirectoryTestCase extends TestCase
 {
    ...
    {
       //Test n1a : directory does not allow to GET its content
       ...
       //Test n1a : directory allows to GET its content
       ...
       //Test n2a : tests the HEAD method
       response = handle(directory, webSiteURL, testFileUrl, Method.HEAD, null);
       assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
 
       //Test n2b : try to GET a file that does not exist
       response = handle(directory, webSiteURL, webSiteURL + "123456.txt", Method.GET, null);
       assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_NOT_FOUND));
 
       //Test n3a : try to put a new representation, but the directory is by default read only
       ...
       //Test n3b : try to put a new representation, the directory is no more read only
       directory.setModifiable(true);
       response = handle(directory, webSiteURL, baseFileUrl, Method.PUT,
             new StringRepresentation("this is a test"));
       assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
 
       //Test n4 : Try to get the representation of the new file
       ...
       //Test n5 : add a new representation of the same base file
       ...
       //Test n6b : delete a file that does not exist
       response = handle(directory, webSiteURL, testFileUrl, Method.DELETE, null);
       assertTrue(response.getStatus().equals(Status.CLIENT_ERROR_NOT_FOUND));
 
       //Test n6c : delete a directory
       ...
       //Test n7a : put one representation of the base file (in french language)
       response = handle(directory, webSiteURL, baseFileUrlFr, Method.PUT,
             new StringRepresentation("message de test"));
       assertTrue(response.getStatus().equals(Status.SUCCESS_OK));
 
       //Test n7b : put another representation of the base file (in french language) but the extensions are mixed
       ...
       //Test n7c : delete the file representation of the resources with no content negotiation
       ...
       //Test n7d : put another representation of the base file (in french language) but the extensions are mixed
       ...
       //Test n7e : delete the file representation of the resources with content negotiation
       directory.setNegotiationEnabled(true);
       response = handle(directory, webSiteURL, baseFileUrlFr, Method.DELETE, null);
       assertTrue(response.getStatus().equals(Status.SUCCESS_NO_CONTENT));
       response = handle(directory, webSiteURL, baseFileUrlFr, Method.HEAD, null);
       ...
       //Test n8 : should not delete the english representation
