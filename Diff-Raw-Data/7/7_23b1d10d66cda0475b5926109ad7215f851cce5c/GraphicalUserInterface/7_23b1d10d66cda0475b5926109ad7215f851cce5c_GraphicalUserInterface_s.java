 ...
 public class GraphicalUserInterface extends JFrame implements IObserver {
     ...
     public GraphicalUserInterface(final GameTableInterface controller) {
         ...
         mnExit.addActionListener(new ActionListener() {
             ...
             public void actionPerformed(ActionEvent arg0) {
                 if (JOptionPane.showConfirmDialog(contentPane, "Wirklich beenden?", "Beenden", JOptionPane.YES_NO_OPTION) == 0) {
                     System.exit(0);
                 }
             }
         });
         mnExit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F4, Event.ALT_MASK));
         mnExit.setIcon(new ImageIcon(this.getClass().getResource("/off.png").getPath()));
         mnGame.addSeparator();
         mnGame.add(mnExit);
         ...
         panetAreaStatus.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER);
         panetAreaStatus.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
     ...
     public void update(IOEvent e) {
         if (e instanceof IOMsgEvent) {
             ...
             figures.changePlayer(playerID, controller.getCurrentPlayer().getFigureList().size());
     ...
     private void repaintCardLabels() {
         ...
         for (int i = 0; i < cards.length; i++) {
             if (i < cardListSize) {
                 int card = controller.getCurrentPlayer().getCardList().get(i).getValue();
                 cards[i].setIcon(new ImageIcon(getClass().getResource(String.format("/%d.gif", card))));
     ...
     private int getValueForCardIcon() {
         ...
         if (up != null) {
             ...
             int card = controller.getCurrentPlayer().getCardList().get(index).getValue();
             ret = card;
         }
         return ret;
     }
     ...
     private void winnerDialog() {
         if (controller.currentPlayerHaswon()) {
             Object[] options = { "Exit" };
             String won = String.format("Glckwunsch Spieler %d du hast gewonnen!!", controller.getCurrentPlayerID());
     ...
     private int cardAceDialog() {
         int retVal = CARD11;
         Object[] options = { "1", "11" };
         int decision = JOptionPane.showOptionDialog(this, "Wieviel mchtest du laufen?", "Laufwert?", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, icon, options, options[1]);
     ...
     private int card4Dialog(int cardval) {
         ...
         int decision = JOptionPane.showOptionDialog(this, "In welche Richtung mchtest du laufen?", "Welche Richtung?", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, icon, options, options[1]);
     ...
     private void jokerSpinnerDialog(final PlayerInterface current) {
         ...
         int decision = JOptionPane.showOptionDialog(contentPane, input, "Karte auswhlen", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, icon, null, null);
         if (decision == JOptionPane.YES_OPTION) {
