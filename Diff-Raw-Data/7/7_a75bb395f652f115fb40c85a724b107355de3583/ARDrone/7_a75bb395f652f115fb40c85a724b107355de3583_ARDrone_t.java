 ...
 public class ARDrone {
 	...
 	private String DEBUG = "Drone Control";
 	...
 	private void changeState(State newstate) throws IOException {
 		...
 		synchronized (state_mutex) {
 			if (state != newstate) {
 				Log.v(DEBUG, "State changed from " + state + " to " + newstate);
 				state = newstate;
 	...
 	public void changeToErrorState(Exception ex) {
 		synchronized (state_mutex) {
 			...
 			Log.v(DEBUG, "State changed from " + state + " to " + State.ERROR + " with exception ", ex);
 			state = State.ERROR;
 			state_mutex.notifyAll();
 		}
 	}
 	...
 	public void navDataReceived(NavData nd) {
 		...
 		try {
 			...
 			Log.v(DEBUG, "ERROR: Error changing the state", e);
 		}
 		...
 		synchronized (state_mutex) {
 			while (true) {
 				if ((System.currentTimeMillis() - since) >= how_long) {
 					...
 					throw new IOException("Connection Error = State.Error");
 				} else if (state == State.DISCONNECTED) {
 					throw new IOException("Connection Error = State.Disconnected");
 				}
