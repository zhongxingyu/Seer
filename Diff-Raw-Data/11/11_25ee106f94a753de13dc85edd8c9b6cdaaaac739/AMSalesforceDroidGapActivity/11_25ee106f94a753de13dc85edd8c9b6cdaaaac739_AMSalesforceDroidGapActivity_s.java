 ...
 public class AMSalesforceDroidGapActivity extends SalesforceDroidGapActivity
 	...
 	private void mapInit(Bundle savedInstanceState) throws AMException {
 		// Initialize varibles
 		mapLayout = getLayoutInflater().inflate(
 				R.layout.activity_native_map_acivity, null);
 	...
 	public void onCreate(Bundle savedInstanceState) {
 		...
 		/*
 		 * Inflate and add the main view layout and menu layout to root sliding
 		 * menu layout. Menu layout should be added first..
 		 */
 		menuLayout = getLayoutInflater().inflate(R.layout.sliding_menu_layout,
 				null);
 		...
 		slidingMenuListView = (ListView) menuLayout
 				.findViewById(R.id.sliding_menu_list_view);
 		showSlidingMenuButton = (Button) mainLayout
 				.findViewById(R.id.show_menu_button);
 		...
 		/*
 		 * Initialize the menu adapter and set to list view to load menu from
 		 * the XML file.
 		 */
 		...
 		nativeSettingsButton = (ImageButton) mapLayout
 				.findViewById(R.id.settings_popup);
	...
	private static final int ASK_FEEDBACK_AFTER_DAYS = 0;
 	...
 	public void onResume() {
 		...
 		if (!isConnected()) {
 			...
 			new AlertDialog.Builder(this)
 					.setTitle("Error")
 					.setMessage(
 							"This device is not currently connected to the internet, please restart the application when a connection is available")
 					.setNeutralButton("Ok",
 	...
 	class FeedbackDialogListener implements DialogInterface.OnClickListener {
 		...
 		public FeedbackDialogListener(Context ctx,
 	...
 	public void onItemClick(AdapterView<?> adapterView, View view,
 		...
 		SlidingMenuItem menuItem = (SlidingMenuItem) menuAdapter
 				.getItem(position);
 	...
 	private void showPopup(final Activity context, Point p) {
 		...
 		View layout = layoutInflater.inflate(R.layout.popup_settings_layout,
 				viewGroup);
 		...
 		// Log.d("NATIVE MAP Activity", "Setting POINT: "+ pbtn.x + OFFSET_X +
 		// " " + pbtn.y + OFFSET_Y);
 	...
 	private void addRoutesTable(PopupWindow popup) {
 		...
 		TableLayout tl = (TableLayout) popup.getContentView().findViewById(
 				R.id.tableLayout1);
 		int cnt = 1;
 		TableRow tr = null;
 		CheckBox cb = null;
 	...
 	// Method to handle Toggle button to set flag for Live Bus updates from
 	// Teletrack
