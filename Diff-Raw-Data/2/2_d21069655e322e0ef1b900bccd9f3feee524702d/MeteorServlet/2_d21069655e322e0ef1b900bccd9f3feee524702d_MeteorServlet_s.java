 ...
 public class MeteorServlet extends HttpServlet {
 	...
 	public void doGet(HttpServletRequest req, HttpServletResponse res)
 		...
 		try {
 			...
 			if (parms.getForceRefresh()) {
 				sess.removeAttribute(parms.getSsn());
 			}
 
 			try {
 				String cached = (String) sess.getAttribute(parms.getSsn());
 				if(cached != null){ xml = cached; }
 			...
 			if (xml == null && !"query".equals(level)) {
 				...
 				try{
 					xml = indexQueryService.query(parms);
 					sess.setAttribute(parms.getSsn(), xml);
 			...
 			// If an Exception occurs, return the error to the client.
				...
				log.error("Error Transforminx XSL: " + xslFile, e);
 	...
 	/**
 	 * builds the MeteorParameters object.
 	 * @param ssn Social Security Number
 	 * @param dob Date of Birth
 	 * @param institutionID Institution ID
 	 * @param role User's Role
 	 * @return Returns a MeteorParameter object
 	 */
 	private void buildParameters(
 		...
 		parms.setRole(resource.getProperty("meteor.default.role"));
 		...
 		while (enum.hasMoreElements()) {
 			...
 			if (key.equalsIgnoreCase("SSN")) {
 			...
 			} else if (key.equalsIgnoreCase("INSTITUTION")) {
 				parms.setInstitutionID(value);
 			} else if (key.equalsIgnoreCase("ROLE")) {
 				String role = null;
 				if ("FAA".equals(value)) {
 					role = MeteorParameters.FAA;
 				} else if ("STUDENT".equals(value)) {
 					role = MeteorParameters.STUDENT;
 				} else {
 					throw new ParameterException("Invalid Role");
 				}
 				...
 				if(! acceptPassedAssertion ){
 					log.error("An assertion was passed into the Access Provider " +
 					          ...
 					          "The received assertion will be ignoored");
 					continue;
 				}
 				try {
 		...
 		// OK if DOB is null
 		//if(parms.getDob() == null) throw new ParameterException("Invalid Date of Birth (null)");
