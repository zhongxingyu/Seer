 ...
 public class IndexQueryService {
 			...
 			if(elapsedTime >= timeout){
 				...
 				for(int j = 0; j < t.length; j++){
 					...
 					if(log.isErrorEnabled()){
 						...
 						if(thread instanceof DataProviderThread){
 							DataProviderThread dpt = (DataProviderThread)thread;
 							DataProvider dp = dpt.getProvider();
 							if(dp != null){
 								name = dp.getURL().toString();
 							} else {
 								name = "Unknown";
 							}
 						} else {
 	...
 	private SecurityToken getSecurityToken(MeteorParameters params) throws AssertionException{
 		...
 		if( token == null && ! requirePassedAssertion){
 			try{
 				...
 				if(params.getRole().equals(SecurityToken.roleBORROWER)){
 		...
 		while(iterator.hasNext()){
 			...
 			try {
 				...
 				log.error(dProvider.getName() + " returned completely bogus data that didn't parse at all! " +
 				          "Please taunt them and tell them their mother was a hamster, and their father smelt of elderberries!");
 			}
 		}
 	...
 	public String query(MeteorParameters params, HttpSession session) throws AuthenticationLevelException{
 		...
 		if (params.getForceRefresh()) {
 			...
 			session.removeAttribute(params.getSsn());
 		}
 
 		if(session != null){
 			try {
 				responseData = (ResponseData) session.getAttribute(params.getSsn());
 		...
 		if(responseData == null) {
 			try {
 				...
 				return createErrorXML("E", e.getMessage());
