 ...
 public class ProductDAO implements ProductDAOInterface {
 	...
 	private final static String GET_SELECTION = "SELECT SQL_CALC_FOUND_ROWS * FROM products JOIN auctions ON products.id_product = auctions.product_id WHERE sub_category_id = ? AND status = 'inProgress' AND end_time > ?"
 			+ "LIMIT ?, ?";
	private final static String GET_ROWS_COUNT_BY_SYBCATEGORY_ID = "SELECT COUNT(id_product) FROM products JOIN auctions ON products.id_product = auctions.product_id WHERE deleted=false AND sub_category_id = ? AND status= 'inProgress'";
