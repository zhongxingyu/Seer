 ...
 {
   String baseURL = "http://untappdpicker.heroku.com/";
   // String baseURL = "http://localhost:8080/";
    private final int COOKIE_AGE = 60 * 60 * 24 * 365;
          ...
          throws ServletException, IOException
    {
       ...
       if (req.getParameter("action") != null && req.getParameter("action").equals("getBeers"))
       {
          ...
          Map<Beer, Integer> beers = c.getOptions(v, AssetManager.getInstance(token).getUser(),
                token);
 
          System.out.println("READ READ = " + beers.size());
 
          String retString = "<HTML><BODY><TABLE><TR><TD valign=top>";
          int count = 1;
          for (Entry<Beer, Integer> b : beers.entrySet())
             if (b.getValue() > 1)
                retString += "<p>" + (count++) + ": <A HREF=\"https://untappd.com/beer/"
                      + b.getKey().getId() + "\">" + b.getKey().getName() + "</A> ("
                      + b.getKey().getScore() + ")</p>";
          retString += "</TD><TD>";
       ...
       else if (req.getParameter("code") != null)
       {
          ...
          String url = "https://untappd.com/oauth/authorize/?client_id=" + System.getenv("UT_ID")
                ...
                + "&response_type=code&redirect_url=" + baseURL + "&code=" + code;
          String response = "";
          ...
          String result = "<HTML><BODY><SCRIPT>window.location.href='/'</SCRIPT></BODY></HTML>";
          // String result = "";
          PrintStream ps = new PrintStream(resp.getOutputStream());
          ps.println(result);
          ps.close();
       ...
       else if (req.getParameter("action") != null && req.getParameter("action").equals("login"))
       {
          ...
          else
          {
             ...
             String result = "<HTML><BODY><SCRIPT>window.location.href="
                   + "'http://untappd.com/oauth/authenticate/?client_id=" + System.getenv("UT_ID")
                   + "&client_secret=" + System.getenv("UT_SECRET")
                   + "&response_type=code&redirect_url=" + baseURL + "'</SCRIPT></BODY></HTML>";
       ...
       else if (req.getParameter("action") != null && req.getParameter("action").equals("land"))
       {
          String result = "<HTML><BODY><A HREF='login'>I WANT A BEER</A></BODY></HTML>";
          PrintStream ps = new PrintStream(resp.getOutputStream());
          ps.println(result);
          ps.close();
       }
       else if (req.getParameter("action") != null && req.getParameter("action").equals("manadd"))
       {
          ...
          BeerScoreFinder bsf = new BeerScoreFinder(getUserToken(req));
          BeerScore score = bsf.getScore(new Beer(beerid, "Unknown"));
 
          Drink d = new Drink(new Beer(beerid, "Unknown " + beerid), System.currentTimeMillis(),
                new User("ManualUser"), venueid);
          if (score != null)
             d = new Drink(new Beer(beerid, score.getName()), System.currentTimeMillis(), new User(
                   "ManualUser"), venueid);
          manager.addDrink(d);
       }
       else
