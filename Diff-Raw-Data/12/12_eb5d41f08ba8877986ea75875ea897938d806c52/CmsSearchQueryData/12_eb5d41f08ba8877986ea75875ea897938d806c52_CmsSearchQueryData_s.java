 ...
 /**
  * Stores all information to create a Search query.<p>
  * 
  * Use the {@link #toString()} to get the Search Query.<p>
  */
 public class CmsSearchQueryData implements IsSerializable {
 
     /** Date format used by Search. */
     public static final String SEARCH_DATEFORMAT = "yyyy-MM-dd'T'HH:mm:ss'Z'";
      ...
      */
     public CmsSearchQueryData(CmsSearchConfig config) {
      ...
      * 
      * @param config the Search search configuration
      * @param parameterString the parameter as string
      * 
      * @return the reconstructed search data object
      */
     public static CmsSearchQueryData fromParameterString(CmsSearchConfig config, String parameterString) {
 
         String parameters = URL.decode(parameterString);
         CmsSearchQueryData data = new CmsSearchQueryData(config);
         if (parameters != null) {
         ...
         Map<String, String[]> params = CmsSearchStringUtil.createParameterMap(parameters);
      ...
      * @return the query
      */
     public static CmsSearchQueryData fromShortQuery(CmsSearchConfig config, String parameterString) {
     ...
     public String createQueryString(boolean isTitle) {
        ...
        buf.append(cerateFilterQuery("type", m_resourceTypes, true));
         ...
         for (String facet : m_config.getFacetFieldNames()) {
             if (!facet.equals(CmsSearchConfig.SPECIAL_SPACER_FIELD)) {
         ...
         if ((m_startDate != null) || (m_endDate != null)) {
             ...
             if (m_startDate != null) {
                 startDate = DateTimeFormat.getFormat(SEARCH_DATEFORMAT).format(m_startDate);
             }
             if (m_endDate != null) {
                 endDate = DateTimeFormat.getFormat(SEARCH_DATEFORMAT).format(m_endDate);
             }
