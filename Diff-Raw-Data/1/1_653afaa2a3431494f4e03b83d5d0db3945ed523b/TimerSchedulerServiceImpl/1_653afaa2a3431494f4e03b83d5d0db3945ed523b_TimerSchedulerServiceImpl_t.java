 ...
 public class TimerSchedulerServiceImpl implements SchedulerService { 
 	...
 	private static Log log = 
 		LogFactory.getLog( TimerSchedulerServiceImpl.class );
 
 	/**
 	 * Registered task list
 	 */
 	private Set<TaskDefinition> registeredTasks = 
 		new HashSet<TaskDefinition>(); 
 	
 	/**
 	 * Scheduled Task Map
 	 */
 	private static Map<Integer, TimerSchedulerTask> scheduledTasks = 
 		new HashMap<Integer, TimerSchedulerTask>();
 
 	/**
 	 * A single timer used to keep track of all scheduled tasks. The Timer's
 	 * associated thread should run as a daemon.  
 	 * 
 	 * A deamon thread is called for if the timer will be used to schedule 
 	 * repeating "maintenance activities", which must be performed as long 
 	 * as the application is running, but should not prolong the lifetime 
 	 * of the application.
 	 * 
 	...
 	public SortedMap<String,String> getSystemVariables() {
 		TreeMap<String,String> systemVariables = 
 			new TreeMap<String,String>(new InsertedOrderComparator());
 		...
 		systemVariables.put("SCHEDULER_MILLIS_PER_SECOND", 
 				String.valueOf(SchedulerConstants.SCHEDULER_MILLIS_PER_SECOND));
