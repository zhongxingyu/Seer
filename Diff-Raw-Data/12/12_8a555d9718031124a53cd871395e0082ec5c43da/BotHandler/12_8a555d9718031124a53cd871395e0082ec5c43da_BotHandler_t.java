 ...
 public class BotHandler
 {
     ...
     public void onPlayerKick(PlayerKickEvent event)
     {
         ...
         if (data.getBotTracker().autoipkick.contains(event.getPlayer().getAddress().toString().split(":")[0]))
         {
             event.setLeaveMessage(null);
             return;
         }
     ...
     public void onPlayerJoin(PlayerJoinEvent event)
     {
         try
         {
             ...
             if (Permissions.JOIN.getPermission(event.getPlayer()))
             {
                 ...
                 if (data.getBotTracker().reanibo && Permissions.NOTIFY.getPermission(event.getPlayer()))
                 {
                     event.getPlayer().sendMessage(Settings.prefix + "\247c" + Settings.connectInvasion);
                 }
                 if (data.getBotTracker().reanibo && Permissions.NOTIFY.getPermission(event.getPlayer()) && Settings.interval > 100000)
                 {
             ...
             if (!data.getBotTracker().reanibo)
             {
                 ...
                 data.getBotTracker().trackPlayer(event.getPlayer(), event.getPlayer().getAddress().toString().split(":")[0]);
             ...
             if (data.getBotTracker().botcts > Settings.accounts + 2 && data.getBotTracker().reanibo)
             {
             ...
             long math = data.getBotTracker().time - data.getBotTracker().lasttime;
             ...
             botclass.getUtility().getDebug().debug("Time: " + data.getBotTracker().time);
             ...
             botclass.getUtility().getDebug().debug("Lasttime: " + data.getBotTracker().lasttime);
             botclass.getUtility().getDebug().debug("BotCts: " + data.getBotTracker().botcts + " Accs: " + Settings.accounts);
