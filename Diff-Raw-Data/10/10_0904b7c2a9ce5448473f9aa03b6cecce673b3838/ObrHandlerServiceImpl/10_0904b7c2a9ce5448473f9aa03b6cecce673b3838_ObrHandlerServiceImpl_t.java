 ...
 class ObrHandlerServiceImpl extends ServiceTracker implements ObrHandlerService {
     ...
     public Object addingService(ServiceReference reference) {
         if (this.getTrackingCount() == 1) {
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "addingService",
     ...
     private synchronized void _setupRepository(File repoDir) throws Exception {
         ...
         if (repoFile != null && repoFile.exists()) {
             ...
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "_setupRepository", "Thread #{0}: updateRepository took {1} ms",
                     new Object[]{tid, t2 - t});
         } else {
             long t = System.currentTimeMillis();
             repository = createRepository(repoFile, repoDir);
             long t2 = System.currentTimeMillis();
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "_setupRepository", "Thread #{0}: createRepository took {1} ms",
                     new Object[]{tid, t2 - t});
         }
 ...
 //                new Object[]{tid, t2 - t});
     ...
     private Repository createRepository(File repoFile, File repoDir) throws IOException {
         ...
         for (File jar : findAllJars(repoDir)) {
             ...
             if (r == null) {
             	logger.logp(Level.WARNING, "ObrHandlerServiceImpl", "createRepository", "{0} not an OSGi bundle", jar.toURI().toURL());
             } else {
                 resources.add(r);
             }
         }
         Repository repository = dmh.repository(resources.toArray(new Resource[resources.size()]));
         logger.logp(Level.INFO, "ObrHandlerServiceImpl", "createRepository", "Created {0} containing {1} resources.",
     ...
     private Repository updateRepository(File repoFile, File repoDir) throws Exception {
         ...
         if (isObsoleteRepo(repository, repoFile, repoDir)) {
             ...
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "updateRepository", "Recreating {0}", new Object[]{repoFile});
             repository = createRepository(repoFile, repoDir);
         }
         return repository;
     }
     ...
     private boolean isObsoleteRepo(Repository repository, File repoFile, File repoDir) {
         ...
         for (File jar : findAllJars(repoDir)) {
             if (jar.lastModified() > lastModifiedTime) {
                 logger.logp(Level.INFO, "ObrHandlerServiceImpl", "isObsoleteRepo", "{0} is newer than {1}", new Object[]{jar, repoFile});
                 return true;
             }
         ...
         if (totalSize != 0) {
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "isObsoleteRepo", "Change in size detected by {0} bytes", new Object[]{totalSize});
             return true;
         }
         return false;
     }
     ...
     /* package */ boolean resolve(final Resolver resolver, Resource resource) {
         ...
         logger.logp(Level.INFO, "ObrHandlerServiceImpl", "resolve", "At the end of first pass, resolver outcome is \n: {0}",
 ...
 //        }
     ...
     private Resource findResource(String name, String version) {
         ...
         if (repositoryAdmin == null) {
             logger.logp(Level.WARNING, "ObrHandlerServiceImpl", "findResource",
         ...
         try {
             ...
             logger.logp(Level.INFO, "ObrHandlerServiceImpl", "findResource",
     ...
     private void printResolverOutput(Resolver resolver) {
         ...
         logger.logp(Level.INFO, "ObrHandlerServiceImpl", "printResolverOutput", "OBR resolver state: {0}", new Object[]{sb});
     }
