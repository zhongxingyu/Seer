 ...
 public class LayerExtTriangle implements LayerDelaunay {
 	...
 	private static Logger logger = Logger.getLogger(LayerExtTriangle.class.getName());
 	//Final list
     ...
     private void Unitize()
     {
     	...
     	for(int idv=0;idv<this.vertices.size();idv++)
     	{
     		...
     		this.vertices.set(idv, new Coordinate(v.x-unitize_translation_vec.x,v.y-unitize_translation_vec.y));
     	}
     ...
     private void readSegsFile(final String polyFileName,ArrayList<Coordinate> localVertices) throws LayerDelaunayError
     {
     	this.segments.clear();
       	////////////////////////////////////////////
     	// Read model.1.poly
     ...
     private void readNeighFile(final String eleFileName) throws LayerDelaunayError
     {
 	...
 	private void writePolyFile(final String filepath) throws LayerDelaunayError
     {
     	try
     	{
 			...
 			out.printf("%d 2 0 0%s",vertices.size(),EOL); // <# of vertices> <dimension (must be 2)>  <# of attributes> <# of boundary markers (0 or 1)> 
 	
 			// write vertices
 			int coordinateIndex=1;
 			for(Coordinate vertice : vertices)
 			{
 				out.printf("%d %s %s%s",coordinateIndex,Double.toString(vertice.x),Double.toString(vertice.y),EOL); //<vertex #> <x> <y>  [attributes] [boundary marker] 
 				coordinateIndex++;
 			}
 			// write segments
 			out.printf("%d 0%s",segments.size(),EOL); //<# of segments> <# of boundary markers (0 or 1)> 
 			int segmentIndex=1;
 			for(IntSegment segment : segments)
 			{
 				out.printf("%d %d %d%s",segmentIndex,segment.getA() +1,segment.getB()+1,EOL); //<segment #> <endpoint> <endpoint>  [boundary marker]
 				segmentIndex++;
 			}
 			...
 			{
 				out.printf("%d %s %s%s",holeIndex,Double.toString(holePosition.x),Double.toString(holePosition.y),EOL); //<hole #> <x> <y> 
 				holeIndex++;
 			}			
 			out.close();
 		} catch (FileNotFoundException e) {
 	...
 	public String processDelaunay(String prefix,int delaunayIndex,int maxSteiner,boolean steinerOnBoundaries,boolean savePolyToRefine) throws LayerDelaunayError {
 		try {
 			...
 			if(savePolyToRefine)
 			{
 				finalName=this.tempdir+File.separatorChar+"firstPass"+delaunayIndex+".poly";
