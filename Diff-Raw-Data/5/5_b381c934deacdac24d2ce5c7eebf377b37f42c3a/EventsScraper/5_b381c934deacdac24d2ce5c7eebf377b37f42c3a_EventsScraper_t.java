 ...
 public class EventsScraper extends BasicScraper {
 	...
 	/**
 	 * Gibt den Adapter fr den Spinner zurck
 	 * 
 	...
 	/**
 	 * Gibt einen ListAdapter mit allen Events zurck
 	 * 
 	...
 	/**
 	 * Gibt einen ListAdapter mit den Modulen zurck
 	 * 
 	...
 	/**
 	 * Gibt einen ListAdapter zurck der durch die Anmeldungsdaten anzeigt
 	 * 
 	...
 	/**
 	 * Gibt einzelne Events in einem ListAdapter zurck.
 	 * 
 	 * @param content
 	 *            Content div Element
 	 * @return ListAdapter
 	...
 	private ListAdapter getApplicationSingleItems(Element content) {
 		...
 		if (coursestatusTable != null) {
 			...
 			if (moduleTable.size() > 0) {
 				...
 				//Adapter zum zurckgeben erstellen
 	...
 	/**
 	 * Gibt einen Adapter zurck welcher die Kategorien enthalten um tiefer zu
 	 * gehen. Speichert auerdem den Link zu diesen Kategorien in applyLink ab
 	 * 
 	 * @param content
 	 *            Das Seitenelement, welches diese informationen enthlt
 	...
 	/**
 	 * Gibt einen Adapter zurck, welcher auf der Hauptseite angezeigt wird.
 	 * Auerdem speichert er die Links fr die Einzelnen Seiten in den
 	 * eventLinks ab
 	 * 
 	 * @return ListAdapter mit der MenNavigation
 	 * @author Daniel Thiem
 	 * 
 	 */
 	private ListAdapter getMenuAdapter() {
 		...
 		// ArrayList kopieren um Links nicht neu laden zu mssen
