  ...
  * and add it to the list obtained from getOverlays() of {@link MapView}.
  * 
  * This class implements a form of Gesture Handling similar to
  * {@link android.view.GestureDetector.SimpleOnGestureListener} and
  * {@link GestureDetector.OnGestureListener}. The difference is there is an
  * additional argument for the item.
  * 
 ...
 public abstract class Overlay implements OverlayConstants {
 	 ...
 	 *            drawable itself.
 	 */
 	protected synchronized static void drawAt(
 			final android.graphics.Canvas canvas,
 			final android.graphics.drawable.Drawable drawable, final int x,
 			final int y, final boolean shadow) {
 		drawable.copyBounds(mRect);
 		drawable.setBounds(mRect.left + x, mRect.top + y, mRect.right + x,
 				mRect.bottom + y);
 	...
 	public Overlay() {
 		this(MyResourceProxy.getInstance());
 	}
 	 ...
 	 */
 	// protected abstract void draw(final Canvas c, final MapView osmv, final
 	// boolean shadow);
