 ...
 public class RegexObject extends PyObject {
     ...
     public MatchObject match(String string, int pos, int endpos) {
         if (endpos > string.length()) endpos = string.length();
         if (endpos < pos) endpos = pos;
 
         MatchResult result = doMatch(new PatternMatcherInput(string, pos, endpos-pos));
         if (result == null) return null;
     ...
     private MatchResult doMatch(Object input) {
         ...
         if (input instanceof String) {
             if (!matcher.matchesPrefix((String)input, code)) return null;
         } else {
             if (!matcher.matchesPrefix((PatternMatcherInput)input, code)) return null;
         }
     ...
     public MatchObject search(String string, int pos, int endpos) {
         ...
         MatchResult result = doSearch(new PatternMatcherInput(string, pos, endpos-pos));
         if (result == null) return null;
         return new MatchObject(this, string, pos, endpos, result);
     }
     ...
     private MatchResult doSearch(Object input) {
         ...
         if (input instanceof String) {
             if (!matcher.contains((String)input, code)) return null;
         } else {
             if (!matcher.contains((PatternMatcherInput)input, code)) return null;
         }
     ...
     public PyTuple subn(PyObject repl, String string, int count) {
         ...
         while (n < count && !match.endOfInput()) {
             if (!matcher.contains(match, code)) break;
             n++;
             ...
             if (srepl == null) {
                 MatchObject m = new MatchObject(this, string, lastmatch, string.length(), 
                                         matcher.getMatch());
                 PyObject ret = repl.__call__(m);
                 buf.append(ret.toString());
             } else {
                 if (expand) buf.append(expandMatch(matcher.getMatch(), srepl));
                 else buf.append(srepl);
             }
     ...
     private String fixPattern(String pattern, PyObject groupindex) {
         ...
         while (index < n) {
             if (chars[index++] == '(') {
                 ...
                 if (index > 2 && chars[index-2] == '\\') continue;
                 ...
                 if (index < n && chars[index] == '?') {
                     ...
                     if (index < n && chars[index] == 'P') {
                         ...
                         if (c == '<') {
                             ...
                             if (index == n) throw re.ReError("unmatched <");
                             String name = new String(chars, start, index-start);
                             groupindex.__setitem__(new PyString(name), new PyInteger(group));
                             ...
                             if (c == '=') {
                                 ...
                                 if (index == n) throw re.ReError("?P= not closed");
