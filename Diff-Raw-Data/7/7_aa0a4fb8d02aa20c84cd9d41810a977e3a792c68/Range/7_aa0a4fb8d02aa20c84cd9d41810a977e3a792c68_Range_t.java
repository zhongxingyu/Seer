 ...
 public class Range { // TODO -instantiable superclass
 	...
 	public Table getTable(Paragraph paragraph) {
		...
		if (r._parStart != 0 && getParagraph(r._parStart - 1 -_parStart).isInTable()
				&& getParagraph(r._parStart - 1 - _parStart)._sectionEnd >= r._sectionStart) {
 		...
 		for (; tableEnd < limit; tableEnd++) {
			if (!getParagraph(tableEnd-_parStart).isInTable()) {
 				break;
 			}
 		}
