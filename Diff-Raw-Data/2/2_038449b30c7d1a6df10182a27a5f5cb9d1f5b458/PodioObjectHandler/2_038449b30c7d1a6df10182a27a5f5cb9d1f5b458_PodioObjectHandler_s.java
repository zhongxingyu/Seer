 ...
 public class PodioObjectHandler {
 	...
 	public void writeShows(PodioShow... pshow) throws SQLException {
 		Connection c = DBConnector.getInstance().getConnection();
 		PreparedStatement ps = c
				.prepareStatement("INSERT INTO shows (showID, bandID, date, promoID, state, comments, lastEdit) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY MODIFY showID=?, bandID=?, date=?, promoID=?, state=?, comments=?, lastEdit=?");
