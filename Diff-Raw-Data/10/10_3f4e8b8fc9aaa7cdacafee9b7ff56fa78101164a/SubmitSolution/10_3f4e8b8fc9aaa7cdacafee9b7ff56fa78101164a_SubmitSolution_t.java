 	...
 	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
 		...
 		for (int partnerID : partnerIDs) {
 			...
 			if (submission.getSubmitters().size() < task.getMaxSubmitters() && partnerParticipation != null && partnerParticipation.getLecture().getId() == task.getTaskGroup().getLecture().getId() && submissionDAO.getSubmissionLocked(task, partnerParticipation.getUser()) == null) {
 				...
 				tx.rollback();
 				template.printTemplateHeader("Ungltige Anfrage");
 				out.println("<div class=mid>Ein ausgewhlter Partner hat bereits eine eigene Abgabe initiiert oder Sie haben bereits die maximale Anzahl von Partnern ausgewhlt.</div>");
 				template.printTemplateFooter();
 				return;
 			}
 		}
 			...
 			while (iter.hasNext()) {
 				...
 				if (!item.isFormField()) {
 					...
 					if (!m.matches()) {
 						...
 						if (uploadFor > 0) {
 							out.println("<br>Fr Experten: Der Dateiname muss dem folgenden regulren Ausdruck gengen: " + Util.escapeHTML(pattern.pattern()));
 						}
 						template.printTemplateFooter();
 						return;
 					}
