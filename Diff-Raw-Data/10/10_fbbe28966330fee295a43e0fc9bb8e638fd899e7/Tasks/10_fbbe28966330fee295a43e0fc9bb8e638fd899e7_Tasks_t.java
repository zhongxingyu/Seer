 ...
 public class Tasks extends Controller {
 	...
 	public static Result getTasks(String pEnvName, String pRunId, String pTaskId) {
 		...
 		try {
 			if ((pEnvName != null) && (!pEnvName.trim().isEmpty())) {
 				...
 				if (parentTask != null) {
 					...
 					tasks = Task.find.fetch("parent").where().eq("environment", parentTask.environment).or(Expr.eq("path", parentTask.path), Expr.startsWith("path", parentTask.path+"/")).orderBy("path asc").findList();
 				}
 			}
 		...
 		// ------------------------------------------------
		List<ValueOccurrence> valueOccurrences = new ArrayList<ValueOccurrence>();
		List<Value> tempValues = values;
		//System.out.println("------> "+tempValues.size());
		while (tempValues.size() != 0) {
			//System.out.println("------> "+tempValues.size());
			valueOccurrences.addAll(ValueOccurrence.find.where().in("value", tempValues.subList(0, Math.min(1000, tempValues.size()))).in("run", runs).findList());
			tempValues = tempValues.subList(Math.min(1000, tempValues.size()), tempValues.size());
		}
		//System.out.println("------> "+tempValues.size());
 		...
 		for (Run run : runs) {
 			...
 			gsonRun.put("description", run.description);
 			if (cpt < runs.size()) {
 				Run nextRun = runs.get(cpt);
 				//System.out.println(cpt+" "+run.getId()+"->"+nextRun.getId());
 				Result checkResult = checkMergeAllow(run.getId(), nextRun.getId(), run, nextRun);
 				if (checkResult == null) {
 					gsonRun.put("mergeTarget", nextRun.getId());
 				}
 			}
 	...
 	public static Result mergeRuns(Long runIdSrc, Long runIdTrg) {
 		...
 		Result checkRes = checkMergeAllow(runIdSrc, runIdTrg, runSrc, runTrg);
 		if (checkRes != null) {
 			return checkRes;
 		}
 	 ...
 	 * @return
 	 */
 	private static Result checkMergeAllow(Long runIdSrc, Long runIdTrg, Run runSrc, Run runTrg) {
 		if (runSrc == null) {
 			//Logger.debug("Run not found "+runIdSrc);
 			return badRequest("Run not found "+runIdSrc);
 		}
 		if (runTrg == null) {
 			//Logger.debug("Run not found "+runIdTrg);
 			return badRequest("Run not found "+runIdTrg);
 		}
 		
 		// Some checks
 		if ((runSrc.environment != null) && (runTrg.environment != null) && (!runSrc.environment.equals(runTrg.environment))) {
 			//Logger.debug("Not on the same enviroment ("+runSrc.environment+" and "+runTrg.environment+")");
 			return badRequest("Not on the same enviroment ("+runSrc.environment+" and "+runTrg.environment+")");
 		}
 		if (runSrc.description != null) {
 			//Logger.debug("Source is a real run (description not null)");
 			return badRequest("Source is a real run (description not null)");
 		}
 		if (runTrg.description == null) {
 			//Logger.debug("Not real target run (description is null)");
