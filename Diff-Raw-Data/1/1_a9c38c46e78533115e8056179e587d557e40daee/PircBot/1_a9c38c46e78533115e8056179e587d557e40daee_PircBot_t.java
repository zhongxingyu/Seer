 ...
 public abstract class PircBot implements ReplyConstants {
 	...
 	private static final int HOP_ADD = 5;
 	private static final int HOP_REMOVE = 6;
 	private static final int AOP_ADD = 7;
 		...
 		if (_channelPrefixes.indexOf(target.charAt(0)) >= 0) {
 			...
 			for (int i = 0; i < params[0].length(); i++) {
 				...
 				if (atPos == '+' || atPos == '-') {
 					...
 					if (pn == '+') {
 						this.updateUser(channel, HOP_ADD, params[p]);
 						onVoice(channel, sourceNick, sourceLogin,
 								sourceHostname, params[p]);
 					} else {
 						this.updateUser(channel, HOP_REMOVE, params[p]);
 						onDeVoice(channel, sourceNick, sourceLogin,
 								sourceHostname, params[p]);
 					}
 					p++;
 					...
 					if (pn == '+') {
 						this.updateUser(channel, AOP_ADD, params[p]);
 						onVoice(channel, sourceNick, sourceLogin,
 								sourceHostname, params[p]);
 					} else {
 						this.updateUser(channel, AOP_REMOVE, params[p]);
 						onDeVoice(channel, sourceNick, sourceLogin,
 								sourceHostname, params[p]);
 					}
 					p++;
 	...
 	private String _version = "PircBot " + VERSION
