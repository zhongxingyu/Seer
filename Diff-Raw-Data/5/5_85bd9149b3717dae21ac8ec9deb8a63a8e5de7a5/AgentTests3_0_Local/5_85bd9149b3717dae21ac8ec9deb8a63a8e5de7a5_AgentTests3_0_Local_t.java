 ...
 * $Id: AgentTests3_0_Local.java,v 1.4 2008-08-29 20:31:28 nithyas Exp $
 ...
 /**
  * This class tests is for Web Agents 3.0 and above
  * Tests the HotSwappable property of the Agents using 
  * Centralised Configuration
  */
 
 public class AgentTests3_0_Local extends TestCommon {
     ...
     throws Exception {
         ...
         try {
             ...
             if (strAgentBeingTested.contains("3.0")) { 
                 if (strHeaderFetchMode  == null) {
                     strHeaderFetchMode = rbg.getString(strGblRB + 
                             ".headerFetchMode");
                 }
                         
             // Checking if agent configuration is local
             String strIsCentralized = rbg.getString(strGblRB + 
                             ".30agentConfigurationType");
             if (!strIsCentralized.equals("local")) {
                 Reporter.log("These test cases are for 3.0 agents with local " +
                         " configuration only. Agent configuration is : " + 
                         strIsCentralized + ". Hence skipping test cases.");
                 log(Level.FINE, "setup", "These test cases are for 3.0 " + 
                         "agents with local configuration only. " +
                         "Agent configuration is : " + strIsCentralized + 
                         ". Hence skipping test cases.");
                 assert(false);
             }
