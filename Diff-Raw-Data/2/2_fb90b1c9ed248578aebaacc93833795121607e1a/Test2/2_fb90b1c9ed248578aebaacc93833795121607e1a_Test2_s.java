 ...
 public class Test2 implements AsynchMessageReceiver {
 	...
 	try {
 	    ...
 	    System.out.println("Loggar in som " +
 			       ...
 			       names[0].confNo + ")");
 	    if (!foo.login(names[0].confNo, argv.length > 1 ? argv[1] : crtReadLine("lsenord> "), false)) {
 		System.err.println("Inloggningen misslyckades!");
 		System.exit(-32);
 	    }
 	    System.out.println("Inloggad. Vlkommen till LysKOM!");
 		...
 		if (rc == 0) { // default-action (read next text)
 			...
 			if (nextConf == -1) System.out.println("Det finns inga fler olsta mten.");
 			else {
 			    System.out.println("Gr till mte: " + confNoToName(nextConf));
 	...
 	String curConf = conf == -1 ? "Ej nrvarande i ngot mte" : confNoToName(conf);
 	return curConf + "> ";
     }
 	...
 	if (cmd.equals("")) { // terse inlgg ( <txtno>)
 	    String txt = st.hasMoreTokens() ? st.nextToken() : null;
 	    if (txt == null) throw new CmdErrException("du mste ange inlggsnummer");
 	    int txtNo = 0;
 	    try {
 		txtNo = Integer.parseInt(txt);
 	    } catch (NumberFormatException ex) {
 		throw(new CmdErrException("trasigt inlggsnummer: " + txt));
 	    }
 	...
 	if (cmd.equals("k")) { // kommentera (k [txtno])
 		...
 		try {
 		    ...
 		    throw new CmdErrException("trasigt inlggsnummer: " + opt);
 		}
 	    }
 	...
 	if (cmd.equals("i")) { // skriv inlgg i nuvarande mte
 	    StringBuffer textb = new StringBuffer();
 	    System.out.println("-- Inlgg i mte " + confNoToName(foo.getCurrentConference()));
 	    ...
 	    else System.out.println("misslyckades att skapa inlgg.");
 	    return 1;
 	}		 
 	...
 	if (cmd.equals("u")) { // terse urinlgg
 	    // footnotes breaks the chain
 	    System.out.print("Sker efter urinlgg fr text " + t.getNo() + "... ");
 	...
 	if (cmd.equals("g")) { // g (till mte) (g <mtesnamn>)
 	    ...
 	    if (names.length == 0) {
 		System.out.println("%Fel: mtet finns inte");
 		return 1;
 	    }
 	    if (names.length > 1) {
 		System.out.println("%Fel: flertydigt mtesnamn");
 		for (int i=0; i < names.length; i++)
 		    System.out.println("-- alternativ " + (i+1) + ": " + names[i].getNameString());
 		return 1;
 	    }
 	    System.out.println("-- g till mte: " + names[0].getNameString());
 	    foo.changeConference(names[0].getNo());
 	    return 1;
 	}
 	if (cmd.equals("nm")) { // nsta mte
 	    int nextConf = foo.nextUnreadConference(true);
 	    if (nextConf != -1)
 		System.out.println("nsta mte - " + confNoToName(nextConf));
 	    else
 		System.out.println("Det finns inga fler olsta mten.");
 	    return 1;
 	}
 	if (cmd.equals("q") || cmd.equals(".")) return -1; // avsluta
 	throw(new CmdErrException("Frstod inte \"" + cmd + "\""));
