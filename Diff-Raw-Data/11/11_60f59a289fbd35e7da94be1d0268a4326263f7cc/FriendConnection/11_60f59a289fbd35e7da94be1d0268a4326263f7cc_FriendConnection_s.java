 ...
 public class FriendConnection {
 	 ...
 	 * @param _remoteFriend
 	 */
 	public FriendConnection(GlobalManagerStats stats, QueueManager _queueManager, ConnectionEndpoint remoteFriendAddr, Friend _remoteFriend, FileListManager _filelistManager, FriendConnectionListener _listener) {
 		remoteFriendAddr.addProtocol(new ProtocolEndpointTCP(remoteFriendAddr.getNotionalAddress()));
 		...
 		sharedSecret[0] = OneSwarmSslTransportHelperFilterStream.SHARED_SECRET_FOR_SSL_STRING.getBytes();
 		...
 		this.connection = NetworkManager.getSingleton().createConnection(remoteFriendAddr, new OSF2FMessageEncoder(), new OSF2FMessageDecoder(), true, false, sharedSecret);
 		...
 		this.connection.connect(null, false, new ConnectionListener() {
 			...
 			public void connectSuccess(ByteBuffer remaining_initial_data) {
				...
				updateFriendConnectionLog(true, "connected to: " + getRemoteIp().getHostAddress() + ":" + getRemotePort());
 				...
 				friendConnectionQueue = queueManager.registerConnectionForQueueHandling(FriendConnection.this);
 	 ...
 	 */
 	public FriendConnection(GlobalManagerStats stats, QueueManager _queueManager, NetworkConnection _connection, Friend _remoteFriend, FileListManager _filelistManager, FriendConnectionListener _listener) {
 		this.queueManager = _queueManager;
 		this.outgoing = false;
 	...
 	public void clearTimedOutForwards() {
 		...
 		for (Integer key : timedOutIds) {
 			...
 			if (overlayPathLogger.isEnabled()) {
 				try {
 					if (f != null) {
 						overlayPathLogger.log(System.currentTimeMillis() + ", timeout_forward, " + f.getChannelId() + ", " + f.getBytesForwarded() + ", " + f.getAge() + ", " + f.getRemoteFriend().getNick() + ", " + f.getRemoteIpPort());
 					}
 				} catch (Exception e) {
 					e.printStackTrace();
 				}
 	...
 	public void close() {
 		...
 		List<OverlayTransport> transports = new LinkedList<OverlayTransport>(overlayTransports.values());
 	...
 	private void connectionException(Throwable error) {
 		logger.warning(getDescription() + ": got exception in " + "OSF2F session (" + connection + "/" + remoteFriend.getNick() + ") disconnecting: " + error.getMessage() + " (from: " + error.toString() + ")");
 		...
 		if (friendLogMessage == null) {
 			...
 			expectedError = true;
 		} else if (friendLogMessage.startsWith("An existing connection was forcibly closed by the remote host")) {
 			expectedError = true;
 		}
 	...
 	private void updateFriendConnectionLog(boolean append, String message) {
 		...
 		logger.fine(remoteFriend.getNick() + ": " + message);
 	}
 	...
 	private void deregisterOverlayForward(int channelId, boolean sendReset) {
 		...
 		if (overlayPathLogger.isEnabled()) {
 			try {
 				if (f != null) {
 					overlayPathLogger.log(System.currentTimeMillis() + ", deregistered_forward, " + f.getChannelId() + ", " + f.getBytesForwarded() + ", " + f.getAge() + ", " + f.getRemoteFriend().getNick() + ", " + f.getRemoteIpPort());
 				}
 			} catch (Exception e) {
 				e.printStackTrace();
 			}
 	...
 	void deregisterOverlayTransport(OverlayTransport transport) {
 		...
 		try {
 			...
 			if (overlayPathLogger.isEnabled()) {
 				overlayPathLogger.log(System.currentTimeMillis() + ", deregistered_transport, " + exists.getChannelId() + ", " + exists.getBytesIn() + ", " + exists.getBytesOut() + ", " + exists.getAge() + ", " + exists.getPathID());
 			}
 	...
 	public void doKeepAliveCheck() {
 		if (friendConnectionQueue.getLastMessageSentTime() > KEEP_ALIVE_FREQ) {
 			connection.getOutgoingMessageQueue().addMessage(new BTKeepAlive(OSF2FMessage.CURRENT_VERSION), false);
 		}
 	}
 		...
 		if (message instanceof OSF2FChannelDataMsg) {
 			...
 			if (overlayTransports.containsKey(channelId)) {
 				...
 				if (!recentlyClosedChannels.containsKey(msg.getChannelId())) {
 					logger.warning(getDescription() + ": got channel message for unregistered channel id " + msg.getChannelId() + " / " + msg.getCreatedTime() );
 				}
 			}
 
 		} else {
 			logger.warning(getDescription() + ": handleChannelMsg " + "got non OSF2FChannelMsg message: " + message.getDescription());
 		}
 	}
 	...
 	private void handleChannelReset(Message message) {
 		...
 		if (message instanceof OSF2FChannelReset) {
 			...
 			if (overlayTransports.containsKey(msg.getChannelId())) {
 				...
 				if (!recentlyClosedChannels.containsKey(msg.getChannelId())) {
 					logger.warning(getDescription() + ": got channel reset for unregistered channel id " + msg.getChannelId());
 				}
 			}
 		} else {
 			Debug.out(remoteFriend.getNick() + ": handleChannelReset " + "got non OSF2FChannelReset message: " + message.getDescription());
 		}
 	...
 	private void handleChannelSetup(Message message) {
 		if (message instanceof OSF2FHashSearchResp) {
 			...
 			Debug.out(remoteFriend.getNick() + ": handleChannelSetup " + "got non channelSetup message: " + message.getDescription());
 		}
 	}
 	...
 	private void handleFileListRequest(OSF2FTextSearch message) {
 		try {
 			if (numFileListRequestsReceived > 0) {
 				updateFriendConnectionLog(true, "strange, remote friend is sending more than 1 file list request");
 				...
 				String warnMsg = "banning friend for 10 minutes, sent more than " + MAX_FILE_LIST_REQUESTS + " file list requests (DOS?)";
 				updateFriendConnectionLog(true, warnMsg);
 				remoteFriend.setFriendBannedUntil("file list request spam", System.currentTimeMillis() + 10 * 60 * 1000);
 			...
 			Debug.out("error when parsing file list request, '" + message.getSearchString() + "' is not a number");
 		}
 	...
 	private void handleHandshake(Message message) {
 		...
 		if (message instanceof OSF2FHandshake) {
 			...
 			if (!connection.isConnected()) {
 				Debug.out("got handshake message but connection is already closed: " + remoteFriend.getNick());
 				updateFriendConnectionLog(true, "got handshake message but connection is already closed");
 				close();
 				return;
 			}
 			...
 			if (outgoing && this.hasExtendedDHTKeyNegotiationSupport() && remoteFriend.isDhtLocationConfirmed() == false) {
 			...
 			if (remoteFriend.isRequestFileList()) {
 				...
 				filelistManager.receivedFriendFileList(remoteFriend, OSF2FMessage.FILE_LIST_TYPE_COMPLETE, fileList);
 	...
 	private void sendDhtLocation() {
 		 ...
 		 */
 		sendMessage(new OSF2FDhtLocation(OSF2FMessage.CURRENT_VERSION, remoteFriend.getDhtWriteLocation(), remoteFriend.getDhtReadLocation()));
 	}
 	...
 	private void handleMetainfoRequest(Message message) {
 		if (message instanceof OSF2FMetaInfoReq) {
 			...
 			if (metaInfoRequestHandler.canRespond(req)) {
 				...
 				Debug.out("got meta info request message " + "with unknown channel Id: " + req.getDescription());
 			}
 		} else {
 			Debug.out("handleMetainfoRequest got non " + "metainfoRequest message: " + message.getDescription());
 		}
 	}
 	...
 	private void handleMetainfoResponse(Message message) {
 		if (message instanceof OSF2FMetaInfoResp) {
 			...
 			if (channelId == 0) {
 				...
 				Debug.out("got meta info resp message " + "with unknown channel Id: " + resp.getDescription());
 			}
 
 		} else {
 			Debug.out("handleMetaInfoResponse got non " + "metaInfoResponse message: " + message.getDescription());
 		}
 	}
 	...
 	private void handleChat(Message message) {
 		if (message instanceof OSF2FChat) {
 			...
 			if (remoteFriend.isAllowChat() == false) {
 				logger.warning(getDescription() + "Received chat from " + this + " even though not allowed, dropping.");
 				return;
 			}
 
 			logger.finer(getDescription() + "received chat: " + chat.getPlainText() + " from " + this.getRemoteFriend().getNick());
 			ChatDAO.get().queuePlaintextMessageForProcessing(chat.getPlainText(), this.getRemoteFriend());
 		} else {
 			Debug.out(getDescription() + "handleChat got non " + "chat message: " + message.getDescription());
 		}
 	}
 		...
 		if (message instanceof OSF2FSearch) {
 			...
 			if (msg.getSearchID() == 0) {
 				if (message instanceof OSF2FTextSearch) {
 					...
 					Debug.out(getDescription() + "handleSearch got search with id=0, " + "but it wasn't an TextSearch!");
 				}
 			} else {
 			...
 			Debug.out(getDescription() + "handleSearch got non " + "OSF2FSearch message: " + message.getDescription());
 		}
 	...
 	private void handleTextSearchReponse(Message message) {
 		if (message instanceof OSF2FTextSearchResp) {
 			...
 			if (resp.getFileListType() == OSF2FMessage.FILE_LIST_TYPE_COMPLETE) {
 				...
 				Debug.out(getDescription() + "File list type: " + resp.getFileListType() + " not implemented");
 			}
 		} else {
 			Debug.out(getDescription() + "handleFileListResponse " + "got non fileListResponse message: " + message.getDescription());
 		}
 	}
 	...
 	private void handleDhtLocationMessage(OSF2FDhtLocation message) {
 		...
 		if (outgoing) {
 			...
 			if (!Arrays.equals(readLocation, storedReadLocation)) {
 				updateFriendConnectionLog(true, "got inconsistent dht read location data, not confirming dht location");
 				return;
 			}
 			...
 			if (!Arrays.equals(writeLocation, storedWriteLocation)) {
 				updateFriendConnectionLog(true, "got inconsistent dht write location data, not confirming dht location");
 				return;
 			}
 			/*
 			 ...
 			 */
 			sendMessage(new OSF2FDhtLocation(OSF2FMessage.CURRENT_VERSION, remoteFriend.getDhtWriteLocation(), remoteFriend.getDhtReadLocation()));
 			updateFriendConnectionLog(true, "stored dht location, sending dht location confirmation");
 		}
 	}
 	...
 	public boolean isTimedOut() {
 		if (!handShakeReceived && System.currentTimeMillis() - connectionTime > INITIAL_HANDSHAKE_TIMEOUT) {
 		...
 		if (!isFileListReceived() && System.currentTimeMillis() - connectionTime > TIMOUT_FILELIST) {
 			logger.finer(getDescription() + "closing friend connection: " + this + " (filelist timed out)");
 			Debug.out("closing friend connection: " + this + " (filelist timed out), timeSinceLastMsg=" + timeSinceLastMsg + " bytes sent=" + (getProtocolBytesUploaded() + getDataBytesUploaded()) + " recv=" + (getProtocolBytesDownloaded() + getDataBytesDownloaded()) + " friend status: " + remoteFriend.getStatus());
 		...
 		if (lastByteRecvTime != 0 && timeSinceLastMsg > KEEP_ALIVE_TIMEOUT) {
 			logger.finer(getDescription() + "closing friend connection: " + this + " (timed out," + timeSinceLastMsg + ")");
 	...
 	void registerOverlayForward(OSF2FSearchResp currentSetupMsg, FriendConnection conn, OSF2FSearch search, boolean searcherSide) throws FriendConnection.OverlayRegistrationError {
 		lock.lock();
 		try {
 			if (overlayForwards.containsKey(currentSetupMsg.getChannelID())) {
 				...
 				if (existingNick.equals(currentNick)) {
 					throw new OverlayRegistrationError(getRemoteFriend().getNick(), currentSetupMsg.getChannelID(), "existing channel id: exid=" + Integer.toHexString(existing.getChannelId()) + " exbytes=" + existing.getBytesForwarded() + " exage=" + existing.getAge() + " extarget=" + existingNick + " cutarget=" + currentNick + " exside=" + existing.isSearcherSide() + " cuside=" + searcherSide + " registered=" + overlayForwards.size() + " ctype=" + currentType + " extype=" + existingType);
 				} else {
 					throw new OverlayRegistrationError("collision", currentSetupMsg.getChannelID(), "colliding channel id: exid=" + Integer.toHexString(existing.getChannelId()) + " exbytes=" + existing.getBytesForwarded() + " exage=" + existing.getAge() + " extarget=" + existingNick + " cutarget=" + currentNick + " exside=" + existing.isSearcherSide() + " cuside=" + searcherSide + " registered=" + overlayForwards.size() + " ctype=" + currentType + " extype=" + existingType);
 				}
 			}
 			overlayForwards.put(currentSetupMsg.getChannelID(), new OverlayForward(currentSetupMsg.getChannelID(), conn, search, currentSetupMsg, searcherSide));
 	...
 	void registerOverlayTransport(OverlayTransport transport) throws OverlayRegistrationError {
 		...
 		try {
 			...
 			int pathID = transport.getPathID();
 			if (overlayTransports.containsKey(channelId)) {
 				Debug.out(getDescription() + "tried to register existing channel id, " + "this should _never_ happen " + this);
 				throw new FriendConnection.OverlayRegistrationError(getRemoteFriend().getNick(), channelId, "existing channel id");
 
 			}
 			if (overlayTransportPathsId.containsKey(pathID)) {
 				Debug.out(getDescription() + "tried to register existing path id, " + "this should _never_ happen " + this);
 				throw new FriendConnection.OverlayRegistrationError(getRemoteFriend().getNick(), channelId, "existing path id");
 			}
 	...
 	private void sendFileListResponse(byte type, int searchId, int lastId, boolean sendResponseOnNoChange) {
 		...
 		if (lastFileListSentToFriend == fileListToSendToFriend.getListId()) {
 			...
 			logger.finer(getDescription() + "file list for " + this.getRemoteFriend().getNick() + " unchanged, skipping file list send");
 			...
 			logger.finer(getDescription() + "file list for " + this.getRemoteFriend().getNick() + " changed, sending new list");
 
 			lastFileListSentToFriend = fileListToSendToFriend.getListId();
 			if (this.hasExtendedFileListsSupport() == false) {
 				logger.finer(getDescription() + "sending basic flist: " + this.getRemoteFriend().getNick());
 				bytesToSend = FileListManager.encode_basic(fileListToSendToFriend, true);
 			} else {
 				logger.finer(getDescription() + "sending extended flist" + this.getRemoteFriend().getNick());
 				bytesToSend = FileListManager.encode_extended(fileListToSendToFriend, true);
 				logger.finest(getDescription() + " the bytes of extended: " + ByteFormatter.encodeString(bytesToSend));
 			}
 		}
 		logger.finer(getDescription() + "sending custom list to friend, bytes=" + bytesToSend.length + " took " + (System.currentTimeMillis() - t) + "ms to generate");
 		int channelId = 0;
 		OSF2FTextSearchResp msg = new OSF2FTextSearchResp(OSF2FMessage.CURRENT_VERSION, OSF2FMessage.FILE_LIST_TYPE_COMPLETE, searchId, channelId, bytesToSend);
 	...
 	private void sendHandshake() {
 		if (remoteFriend.isAllowChat()) {
 			...
 			connection.getOutgoingMessageQueue().addMessage(new OSF2FHandshake((byte) 1, my_flags), false);
 		}
 	}
 	...
 	private void sendMessage(OSF2FMessage msg, QueueBuckets queueBuckets, boolean skipQueue) {
 		if (!handShakeReceived) {
 			...
 			logger.finer(getDescription() + "waiting for handshake to complete, queue size: " + bufferedMessages.size());
 		} else {
 			friendConnectionQueue.queuePacket(QueueBuckets.CONTROL, msg, skipQueue);
 		}
 	}
 
 	public void sendMetaInfoRequest(byte type, int channelId, byte[] infoHash, int lengthHint, PluginCallback<byte[]> callback) {
 		if (type == OSF2FMessage.METAINFO_TYPE_BITTORRENT || type == OSF2FMessage.METAINFO_TYPE_THUMBNAIL) {
 			metaInfoRequestHandler.sendMetaInfoRequest(type, channelId, infoHash, lengthHint, callback);
 		} else {
 	...
 	private void updateFriend() {
 		...
 		if (outgoing) {
 			remoteFriend.setLastConnectPort(connection.getEndpoint().getNotionalAddress().getPort());
 		}
 		logger.fine(getDescription() + "updating friend: out=" + outgoing + " friend=" + remoteFriend);
 	}
 	...
 	public static int getHashOf(byte[] publicKey, InetAddress addr, int port) {
 		return Arrays.hashCode(publicKey) ^ (int) OneSwarmSslTools.unsignedIntToLong(addr.getAddress()) ^ port;
 	}
 	...
 	private class FileListRequestHandler {
 		...
 		public void handleFileListResponse(OSF2FTextSearchResp resp) {
 			...
 			try {
 				List<byte[]> newInfoHashes = filelistManager.receivedFriendFileList(remoteFriend, type, fileList, FriendConnection.this.hasExtendedFileListsSupport());
 				// valid list
 				notifyListenerComplete(filelistManager.getFriendsList(remoteFriend));
 
 				// now, try so sync up all the thumbnails
 
 				final List<byte[]> neededThumbnails = filelistManager.getMetaInfoManager().getTorrentThumbnailNeeded(newInfoHashes);
 
 				sendNextImageRequest(neededThumbnails);
 
 			} catch (IOException e) {
 				logger.warning(getDescription() + ": got decode error when " + "processing file list: " + this + ": " + e.getMessage());
 			}
 		}
 		...
 		public void notifyListenerComplete(FileList fileList) {
 			...
 			if (connection.isConnected()) {
 				...
 				if (!filelistReceived) {
 					remoteFriend.handShakeCompleted(FriendConnection.this.hashCode(), hasExtendedFileListsSupport(), hasChatSupport());
 			...
 			if( logger.isLoggable(Level.FINEST) ) {
 				if (!message.getID().equals(OSF2FMessage.ID_OS_CHANNEL_DATA_MSG) || packetNum++ % 100 == 0) {
 					if (!message.getID().equals(BTKeepAlive.ID_BT_KEEP_ALIVE)) {
 							logger.finest(getDescription() + " got message: " + message.getDescription() + "\t::" + FriendConnection.this);
 					}
 					// printRatelimitInfo();
 				}
 			}
 	...
 	private class MetaInfoRequestHandler {
 		...
 		public boolean canRespond(OSF2FMetaInfoReq req) {
 			...
 			byte[] data = filelistManager.getMetaInfoManager().getMetaInfo(remoteFriend, req.getMetaInfoType(), infoHash);
 			if (data != null) {
 				canRespond = true;
 			}
 		...
 		public void handleMetaInfoRequest(final OSF2FMetaInfoReq req) {
 			...
 			final byte[] data = filelistManager.getMetaInfoManager().getMetaInfo(remoteFriend, type, infoHash);
 
 			final long infoHashhash = filelistManager.getInfoHashhash(infoHash);
 			final int startByte = req.getStartByte();
 
 			final int requestSize = Math.min(OSF2FMessage.METAINFO_CHUNK_SIZE, data.length - startByte);
 
 			// sanity checks
 			if (requestSize < 0 || startByte + requestSize > data.length) {
 				Debug.out(getDescription() + "got strange metainfo request, length=" + data.length + " startpos=" + startByte + " requestSize=" + requestSize + "\t" + FriendConnection.this);
 				return;
 
 			} else {
 				int delay = OSF2FMain.getSingelton().getOverlayManager().getSearchDelayForInfohash(remoteFriend, infoHash);
 				delayedExecutor.queue(delay, new TimerTask() {
 					...
 					public void run() {
 						...
 						OSF2FMetaInfoResp msg = new OSF2FMetaInfoResp(OSF2FMessage.CURRENT_VERSION, req.getChannelId(), type, infoHashhash, startByte, data.length, chunk);
 						sendMessage(msg);
 					}
 				});
 			}
 		...
 		public void sendMetaInfoRequest(byte type, int channelId, byte[] infohash, int lengthHint, PluginCallback<byte[]> callback) {
 			...
 			if (!requests.containsKey(infohashhash)) {
 				requests.put(infohashhash, new MetaInfoRequest(type, channelId, infohash, lengthHint));
 		...
 		private class MetaInfoRequest {
 			...
 			public void sendInitialMetaInfoRequest(byte type) {
 				if (lengthHint > 0) {
 					...
 					int startByte = 0;
 					OSF2FMetaInfoReq req = new OSF2FMetaInfoReq(OSF2FMessage.CURRENT_VERSION, channelId, type, startByte, infohash);
 					sendMessage(req);
 				}
 			}
 	...
 	public class OverlayForward {
 		...
 		public OverlayForward(int channelId, FriendConnection conn, OSF2FSearch sourceMessage, OSF2FSearchResp setup, boolean searcherSide) {
 	...
 	public void closeChannel(int channelId) {
 		...
 		if (overlayForward != null) {
 			...
 			if (overlayPathLogger.isEnabled()) {
 				overlayPathLogger.log(System.currentTimeMillis() + ", close_forward, " + overlayForward.getChannelId() + ", " + overlayForward.getBytesForwarded() + ", " + overlayForward.getAge() + ", " + overlayForward.getRemoteFriend().getNick() + ", " + overlayForward.getRemoteIpPort());
 			}
 		}
