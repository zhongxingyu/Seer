...
import org.osgi.framework.BundleException;
 ...
 public class PlatformLogWriter implements ILogListener {
 	...
 	protected void writeHeader() throws IOException {
 		...
 		write("BootLoader constants: OS=" + InternalPlatform.getDefault().getEnvironmentInfoService().getOS()); //$NON-NLS-1$
 		write(", ARCH=" + InternalPlatform.getDefault().getEnvironmentInfoService().getOSArch()); //$NON-NLS-1$
 		write(", WS=" + InternalPlatform.getDefault().getEnvironmentInfoService().getWS()); //$NON-NLS-1$
 		writeln(", NL=" + InternalPlatform.getDefault().getEnvironmentInfoService().getNL()); //$NON-NLS-1$
 
 		// Add the command-line arguments used to invoke the platform.
 		String[] args = InternalPlatform.getDefault().getAppArgs();
