 ...
 public class Frame extends JFrame implements ActionListener {
 	...
 	private JLabel studentViewLabel1, studentViewLabel2, studentViewLabel3;
 	...
 	public void createTab(File file) {
 		...
 		try {
 			...
 			int index = Global.getGlobal().getActiveProjectIndex();
 
 			fileHandler.openFile(file.getAbsolutePath(), Global.getGlobal().getActiveProject());
 			p = Global.getGlobal().getProjects().get(index);
 			p.getTables().clear();
 	...
 	public void showStudent() {
 		...
 		if (studentViewLabel1 != null) {
 			studentPanel.remove(studentViewLabel1);
 		}
 		if (studentViewLabel2 != null) {
 			studentPanel.remove(studentViewLabel2);
 		}
 		if (studentViewLabel3 != null) {
 			studentPanel.remove(studentViewLabel3);
 		}
 		...
 		if (i != -1) {
 			...
 			Global.getGlobal().getActiveProject().getHead().getDataLinkedList().get(i).get(0).findThreeStrings(linkedList);
 
 			if (linkedList.size() > 0) {
 				studentViewLabel1 = new JLabel(linkedList.get(0));
 				studentViewLabel1.setBounds(40, 220, 200, 20);
 				studentViewLabel1.setForeground(Color.white);
 				studentPanel.add(studentViewLabel1);
 			}
