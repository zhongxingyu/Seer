 ...
 class DsCompletionProcessor implements IContentAssistProcessor,DsSyntax {
 	...
 	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
 		...
 		String fetchType = null;
 					...
 					TagInfo previousOpenTagClosedInfo = getPreviousOpenTagClosedInfo(viewer, previousOpendTag.getTag(), previousOpendTag.getIndex()); 
 										
 						if(previousOpendTag.getIndex() > previousClosingTagInfo.getIndex()){
 							//if we found opening before closing tag then we -> opening based sugession
 							//opening index > closing index  means we near to opening more..--> opening
 							
 							if(previousOpenTagClosedInfo != null){
 								
 								if(previousOpenTagClosedInfo.getTag().equals(TAG_WITH_CLOSING_CHAR_AND_NO_CLOSING_TAG)){
 									//suggest the opened tag for the proposal and get the children of that
 									//levelTag =  previousTag.getTag();
 									
 									TagInfo nextOpendTag = getNextOpenedTagInfo(viewer, offset); //get the current tag info
 									...
 									String closingTag = previousOpenTagClosedInfo.getTag();  // open and close with this tag
 									boolean isItValidClosing = previousOpenTagClosedInfo.isDetected();
 									
 									TagInfo nextOpendTag = getNextOpenedTagInfo(viewer, offset);
 									
 									String nextTag = nextOpendTag.getTag();
 									boolean validClosingWithoutAttrib = nextOpendTag.isValidClosingwithNoAttribute();
 							...
 							//if we found closing before opening tag then we -> closing based sugession.
 							//closing index > opening index means we near to closing to more --> closing
 								levelTag = previousClosingTagInfo.getTag();
 								fetchType = SIBLINGS;
 							
 						}else if(previousOpenTagClosedInfo.getTag().equals(TAG_WITH_CLOSING_CHAR_AND_NO_CLOSING_TAG)
 						...
 						}else if(previousOpendTag.getTag().equals(NO_PRIVIOUS_TAG) && previousOpendTag.getIndex() == previousClosingTagInfo.getIndex()){
 		  ...
 		  ICompletionProposal[] result = new ICompletionProposal[proposalList.size()];
 		    ...
 		    for (Iterator i = proposalList.iterator(); i.hasNext();) {
 		      ...
 		      result[index] = new CompletionProposal(
 		          ...
 		          offset, 			 			//replacementOffset the offset of the text to be replaced
 		          0,							//replacementLength the length of the text to be replaced
 		          keyWord.length()+2,			//cursorPosition the position of the cursor following the insert relative to replacementOffset
 	...
 	private TagInfo getNextOpenedTagInfo(ITextViewer viewer,int documentOffset){
 		...
 		return new TagInfo(tag,
 				validClosingWithNoAttributes, validClosingWithAttributes);
 	 ...
 	 */
     private TagInfo getPreviousOpenTagClosedInfo(ITextViewer viewer,String tag,int tagStratingIndex){
 			...
 			while (((forwordIndex) <= viewer.getBottomIndexEndOffset())) {
 				...
 				if (currChar == '>') {
 					...
 					if (outerSlashDetected) {
 						...
 						while (((internalForwordIndex) <= viewer
 								.getBottomIndexEndOffset())) {
 
 							char internaCurrChar = viewer.getDocument()
 							...
 							if (closingSeqDetected) {
 								if (internaCurrChar != '/' && internaCurrChar != '<'&& internaCurrChar != '>') {
     ...
     private class TagInfo{
     	...
     	public TagInfo(String tag ,boolean validClosingwithNoAttribute,boolean validClosingWithAtrribute){
     		this.tag = tag;
