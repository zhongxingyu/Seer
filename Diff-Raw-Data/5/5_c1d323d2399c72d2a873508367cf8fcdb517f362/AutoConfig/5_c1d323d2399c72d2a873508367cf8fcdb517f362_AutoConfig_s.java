 ...
 //                String destination = properties.getProperty("destination", properties.getProperty("destinationName"));
     ...
     private void processJndiRefs(String moduleId, JndiConsumer jndiConsumer, AppResources appResources, ClassLoader classLoader) throws OpenEJBException {
         ...
         for (ResourceRef ref : jndiConsumer.getResourceRef()) {
             ...
             //skips references such as javax.ws.rs.core.Context
             if (isWSSpecificReferenceType(classLoader, ref)) {
             	continue;
             }
 	...
 	private boolean isWSSpecificReferenceType(ClassLoader classLoader, ResourceRef ref) {
 		try {
 			if (ref.getInjectionTarget().size() > 0) {
 				InjectionTarget target = ref.getInjectionTarget().iterator().next();
 				String targetClassString = target.getInjectionTargetClass();
 				String targetNameString = target.getInjectionTargetName();	
 				Class targetClass = classLoader.loadClass(targetClassString);
 				Field targetField = targetClass.getDeclaredField(targetNameString);
 				if (targetField.getAnnotation(Context.class) != null) {
 					System.out.println("Ignored javax.ws.rs.core.Context JNDI Resource for field '"+targetNameString+"' in type '"+targetClassString+"'");
 					return true;
 				}
 			}
 		} catch (Exception e) {}
