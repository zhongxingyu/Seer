 ...
 public final class SigningUtil {
     ...
     /** Class logger. */
     private static Logger log = Logger.getLogger(SigningUtil.class);
      ...
      * 
      * It is up to the caller to ensure that the specified algorithm URI is consistent with
      * the type of signing key supplied in the signing credential.
      * 
      ...
      * Compute the signature or MAC value over the supplied input.
      * 
      * It is up to the caller to ensure that the specified algorithm ID and isMAC flag are consistent with
      * the type of signing key supplied in the signing credential.
      * 
      * @param signingCredential the credential containing the signing key
      ...
      * 
      * It is up to the caller to ensure that the specified algorithm ID is consistent with
      * the type of signing key supplied.
      * 
     ...
     public static byte[] sign(PrivateKey signingKey, String jcaAlgorithmID, byte[] input) throws SecurityException {
         if (log.isDebugEnabled()) {
             log.debug(String.format("Computing signature over input using private key of type '%s' and JCA algorithm ID '%s'",
         ...
         try {
             ...
             if (log.isDebugEnabled()) {
                 log.debug(String.format("Computed signature: '%s'", new String(Hex.encode(rawSignature))));
             }
             return rawSignature;
     ...
     public static byte[] signMAC(Key signingKey, String jcaAlgorithmID, byte[] input) throws SecurityException {
         if (log.isDebugEnabled()) {
             log.debug(String.format("Computing MAC over input using key of type '%s' and JCA algorithm ID '%s'",
                     signingKey.getAlgorithm(), jcaAlgorithmID));
         }
         ...
         try {
             ...
             if (log.isDebugEnabled()) {
                 log.debug(String.format("Computed MAC: '%s'", new String(Hex.encode(rawMAC))));
             }
             return rawMAC;
         } catch (GeneralSecurityException e) {
     ...
     /**
      * Verify the signature value computed over the supplied input against
      * the supplied signature value.
      * 
      * It is up to the caller to ensure that the specified algorithm URI are consistent with
      * the type of verification credential supplied.
      * 
      ...
      * @return true if the signature value computed over the input using the supplied key and algorithm ID is
      *          identical to the supplied signature value
      * @throws SecurityException thrown if the signature computation or verification process results in an error
      */
     public static boolean verifyWithURI(Credential verificationCredential, String algorithmURI,
      ...
      * 
      * It is up to the caller to ensure that the specified algorithm ID is consistent with
      * the type of verification key supplied.
      * 
     ...
     public static boolean verify(PublicKey verificationKey, String jcaAlgorithmID, byte[] signature, byte[] input) 
         ...
         if (log.isDebugEnabled()) {
             log.debug(String.format("Verifying signature over input using public key of type '%s' and JCA algorithm ID '%s'",
     ...
     /**
      * Verify the Message Authentication Code (MAC) value computed over the supplied input against
      * the supplied MAC value.
      * 
      * It is up to the caller to ensure that the specified algorithm ID is consistent with
      * the type of verification key supplied.
      * 
      ...
      * @return true if the MAC value computed over the input using the supplied key and algorithm ID is
      *          identical to the supplied MAC signature value
