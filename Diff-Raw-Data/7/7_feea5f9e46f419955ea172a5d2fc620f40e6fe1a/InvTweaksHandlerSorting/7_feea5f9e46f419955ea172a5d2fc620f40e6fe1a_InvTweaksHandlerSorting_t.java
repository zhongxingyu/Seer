 ...
 public class InvTweaksHandlerSorting extends InvTweaksObfuscation {
         ...
         if (algorithm != ALGORITHM_DEFAULT) {
             ...
             if (algorithm == ALGORITHM_INVENTORY) {
                 ...
                 if (globalContainer.hasSection(InvTweaksContainerSection.CRAFTING_IN)) {
                     List<pq> craftingSlots = globalContainer.getSlots(InvTweaksContainerSection.CRAFTING_IN);
                     int emptyIndex = globalContainer.getFirstEmptyIndex(InvTweaksContainerSection.INVENTORY);
                     if (emptyIndex != -1) {
                         for (pq craftingSlot : craftingSlots) {
                 ...
                 for (int i = size - 1; i >= 0; i--) {
                     ri from = containerMgr.getItemStack(i);
                     if (from != null) {
 
                         // Move armor parts
 	                    rg fromItem = getItem(from);
 	                    if (isDamageable(fromItem)) {
 	                        if (sortArmorParts) {
 	                             if (isItemArmor(fromItem)) {
 	                            	 ps fromItemArmor = asItemArmor(fromItem);
 	                                 if (globalContainer.hasSection(InvTweaksContainerSection.ARMOR)) {
 	                                     List<pq> armorSlots = globalContainer.getSlots(InvTweaksContainerSection.ARMOR);
 	                                     for (pq slot : armorSlots) {
 	                                    	boolean move = false;
 	                                    	if (!hasStack(slot)) {
 	                                    		move = true;
 	                                    	}
                         ...
                         // Stackable objects are never damageable
                             ...
                             for (Integer lockPriority : lockPriorities) {
                                 if (lockPriority > 0) {
                                     ri to = containerMgr.getItemStack(j);
         ...
         if (lockPriorities[i] <= priority) {
             ...
             // Try to swap/merge
                 ...
                 if (canBeSwappedOrMerged) {
                     ...
                     ri remains = containerMgr.getItemStack(i);
     ...
     private boolean isOrderedBefore(int i, int j) {
         ...
         ri iStack = containerMgr.getItemStack(i),
     ...
     private int getItemOrder(ri item) {
