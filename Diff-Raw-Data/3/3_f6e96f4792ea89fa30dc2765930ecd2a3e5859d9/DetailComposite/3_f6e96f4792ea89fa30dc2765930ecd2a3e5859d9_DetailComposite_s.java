 ...
 public class DetailComposite extends Composite {
 	...
 	public DetailComposite(MDirtyable dirtyable, final Composite parent) {
 		...
 		dbc.bindValue(SWTObservables.observeImage(imageLabel), scaledImage, new UpdateValueStrategy(
 	...
 	private Text createText(final Composite parent, final String labelText, final String property) {
 		...
 		if (property != null) {
 			if (property.equals("name")) {
 				dbc.bindValue(SWTObservables.observeText(text, SWT.Modify), new AggregateNameObservableValue(
 						contactValue));
 			} else {
