 ...
 public class Stalemate extends JavaPlugin implements Listener {
 	...
 	public void onEnable()
 	{
 		...
 		registerCommand(getSetting("help_cmd", "help"), getSetting("help_help", "Shows the help page."), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				{
 					sender.sendMessage(synColor(ChatColor.LIGHT_PURPLE)+"/stalemate "+synColor(ChatColor.RESET)+key.toLowerCase()+" - "+synColor(ChatColor.AQUA)+helpMap.get(key));
 				}
 			}
 		...
 		registerCommand(getSetting("remove_trap_cmd", "rtrap"), getSetting("remove_trap_help", "Defuses a trap. Has a chance to fail."), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				{
					sender.sendMessage(parseColors(getSetting("players_only_msg", "<xml><font color=\"Red\">Only players may use this command.</font></xml>")));
 					return;
 				}
 				// TODO: Finish
 			}
 		...
 		registerCommand(getSetting("join_war_cmd", "joinwar"), getSetting("join_war_help", "Joins a war"), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				if (args.length < 2)
 				{
 					sender.sendMessage(parseColors(getSetting("join_war_syntax", "Syntax: /stalemate "+args[0]+" &lt;teamname&gt;")));
 					return;
 				}
 		...
 		registerCommand(getSetting("place_trap_cmd", "ptrap"), getSetting("place_trap_help", "Places a new trap."), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				if (!(sender instanceof Player))
 				{
 					if (args.length < 6)
 					{
 						sender.sendMessage(parseColors(getSetting("syntax_trap_console_msg", "Syntax: stalemate "+args[0]+" <world> <x> <y> <z>")));
 						return;
 					}
 					World w;
 					int x, y, z;
 					...
 					if (args.length < 2)
 					{
 						sender.sendMessage(parseColors(getSetting("syntax_trap_player_msg", "Syntax: /stalemate "+args[0]+" <trap_type>")));
 						return;
 					}
 					Player p = (Player) sender;
 					String trapName = args[1];
 					if (!trapMap.containsKey(trapName.toUpperCase()))
 					{
 						sender.sendMessage(parseColors(getSetting("no_trap_name", "<xml><font color=\"Purple\">That trap type does not exist!</font></xml>")));
 						return;
 					}
 					placeTrap(trapName, p.getLocation());
 					sender.sendMessage(parseColors(getSetting("trap_place_success_msg", "<xml>Trap Placed.</xml>")));
 				}
 			}
 		...
 		registerCommand(getSetting("join_cmd", "join"), getSetting("join_help", "Joins the team of the specified player."), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				if (args.length < 2) {
 					sender.sendMessage(parseColors(getSetting("syntax_join_msg", "<xml>/stalemate "+args[0]+" &lt;player&gt; or to create a team: /stalemate "+args[0]+" &lt;teamname&gt;</xml>")));
 					Team yours = null;
 					for (Team tt : Team.list())
 					{
 					...
 					else sender.sendMessage("You do not have a team.");
 					return;
 				}
 		...
 		registerCommand(getSetting("leave_cmd", "leave"), getSetting("leave_help", "Leaves the current round."), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				...
 				if (!(sender instanceof Player))
 				{
					sender.sendMessage(parseColors(getSetting("players_only_msg", "<xml><font color=\"Red\">Only players may use this feature.</font></xml>")));
 					return;
 				}
 				for (RoundController rc : rounds)
 				{
 		...
 		registerCommand(getSetting("class_cmd", "class"), getSetting("class_help", "Changes your class (Gives you a class's items)"), new CommandCallback() {
 			...
 			public void onCommand(CommandSender sender, String[] args) {
 				if (!(sender instanceof Player))
 				{
 					sender.sendMessage(parseColors(getSetting("players_only_msg", "<xml>Only players may use this command.</xml>")));
 					return;
 				}
 				...
 				{
					sender.sendMessage(parseColors(getSetting("class_nochange", "<xml><font color=\"Red\">You may not change your class once you have selected it.</font></xml>")));
 					return;
 				}
 				if (args.length < 2)
 				{
 					sender.sendMessage(parseColors(getSetting("class_syntax", "<xml>Syntax: /stalemate "+args[0]+" &lt;classname&gt;</xml>")));
 					return;
 				}
 				SoldierClass s = SoldierClass.fromName(args[1]);
 				if (s == null)
 				{
 					sender.sendMessage(parseColors(getSetting("class_nonexist_msg", "<xml><font color=\"Red\">That class does not exist!</font></xml>")));
 					return;
 				}
 				if (!sender.hasPermission(s.getPermission()))
 				{
					sender.sendMessage(parseColors(getSetting("no_perm_msg", "<xml><font color=\"Red\">You do not have permission to change to the requested class.</font></xml>")));
 					return;
 				}
 	...
 	protected static boolean isInRound(Player sender) {
 		// TODO: Finish
 		for (RoundController rc : Stalemate.getInstance().rounds)
 		{
 			if (rc.getPlayers().contains(sender.getName().toUpperCase()))
 			{
 				return true;
 	...
 	protected static RoundController getRound(Player sender) {
 		for (RoundController rc : Stalemate.getInstance().rounds)
 		{
 			if (rc.getPlayers().contains(sender.getName().toUpperCase()))
 				return rc;
 	...
 	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String args[])
 	{
 		...
 		String name = args[0];
 		CommandCallback cb = cmdMap.get(name.toUpperCase());
 		if (cb == null)
 		{
 			sender.sendMessage(parseColors(getSetting("invalid_cmd_msg", "<xml>Invalid Command. Please type /stalemate "+getSetting("help_cmd", "help")+" for help.</xml>")));
 			return true;
 		}
 		cb.onCommand(sender, args);
 		return true;
 	}
 	...
 	private void generateConfig(File f) throws IOException
 	{
 		...
 		InputStream in = getResource("config.xml");
 	...
 	public void onEnable0() throws Throwable
 	{
 		...
 		for (int i = 0; i < classes.getLength(); i++)
 		{
 			...
 			for (int j = 0; j < classList.getLength(); j++)
 			{
 				...
 				if (permission.equals("")) permission = "stalemate.basic";
