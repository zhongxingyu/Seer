 ...
 abstract public class AbstractWebdavServlet extends HttpServlet implements DavConstants {
     ...
     protected void service(HttpServletRequest request, HttpServletResponse response)
         ...
         WebdavRequest webdavRequest = createWebdavRequest(request);
         boolean noCache = ! isResponseCacheable(webdavRequest);
         WebdavResponse webdavResponse = createWebdavResponse(response, noCache);
         try {
             ...
             if (ifNoneMatch(webdavRequest, resource)) {
                 switch (methodCode) {
             ...
             if (ifMatch(webdavRequest, resource)) {
                 response.setStatus(DavServletResponse.SC_PRECONDITION_FAILED);
                 return;
             }
     ...
     protected void doMkCol(WebdavRequest request, WebdavResponse response,
         ...
         if (request.getContentLength() > 0 || request.getHeader("Transfer-Encoding") != null) {
             if (isMkColBodyAllowed(request, resource)) {
                 parentResource.addMember(resource, getInputContext(request, request.getInputStream()));
             } else {
     ...
     protected void doDelete(WebdavRequest request, WebdavResponse response,
                             DavResource resource) throws IOException, DavException {
         // only depth infinity is allowed
         int depth = request.getDepth(DEPTH_INFINITY);
     ...
     protected boolean isResponseCacheable(WebdavRequest request) {
         ...
         // DeltaV only allows VERSION-CONTROL and REPORT to be
         // cacheable
         if (DavMethods.isDeltaVMethod(request)) {
             if (! (methodCode == DavMethods.DAV_VERSION_CONTROL ||
                    methodCode == DavMethods.DAV_REPORT))
                 return false;
         }
         return true;
     }
     ...
     protected boolean ifNoneMatch(WebdavRequest request,
         ...
         if (resource.exists()) {
             if (request.getMethod().equals(DavMethods.METHOD_GET) ||
                 request.getMethod().equals(DavMethods.METHOD_HEAD)) {
     ...
     /**
      * Returns <code>true</code> if a message body is allowed for a
      * MKCOL on the given resource, or <code>false</code> if not. This
      * implementation always returns <code>false</code>.
      */
     protected boolean isMkColBodyAllowed(WebdavRequest request,
                                          DavResource resource) {
         return false;
     }
 
     /**
      * Returns a new <code>WebdavRequest</code>.
      */
     protected WebdavRequest createWebdavRequest(HttpServletRequest request) {
         return new WebdavRequestImpl(request, getLocatorFactory());
     }
 
     /**
      * Returns a new <code>WebdavResponse</code>.
      */
