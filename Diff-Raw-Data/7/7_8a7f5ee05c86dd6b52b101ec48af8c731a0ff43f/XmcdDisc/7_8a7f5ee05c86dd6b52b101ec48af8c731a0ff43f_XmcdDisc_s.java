 ...
 public class XmcdDisc {
     ...
     public static XmcdDisc fromXmcdFile(String contents, FreedbGenre freedbGenre)
         ...
         try {
             ...
             throw new XmcdFormatException("Bad number format", e);
         }
     }
     ...
     private static short parseDiscLength(Matcher m, AlbumRow albumRow)
        ...
        // TODO fix this hack
         int tempHack = Integer.parseInt(m.group(1));
         if (tempHack > Short.MAX_VALUE) {
             return Short.MAX_VALUE;
         } else {
             return (short)tempHack;
         }
     ...
     private static void parseDiscTitle(Matcher m, AlbumRow albumRow)
         ...
         String[] titleParts = XmcdRegEx.SPLITTER.split(title, 2);
         albumRow.artistName = normalize(titleParts[0]);
         if (titleParts.length > 1) {
             // Album title is specified
             albumRow.title = normalize(titleParts[1]);
         } else {
     ...
     private static void parseDiscGenre(Matcher m, AlbumRow albumRow) {
         ...
         if (!m.find()) {
             ...
             albumRow.freeTextGenre = normalize(FreedbGenre.values()[albumRow.freedbGenre].name());
         } else {
             albumRow.freeTextGenre = normalize(m.group(1));
         }
     }
     ...
     private static List<TrackRow> parseTrackTitles(Matcher m, AlbumRow albumRow, List<Integer> trackFrameOffsets,
         ...
         List<Short> lengths =
                 Conversions.toTrackLengthsSeconds(trackFrameOffsets, totalLenSec);
         ...
         for (Pair<Byte, String> track : trackTitles) {
             ...
             trackRow.albumRow = albumRow;
             trackRow.trackNum = track.fst;
             trackRow.lenInSec = lengths.get(track.fst);
             String[] titleParts = XmcdRegEx.SPLITTER.split(track.snd, 2);
             if (titleParts.length > 1) {
                 // Track has a specified artist
                 trackRow.artistName = normalize(titleParts[0]);
                 trackRow.title = normalize(titleParts[1]);
             } else {
                 // Track artist is the same as the album artist
                 trackRow.artistName = albumRow.artistName;
                 trackRow.title = normalize(titleParts[0]);
             }
     ...
     /**
      * Formats strings in a normalized form: Title Case with no leading or trailing whitespace.
      */
     private static String normalize(String str) {
         return WordUtils.capitalizeFully(str.trim(), DELIMITERS);
     }
 }
