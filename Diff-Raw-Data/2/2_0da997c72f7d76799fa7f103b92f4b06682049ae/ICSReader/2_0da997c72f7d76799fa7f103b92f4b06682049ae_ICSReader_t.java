 ...
 */
 ...
 public class ICSReader extends FormatReader {
   ...
   /** Used for testing. */
   private Map<String, TrackKeyValueClass> keyValueMap =
       new HashMap<String, TrackKeyValueClass>();
   private int count = 0;
   ...
   //TODO ARG for testing: protected void initFile(String id) throws FormatException, IOException {
     super.initFile(id);
  ...
  //TODO ARG for testing protected void oldInitFile(String id) throws FormatException, IOException {
     super.initFile(id);
   ...
   private void trackKeyValues(String id, String key, String value,
                               String parent) {
     key = key.toLowerCase();
     TrackKeyValueClass keyValueClass = keyValueMap.get(key);
     if (null == keyValueClass) {
       keyValueClass = new TrackKeyValueClass(id, value, parent);
       keyValueMap.put(key, keyValueClass);
     }
     else {
   ...
   private void dumpSortedKeys() {
     ...
     List<String> keyList = new ArrayList(lowercaseKeys);
     Collections.sort(keyList);
     for (String key : keyList) {
   ...
   /* Searches through subdirectories for ".ics" and "*.ids" files and loads
    * them.
    */
   private void recursiveSearch(boolean old, File dir) {
     ...
     if (null != files) {
       for (File f: files) {
         if (f.isDirectory()) {
           recursiveSearch(old, f);
         }
         else {
           String id = f.getAbsolutePath();
           if (id.toLowerCase().endsWith(".ids")
               || id.toLowerCase().endsWith(".ics")) {
             try {
               ...
               System.out.println("Exception processing " + id +
                                    " " + e.getMessage());
             }
           }
         }
