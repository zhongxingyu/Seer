 ...
 public class GeneAnalytics {
     ...
     static double avgIntronSize() {
         ...
         //		System.out.println("totalLength: "+totalLength);
         //		System.out.println("geneIdLengths.size(): "+geneIdLengths.size());
         double avgIntronLength = 0;
         if (intronLength.size() != 0) {
             avgIntronLength = totalLength / intronLength.size();
         }
     ...
     static double avgIntergenicSize() {
         ...
         double avgIntergenicLength = 0;
 
         if (intergenic.size() != 0) {
             avgIntergenicLength = totalLength / intergenic.size();
         }
     ...
     /*	#8
     8. Proportion of CDS nucleotides per contig.
     Total number of nucleotides in all CDS regions divided by
     total number of nucleotides in the reference sequence.
     If one or more CDS regions overlap, choose the longest
     CDS region for calculations (see Figure 3 below)
      */
     static double cdsNucProportion() {
         ...
         while (gffScanner.hasNext()) {
             ...
             if (type.equals("CDS")) {
                 ...
                 start.add(Math.min(startNuc, endNuc));
                 end.add(Math.max(startNuc, endNuc));
             }
         }
         ...
         System.out.println("usedcds " + usedCDS + "  totalnuc " + totalNuc);
     ...
     static HashMap<String, String> predictProtein() { // apparently this is reading the fasta file off by two
         ...
         for (String transcriptId : transcriptIds.keySet()) {
             ...
             for (HashMap<String, String> gffLine : gffLines) {
                 ...
                 System.out.println("startNuc: " + startNuc);
                 System.out.println("endNuc: " + endNuc);
             ...
             System.out.println("sort fastaOrdering: " + fastaOrdering);
             if (plusStrand) {
                 Collections.reverse(fastaOrdering);
                 System.out.println("reverse fastaOrdering: " + fastaOrdering);
             }
 
             String workingLine = "";
             for (int i = 0; i < fastaOrdering.size(); i++) {
                 workingLine += fastaLines.get(fastaOrdering.get(i));
                 System.out.println("workingLine: " + workingLine);
             }
            workingLine = new StringBuffer(workingLine).reverse().toString();
            String output = "";
 
             for (int i = 0; i + 3 < workingLine.length(); i += 3) {
                 //System.out.println("fastaLine.substring("+i+","+(i+3)+"): "+fastaLine.substring(i, i+3));
                if (!plusStrand) {
                     String t = codonTranslate.get(complement(workingLine.substring(i, i + 3)));
                     if(t == null){
                         System.out.println("NULL");
                     }
                     output += t;
                 } else {
                     String t = codonTranslate.get(reverseComplement(workingLine.substring(i, i + 3)));
                     if(t == null){
                         System.out.println("NULL");
                     }
                     output += t;
            ...
            transToSeq.put(transcriptId, output);//new StringBuffer(output).reverse().toString());
             //			transToSeq.put(transcriptId, output);
         }
