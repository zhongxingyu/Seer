 ...
 public class DefinitionInformationWizardPage extends WizardPage implements ISelectionChangedListener{
 	...
 	public void createControl(Composite parent) {
 		...
 		versionStrategy.setBeforeSetValidator(new IValidator() {
 			...
 			public IStatus validate(Object value) {
 				if(!FileUtil.isValidName(NamingUtils.toConnectorDefinitionFilename(definition.getId(),value.toString(), true))){
 		...
 		idStrategy.setBeforeSetValidator(new IValidator() {
 			...
 			@Override
 			public IStatus validate(Object value) {
 				if(!FileUtil.isValidName(NamingUtils.toConnectorDefinitionFilename(value.toString(), definition.getVersion(), true))){
 					return  ValidationStatus.error(Messages.invalidFileName) ;
 				}
 				return Status.OK_STATUS;
 			}
 		...
 		context.bindValue(definitionValidator.observeValidatedValue(observableIdText), EMFObservables.observeValue(definition, ConnectorDefinitionPackage.Literals.CONNECTOR_DEFINITION__ID),idStrategy,null) ;
 		context.bindValue(definitionValidator.observeValidatedValue(observableVersionText), EMFObservables.observeValue(definition, ConnectorDefinitionPackage.Literals.CONNECTOR_DEFINITION__VERSION),versionStrategy,null) ;
 		...
 		context.bindValue(SWTObservables.observeText(displayNameText, SWT.Modify), PojoProperties.value(DefinitionInformationWizardPage.class, "displayName").observe(this)) ;
 	...
 	protected void createCategoryVieer(Composite mainComposite) {
 		...
 		createButton.setLayoutData(GridDataFactory.fillDefaults().grab(true, false).hint(DEFAULT_BUTTON_WIDTH_HINT, SWT.DEFAULT).create()) ;
 	...
 	protected void createDependenciesViewer(Composite mainComposite) {
 		...
 		context.bindValue(ViewersObservables.observeInput(viewer), EMFObservables.observeValue(definition, ConnectorDefinitionPackage.Literals.CONNECTOR_DEFINITION__JAR_DEPENDENCY)) ;
