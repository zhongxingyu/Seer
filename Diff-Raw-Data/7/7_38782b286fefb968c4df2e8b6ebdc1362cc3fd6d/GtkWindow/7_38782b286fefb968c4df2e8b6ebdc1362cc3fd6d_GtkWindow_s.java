...
import org.gnome.gdk.EventExpose;
 ...
 public class GtkWindow extends org.gnome.gtk.Window implements ClientWindow {
 	...
 	public GtkWindow(Client client, int id, int x, int y, int w, int h, Map<String,Object> metadata, boolean override_redirect) {
         ...
         this.mapEvents();
 		EventMask[] masks = new EventMask[] {EventMask.STRUCTURE, EventMask.FOCUS_CHANGE,
 				EventMask.SCROLL, EventMask.VISIBILITY_NOTIFY, EventMask.EXPOSURE,
 	...
 	protected	void mapEvents() {
 		//Here we just map events to the same method name used in the pygtk version:
 	...
 	public void update_metadata(Map<String,Object> newMetadata) {
         ...
         /*
         WindowState state = this.getWindow().getState();
         if (state==WindowState.WITHDRAWN) {
         	//this.set_wmclass(*self._metadata.get("class-instance", ("xpra", "Xpra")))
             if not (self.flags() & gtk.REALIZED):
                 self.set_wmclass(*self._metadata.get("class-instance",
                                                      ("xpra", "Xpra")))
         }
 
         List<Object> icon = (List<Object>) metadata.get("icon");
         if (icon!=null) {
         	Integer width = (Integer) icon.get(0); 
         	Integer height = (Integer) icon.get(1); 
         	String coding = (String) icon.get(2); 
         	byte[] data = (byte[]) icon.get(3);
             assert coding.equals("premult_argb32");
             ImageSurface cairo_surf = new ImageSurface(Format.ARGB32, width.intValue(), height.intValue());
     ...
     */
 	...
 	public void draw(int _x, int _y, int width, int height, String coding, byte[] img_data) {
         ...
         if (!coding.equals("rgb24")) {
 			...
 			try {
 				/*
 				byte[] bytes = new byte[img_data.length()];
 				int index = 0;
 				for (char c : img_data.toCharArray())
 					bytes[index++] = (byte) c;
 					*/
 	        	...
 	        	//	this.savePixbuf("OK-pixbuf-"+System.currentTimeMillis(), img_data);
 				...
 				//this.backing.draw_pixbuf(gc, pixbuf, 0, 0, x, y, width, height);
 			}
 			catch (IOException e) {
             ...
             //this._backing.draw_rgb_image(gc, x, y, width, height, dither, img_data);
         }
     ...
     public boolean do_expose_event_test(Widget source, EventExpose event) {
         ...
         cr.setSource(0.4, 0.5, 0.9, 1.0);
         cr.rectangle(r.getX(), r.getY(), r.getWidth(), r.getHeight());
         cr.fill();
     	this.log("do_expose_event("+source+", "+event+") filling "+r);
         return true;
     }
     public boolean do_expose_event(Widget source, EventExpose event) {
     	//this.log("expose event: "+event);
         WindowState state = this.getWindow().getState();
         //if (!this.flags() & Flags.MAPPED)   return False;
         if (state==WindowState.WITHDRAWN)
         	return false;
         if (event.getWindow()!=this.getWindow())
         	this.log("expose incorrect window?!");
         Rectangle r = event.getArea();
         Context cr = new Context(event);
         cr.setOperator(Operator.SOURCE);
         cr.setSource(this.backing, 0d, 0d);
         cr.paint();
     	this.log("expose event showing "+this.backing+" to "+r);
 		if (save_backing)
 			try {
 				this.backing.writeToPNG("backing-exposed-"+System.currentTimeMillis()+".png");
 			}
 			catch (IOException e) {
     ...
     public void do_motion_notify_event(EventMotion event) {
         ...
         //this.client.send_mouse_position("pointer-position", this.id, pointer, modifiers])
     }
 
     protected	void button_action(Object button, Object event, boolean depressed) {
     	this.log("button");
         //(pointer, modifiers) = self._pointer_modifiers(event)
         //this.client.send_positional("button-action", this.id, button, depressed, pointer, modifiers);
     }
     ...
     protected void focus_change(Object... args) {
         //this.client.update_focus(this.id, this.getProperty("has-toplevel-focus"));
     }
 }
