 ...
 public class HPCDataProvider implements DataServerAbstraction {
 	...
 	public MeteorDataResponse getData(MeteorContext context, String ssn) {
		...
		mess.setRecipientID(res.getProperty("HPC.DATAPROVIDER.ACCESSPOINT","HPC.DATAPROVIDER.ACCESSPOINT"));
		mess.setTransactionType(res.getProperty("EEAT-HPCP-DATASOURCE","EEAT-HPCP-DATASOURCE"));
		mess.setMessageID(res.getProperty("MessageID","Hi John"));
 		
 		if (context.getSecurityToken().getRole().equals(SecurityToken.roleSTUDENT)) {					
			mess.setContent("<BorrowerSSN>"+ ssn + "</BorrowerSSN>", res.getProperty("ContentID","METEOR001"));
 		} else {					
			mess.setContent("<StudentSSN>"+ ssn + "</StudentSSN>", res.getProperty("ContentID","METEOR001"));
 		}
