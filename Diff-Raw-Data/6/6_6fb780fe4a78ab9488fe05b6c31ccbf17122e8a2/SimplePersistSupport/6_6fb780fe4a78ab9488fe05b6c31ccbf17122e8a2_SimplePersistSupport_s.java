 ...
 public class SimplePersistSupport {
 	 ...
 	 * @param multipleLoadingStrategy
 	 *            the multiple loading strategy
 	 * @param keyElementNames
 	 *            the key element names
 	 * @param keyElementValues
 	 *            the key element values
 	 * @return the set< c>
 	 */
 	public static <T> Set<T> findNodesByPrimaryKeyElements(
 		...
 		try {
 			...
 			Assertions.checkCondition("sameSize",
 					keyElementNames.length == keyElementValues.length);
 			...
 			for (int i = 0, size = keyElementNames.length; i < size; i++) {
 				final String keyString = MessageFormat.format(
 						SimplePersistSupport.KEY_VALUE, keyElementNames[i]);
 				fakeBeanDescriptor.properties.put(keyString, Conversion
 						.convert(keyElementValues[i], String.class));
 				attributesToHash.add(keyString);
 			}
 			final String hash = SimplePersistSupport
 					.createHashUsingOnlyPrimaryKey(nodeType,
 							fakeBeanDescriptor, attributesToHash);
 			final String jcrNodeName = SimplePersistSupport.getNodeName(
 					JcrNodeType.NODE, fakeBeanDescriptor, null);
 			final String xpath = MessageFormat.format("{0}//{1}[@{2}=''{3}'']",
 					rootPath, jcrNodeName,
 					SimplePersistSupport.LOCAL_HASH_VALUE, hash);
