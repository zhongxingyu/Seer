 ...
 public class TestResultsGenerator extends Task {
 		...
 		if (sourceDirectory.exists()) {
 			...
 			for (int i = 0; i < xmlFileNames.length; i++) {
 				if (xmlFileNames[i].getPath().endsWith(".xml")) {
 					...
 					String tmp = ((platformSpecificTemplateList.equals("")) ? formatRow(xmlFileNames[i].getPath(), errorCount, true)
 							: formatRowReleng(xmlFileNames[i].getPath(), errorCount, true));
 					replaceString = replaceString + tmp;
 	...
 	private void formatCompileErrorRow(String fileName, int errorCount, int warningCount, StringBuffer buffer) {
 		...
 		String shortName = fileName.substring(i + hrefCompileLogsTargetPath2.length());
 
 		buffer.append("<tr>\n<td>\n").append("<a href=").append("\"").append(hrefCompileLogsTargetPath2).append(shortName).append("\">")
 				.append(shortName).append("</a>").append("</td>\n").append("<td align=\"center\">").append("<a href=").append("\"")
 				.append(hrefCompileLogsTargetPath2).append(shortName).append("#ERRORS").append("\">").append(errorCount).append("</a>")
 				.append("</td>\n").append("<td align=\"center\">").append("<a href=").append("\"").append(hrefCompileLogsTargetPath2)
 				.append(shortName).append("#OTHER_WARNINGS").append("\">").append(warningCount).append("</a>").append("</td>\n").append("</tr>\n");
 	}
 
 	private void formatAccessesErrorRow(String fileName, int forbiddenAccessesWarningsCount, int discouragedAccessesWarningsCount, StringBuffer buffer) {
 		...
 		if (fileName.endsWith(".xml")) {
 			...
 			if (shortName.startsWith(prefix)) {
 				...
 				if (errorCount == -1) {
 					...
 					aString = aString + "<a href=" + "\"" + hrefTestResultsTargetPath + "/" + fileName.substring(begin + 1, fileName.length() - 4)
 							+ ".html" + "\">" + displayName + "</a>";
 				}
