...
import org.meteorologaaguascalientes.control.forecast.ForecastsFactory;
 ...
 public class ServiceFacade {
     ...
     public List<SortedMap<Date, Double>> getData(String variable, String forecast) {
         ...
         try {
             DataAccessAdapter da = Config.getInstance().getDataAccessFactory().createDataAccess();
            AbstractVariableService variableServiceByKey = ServicesFactory.getInstance().getVariableServiceByKey(variable);
            variableServiceByKey.setForecast(ForecastsFactory.getInstance().getForecastById(forecast));
            data = variableServiceByKey.getDataAndPrediction(da);
         } catch (DataAccessException ex) {
             Logger.getLogger(ServiceFacade.class.getName()).log(Level.SEVERE, null, ex);
