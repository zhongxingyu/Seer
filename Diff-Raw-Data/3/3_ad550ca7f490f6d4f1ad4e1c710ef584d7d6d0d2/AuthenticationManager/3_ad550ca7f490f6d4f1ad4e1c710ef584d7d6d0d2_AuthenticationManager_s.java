 ...
 public final class AuthenticationManager implements IAuthenticationManager {
 	...
 	private static final String SECURITY_NODE = "com.aptana.ide.core.io.auth";
 	...
 	public char[] getPassword(String authId) {
 		...
 		if (preferences.nodeExists(authId)) {
 			try {
 				...
 				IdeLog.logImportant(CoreIOPlugin.getDefault(), "Getting security props failed", e);
 			}
 		}
 	...
 	public void setPassword(String authId, char[] password, boolean persistent) {
 		...
 		try {
 			...
 			IdeLog.logImportant(CoreIOPlugin.getDefault(), "Saving security props failed", e);
 		}
 	}
