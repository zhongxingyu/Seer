 ...
 public class NpcDropController {
 	...
 	private static int superChance = 20000;
 	private static int rareChance = 500;
 	...
 	public static boolean gemFix(int id){
 		switch(id){
 		case 1619:
 		case 1623:
 		case 1621:
 	...
 	public static boolean rareDropTableFix(int id){
 		switch(id){
 			case 1438:
 			case 1439:
 			case 1440:
 	...
 	public Item[] getDrops() {
 		...
 		if (drop != null) {
 			if(!rareDropTableFix(drop.getId())){
 			...
 			if(rareDropTableFix(drop.getId())){
 				dropItem = new Item(drop.getId(), 1);
 			}
 		}
 	...
 	public void setDrop() {
 		//int hp = NpcDefinition.forId(getNpcId()).getHitpoints();
 		//hp = hp > 255 ? 255 : hp;
 		//superChance = 500;//1000 - (500 * (hp/255));
 		//rareChance = 50;//100 - (50 * (hp/255));
 		//uncommonChance = 4;//20 - (10 * (hp/255));
 		int c = 0, u = 0, r = 0, s = 0;
 		for (NpcDropItem item : drops) {
 			if(isGem(item.getId())){
 				if(item.getChance() == 2){
 		...
 		for (NpcDropItem item : drops) {
 			...
 			if ((drop.getCount().length > 1 || drop.getCount()[0] > 1) && !drop.getItem().getDefinition().isStackable() && drop.getItem().getDefinition().isNoteable() && !rareDropTableFix(drop.getId()) && !isHerb(drop.getId())) {
 				drop.setNoted();
 			}
 			if (item.getChance() == 2) {
 				common[c2] = drop;
