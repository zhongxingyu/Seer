 ...
 public class IndexProviderService
 {
 	...
 	public DataProviderList getDataProviders (DistributedRegistry registry, ResponseData response, MeteorParameters params, LoanLocator loanLocator)
 	{
 		...
 		if (iProviders.isEmpty())
 		{
			...
			log.info("No index providers found in the registry.  Switching to broadcast mode for this request");
 			dProviders.addAll(registry.getDataProviders());
 			return dProviders;
 		}
 		...
 		while (iterator.hasNext())
 		{
 			...
 			catch (IndexException ex)
 			{
				...
				log.info("Error in response from Index Provider.  Switching to broadcast mode for this request");
 				dProviders.addAll(registry.getDataProviders());
 			}
