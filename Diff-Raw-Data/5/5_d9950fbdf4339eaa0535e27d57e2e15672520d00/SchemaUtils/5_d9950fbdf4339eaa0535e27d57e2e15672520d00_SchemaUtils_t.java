...
import java.util.Date;
 ...
 public class SchemaUtils {
 	...
 	static TradeConstraintsStatus evaluateConstraint(VariableValues variableValues, ConstraintFormula constraint) throws Exception {
 	...
 	static TradeConstraintsStatus evaluateConstraints(VariableValues variableValues, List<ConstraintFormula> constraints) throws Exception {
 		...
 		for (ConstraintFormula constraint : constraints) {
 			if (constraint.getForEach() == ForEach.NONE) {
 				TradeConstraintsStatus status = evaluateConstraint(variableValues, constraint);
 	...
 	static TradeConstraintsStatus evaluateForEachConstraints(Trade trade, VariableValues variableValues, List<ConstraintFormula> constraints) throws Exception {
 		...
 		for (ConstraintFormula constraint : constraints) {
 			if (constraint.getForEach() == ForEach.TYPEREF) {
 				...
 				for (DataItem d : allData) {
 					if (areIdentical(d.getReference(), r) && d.hasValue()) {
 						...
 						variables.put(
 								Variable.newBuilder()
 									...
 									.build(),
 								readBigInteger(d.getValue()));
 						
 						TradeConstraintsStatus status = evaluateConstraint(variables, constraint);
 						if (status == TradeConstraintsStatus.CONSTRAINTS_BROKEN) {
 							return TradeConstraintsStatus.CONSTRAINTS_BROKEN;
 						}
 					}
 	...
 	static boolean isSchemaSensible(TransactionSchema schema) {
 		// TODO: implement
 		System.out.println("isSchemaSensible not implemented yet");
 		return true;
 	}
 	
 	static public boolean isInitialListingValid(TransactionSchema schema, List<SinglePartyData> data) throws Exception {
 	...
 	static public TradeCombinationResult tryAddTrade(Trade original, Trade newTrade) throws Exception {
 		...
 		TradeConstraintsStatus forEachStatus = evaluateForEachConstraints(provisionalTrade, variableValues, provisionalTrade.getSchema().getConstraintsList());
