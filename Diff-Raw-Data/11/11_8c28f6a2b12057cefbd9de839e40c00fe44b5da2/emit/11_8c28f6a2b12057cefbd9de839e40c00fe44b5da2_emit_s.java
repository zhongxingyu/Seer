 ...
 */
 ...
 public class emit {
  ...
  /** TUM changes; proposed by Henning Niss 20050628: Type arguments for class declaration */
   ...
   /** Whether to store tables in files */
 ...
 //	    out.println("              " + pre("result") + " = new java_cup.runtime.Symbol(" + 
   ...
   protected static void emit_production_table(PrintWriter out)
       ...
       for (int i = 0; i<production.number(); i++)
 	{
       ...
       if (store_tables) {
           out.println("  protected static short _production_table[][];");
           store_table("production_table.bin", prod_table);
       } else {
   ...
   protected static void do_action_table(
       ...
       for (int i = 0; i < act_tab.num_states(); i++)
 	{
       ...
       if (store_tables) {
           ...
           store_table("action_table.bin", action_table);
       } else {
   ...
   protected static void do_reduce_table(
       ...
       for (int i=0; i<red_tab.num_states(); i++)
 	{
       ...
       if (store_tables) {
           ...
           store_table("reduce_table.bin", reduce_goto_table);
       } else {
   ...
   public static void parser(
 	...
 	out.println("import " + import_list.elementAt(i) + ";");
       ...
       out.println("     super();");
       if (store_tables) {
         out.println("    load_tables();");
       }
       out.println("  }");
       if (!suppress_scanner) {
 	  out.println();
 	  out.println("  /** Constructor which sets the default scanner. */");
 	  out.println("  public " + parser_class_name + "(java_cup.runtime.Scanner s) {");
       out.println("     super(s);");
       if (store_tables) {
         out.println("     load_tables();");
       }
 	  ...
 	  out.println("  /** Constructor which sets the default scanner. */");
 	  out.println("  public " + parser_class_name + 
 		      "(java_cup.runtime.Scanner s, java_cup.runtime.SymbolFactory sf) {");
       out.println("     super(s, sf);");
       if (store_tables) {
         out.println("     load_tables();");
       }
       out.println("  }");
       ...
       if (store_tables) {
         out.println();
         out.println("  /** Load the tables from files. */");
         out.println("  public void load_tables() {");
         emit_load_table(out, "production_table.bin", "_production_table");
         emit_load_table(out, "action_table.bin", "_action_table");
         emit_load_table(out, "reduce_table.bin", "_reduce_table");
         out.println("  }");
       }
