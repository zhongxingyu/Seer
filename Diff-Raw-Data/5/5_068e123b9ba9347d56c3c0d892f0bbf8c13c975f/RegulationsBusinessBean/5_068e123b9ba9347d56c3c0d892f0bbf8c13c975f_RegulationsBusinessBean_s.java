 ...
 * $Id: RegulationsBusinessBean.java,v 1.103 2003/12/30 11:50:04 palli Exp $
 ...
 public class RegulationsBusinessBean extends com.idega.business.IBOServiceBean implements RegulationsBusiness {
 	...
 	public int saveRegulation(String regID, Date periodFrom, Date periodTo, String name, String amount, String conditionOrder, String operation, String paymentFlowType, String vatEligible, String regSpecType, String conditionType, String specialCalculation, String vatRule, String changedBy, String discount, String maxAmountdiscount) throws RegulationException, RemoteException {
 		...
 		//		"momsersttning" have values when push SAVE-button.
 
 		if (operation.compareTo("0") == 0) {
 			throw new RegulationException(KEY_ERROR_PARAM_MAIN_OPERATION, "Huvudverksamhet mste vljas");
 		}
 
 		if (paymentFlowType.compareTo("0") == 0) {
 			throw new RegulationException(KEY_ERROR_PARAM_PAYMENT_FLOW_TYPE, "Strm mste vljas");
 		}
 		...
 		if (conditionType.compareTo("0") == 0) {
 			throw new RegulationException(KEY_ERROR_PARAM_CONDITION_TYPE, "Villkorstyp mste vljas");
 		}
 		...
 		if (regSpecType.compareTo("0") == 0) {
 			throw new RegulationException(KEY_ERROR_REG_SPEC_TYPE, "Regelspecificationstyp mste vljas");
 		}
 
 		if (vatEligible.compareTo("0") == 0) {
 			throw new RegulationException(KEY_ERROR_VAT_ELIGIBLE, "Momsersttning mste vljas");
 		}
 
 		if (periodFrom.after(periodTo)) {
 			throw new RegulationException(KEY_ERROR_PARAM_DATE_ORDER, "Frn datum kan ej vara senare n tom datum!");
 		}
 	...
 	public Collection findAllConditionSelections(String operationID) {
 		...
 		arr.add(new ConditionHolder(RuleTypeConstant.CONDITION_ID_SCHOOL_YEAR, "rskurs", LP + "aarskurs", "se.idega.idegaweb.commune.accounting.regulations.business.RegulationsBusiness", "findAllSchoolYearIntervals", "", ""));
 		...
 		arr.add(new ConditionHolder(RuleTypeConstant.CONDITION_ID_AGE_INTERVAL, "lder", LP + "alder", "se.idega.idegaweb.commune.accounting.regulations.business.RegulationsBusiness", "findAllAgeIntervals", "", ""));
 
 		arr.add(new ConditionHolder(RuleTypeConstant.CONDITION_ID_STADSBIDRAG, "Statsbidragsberttigad", LP + "statsbidragsberattigad", "se.idega.idegaweb.commune.accounting.regulations.business.RegulationsBusiness", "getYesNo", "getLocalizationKey", ""));
 		...
 		if (operationID.compareTo(SchoolCategoryBMPBean.CATEGORY_HIGH_SCHOOL) == 0) {
 			arr.add(new ConditionHolder(RuleTypeConstant.CONDITION_ID_STUDY_PATH, "Studievg", LP + "studievag", "se.idega.idegaweb.commune.accounting.school.business.StudyPathBusiness", "findAllStudyPaths", "getCode", ""));
 		}
