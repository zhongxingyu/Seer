 ...
 public class BluetoothHeadsetService extends Service {
     ...
     public void onCreate() {
         ...
         mBtHandsfree = BluetoothHandsfree.init(this); //TODO
    ...
    synchronized private BluetoothDevice getCurrentDevice() {
         ...
         public void onReceive(Context context, Intent intent) {
             ...
             if ((state == BluetoothProfile.STATE_CONNECTED ||
                 ...
                 case BluetoothAdapter.STATE_ON:
                     Log.d(TAG,"Bluetooth is poweredON...Start AG");
                     mAg.start(mIncomingConnectionHandler);
                     mBtHandsfree.onBluetoothEnabled();
                     break;
                 case BluetoothAdapter.STATE_TURNING_OFF:
                     Log.d(TAG,"Bluetooth is poweredOFF...Stop AG");
     ...
     private final IBluetoothHeadset.Stub mBinder = new IBluetoothHeadset.Stub() {
         ...
         //New AIDL functions implemented here
         public boolean connectAudio() {
             enforceCallingOrSelfPermission(BLUETOOTH_PERM, "Need BLUETOOTH permission");
             log("AudioOn  connectAudio" );
             return mBtHandsfree.audioOn();
         }
 
         public boolean disconnectAudio() {
             enforceCallingOrSelfPermission(BLUETOOTH_PERM, "Need BLUETOOTH permission");
             log("AudioOf  disconnectAudio" );
             mBtHandsfree.audioOff();
             return true; //TODO..Can we change the audioOff to return true or
             // false.
         }
 
         public boolean isAudioOn() {
             return mBtHandsfree.isAudioOn(); //TODO. What to check and return
         }
 
         public void phoneStateChanged(int numActive, int numHeld, int callState,
                                       String number, int type) {
             HeadsetCallState newState = new HeadsetCallState(numActive, numHeld, callState, number, type);
             mBtHandsfree.phoneStateChanged(newState);
         }
 
         public void roamChanged(boolean roam) {
             mBtHandsfree.roamChanged(roam);
         }
 
         public void clccResponse(int index, int direction, int status, int mode,
                                  boolean mpty,String number, int type) {
             mBtHandsfree.clccResponse(index, direction, status, mode, mpty, number, type);
         }
