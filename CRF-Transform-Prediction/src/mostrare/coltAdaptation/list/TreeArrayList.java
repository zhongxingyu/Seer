...
import mostrare.coltAdaptation.function.TreeProcedure;
import mostrare.tree.Tree;
import cern.colt.list.AbstractList;

/**
 * Resizable list holding <code>Tree</code> elements; implemented with arrays. This class is an
 * adaptation of <code>cern.colt.list.ObjectArrayList</code> of the colt API.
 * 
 * @author missi
 */
public class TreeArrayList extends AbstractList
{
	/**
	 * 
	 */
	 ...
	 */
	public TreeArrayList(Tree[] elements)
	{
		elements(elements);
	}
	 ...
	 */
	public TreeArrayList(int initialCapacity)
	{
		this(new Tree[initialCapacity]);
		size = 0;
	}
	 ...
	 */
	public void addAllOfFromTo(TreeArrayList other, int from, int to)
	{
	 ...
	 */
	public void beforeInsert(int index, Tree element)
	{
	 ...
	 *                index is out of range (<code>index &lt; 0 || index &gt; size()</code>).
	 */
	public void beforeInsertAllOfFromTo(int index, TreeArrayList other, int from, int to)
	{
	 ...
	 * multiple elements equal to the specified Tree, there is no guarantee which instance will be
	 * found.
	 * 
	 * @param key
	 ...
	 */
	public int binarySearch(Tree key)
	{
	 ...
	 * @see java.util.Arrays
	 */
	public int binarySearchFromTo(Tree key, int from, int to)
	{
		int low = from;
		int high = to;
		...
		{
			int mid = (low + high) / 2;
			Tree midVal = elements[mid];
			int cmp = ((Comparable<Tree>) midVal).compareTo(key);
	 ...
	 * an infinite loop. If the receiver contains multiple elements equal to the specified Tree,
	 ...
	 */
	public int binarySearchFromTo(Tree key, int from, int to, java.util.Comparator comparator)
	{
	 ...
	 * elements, but do not share the same array to index them; So modifying an Tree in the copy
	 * modifies the Tree in the receiver and vice versa; However, structurally modifying the copy
	 * (for example changing its size, setting other Trees at indexes, etc.) does not affect the
	...
	public Object clone()
	{
		TreeArrayList v = (TreeArrayList) super.clone();
		v.elements = (Tree[]) elements.clone();
		return v;
	}
	 ...
	 */
	public boolean contains(Tree elem, boolean testForEquality)
	{
	 ...
	 * the same array to index them; So modifying an Tree in the copy modifies the Tree in the
	 * receiver and vice versa; However, structurally modifying the copy (for example changing its
	 * size, setting other Trees at indexes, etc.) does not affect the receiver and vice versa.
	 * 
	 * @return a copy of the receiver.
	 */
	public TreeArrayList copy()
	{
		return (TreeArrayList) clone();
	}
	 ...
	 */
	public void delete(Tree element, boolean testForEquality)
	{
	 ...
	 */
	public TreeArrayList elements(Tree[] elements)
	{
	...
	{
		elements = mostrare.coltAdaptation.ArraysForTree.ensureCapacity(elements, minCapacity);
	}

	/**
	 * Compares the specified Tree with the receiver for equality. Returns true if and only if the
	 * specified Tree is also an TreeArrayList, both lists have the same size, and all corresponding
	 * pairs of elements in the two lists are equal. In other words, two lists are defined to be
	 * equal if they contain the same elements in the same order. Two elements <code>e1</code> and
	 * <code>e2</code> are <i>equal</i> if <code>(e1==null ? e2==null :
	 * e1.equals(e2))</code>.)
	 * 
	 * @param otherObj
	 *            the Tree to be compared for equality with the receiver.
	 * @return true if the specified Tree is equal to the receiver.
	 */
	@Override
	 ...
	 * Compares the specified Tree with the receiver for equality. Returns true if and only if the
	 * specified Tree is also an TreeArrayList, both lists have the same size, and all corresponding
	 * pairs of elements in the two lists are the same. In other words, two lists are defined to be
	 * equal if they contain the same elements in the same order. Tests elements for equality or
	 * identity as specified by <code>testForEquality</code>. When testing for equality, two
	...
	{ // delta
		if (!(otherObj instanceof TreeArrayList))
		{
			return false;
		}
		if (this == otherObj)
		...
		if (otherObj == null)
			return false;
		TreeArrayList other = (TreeArrayList) otherObj;
		if (elements == other.elements())
			return true;
		if (size != other.size())
			return false;

		Tree[] otherElements = other.elements();
		Tree[] theElements = elements;
		if (!testForEquality)
		{
			for (int i = size; --i >= 0;)
			{
	 ...
	 */
	public void fillFromToWith(int from, int to, Tree val)
	{
	 ...
	 */
	public boolean forEach(TreeProcedure procedure)
	{
		Tree[] theElements = elements;
		int theSize = size;
	 ...
	 *            index of element to return.
	 */
	public Tree getQuick(int index)
	{
		return elements[index];
	}
	 ...
	 */
	public int indexOf(Tree element, boolean testForEquality)
	{
	 ...
	 *                to&gt;=size())</code>).
	 */
	public int indexOfFromTo(Tree element, int from, int to, boolean testForEquality)
	{
		if (size == 0)
			return -1;
	 ...
	 *         <code>-1</code> if the element is not found.
	 */
	public int lastIndexOf(Tree element, boolean testForEquality)
	{
	 ...
	 *                index is out of range (<code>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</code>).
	 */
	public int lastIndexOfFromTo(Tree element, int from, int to, boolean testForEquality)
	{
		if (size == 0)
			return -1;
		checkRangeFromTo(from, to, size);
	 ...
	 * @see Comparator
	 */
	public void mergeSortFromTo(int from, int to, java.util.Comparator<Tree> c)
	{
		if (size == 0)
			return;
		checkRangeFromTo(from, to, size);
	 ...
	 *                index is out of range (<code>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</code>).
	 */
	public TreeArrayList partFromTo(int from, int to)
	{
		if (size == 0)
			return new TreeArrayList(0);

		checkRangeFromTo(from, to, size);

		Tree[] part = new Tree[to - from + 1];
		System.arraycopy(elements, from, part, 0, to - from + 1);
		return new TreeArrayList(part);
	}
	 ...
	 */
	public boolean removeAll(TreeArrayList other, boolean testForEquality)
	{
		if (other.size == 0)
	 ...
	 */
	public void replaceFromToWithFrom(int from, int to, TreeArrayList other, int otherFrom)
	{
		int length = to - from + 1;
		if (length > 0)
		{
	 ...
	 * </pre>
	 */
	public void replaceFromToWithFromTo(int from, int to, TreeArrayList other, int otherFrom,
			int otherTo)
	{
	...
	public void replaceFromWith(int from, java.util.Collection other)
	{
		...
		for (int i = 0; i < limit; i++)
			elements[index++] = (Tree) e.next(); // delta
	}
	 ...
	 * @return <code>true</code> if the receiver changed as a result of the call.
	 */
	public boolean retainAll(TreeArrayList other, boolean testForEquality)
	{
		if (other.size == 0)
		{
			if (size == 0)
	 ...
	 *                index is out of range (index &lt; 0 || index &gt;= size()).
	 */
	public void set(int index, Tree element)
	{
		if (index >= size || index < 0)
			throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + size);
		elements[index] = element;
	 ...
	 *            element to be stored at the specified position.
	 */
	public void setQuick(int index, Tree element)
	{
		elements[index] = element;
	}
	 ...
	 */
	public TreeArrayList times(int times)
	{
		TreeArrayList newList = new TreeArrayList(times * size);
		for (int i = times; --i >= 0;)
		{
	 ...
	 */
	public Tree[] toArray(Tree array[])
	{
		if (array.length < size)
			array = (Tree[]) java.lang.reflect.Array.newInstance(array.getClass()
	...
	public java.util.ArrayList toList()
	{
		int mySize = size();
		Tree[] theElements = elements;
		java.util.ArrayList<Tree> list = new java.util.ArrayList<Tree>(mySize);
	...
	{
		elements = mostrare.coltAdaptation.ArraysForTree.trimToCapacity(elements, size());
	}
}
