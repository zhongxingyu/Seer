 ...
 //			StringBuffer 
 	...
 	private void createPaymentFiles(String fileName1, String fileName2, String schoolCategory, IWTimestamp executionDate, IWTimestamp paymentDate, Locale currentLocale) throws IOException {
 		...
 		if (phOutsideCommune != null && !phOutsideCommune.isEmpty()) {
 			...
 			try {							
 				createPaymentFilesExcel(recOutside, fileName2 + ".xls","Checkutbetalning "+localizedSchoolCategoryName+", vriga anordnare, "+executionDate.getDateString("yyyy-MM-dd"));				
 			}
 			catch (IOException e3) {
 				e3.printStackTrace();
 			}
 	...
 	public void createPaymentFilesExcel(Collection data, String fileName, String headerText) throws IOException {
 		if (data != null && !data.isEmpty()) {
 			...
 			sheet.createRow((short) (rowNumber + 1)).createCell(row.getFirstCellNum()).setCellValue(numberOfRecords + " bokfringsposter,   Kreditbelopp totalt:  - " + getNumberFormat().format(totalAmount) + ",   Debetbelopp totalt: " + getNumberFormat().format(totalAmount));
 			...
 			while (it.hasNext()) {
 				...
 				if (!iRecs.isEmpty()) {					
 					...
 					while (irIt.hasNext()) {
 						...
 						row = sheet.createRow(rowNumber++);
 						row.createCell(cellNumber++).setCellValue("Extraersttning 6% p totalt utbetalt checkbelopp");
