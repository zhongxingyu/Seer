  ...
  */
 	public boolean start()
 	{
 		...
 		broadcast(prtcl.makePacket(game.toString(), "GAMES"));
 		if(inSession == false)
 		{
 			inSession = true;
  ...
  */	
 	private void promptTurn()
 	{
 		...
 		data = new char[prtcl.getLength(packet)+1];
 		//packet = new String(data);
 		//packet = packet.substring(0, packet.length()-1);
 System.out.println("Before if-else: " + packet);		
 		if(prtcl.getCommand(packet).toLowerCase().equals("help"))
 		{
 			curr.sendMessage(prtcl.makePacket(helpMessage("help"), "message"));
			curr.getMessage();
 		}
 		else
 		{
 	
 			curr.read(data, prtcl.getLength(packet)+1);		
 		
 			packet = new String(data);
 			packet = packet.substring(0, packet.length()-1);	
 			System.out.println("Z: " +packet);
 			tokens = packet.split(" ");
 	...
 	{
 		String message = "";
 		String[] msgsTypes = {"Help", "Login", "Remove", "Bye"};
 		...
 		for(int i = 0; i < msgsTypes.length; i++)
 		{
 			message += msgsTypes[i] + " " + msgs.get(msgsTypes[i].toLowerCase()) + "\n" ;
 		}
